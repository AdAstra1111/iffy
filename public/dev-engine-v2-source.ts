...
${version.plaintext.slice(0, 25000)}`;
      ...
      if (blockerCount === 0 && parsed.convergence) {
        parsed.convergence.status = "converged";
      }

      if (guardrails.policy.engineMode === "hard-lock") {
        const violations = validateOutput(parsed, guardrails.policy);
        if (violations.length > 0) {
          console.warn(`[dev-engine-v2] hard-lock violations: ${violations.join("; ")}`);
          const regenPrompt = buildRegenerationPrompt(violations);
          const retryRaw = await callAI(LOVABLE_API_KEY, PRO_MODEL, systemPrompt + "\n\n" + regenPrompt, userPrompt, 0.15, 6000);
          const retryParsed = await parseAIJson(LOVABLE_API_KEY, retryRaw);
          Object.assign(parsed, retryParsed);
          parsed._guardrail_retry = true;
        }
      }
      ...
      const { data: run, error: runErr } = await supabase.from("development_runs").insert({
        ...
        schema_version: SCHEMA_VERSION,
        guardrail_hash: guardrails.hash,
        guardrail_profile: guardrails.profileName,
      }).select().single();
      if (runErr) throw runErr;

      const extractedCore = parsed.extracted_core || {};
      const { data: currentVersion } = await supabase.from("project_document_versions")
        .select("inherited_core, drift_snapshot").eq("id", versionId).single();
      const inheritedCore = (currentVersion?.inherited_core as Record<string, string>) || {};

      await supabase.from("project_document_versions")
        .update({ drift_snapshot: { ...parsed, extracted_core: extractedCore } })
        .eq("id", versionId);

      if (Object.keys(inheritedCore).length > 0 && Object.values(inheritedCore).some(v => v)) {
        const drift = detectDrift(extractedCore, inheritedCore);
        if (drift.level !== "none") {
      ...
      await supabase.from("dev_engine_convergence_history").insert({
        ...
        document_id: documentId,
        version_id: versionId,
        creative_score: parsed.ci_score ?? scores.ci_score ?? 0,
        greenlight_score: parsed.gp_score ?? scores.gp_score ?? 0,
        gap: parsed.gap ?? scores.gap ?? 0,
        allowed_gap: parsed.allowed_gap ?? scores.allowed_gap ?? 25,
        convergence_status: parsed.convergence?.status || "not_started",
        trajectory: parsed.trajectory || null,
      });

      if (!currentVersion?.inherited_core || Object.keys(inheritedCore).length === 0) {
        await supabase.from("project_document_versions")
          .update({ inherited_core: extractedCore })
          .eq("id", versionId);
      }
    ...
    // ══════════════════════════════════════════════
    // NOTES — tiered generation with resolution tracking
    // ══════════════════════════════════════════════
    if (action === "notes") {
      const { projectId, documentId, versionId, analysisJson } = body;
      if (!projectId || !documentId) throw new Error("projectId, documentId required");

      let analysis = analysisJson;
      if (!analysis) {
        const effectiveVersionId = versionId;
        if (effectiveVersionId) {
          const { data: lastRun } = await supabase.from("development_runs")
            .select("output_json").eq("version_id", effectiveVersionId).eq("run_type", "ANALYZE")
            .order("created_at", { ascending: false }).limit(1).single();
          analysis = lastRun?.output_json;
        }
      }
      ...
      const parsed: any = {
        ...
        high_impact_notes: analysis.high_impact_notes || [],
        polish_notes: analysis.polish_notes || [],
        convergence: analysis.convergence || {},
        rewrite_plan: analysis.rewrite_plan || [],
        protect: analysis.protect || [],
      };

      const allTieredNotes = [
        ...(parsed.blocking_issues || []).map((n: any) => ({ ...n, severity: "blocker" })),
        ...(parsed.high_impact_notes || []).map((n: any) => ({ ...n, severity: "high" })),
        ...(parsed.polish_notes || []).map((n: any) => ({ ...n, severity: "polish" })),
      ];

      const { data: existingNotes } = await supabase.from("development_notes")
        .select("note_key, resolved")
        .eq("document_id", documentId);
      const existingUnresolved = (existingNotes || []).filter(n => !n.resolved);
      const currentNoteKeys = new Set(allTieredNotes.map((n: any) => n.id).filter(Boolean));
      const previouslyResolved = new Set((existingNotes || []).filter(n => n.resolved).map(n => n.note_key));

      for (const existingNote of existingUnresolved) {
        if (!currentNoteKeys.has(existingNote.note_key)) {
          await supabase.from("development_notes")
            .update({ resolved: true, resolved_at: new Date().toISOString() })
            .eq("note_key", existingNote.note_key)
            .eq("document_id", documentId)
            .eq("resolved", false);
        }
      }
...
${version.plaintext.slice(0, 20000)}`;
      ...
      const { data: run } = await supabase.from("development_runs").insert({
        ...
        output_json: {
          ...
          source_doc_type: srcDoc?.doc_type,
          target_format: targetOutput,
          change_summary: parsed.change_summary || "",
          char_count: (parsed.converted_text || "").length,
        },
        schema_version: SCHEMA_VERSION,
      }).select().single();

      return new Response(JSON.stringify({ run, newDoc, newVersion }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // ══════════════════════════════════════════════
    // CREATE-PASTE — text paste as new document
    // ══════════════════════════════════════════════
    if (action === "create-paste") {
      const { projectId, title, docType, text } = body;
      if (!projectId || !title || !text) throw new Error("projectId, title, text required");

      const resolvedType = docTypeMap[(docType || "").toUpperCase()] || docType || "script";
      ...
      const { data: version, error: vErr } = await supabase.from("project_document_versions").insert({
        document_id: doc.id,
        version_number: 1,
        label: "v1 (pasted)",
        plaintext: text,
      ...
      return new Response(JSON.stringify({ document: doc, version }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // ══════════════════════════════════════════════
    // SCRIPT-PLAN → WRITE-BATCH → ASSEMBLE
    // ══════════════════════════════════════════════
    if (action === "script-plan") {
      ...
      const { data: srcDoc } = await supabase.from("project_documents")
        .select("title, doc_type").eq("id", documentId).single();

      const userPrompt = `SOURCE: ${srcDoc?.title || "Untitled"} (${srcDoc?.doc_type || "blueprint"})

MATERIAL:
${version.plaintext.slice(0, 15000)}`;

      const raw = await callAI(LOVABLE_API_KEY, PRO_MODEL, SCRIPT_PLAN_SYSTEM, userPrompt, 0.3, 8000);
      const parsed = await parseAIJson(LOVABLE_API_KEY, raw);

      const { data: scriptDoc, error: sdErr } = await supabase.from("project_documents").insert({
        project_id: projectId,
        user_id: user.id,
        title: `${srcDoc?.title || "Script"} — Screenplay`,
        file_name: `${srcDoc?.title || "Script"} — Screenplay`,
        file_path: "",
      ...
      const { data: scriptVersion, error: svErr } = await supabase.from("project_document_versions").insert({
        ...
        plaintext: "",
        created_by: user.id,
        change_summary: `Planned: ${parsed.total_scenes} scenes, ~${parsed.target_pages} pages`,
        inherited_core: {},
        source_document_ids: [documentId],
      }).select().single();
      ...
      const allScenes = parsed.acts?.flatMap((a: any) => a.scenes || []) || [];
      const batches: any[][] = [];
