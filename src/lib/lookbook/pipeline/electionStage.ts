...
function getScoringContext(
  ...
  sceneContext?: { sceneCharacterKeys?: string[]; sceneLocationKey?: string | null; sceneNumber?: number; sceneWardrobeStateMap?: Record<string, string> },
): ScoringContext {
  ...
  return {
    ...
    slotIntent: {
      ...
      sceneCharacterKeys: sceneContext?.sceneCharacterKeys,
      sceneLocationKey: sceneContext?.sceneLocationKey,
      sceneNumber: sceneContext?.sceneNumber,
      sceneWardrobeStateMap: sceneContext?.sceneWardrobeStateMap,
    },
  };
}
...
export function pickForegroundImages(
  ...
  const scoringCtx = getScoringContext(ctx, slideType, boundPrincipalIds, hasSceneEvidence, sceneContext);

  // Orientation-aware pre-filtering: strongly prefer correct orientation
  const LANDSCAPE_SLIDES = new Set(['cover', 'closing', 'world', 'creative_statement', 'themes', 'comparables']);
  const PORTRAIT_SLIDES = new Set(['poster_directions']);
  const requiredOrientation = LANDSCAPE_SLIDES.has(slideType) ? 'landscape' : PORTRAIT_SLIDES.has(slideType) ? 'portrait' : null;
...
export function runElectionStage(
  ...
  // Build scene-aware context from SceneEvidence (sorted by sceneNumber for deterministic ordering)
  const canonScenes = (narrativeEvidence?.sceneEvidence || [])
    .filter(s => s.fromSceneIndex && s.sceneNumber != null)
    .sort((a, b) => (a.sceneNumber || 0) - (b.sceneNumber || 0));

  // Scene-centric slide types that get per-scene binding
  const SCENE_CENTRIC_SLIDES = ['key_moments', 'story_engine', 'world', 'themes'] as const;

  // Build a round-robin scene assignment for scene-centric slides.
  // Each scene-centric slide spec gets a specific canonical scene (wrapping if fewer scenes than slides).
  let sceneCentricIndex = 0;

  if (canonScenes.length > 0) {
    console.log(`[Election:scene] ${canonScenes.length} canonical scenes (fromSceneIndex), ` +
      `scenes=[${canonScenes.map(s => `#${s.sceneNumber}(chars=${(s.characterKeys||[]).length},loc=${s.locationKey||'none'})`).join(', ')}]`);
  }
  ...
  /** Get scene context for a given slide type — per-scene for scene-centric, location-only for others */
  const getSceneContextForSlide = (slideType: string) => {
    if (canonScenes.length === 0) return undefined;

    if ((SCENE_CENTRIC_SLIDES as readonly string[]).includes(slideType)) {
      // Assign a specific canonical scene (round-robin across scene-centric slides)
      const scene = canonScenes[sceneCentricIndex % canonScenes.length];
      sceneCentricIndex++;
      const ctx = {
        sceneCharacterKeys: scene.characterKeys || [],
        sceneLocationKey: scene.locationKey || null,
        sceneNumber: scene.sceneNumber,
        sceneWardrobeStateMap: scene.wardrobeStateMap || {},
      };
      console.log(`[Election:scene] ${slideType} → scene #${ctx.sceneNumber} chars=[${ctx.sceneCharacterKeys.join(',')}] loc=${ctx.sceneLocationKey || 'none'}`);
      return ctx;
    }

    // Non-scene-centric slides: no scene binding
    return undefined;
  };
  ...
  for (const spec of SLIDE_ELECTION_SPECS) {
    ...
    const sceneCtx = getSceneContextForSlide(spec.slideType);

    // Special handling for cover/closing — use poster hero
    if (spec.slideType === 'cover' || spec.slideType === 'closing') {
      const bgUrl = coverImageUrl || pickBackgroundImage(primaryPool, ctx, spec.slideType, fallbackPool, boundPrincipalIds, hasSceneEvidence, selCtx, sceneCtx);
      if (bgUrl) {
    ...
    if (spec.needsBackground) {
      bgUrl = pickBackgroundImage(primaryPool, ctx, spec.slideType, fallbackPool, boundPrincipalIds, hasSceneEvidence, selCtx, sceneCtx);
      if (bgUrl) {
        ctx.usedBackgroundUrls.push(bgUrl);
        trackSelection(ctx, bgUrl, spec.slideType);
        const bgImg = ctx.urlToImage.get(bgUrl);
    ...
    if (spec.maxForeground > 0) {
      ...
      if (spec.slideType === 'story_engine') {
        ...
        fgUrls = pickForegroundImages(sePool, spec.slideType, spec.maxForeground, ctx, bgUrl ? [bgUrl] : [], boundPrincipalIds, hasSceneEvidence, selCtx, sceneCtx);
      } else if (spec.slideType === 'visual_language') {
        const vlPool = [...(sectionPools.texture || []), ...(sectionPools.motifs || [])];
        fgUrls = pickForegroundImages(vlPool, spec.slideType, spec.maxForeground, ctx, bgUrl ? [bgUrl] : [], boundPrincipalIds, hasSceneEvidence, selCtx, sceneCtx);
      } else {
        fgUrls = pickForegroundImages(combinedPool, spec.slideType, spec.maxForeground, ctx, bgUrl ? [bgUrl] : [], boundPrincipalIds, hasSceneEvidence, selCtx, sceneCtx);
      }
      fgUrls.forEach(u => {
