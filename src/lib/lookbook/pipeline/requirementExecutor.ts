      ...
      while (
        ...
        while (retryAttempt <= MAX_RETRIES_PER_CALL) {
          try {
            ...
            // If the error is a transient fetch failure and we have retries left, retry
            const isTransient = error && (
              String(error.message || error).includes('Failed to fetch') ||
              String(error.message || error).includes('FetchError') ||
              String(error.message || error).includes('network')
            );

            if (isTransient && retryAttempt < MAX_RETRIES_PER_CALL) {
              retryAttempt++;
              const backoff = RETRY_BACKOFF_MS * retryAttempt;
              log(`[${section}] batch ${callCount} transient failure, retrying in ${backoff}ms (attempt ${retryAttempt}/${MAX_RETRIES_PER_CALL})`);
              await new Promise(resolve => setTimeout(resolve, backoff));
              continue;
            }
            break;
          } catch (e: any) {
            const isTransientCatch = (
              String(e.message || '').includes('Failed to fetch') ||
              String(e.message || '').includes('FetchError') ||
              String(e.message || '').includes('network')
            );

            if (isTransientCatch && retryAttempt < MAX_RETRIES_PER_CALL) {
              retryAttempt++;
              const backoff = RETRY_BACKOFF_MS * retryAttempt;
              log(`[${section}] batch ${callCount} fetch error, retrying in ${backoff}ms (attempt ${retryAttempt}/${MAX_RETRIES_PER_CALL})`);
              await new Promise(resolve => setTimeout(resolve, backoff));
              continue;
            }
        ...
        if (batchError) {
          log(`[${section}] batch ${callCount} FAILED: ${batchError.message || batchError}`);
          totalFailed += count;
          consecutiveFailures++;
        } else {
          const genResults = batchData?.results || [];
...
const SHOT_TYPE_COMPAT: Record<string, string[]> = {
  ...
  identity_profile: ['close_up', 'profile'],
