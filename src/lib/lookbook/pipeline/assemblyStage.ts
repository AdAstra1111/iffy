...
export function runAssemblyStage(input: AssemblyInput): SlideContent[] {
  ...
  // ── 1. COVER ──
  {
    ...
    slides.push({
      ...
      _heroAnchorId: (canonImages.poster_directions as SectionImageResult)?.heroAnchorId ?? null,
      _heroAnchorInjected: (canonImages.poster_directions as SectionImageResult)?.heroAnchorInjected ?? false,
    });
  }
  ...
  // ── 10. HERO FRAMES ──
  {
    const e = getElection(electionResult, 'key_moments:hero_frames');
    const heroSection = canonImages.hero_frames as SectionImageResult | undefined;
    if (e.foregroundUrls.length > 1) {
      slides.push({
        type: 'key_moments' as any,
        slide_id: makeSemanticSlideId('key_moments', 'hero_frames'),
        title: 'Hero Frames',
        body: 'Cinematic anchor stills — the visual truth that defines the project.',
        imageUrl: e.foregroundUrls[0] || undefined,
        imageUrls: e.foregroundUrls,
        composition: 'montage_grid',
        roledImages: e.roledImages,
        _debug_image_ids: heroSection?.imageIds,
        _debug_provenance: heroSection ? toSlideProvenance(heroSection) : [],
        _has_unresolved: heroSection?.unresolvedCount ? heroSection.unresolvedCount > 0 : false,
        _heroAnchorId: heroSection?.heroAnchorId ?? null,
        _heroAnchorInjected: heroSection?.heroAnchorInjected ?? false,
      });
    }
  }
  ...
  // ── Resolution status enforcement ──
  for (const slide of slides) {
    const hasAnyImage = !!(slide.backgroundImageUrl || slide.imageUrl || (slide.imageUrls && slide.imageUrls.length > 0) ||
      (slide.characters && slide.characters.some(c => c.imageUrl)));
    if (slide._has_unresolved) {
      slide._resolutionStatus = hasAnyImage ? 'partial' : 'unresolved';
    } else {
      slide._resolutionStatus = 'resolved';
    }
  }
