...
describe('ranking integration with visual similarity', async () => {
  ...
  it('metadata-locked candidate can be demoted by weak visual similarity', () => {
    ...
    // unlocked character image is now excluded by fail-closed gate
    // So test with two locked images with different similarity scores
    const lockedStrong = makeImage({
      id: 'locked-strong-visual',
      subject_type: 'character',
    ...
    const similarities: Record<string, VisualSimilarityResult> = {
      ...
      'locked-strong-visual': makeResult(90),
    };

    const withSim = rankCharacterCandidates([locked, lockedStrong], fullAnchorSet, null, similarities);
  ...
  it('no similarity data = no adjustment, no crash', () => {
    const img = makeImage({ id: 'img-nosim', subject_type: 'character', generation_config: { identity_locked: true } });
