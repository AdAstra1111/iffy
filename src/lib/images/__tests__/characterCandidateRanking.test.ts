...
describe('rankCharacterCandidates', () => {
  ...
  it('locked candidate beats drifted candidate (drifted excluded by gate)', () => {
    const locked = makeImage({ id: 'locked', subject_type: 'character', generation_config: { identity_locked: true } });
    const drifted = makeImage({ id: 'drifted', subject_type: 'character', generation_config: {} });
    const result = rankCharacterCandidates([drifted, locked], partialAnchorSet);
    // Drifted is excluded by fail-closed gate — only locked remains
    expect(result.ranked).toHaveLength(1);
    expect(result.top!.image.id).toBe('locked');
  });

  it('drifted candidate excluded when anchors exist (fail-closed)', () => {
    const drifted = makeImage({ id: 'drifted', subject_type: 'character', generation_config: {} });
    const result = rankCharacterCandidates([drifted], partialAnchorSet);
    // Fail-closed: drifted excluded entirely
    expect(result.ranked).toHaveLength(0);
    expect(result.top).toBeNull();
  });

  it('no penalty when no anchors exist but identity_locked', () => {
    const img = makeImage({ id: 'a', subject_type: 'character', generation_config: { identity_locked: true } });
    ...
    // With identity_locked=true, classifyIdentityContinuity returns strong_match
    expect(result.ranked[0].continuityStatus).toBe('strong_match');
  });
  ...
  it('all surfaces agree: requiredVisualSet ranking matches compare ranking', () => {
    // Drifted is excluded by fail-closed gate, only locked survives
    const locked = makeImage({ id: 'locked', subject_type: 'character', generation_config: { identity_locked: true } });
    const drifted = makeImage({ id: 'drifted', subject_type: 'character', generation_config: {} });
    ...
    expect(result1.ranked).toHaveLength(1);
  });
});
