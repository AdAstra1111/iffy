...
import { isCharacterImageEligible } from './characterImageEligibility';
...
export function rankCharacterCandidates(
  ...
  // ── CANONICAL IDENTITY GATE (fail-closed): exclude drift/blocked images before ranking ──
  const eligible = candidates.filter(img => isCharacterImageEligible(img as any, 'character_identity'));
  if (eligible.length === 0) {
    return { ranked: [], top: null, topReason: 'All candidates excluded by identity gate (fail-closed)' };
  }
  if (eligible.length < candidates.length) {
    console.warn('[IDENTITY_GATE_RANKING]', {
  ...
  const ranked: RankedCandidate[] = eligible.map(image => {
