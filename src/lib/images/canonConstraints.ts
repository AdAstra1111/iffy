 ...
 * 
 * Integrates with:
 * - Global Image Style Policy for photorealistic defaults
 * - World Validation Mode for realism/fantasy governance
 */
...
import { resolveWorldValidationMode, formatWorldValidationPromptBlock, getWorldValidationNegatives, type WorldValidationMode } from '../visual/worldValidationMode';
...
/**
 * Convenience: resolve style policy + world validation mode + build full prompt context for any image generation.
 */
export function buildImageGenerationContext(
  ...
  // Resolve world validation mode from same canon inputs
  const worldValidationMode = resolveWorldValidationMode({
    genres: projectMeta.genres || (projectMeta.genre ? [projectMeta.genre] : []),
    tone_style: projectMeta.tone,
    world_rules: typeof canonState.world_rules === 'string' ? canonState.world_rules : undefined,
    format: projectMeta.format,
    logline: typeof canonState.logline === 'string' ? canonState.logline : undefined,
    premise: typeof canonState.premise === 'string' ? canonState.premise : undefined,
    setting: typeof canonState.setting === 'string' ? canonState.setting : undefined,
  });

  // Add world validation block to canon block
  const worldBlock = formatWorldValidationPromptBlock(worldValidationMode);
  const worldNegatives = getWorldValidationNegatives(worldValidationMode);
  ...
  return {
    ...
    negativePrompt: [
      formatCanonNegativePrompt(constraints, stylePolicy),
      worldNegatives,
    ].filter(Boolean).join(', '),
  };
}
