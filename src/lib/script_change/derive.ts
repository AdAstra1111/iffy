...
async function upsertDerivedDoc(
  ...
  // ── CANONICAL: use createDocumentVersion() instead of direct insert ──
  const { createDocumentVersion } = await import('@/lib/docVersions/createDocumentVersion');
  try {
    await createDocumentVersion({
      ...
      plaintext,
      label: `auto-derived`,
      changeSummary: `Auto-derived ${docType} from script change`,
      generatorId: 'script-change-derive',
      createdBy: userId,
      sourceMode: 'script_derive',
    });
  } catch (err: any) {
    console.error(`[derive] Failed to create ${docType} version:`, err.message);
  }
}
