...
import { propagateCastChange } from '@/lib/visual-truth-staleness';
...
export async function bindActorToProjectCharacter(
  ...
  // Propagate staleness to dependent visual assets (fire-and-forget)
  propagateCastChange(projectId, characterKey, actorId, `Cast bound: ${characterKey} → actor ${actorId}`)
    .catch(err => console.warn('[CastBind] Stale propagation failed:', err));
