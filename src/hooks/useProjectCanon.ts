...
import { propagateCanonChange } from '@/lib/visual-truth-staleness';
...
export function useProjectCanon(projectId: string | undefined) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (merged) => {
      ...
      if (projectId) {
        propagateCanonChange(projectId, 'Canon updated')
          .catch(err => console.warn('[Canon] Stale propagation failed:', err));
      }
    },
    onError: (err: any) => {
