...
const FUNC_BASE = `/api/supabase-proxy/functions/v1/narrativebeat-atomiser`;
...
export function useNarrativebeatAtoms({ projectId, enabled = true }: UseNarrativebeatAtomsOptions): UseNarrativebeatAtomsReturn {
  ...
  const startPoll = useCallback((delayMs = 5000) => {
    stopPoll();
    const tick = async () => { setIsRefreshing(true); const current = await fetchStatus(); setIsRefreshing(false); if (current.some((a: NarrativebeatAtom) => a.generation_status === 'pending' || a.generation_status === 'generating' || a.generation_status === 'running')) { pollTimer.current = setTimeout(tick, delayMs); } else { setIsGenerating(false); } };
    pollTimer.current = setTimeout(tick, delayMs);
  }, [fetchStatus, stopPoll]);
  useEffect(() => { if (!enabled || !projectId) return; setIsLoading(true); fetchStatus().then(current => { setIsLoading(false); if (current.some((a: NarrativebeatAtom) => a.generation_status === 'pending' || a.generation_status === 'generating' || a.generation_status === 'running')) startPoll(5000); }); return () => stopPoll(); }, [enabled, projectId, fetchStatus, startPoll, stopPoll]);
