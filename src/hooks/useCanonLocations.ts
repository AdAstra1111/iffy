...
import { propagateLocationChange } from '@/lib/visual-truth-staleness';
...
export function useCanonLocations(projectId: string | undefined) {
  ...
  const updateLocationMutation = useMutation({
    ...
    onSuccess: (params) => {
      qc.invalidateQueries({ queryKey: ['canon-locations', projectId] });
      // Propagate staleness for location-dependent visuals
      if (projectId && params.id) {
        propagateLocationChange(projectId, params.id, undefined, 'Location updated')
          .catch(err => console.warn('[Location] Stale propagation failed:', err));
      }
    },
  });
