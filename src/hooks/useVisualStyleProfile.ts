...
import { propagateVisualStyleChange } from '@/lib/visual-truth-staleness';
...
export function useVisualStyleProfile(projectId: string | undefined) {
  ...
  const save = useCallback(async (updates: Partial<VisualStyleProfile>) => {
    ...
    try {
      ...
      setIsAutoFilled(false);
      // Propagate staleness to dependent visual assets
      propagateVisualStyleChange(projectId, 'Visual style updated')
        .catch(err => console.warn('[VSAL] Stale propagation failed:', err));
      await load();
    } catch (err: any) {
