/**
 * useGenreAtoms — hook for genre atom lifecycle
 */
import { useCallback, useEffect, useRef, useState } from 'react';
import { supabase } from '@/integrations/supabase/client';
const FUNC_BASE = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/genre-atomiser`;
...
export interface GenreAtomAttributes {
  genreLabel: string; primaryGenre: string; secondaryGenres: string[];
  subgenreTags: string[]; genreMashupNotes: string; toneTags: string[];
  intendedRating: string; narrativeStructure: string; pacingClass: string[];
  settingClassification: string[]; comparableFilms: string[]; audienceProfile: string;
  genrePurity: string; genreTags: string[]; marketingClassification: string;
  confidence: number; readinessBadge: 'foundation' | 'rich' | 'verified'; generationStatus: string;
}
interface UseGenreAtomsOptions { projectId: string; enabled?: boolean; }
interface UseGenreAtomsReturn {
...
async function call(action: string, projectId: string): Promise<any> {
  ...
  if (!resp.ok) throw new Error(json?.error || `genre-atomiser ${action} failed: HTTP ${resp.status}`);
  if (json?.error) throw new Error(json.error);
  return json;
}
export function useGenreAtoms({ projectId, enabled = true }: UseGenreAtomsOptions): UseGenreAtomsReturn {
  const [atoms, setAtoms] = useState<GenreAtom[]>([]);
