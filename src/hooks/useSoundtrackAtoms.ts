...
export function useSoundtrackAtoms({ projectId, enabled = true }: UseSoundtrackAtomsOptions): UseSoundtrackAtomsReturn {
  ...
  const startPoll = useCallback((delayMs = 5000) => {
    stopPoll();
    const tick = async () => { setIsRefreshing(true); const current = await fetchStatus(); setIsRefreshing(false); if (current.some((a: SoundtrackAtom) => a.generation_status === 'pending' || a.generation_status === 'generating')) pollTimer.current = setTimeout(tick, delayMs); };
