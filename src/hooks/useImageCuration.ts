...
import { assertCharacterImageEligible } from '@/lib/images/characterImageEligibility';
import { assertPrimaryEligible, classifyPremiumImageQuality, isPremiumSection } from '@/lib/images/premiumQualityGate';

/** Map asset_group to canonical section key for gate context */
...
export function useImageCuration(projectId: string) {
   ...
   */
  const setCurationState = useCallback(async (imageId: string, state: CurationState, image?: Partial<ProjectImage>) => {
    if (updating) return;
    setUpdating(imageId);
    try {
      // ── IEL IDENTITY GATE (fail-closed): block approve on drift/blocked images ──
      if (state === 'active' && image) {
        const sectionKey = resolveGateSectionKey((image as any).asset_group || null);
        assertCharacterImageEligible(image as any, 'approve', sectionKey);
        // ── IEL PREMIUM GATE: block activation of premium-fail images in premium sections ──
        if (sectionKey && isPremiumSection(sectionKey)) {
          const quality = classifyPremiumImageQuality(image as any);
          if (quality.status === 'premium_fail') {
            throw new Error(`Cannot activate: ${quality.reasons.join('; ')}`);
          }
        }
      }
  ...
  const setPrimary = useCallback(async (image: ProjectImage) => {
    ...
    try {
      // ── IEL IDENTITY GATE (fail-closed): block setPrimary on drift/blocked images ──
      const sectionKey = resolveGateSectionKey(image.asset_group || null);
      assertCharacterImageEligible(image as any, 'set as primary', sectionKey);
      // ── IEL PRIMARY GATE (fail-closed): block setPrimary on low-quality images in premium sections ──
      assertPrimaryEligible(image as any, sectionKey);
