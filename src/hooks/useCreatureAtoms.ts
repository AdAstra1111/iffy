...
const FUNC_BASE = `/api/supabase-proxy/functions/v1/creature-atomiser`;
    ...
    (delayMs = 5000) => {
      ...
      const tick = async () => {
        ...
        const hasRunning = current.some(
          (a: CreatureAtom) => a.generation_status === 'pending' || a.generation_status === 'generating' || a.generation_status === 'running',
        );
        if (hasRunning) {
