...
const FUNC_BASE = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/structure-atomiser`;
...
export function useStructureAtoms({ projectId, enabled = true }: UseStructureAtomsOptions): UseStructureAtomsReturn {
  ...
  const startPoll = useCallback((delayMs = 5000) => {
    stopPoll();
    const tick = async () => { setIsRefreshing(true); const current = await fetchStatus(); setIsRefreshing(false); if (current.some((a: StructureAtom) => a.generation_status === 'pending' || a.generation_status === 'generating')) pollTimer.current = setTimeout(tick, delayMs); };
