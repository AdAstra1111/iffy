/**
 * useStructureAtoms — hook for structure atom lifecycle
 */
import { useCallback, useEffect, useRef, useState } from 'react';
import { supabase } from '@/integrations/supabase/client';
const FUNC_BASE = `/api/supabase-proxy/functions/v1/structure-atomiser`;

export interface StructureAtom {
  id: string; project_id: string; atom_type: 'structure'; entity_id: string | null;
  ...
  attributes: StructureAtomAttributes | null; created_at: string; updated_at: string;
}
export interface StructureAtomAttributes {
  structureType: string; actCount: number; actBreaks: string[];
  pageCountEstimate: number; sceneCountEstimate: number; midpointLocation: string;
  act1Summary: string; act2Summary: string; act3Summary: string; turningPoints: string[];
  midpointSignificance: string; structuralWeaknesses: string[]; setPieceCount: number;
  pacingAssessment: string; structuralTags: string[]; narrativeEngineeringScore: number;
  comparisonToParity: string; editoriallyCriticalPages: string[]; productionNotes: string;
  confidence: number; readinessBadge: 'foundation' | 'rich' | 'verified'; generationStatus: string;
}
interface UseStructureAtomsOptions { projectId: string; enabled?: boolean; }
interface UseStructureAtomsReturn {
  atoms: StructureAtom[]; isLoading: boolean; isRefreshing: boolean;
...
async function call(action: string, projectId: string): Promise<any> {
  ...
  if (!resp.ok) throw new Error(json?.error || `structure-atomiser ${action} failed: HTTP ${resp.status}`);
  if (json?.error) throw new Error(json.error);
  return json;
}
export function useStructureAtoms({ projectId, enabled = true }: UseStructureAtomsOptions): UseStructureAtomsReturn {
  const [atoms, setAtoms] = useState<StructureAtom[]>([]);
  ...
  const startPoll = useCallback((delayMs = 5000) => {
    stopPoll();
    const tick = async () => { setIsRefreshing(true); const current = await fetchStatus(); setIsRefreshing(false); if (current.some((a: StructureAtom) => a.generation_status === 'pending' || a.generation_status === 'generating' || a.generation_status === 'running')) { pollTimer.current = setTimeout(tick, delayMs); } else { setIsGenerating(false); } };
    pollTimer.current = setTimeout(tick, delayMs);
  }, [fetchStatus, stopPoll]);
  useEffect(() => { if (!enabled || !projectId) return; setIsLoading(true); fetchStatus().then(current => { setIsLoading(false); if (current.some((a: StructureAtom) => a.generation_status === 'pending' || a.generation_status === 'generating' || a.generation_status === 'running')) startPoll(5000); }); return () => stopPoll(); }, [enabled, projectId, fetchStatus, startPoll, stopPoll]);
