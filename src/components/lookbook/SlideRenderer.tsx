...
export function SlideRenderer({ slide, identity, slideIndex, totalSlides, deckFormat = 'landscape' }: SlideRendererProps) {
  ...
  /* Global orientation fix — ensures EXIF-rotated images display upright */
  const imgOrientationCSS = `
    .slide-content img { image-orientation: from-image !important; }
  `;
...
function KeyMomentsSlide({ slide, colors, titleStyle, baseStyle, fontBody, slideIndex, totalSlides, isPortrait }: SlideProps) {
  ...
  const sectionLabel = slide.title?.toLowerCase().includes('hero') ? 'Hero Frames' : slide.title?.toLowerCase().includes('poster') ? 'Hero Frames' : 'Key Moments';
