...
import {
  ...
  Lock, Unlock, Package, Wand2, Zap, CheckCheck, Download, User, Info,
} from 'lucide-react';
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-start gap-2 p-2.5 rounded-md border border-amber-500/40 bg-amber-500/10">
        ...
        <div>
          ...
          <p className="text-[9px] text-amber-600/80 dark:text-amber-400/70 leading-snug">
            Use the Visual Production Pipeline for canonical asset creation (Hero Frames → Production Design → Visual Language → Lookbook). This panel provides utility tools and legacy generation for advanced use only.
          </p>
        </div>
      </div>
      ...
      {/* ── Legacy Tools (Advanced) — collapsed by default ── */}
      <Collapsible>
        <CollapsibleTrigger className="flex items-center gap-2 w-full py-2 px-3 rounded-md border border-amber-500/30 bg-amber-500/5 hover:bg-amber-500/10 transition-colors text-left group">
          <AlertTriangle className="h-3.5 w-3.5 text-amber-500" />
          <span className="text-[10px] font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Legacy Tools (Advanced)</span>
          <ChevronRight className="h-3 w-3 text-amber-500 ml-auto group-data-[state=open]:rotate-90 transition-transform" />
        </CollapsibleTrigger>
      ...
      {/* ── Workflow Action Bar — utility actions only ── */}
      <Card className="border-border/40 bg-muted/10">
        <CardContent className="p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {/* 3. Download All — neutral utility */}
