...
import { GenerationProgressBar, type GenerationStage } from './GenerationProgressBar';
...
export function ActorCandidateReview({ actorId, versions, approvedVersionId, anchorCoverageStatus }: ActorCandidateReviewProps) {
  ...
  }>({ active: false, mode: 'exploratory', total: 0, completed: 0, failed: 0, stage: 'preparing' });
  ...
  const startGeneration = useCallback((genMode: 'exploratory' | 'reference_locked', count: number) => {
    const latestVersion = versions[versions.length - 1];
    if (!latestVersion) {
      toast.error('No version available to generate from');
      return;
    }
    setGenProgress({ active: true, mode: genMode, total: count, completed: 0, failed: 0, stage: 'preparing' });

    // Transition to "generating" after a short prep delay
    setTimeout(() => {
      setGenProgress(p => p.active ? { ...p, stage: 'generating' } : p);
    }, 800);

    generateScreenTest.mutate(
      { actorId, versionId: latestVersion.id, count, mode: genMode },
      {
        onSuccess: (data) => {
          ...
          queryClient.invalidateQueries({ queryKey: ['ai-actor', actorId] });
          // Auto-clear progress after brief display
          setTimeout(() => setGenProgress(p => p.stage === 'complete' ? { ...p, active: false } : p), 3000);
        },
        onError: () => {
          setGenProgress(p => ({ ...p, stage: 'error', active: false }));
          setTimeout(() => setGenProgress(p => p.stage === 'error' ? { ...p, active: false, completed: 0 } : p), 4000);
        },
      }
    );
  ...
  const handleCreateAnother = useCallback(() => {
    ...
    startGeneration(mode, 4);
  }, [coverageInsufficient, startGeneration]);

  const handleExploreOptions = useCallback(() => {
    startGeneration('exploratory', 3);
  }, [startGeneration]);
  ...
  if (isBlocked) {
    return (
      <div className="rounded-lg border border-border/40 bg-card p-6 space-y-5">
        ...
        {(genProgress.active || genProgress.stage === 'complete' || genProgress.stage === 'error') && (
          <GenerationProgressBar
            isActive={genProgress.active}
    ...
    <div className="space-y-4">
      ...
      {latestVersion && (
        <ConvergencePanel
          ...
          hasAnchors={!coverageInsufficient}
        />
      )}
    </div>
  );
