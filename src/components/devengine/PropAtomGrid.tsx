/**
 * PropAtomGrid — card grid + atomise controls for prop atoms
 *
...
import {
  Package, AlertTriangle, CheckCircle2, Loader2, RefreshCw, Sparkles, Clock
} from 'lucide-react';
import type { PropAtom, PropAtomAttributes } from '@/hooks/usePropAtoms';
import { PropAtomDetailDrawer } from './PropAtomDetailDrawer';

interface PropAtomGridProps {
  atoms: PropAtom[];
  isLoading: boolean;
...
function PropTypeBadge({ propType }: { propType: string }) {
  const map: Record<string, string> = {
    held: 'bg-blue-500/15 text-blue-400',
    set_dressing: 'bg-purple-500/15 text-purple-400',
    vehicle: 'bg-orange-500/15 text-orange-400',
    wardrobe_item: 'bg-pink-500/15 text-pink-400',
    weapon: 'bg-red-500/15 text-red-400',
    document: 'bg-yellow-500/15 text-yellow-400',
    technology: 'bg-cyan-500/15 text-cyan-400',
    food: 'bg-green-500/15 text-green-400',
    flora: 'bg-emerald-500/15 text-emerald-400',
    other: 'bg-muted text-muted-foreground',
  };
  const cls = map[propType] || map.other;
  const label = propType?.replace('_', ' ') || 'prop';
  return (
      ...
      <CardContent className="p-3 space-y-2.5">
        ...
        {a?.propType && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <PropTypeBadge propType={a.propType} />
            {a.sizeCategory && (
              <span className="text-xs text-muted-foreground capitalize">{a.sizeCategory}</span>
            )}
          </div>
        )}

        {/* Physical description excerpt */}
        ...
        {a?.productionComplexity && (
          <span className={`text-xs px-1.5 py-0.5 rounded border ${
            a.productionComplexity === 'complex' ? 'bg-red-500/15 text-red-400 border-red-500/30' :
            a.productionComplexity === 'moderate' ? 'bg-amber-500/15 text-amber-400 border-amber-500/30' :
...
export function PropAtomGrid({
  ...
  const [selected, setSelected] = useState<PropAtom | null>(null);
  ...
  const handleCardClick = (atom: PropAtom) => {
    setSelected(atom);
    setDrawerOpen(true);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={handleAtomise}
            disabled={isLoading || isRefreshing}
            className="h-8 text-xs shrink-0"
          >
            <Sparkles className="h-3 w-3 mr-1" />
            {totalCount === 0 ? 'Atomise Props' : 'Regenerate'}
          </Button>
      ...
      {isLoading && totalCount === 0 ? (
        ...
        <div className="text-center py-8 text-sm text-muted-foreground">
          ...
          <p>No prop atoms yet.</p>
          <p className="text-xs mt-1">Click "Atomise Props" to extract from your script.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {atoms.map((atom) => (
