...
interface ActionToolbarProps {
  ...
  /** Production activation: visual production eligible */
  visualActivationEligible?: boolean;
  /** Production activation: visual production already locked */
  visualProductionLocked?: boolean;
  /** Production activation: handler */
...
export function ActionToolbar({
  ...
  visualActivationEligible = false,
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex flex-wrap items-center gap-2 p-2 rounded-lg bg-muted/30 border border-border/50">
        ...
        {/* Production Activation — visual_project_bible (orthogonal to ladder) */}
        {visualProductionLocked && (
          <Badge className="h-8 px-3 text-xs gap-1.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
            <CheckCircle2 className="h-3 w-3" />
            Visual Production Activated
          </Badge>
        )}
        {!visualProductionLocked && visualActivationEligible && (
          <Button size="sm" className="h-8 text-xs gap-1.5 bg-emerald-600 hover:bg-emerald-700"
            onClick={onActivateVisualProduction}
            disabled={activatingVisualProduction || actionsDisabled}>
            {activatingVisualProduction ? <Loader2 className="h-3 w-3 animate-spin" /> : <Eye className="h-3 w-3" />}
            Activate Visual Production
          </Button>
        )}
