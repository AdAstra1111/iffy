/**
 * ActRewriteWorkspace — Act-based workspace UI for beat_sheet sectioned rewrite operations.
 *
 * Mirrors EpisodeRewriteWorkspace but for act units instead of episode units.
 *
 * ARCHITECTURE:
 *   - Top: scope summary (total / affected / preserved) + compact progress
 *   - Left: scrollable act navigator with strong visual status distinction
 *   - Right: reading pane for selected act content
 *   - Bottom: collapsible activity log (demoted from primary)
 *
 * NO scene nesting — just act → prose.
 *
 * SCOPE MATH RULE:
 *   - Progress denominator = affectedActs, never totalActs
 *   - Preserved acts are visually inert (not part of progress)
 */
...
import type { ActUnit, ActUnitStatus } from '@/hooks/useRewritePipeline';
import type { ActivityItem } from '@/components/devengine/ActivityTimeline';

/* ── Act Status Helpers ────────────────────────────────── */

function statusIcon(s: ActUnitStatus): React.ReactElement {
  switch (s) {
    case 'done':
...
function statusBadgeVariant(s: ActUnitStatus): string {
  switch (s) {
    case 'done':
...
function statusLabel(s: ActUnitStatus): string {
  switch (s) {
...
interface ActRewriteWorkspaceProps {
  actUnits: ActUnit[];
  progress: {
    percent: number;
    label: string;
...
export function ActRewriteWorkspace({
  actUnits,
  progress,
  smoothedPercent,
  etaMs,
  ...
  onRestart,
  error,
}: ActRewriteWorkspaceProps) {
  const [selectedAct, setSelectedAct] = useState<string | null>(null);
  const [autoFollow, setAutoFollow] = useState(true);
  const [logOpen, setLogOpen] = useState(false);

  // Auto-follow: track the currently rewriting act
  const rewritingAct = actUnits.find(u => u.status === 'rewriting');
  useEffect(() => {
    if (autoFollow && rewritingAct) {
      setSelectedAct(rewritingAct.sectionId);
    }
  }, [autoFollow, rewritingAct?.sectionId]);

  const handleSelectAct = (sectionId: string) => {
    setSelectedAct(sectionId);
    setAutoFollow(false);
  };

  const selectedUnit = actUnits.find(u => u.sectionId === selectedAct) ?? null;

  // ── Scope counts (truth) ──
  const doneCount = actUnits.filter(u => u.status === 'done').length;
  const failedCount = actUnits.filter(u => u.status === 'failed').length;
  const preservedCount = actUnits.filter(u => u.isPreserved).length;
  const affectedCount = actUnits.filter(u => !u.isPreserved).length;
  const totalCount = actUnits.length;
  const isSelectiveRewrite = preservedCount > 0;

  // Progress is always against AFFECTED acts, not total
  ...
  // Currently rewriting act label for header
  const rewritingLabel = rewritingAct
    ? `Rewriting ${rewritingAct.label} — ${doneCount} of ${affectedCount} affected done`
    : null;
  ...
  return (
    <div className="rounded-lg border border-border/40 bg-card overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-foreground">
              {pipelineStatus === 'planning' ? 'Planning Rewrite…'
                ...
                : pipelineStatus === 'complete'
                  ? (isSelectiveRewrite
                    ? `Selective Rewrite Complete — ${doneCount} of ${affectedCount} acts`
      ...
      {/* ── Main workspace: act list + reading pane ── */}
      <div className="flex" style={{ minHeight: 320, maxHeight: 480 }}>
        ...
        <ScrollArea className="w-[180px] shrink-0 border-r border-border/20">
          <div className="divide-y divide-border/10">
            {actUnits.map((unit) => {
              const isSelected = selectedAct === unit.sectionId;
              const isActive = unit.status === 'rewriting';
              return (
                <button
                  key={unit.sectionId}
                  onClick={() => handleSelectAct(unit.sectionId)}
                  className={cn(
...
function ActPane({ unit }: { unit: ActUnit }) {
  return (
    <div className="flex flex-col h-full">
      {/* Act Header */}
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border/15 bg-muted/15">
        {statusIcon(unit.status)}
        <span className="font-medium text-sm text-foreground">{unit.label}</span>
      ...
      {/* ── Act content (no scene nesting) ── */}
      <ScrollArea className="flex-1">
        <div className="px-3 py-2.5">
          {unit.status === 'rewriting' ? (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              <Loader2 className="h-5 w-5 animate-spin text-primary" />
              <p className="text-xs text-primary font-medium">{unit.label} — Rewriting…</p>
              <p className="text-[10px] text-muted-foreground/60">Content will appear when this act completes.</p>
            </div>
          ) : unit.status === 'queued' ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              <Clock className="h-5 w-5 text-amber-400/60" />
              <p className="text-xs text-amber-400 font-medium">{unit.label} — Queued for rewrite</p>
              <p className="text-[10px] text-muted-foreground/60">
                This act is in the affected set and will be rewritten.
              </p>
            </div>
          ) : unit.status === 'preserved' ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              <ShieldCheck className="h-5 w-5 text-sky-400/60" />
              <p className="text-xs text-sky-400 font-medium">{unit.label} — Preserved unchanged</p>
              <p className="text-[10px] text-muted-foreground/60">
                This act was not in the affected set and retains its original content.
              </p>
              {unit.content && (
            ...
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              ...
              <p className="text-xs text-destructive font-medium">{unit.label} — Failed</p>
              {unit.content && (
