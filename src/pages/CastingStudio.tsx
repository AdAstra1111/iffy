 ...
 * 
 * Includes: Character Brief, Casting Assistant, Show More Options, Auto-Cast,
 * hard constraint enforcement, canonical scoring influence.
 */
...
import {
  ...
  Settings2, MoreHorizontal, Wand2,
} from 'lucide-react';
...
import {
  ...
  refinementToEdgePayload,
} from '@/components/casting/CastingAssistant';
...
export default function CastingStudio() {
  ...
  // Per-character refinement state
  const [refinements, setRefinements] = useState<Record<string, RefinementState>>({});
  const [assistantChar, setAssistantChar] = useState<string | null>(null);
  const [autoCastOpen, setAutoCastOpen] = useState(false);

  const getRefinement = (charName: string) => refinements[charName] || { ...EMPTY_REFINEMENT };
  const setCharRefinement = (charName: string, state: RefinementState) => {
    setRefinements(prev => ({ ...prev, [charName]: state }));
  };
  ...
  const handleGenerate = useCallback(async (
    ...
    charRefinements?: RefinementState,
  ) => {
    if (!projectId || !user?.id) return;
    setIsGenerating(true);
    if (characterFilter) setGeneratingChar(characterFilter);

    const label = explorationMode ? 'Exploring more options' : 'Generating casting options';
    const toastId = toast.loading(
      characterFilter ? `${label} for ${characterFilter}...` : `${label} for all characters...`,
      { duration: 120000 }
    );

    try {
      const existingIds = characterFilter
        ? candidates.filter(c => c.character_key === characterFilter).map(c => c.id)
        : [];

      const { data, error } = await supabase.functions.invoke('generate-casting-candidates', {
        body: {
          projectId,
          candidatesPerCharacter: explorationMode ? 6 : 4,
          characterFilter,
          explorationMode,
          refinements: charRefinements ? refinementToEdgePayload(charRefinements) : null,
      ...
      const constraintInfo = data.constraints_applied?.map((c: any) => {
        const parts = [];
        if (c.constraints.gender) parts.push(`gender: ${c.constraints.gender}`);
        if (c.constraints.age_range) parts.push(`age: ${c.constraints.age_range}`);
        return parts.length ? `${c.character} [${parts.join(', ')}]` : null;
      }).filter(Boolean);

      toast.success(
        `Generated ${data.generated} candidate${data.generated !== 1 ? 's' : ''} across ${data.characters} character${data.characters !== 1 ? 's' : ''}${data.failed > 0 ? ` (${data.failed} failed)` : ''}${explorationMode ? ' (exploration)' : ''}`,
        { id: toastId }
      );
      invalidate();
    } catch (e: any) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="space-y-8">
          {Array.from(characterLanes.entries()).map(([charName, charCandidates]) => (
            <CharacterLane
              ...
              onExplore={() => handleGenerate(charName, true)}
              onAssistant={() => setAssistantChar(charName)}
              onGenerateWithRefinements={() => handleGenerate(charName, false, getRefinement(charName))}
              isGenerating={isGenerating && (generatingChar === null || generatingChar === charName)}
              refinementState={getRefinement(charName)}
            />
          ))}
        </div>
      )}
      ...
      {/* ── Casting Assistant Dialog ── */}
      {assistantChar && (
        <CastingAssistant
          characterName={assistantChar}
          state={getRefinement(assistantChar)}
          onChange={(state) => setCharRefinement(assistantChar, state)}
          onApply={() => handleGenerate(assistantChar, false, getRefinement(assistantChar))}
          open={!!assistantChar}
          onOpenChange={(open) => { if (!open) setAssistantChar(null); }}
        />
      )}
...
interface LaneProps {
  ...
  onExplore: () => void;
  onAssistant: () => void;
  onGenerateWithRefinements: () => void;
  isGenerating: boolean;
  refinementState: RefinementState;
}

function CharacterLane({
  characterName, projectId, candidates, allCandidates, onShortlist, onReject, onUndo,
  onExpand, onToggleCompare, compareIds, onDelete, onGenerate, onExplore, onAssistant,
  onGenerateWithRefinements, isGenerating, refinementState,
}: LaneProps) {
  ...
  const hasRefinements = hasActiveRefinements(refinementState);
  ...
  const stageMsg = STAGE_MESSAGES[characterName.length % STAGE_MESSAGES.length];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-[10px]">
            ...
            {hasRefinements && (
              <Badge variant="outline" className="h-5 text-[10px] text-primary border-primary/40">
                <Settings2 className="h-2.5 w-2.5 mr-0.5" /> Refined
              </Badge>
            )}
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            variant="ghost"
            className="h-7 text-[11px] gap-1"
            onClick={hasRefinements ? onGenerateWithRefinements : onGenerate}
            disabled={isGenerating}
          >
      ...
      {/* Character Brief */}
      <CharacterBrief projectId={projectId} characterKey={characterName} />

      {/* Momentum progress bar — visible during generation */}
      {isGenerating && (
        <div className="space-y-1.5 cast-reveal">
          <div className="h-1 rounded-full bg-muted/30 overflow-hidden">
            <div
              className="h-full rounded-full bg-primary/60 cast-momentum-bar transition-all duration-[8000ms] ease-out"
              style={{ width: '85%' }}
            />
          </div>
          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
            <Loader2 className="h-3 w-3 animate-spin text-primary/60" />
            <span className="cast-pulse-subtle">{stageMsg}</span>
          </div>
        </div>
      )}
        ...
        <div className="rounded-lg border border-dashed border-border/50 p-6 text-center">
          ...
          {allCandidates.length === 0 && (
            <p className="text-[10px] text-muted-foreground mt-1">
              Click "Generate" to create casting options with hard constraint enforcement
            </p>
          )}
        </div>
      ) : (
...
function CandidateCard({ candidate, index = 0, isComparing, onShortlist, onReject, onUndo, onExpand, onToggleCompare, onDelete }: CardProps) {
  ...
  const isKeeper = candidate.status === 'shortlisted';
  const [imgLoaded, setImgLoaded] = useState(false);

  return (
    <div
      className={cn(
        'group relative rounded-lg border overflow-hidden transition-all duration-200 cast-reveal',
        isComparing
          ? 'border-primary ring-2 ring-primary/30'
          : 'border-border/40 hover:border-border/80',
        candidate.status === 'rejected' && 'opacity-50',
        isKeeper && 'cast-keeper-glow',
      )}
    ...
    >
      ...
      <button onClick={onExpand} className="w-full aspect-[3/4] bg-muted/10 relative overflow-hidden">
        ...
        {candidate.generation_config?.hard_constraints && (
          <div className="absolute bottom-1.5 left-1.5">
            <Badge variant="outline" className="text-[8px] h-3.5 px-1 bg-background/80 backdrop-blur border-border/50">
              Constrained
            </Badge>
          </div>
        )}
      ...
      <div className="p-2 space-y-1.5">
        ...
        {candidate.generation_config?.variation && (
          <p className="text-[9px] text-muted-foreground truncate">{candidate.generation_config.variation}</p>
        )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {/* Constraint metadata */}
        {candidate.generation_config?.hard_constraints && (
          <div className="rounded-md bg-muted/30 p-2 text-[10px] space-y-0.5">
            <p className="font-medium text-foreground text-[11px]">Constraints Applied</p>
            {candidate.generation_config.hard_constraints.gender && (
              <p className="text-muted-foreground">Gender: {candidate.generation_config.hard_constraints.gender}</p>
            )}
            {candidate.generation_config.hard_constraints.age_range && (
              <p className="text-muted-foreground">Age: {candidate.generation_config.hard_constraints.age_range}</p>
            )}
            {candidate.generation_config.hard_constraints.ethnicity && (
              <p className="text-muted-foreground">Ethnicity: {candidate.generation_config.hard_constraints.ethnicity}</p>
            )}
          </div>
        )}
        ...
        {candidate.generation_config && Object.keys(candidate.generation_config).length > 0 && (
          <div className="rounded-md bg-muted/30 p-3 text-[10px] text-muted-foreground space-y-0.5">
            ...
            {candidate.generation_config.variation && <p>Style: {candidate.generation_config.variation}</p>}
            {candidate.generation_config.casting_note && <p>Direction: {candidate.generation_config.casting_note}</p>}
            {candidate.generation_config.model && <p>Model: {candidate.generation_config.model}</p>}
            {candidate.generation_config.exploration_mode && (
              <Badge variant="outline" className="text-[9px] h-4 mt-1">Exploration</Badge>
            )}
