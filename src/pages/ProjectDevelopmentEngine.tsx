...
export default function ProjectDevelopmentEngine() {
  ...
  const { data: project, isError: projectNotFound } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('projects')
        .select('title, format, development_behavior, episode_target_duration_seconds, episode_target_duration_min_seconds, episode_target_duration_max_seconds, season_episode_count, assigned_lane, budget_range, genres, comparable_titles, tone, target_audience, guardrails_config, source_pitch_idea_id, project_features')
        .eq('id', projectId!)
        .single();
      if (error) {
  ...
  // ── Production Activation Layer (orthogonal to ladder) ──
  const [activatingVisual, setActivatingVisual] = useState(false);
  const productionFlags = useMemo(
    () => readProductionFlags((project as any)?.project_features as Record<string, any> | null),
    [(project as any)?.project_features],
  );
  const visualActivationEligibility = useMemo(() => {
    const analysis = promotionGateAnalysis || latestAnalysis;
    if (!selectedDoc || !isProductionActivationDoc(selectedDoc.doc_type)) {
      return { eligible: false, reason: 'Not a production activation doc' };
    }
    return canActivateVisualProduction({
      docType: selectedDoc.doc_type!,
      ciScore: analysis?.ci_score ?? null,
      gpScore: analysis?.gp_score ?? null,
      blockers: analysis?.blocking_issues?.filter((n: any) => !n.apply_timing || n.apply_timing === 'now') || [],
      behavior: (analysis?.development_behavior as DevelopmentBehavior) || 'market',
    });
  }, [selectedDoc?.doc_type, promotionGateAnalysis, latestAnalysis]);

  const handleActivateVisualProduction = useCallback(async () => {
    if (!projectId) return;
    setActivatingVisual(true);
    try {
      await activateVisualProduction(projectId);
      toast.success('Visual Production activated — visual system locked');
      qc.invalidateQueries({ queryKey: ['dev-engine-project', projectId] });
    } catch (e: any) {
      toast.error(e.message || 'Activation failed');
    } finally {
      setActivatingVisual(false);
    }
  }, [projectId, qc]);
  ...
  const handleStaleRegenerate = async () => {
    ...
    try {
      ...
      if (selectedDoc.doc_type === 'idea') {
        ...
        toast.error('NEC is auto-generated by the development pipeline. Changes to qualifications trigger automatic regeneration — cannot be manually regenerated.');
        return;
      } else {
  ...
  const handleRewrite = async (decisions?: Record<string, string>, globalDirections?: any[]) => {
    ...
    try {
      ...
      // Character bibles: use per-character single-pass rewrite via rewrite.mutate()
      // This hits the backend's per-character rewrite (line 9355) which processes each
      // character independently with independent LLM calls. CharacterBibleProgress polls
      // the version's meta_json for real per-character progress (bg_generating flag).
              ...
              {!selectedDocId ? (
                ...
                <>
                  ...
                  <ActionToolbar
                    ...
                    visualActivationEligible={visualActivationEligibility.eligible}
                  ...
                  <VisualProductionActivatedCard
                    visible={!!productionFlags.visual_locked}
                    vpbMarkdown={selectedDoc?.doc_type === 'visual_project_bible' ? (selectedVersion as any)?.plaintext : null}
                    documentId={selectedDoc?.doc_type === 'visual_project_bible' ? selectedDocId : null}
                    versionId={selectedDoc?.doc_type === 'visual_project_bible' ? selectedVersionId : null}
                    projectId={projectId}
                    onVersionCommitted={(newVersionId) => {
                      setSelectedVersionId(newVersionId);
                    }}
                    onScrollToSection={(heading: string) => {
                      const textareas = document.querySelectorAll('textarea');
                      for (const ta of textareas) {
                        if (scrollTextareaToHeading(ta as HTMLTextAreaElement, heading)) break;
                      }
                    }}
                  />
                ...
                return (
                   <NotesPanel
                    ...
                    onResolveCarriedNote={async (noteId, action, extra, noteSnapshot) => {
                      ...
                      const resp = await fetch(`/api/supabase-proxy/functions/v1/resolve-carried-note`, {
                        method: 'POST',
