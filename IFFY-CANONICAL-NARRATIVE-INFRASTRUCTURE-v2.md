...
## III.1 System Architecture (May 22, 2026 -- after Red fixes)

IFFY: React/TypeScript SPA -> Supabase (Postgres + Edge Functions) -> Vercel

**Intake Pipeline (canonical order):**

  0. upload           -> storage:scripts
  1. ingest           -> script-intake:ingest_pdf
  2. project_create   -> supabase:projects
  3. scene_extract    -> dev-engine-v2:scene_graph_extract
  3.5 canonicalize    -> canonicalize-scene-substrate   [NEW Stage 3.5]
  4. entity_extract   -> entity-links-engine
  5. nit_dialogue     -> nit-sync:sync_dialogue_characters
  ...
  9. scene_index      -> extract-scene-index

**Key: Stage 3.5 (canonicalize) runs automatically on every script drop.**
It handles format-aware act assignment via proportional split (or beat
sheet if present), writes acts to scene_graph_order, and populates
provenance per-scene.

- **Forward pipeline:** DevSeed -> auto-run propagation engine -> compiled projections
- **Canonicalization pipeline:** Stage 3.5 -- fully wired, deployed, verified
- **Propagation engine:** auto-run/index.ts -- HTTP self-chaining, deterministic regeneration
- **Scene substrate:** scene_graph_scenes -- 6 enrichment columns, sluglines persist at extraction


## III.2 What Was Fixed (2026-05-22)

### Bug 1: scene_graph_atomic_write never persisted sluglines

What was broken: scene_graph_atomic_write (SQL RPC) parsed sluglines
via sgParseSlugline() and received them in the RPC payload, but only
wrote them to scene_graph_versions. NEVER wrote them to
scene_graph_scenes. The column did not exist on the table.

Fix: Migration 20260522134800 added 6 columns: slugline, act,
act_label, page_range_start, page_range_end, source_text_refs
(jsonb, default []). Updated scene_graph_atomic_write SQL RPC to
write slugline during atomic INSERT. Every extraction now persists
sluglines directly on the scene record.

### Bug 2: canonicalize-scene-substrate had no caller

What was broken: The edge function existed on main (built by Trinity
from Architect plan) but was NEVER wired into the intake pipeline.
It was deployable code with no caller.

Fix: Added canonicalize as Stage 3.5 in useScriptDropProject.ts
between scene_extract and entity_extract. Label:
'Canonicalizing scenes...'. Skip/fail lists updated for
non-screenplay docs. Function accepts projectId from POST body.

### Bug 3: verify_jwt=True blocked the function

What was broken: Deployed with JWT verification enabled. Frontend
proxy forwarded user tokens but gateway validated against function
even though function creates its own service-role client.

Fix: Redeployed with --no-verify-jwt. Created Vercel proxy at
api/supabase-proxy/functions/v1/canonicalize-scene-substrate.ts
+ vercel.json entry. Added to deploy.sh function list.

---

## III.3 Verified Data: Scene Enrichment Status (YETI)
...
## III.5 Current Compiled Projections
...
## III.7 Remaining Gaps

| Gap | Impact | Status |
|-----|--------|--------|
| YETI backfill -- pre-fix scenes lack sluglines on record | Verification gate for YETI needs re-extract | Pending decision |
| Page ranges not computed | No source localization in script | Not started |
| source_text_refs empty | Cannot trace scenes to source lines | Not started |
| Story outline has no act boundaries (22 moments unassigned) | Cannot verify per-act balance | Needs design |
| No scene-to-moment mapping | Cannot verify coverage | Needs design |
| No beat-to-scene mapping | Beats reference page numbers not scenes | Needs design |
| No verification gate tooling | Faithfulness checking is manual | Needs design |

---

## III.8 Terminology Map
...
### 1. Canonicalize Scene Substrate — Phase 2 ✅ COMPLETE

**Goal:** Persist sluglines, page ranges, and source_text_refs on scene_graph_scenes.

| Item | Status |
|------|--------|
| Schema migration (6 columns) | ✅ DONE |
| scene_graph_atomic_write persists slugline during INSERT | ✅ DONE |
| Stage 3.5 wired into intake pipeline | ✅ DONE |
| JWT fix (--no-verify-jwt) | ✅ DONE |
| Vercel proxy created | ✅ DONE |
| deploy.sh updated | ✅ DONE |
| Frontend built + deployed | ✅ DONE |
| Act assignment via format-aware split | ✅ DONE |
| Provenance on every scene record | ✅ DONE |

**Owner:** Red (completed)
**Depends on:** Nothing — fully deployed, live on every new script drop
**Note:** YETI was extracted before the fix. See YETI Backfill item below.

### 2. YETI Backfill (optional — depends on decision)

**Question:** Should we re-run the intake pipeline on YETI to populate sluglines/acts on its scene graph?

- Option A: Re-run intake (creates fresh scene graph with all enrichment)
- Option B: Run canonicalize-scene-substrate standalone for project be05e314
- Option C: Leave YETI as pre-fix data, focus iteration loop on new projects

**Owner:** Sebastian (decides)
...
### 4. Revision Propagation Entry Point
...
### 7. Intent Annotations (YETI Pilot)
...
### 8. Semantic Diffs (Early Prototype)
...
|------|------|-------|-------|
| 2026-05-22 | Schema migration — 6 columns on scene_graph_scenes | Red | slugline, act, act_label, page_range_start, page_range_end, source_text_refs |
| 2026-05-22 | scene_graph_atomic_write — persists slugline during atomic INSERT | Red | COALESCE(NULLIF(v_entry->>'slugline',''), NULL) |
| 2026-05-22 | canonicalize-scene-substrate — wired as Stage 3.5 in intake | Red | Between scene_extract and entity_extract (useScriptDropProject.ts) |
| 2026-05-22 | canonicalize-scene-substrate — JWT fix (--no-verify-jwt) | Red | Redeployed v8, function creates own service-role client |
| 2026-05-22 | canonicalize-scene-substrate — body-parsing fix | Red | Now accepts projectId from POST body |
| 2026-05-22 | Vercel proxy created | Red | api/.../canonicalize-scene-substrate.ts + vercel.json entry |
| 2026-05-22 | deploy.sh updated | Red | canonicalize-scene-substrate added to function deploy list |
| 2026-05-22 | Frontend built + deployed to production | Red | Stage 3.5 live |
| 2026-05-22 | Three-layer verification completed | Red | frontend 200, edge function (anon+service-role), SQL synced |
| 2026-05-22 | iffy-deployment-protocol skill created | Red | Single-entry-point deploy discipline: deploy.sh only |
| 2026-05-22 | Migration chain reconciler running | Red | 365/511 migrations tracked, ~146 remaining |
| 2026-05-22 | Act assignment via format-aware proportional split | Red | 83 scenes → 4 acts (or via beat sheet if present) |
| 2026-05-22 | Provenance on every scene record | Red | per-scene: act, scene_number, canonicalized_at, source_version_id |
| 2026-05-22 | Constitution v4 | ChatGPT + Oracle + Red + Sebastian | 12 Articles, 8 Invariants |
| 2026-05-22 | Architecture Whitepaper v1 | ChatGPT | 25 sections, two-pipeline model |
| 2026-05-22 | Working Plan v1 → v2 (this document) | Oracle | Updated with Red's fixes + complete pipeline status |

## V.2 In Progress 🚧

| Date | Item | Owner | Status |
|------|------|-------|--------|
| 2026-05-22 | Migration chain reconciler | Red | 365/511 tracked, iteratively applying remaining ~146 |
| 2026-05-22 | Scene-to-moment mapping design | Oracle (pending) | Waiting on roundtable decision |

## V.3 Blocked ⊘

| Date | Item | Owner | Blocked by |
|------|------|-------|------------|
| — | YETI iteration loop | TBD | Verification gate + YETI backfill decision |
| — | Verification gate | TBD | Scene mapping data model |
...
| Item | Priority | Notes |
|------|----------|-------|
| YETI backfill decision | High | Re-run intake or leave pre-fix data? |
| Verification gate | High | Moment-to-scene + beat-to-scene mapping |
| auto_approve_all mode | High | Needed for iteration loop |
| Scene-to-moment mapping | Medium | Depends on scene substrate stability |
| YETI backfill decision | High | Re-run intake or leave pre-fix?
| Verification gate | High | Moment-to-scene + beat-to-scene mapping |
| auto_approve_all mode | High | Needed for iteration loop |
...
| Feature script version | `c17d29ae` (v2) |
| Intake run | `8c67d0db`, completed, scene_count=83 |
| Canonicalization pass | v1, provenance populated 2026-05-22T11:16:28Z |

## Appendix B: Deployment Protocol

From Red's `iffy-deployment-protocol` skill:
- `deploy.sh` is the ONLY way to deploy
- Never deploy individual edge functions
- Never push SQL without running deploy.sh afterward
- Never skip the frontend build
- Three-layer verification before confirming: frontend 200, edge function (anon + service-role), SQL migration synced

---

## Appendix C: Open Questions for Next Roundtable
...
> The next step is still: **Apply the first iteration cycle to a real project.** That discipline determines whether this becomes real infrastructure or philosophical vapor.

---

*Constitutional version: v4 (2026-05-22)*
*Working plan version: v2 (2026-05-22)*
