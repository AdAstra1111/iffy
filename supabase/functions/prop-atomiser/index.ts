 ...
 * Domain mapping: D2 (prop) / C2 (upstream)
 */
function toCanonEmission(row: Record<string, unknown>): import("../_shared/atomiser-repository.ts").CanonEmission {
  ...
  return {
    ...
    provenance: {
      ...
      pcp_dependencies: ["profession_map", "period", "technology_level"],
    },
    cdg_context: {
      node_id: "D2",
      staleness: "FRESH",
    ...
    generated_by: "prop_atomiser_extract",
...
async function handleExtract(projectId: string) {
  ...
  for (let i = 0; i < toInsert.length; i += 50) {
    ...
    const result = await repo.upsertAtoms(projectId, batchEmissions, "prop");
    if (!result.success) {
      console.error("Insert batch error:", result.errors.join(", "));
      throw new Error(`Failed to insert prop atoms batch: ${result.errors.join(", ")}`);
    }
