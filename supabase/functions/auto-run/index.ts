...
const CI_WEIGHT = 2;
const GP_WEIGHT = 1;
function compositeScore(ci, gp) {
  return ci * CI_WEIGHT + gp * GP_WEIGHT;
}
...
async function hasQualifyingRewriteAttempt(supabase, jobId, currentDoc) {
  try {
    const { data: steps } = await supabase.from("auto_run_steps").select("action, document").eq("job_id", jobId).order("step_index", {
      ascending: true
    });
    if (!steps || steps.length === 0) return false;
    return steps.some((s)=>s.document === currentDoc && (s.action === "rewrite_created_new_version" || s.action === "rewrite_noop_same_content"));
  } catch  {
...
function extractCiGp(res) {
  ...
  const ciRaw = analysis?.ci_score ?? analysis?.scores?.ci_score ?? analysis?.scores?.ci ?? analysis?.ci ?? inner?.ci_score ?? inner?.ci ?? res?.ci_score ?? res?.ci ?? null;
  const gpRaw = analysis?.gp_score ?? analysis?.scores?.gp_score ?? analysis?.scores?.gp ?? analysis?.gp ?? inner?.gp_score ?? inner?.gp ?? res?.gp_score ?? res?.gp ?? null;
  return {
    ci: typeof ciRaw === "number" ? ciRaw : null,
    gp: typeof gpRaw === "number" ? gpRaw : null
  };
}
...
function parseVersionScores(metaJson) {
...
function pickBestScoredVersion(rows) {
...
async function getVersionScoreSnapshot(supabase, versionId) {
  const { data, error } = await supabase.from("project_document_versions").select("meta_json").eq("id", versionId).maybeSingle();
  if (error || !data) {
    return {
      ci: null,
      gp: null,
...
async function persistVersionScores(supabase, params) {
  const { versionId, ci, gp, source, jobId, protectHigher = true, docType, engagement } = params;
  const { data: existingRow, error: existingErr } = await supabase.from("project_document_versions").select("meta_json").eq("id", versionId).maybeSingle();
  if (existingErr || !existingRow) {
    throw new Error(`VERSION_SCORE_READ_FAILED: ${existingErr?.message || `version ${versionId} not found`}`);
  }
  const existingMeta = existingRow.meta_json && typeof existingRow.meta_json === "object" && !Array.isArray(existingRow.meta_json) ? existingRow.meta_json : {};
  ...
  const { error: updateErr } = await supabase.from("project_document_versions").update({
  ...
  try {
    const { data: verDoc } = await supabase.from("project_document_versions").select("document_id, project_documents!inner(project_id)").eq("id", versionId).maybeSingle();
    const projId = verDoc?.project_documents?.project_id;
    if (projId) {
      await emitTransition(supabase, {
        ...
        resultingState: {
          ci: effectiveCi,
          gp: effectiveGp,
          downgraded_blocked: downgradedBlocked,
          score_source: source
        }
      }, {
        critical: false
...
async function getCurrentVersionForDoc(supabase, documentId) {
  const { data } = await supabase.from("project_document_versions").select("id, plaintext, meta_json").eq("document_id", documentId).eq("is_current", true).eq("approval_status", "approved").order("version_number", {
    ascending: false
  }).limit(1).maybeSingle();
  return data || null;
}
...
async function completionGate(supabase, projectId, targetDocument, format, jobId) {
  ...
  {
    const { data: targetDoc1 } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", targetDocument).order("created_at", {
      ascending: false
    }).limit(1).maybeSingle();
    if (!targetDoc1) {
    ...
    const currentVer1 = await getCurrentVersionForDoc(supabase, targetDoc1.id);
    ...
    const { data: _jobRow } = await supabase.from("auto_run_jobs").select("allow_defaults, pinned_inputs").eq("project_id", projectId).order("created_at", {
      ascending: false
    }).limit(1).maybeSingle();
    const _skipCanon = _jobRow?.allow_defaults === true || _jobRow?.pinned_inputs?.skip_canon_alignment === true;
    if (!_skipCanon && currentVer1.plaintext) {
      const alignment = await runCanonAlignmentGate(supabase, projectId, currentVer1.plaintext);
  ...
  // ── Gate 4: Defensibility Validator ────────────────────────────────────────────
  {
    ...
    {
      const { data: approvedVersion } = await supabase.from("project_document_versions").select("id, version_number, ci, gp").eq("document_id", targetDoc.id).eq("approval_status", "approved").maybeSingle();
    ...
    {
      const { data: historicalVersions } = await supabase.from("project_document_versions").select("id, version_number, ci").eq("document_id", targetDoc.id).neq("id", currentVer?.id).order("version_number", {
        ascending: false
      }).limit(10);
      const bestHistCi = (historicalVersions || []).reduce((best, v)=>Math.max(best, v.ci ?? -1), -1);
      ...
      if (ladder) {
        ...
        if (targetIdx > 0) {
          ...
          for (const req of requiredDocs){
            const { data: doc1 } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", req).maybeSingle();
            if (!doc1) {
              missing.push(req);
              continue;
            }
            const { data: ver } = await supabase.from("project_document_versions").select("plaintext").eq("document_id", doc1.id).eq("is_current", true).eq("approval_status", "approved").maybeSingle();
    ...
    if (jobId) {
      const { data: jobHistory } = await supabase.from("auto_run_step_history").select("id, ci_after, stage_document").eq("job_id", jobId).order("created_at", {
        ascending: false
      }).limit(20);
      const sameStageSteps = (jobHistory || []).filter((s)=>s.stage_document === targetDocument).slice(0, 6);
      if (sameStageSteps.length >= 3) {
        const ciHistory = sameStageSteps.map((s)=>s.ci_after ?? 0);
        let declining = true;
        for(let i = 0; i < ciHistory.length - 1; i++){
          if (ciHistory[i] >= ciHistory[i + 1]) {
            declining = false;
            break;
          }
        }
    ...
    {
      const { data: downstreamJobs } = await supabase.from("auto_run_jobs").select("id, target_document, status").eq("project_id", projectId).in("status", [
        "running",
        "completed",
        "paused"
      ]).limit(10);
      ...
      if (targetIdx >= 0 && ladder && downstreamJobs && downstreamJobs.length > 0) {
        const downstreamStages = downstreamJobs.filter((j)=>ladder.indexOf(j.target_document) > targetIdx).map((j)=>j.target_document);
...
const FORMAT_LADDERS = STAGE_LADDERS.FORMAT_LADDERS;
const DOC_TYPE_ALIASES = STAGE_LADDERS.DOC_TYPE_ALIASES;
const FORMAT_SPECIFIC_ALIASES = STAGE_LADDERS.FORMAT_SPECIFIC_ALIASES || {};
...
// This runs once at module load time — prevents silent runtime crashes.
{
  ...
  const allLadderTypes = new Set(Object.values(FORMAT_LADDERS).flat());
  const missing = [
    ...allLadderTypes
  ].filter((dt)=>!registryKeys.has(dt));
  if (missing.length > 0) {
...
const ALL_STAGES = new Set(Object.values(FORMAT_LADDERS).flat());
...
async function getStageBestFromDB(supabase, projectId, docType) {
  const { data: doc1 } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", docType).order("created_at", {
    ascending: false
  }).limit(1).maybeSingle();
  if (!doc1) return null;
  const { data: versions } = await supabase.from("project_document_versions").select("id, meta_json, version_number, approval_status, is_current").eq("document_id", doc1.id).order("version_number", {
    ascending: false
  });
  ...
  const scored = versions.map((v)=>{
    ...
    return {
      ...
      ci: p.ci ?? 0,
      gp: p.gp ?? 0,
      score: compositeScore(p.ci ?? 0, p.gp ?? 0),
      approval_status: v.approval_status
    };
  }).filter((v)=>v.ci > 0 || v.gp > 0).sort((a, b)=>b.score - a.score);
  if (scored.length === 0) return null;
  const best = scored.find((v)=>v.approval_status === "approved") || scored[0];
...
async function getStageBestFromSteps(supabase, jobId, docType, projectId = null) {
  ...
  const CI_SCORED_ACTIONS = [
    "review",
    "rewrite_accepted"
  ];
  const { data: reviewSteps } = await supabase.from("auto_run_steps").select("step_index, ci, gp, gap, output_ref, action").eq("job_id", jobId).eq("document", docType).in("action", CI_SCORED_ACTIONS).not("ci", "is", null).order("step_index", {
    ascending: false
  }).limit(200);
  if (!reviewSteps || reviewSteps.length === 0) {
    if (projectId) {
      ...
      if (dbBest) {
        ...
        return {
          version_id: dbBest.version_id,
          ci: dbBest.ci,
          gp: dbBest.gp,
          gap: null,
          score: dbBest.score,
          step_index: -1,
          versions_considered: 1,
          version_id_source: "meta_json_fallback"
        };
      }
    }
  ...
  function resolveVersionId(step) {
    const ref = step.output_ref;
    if (ref?.output_version_id) return {
      id: ref.output_version_id,
      source: "output_version_id"
    };
    if (ref?.version_id) return {
      id: ref.version_id,
      source: "version_id"
    };
    if (ref?.input_version_id) return {
      id: ref.input_version_id,
      source: "input_version_id"
    };
    return null;
  }
  ...
  for (const step of reviewSteps){
    ...
    if (!byVersion.has(resolved.id) || step.step_index > byVersion.get(resolved.id).step_index) {
      byVersion.set(resolved.id, {
        ...step,
        _vid: resolved.id,
        _vsource: resolved.source
      });
    }
  }
  if (byVersion.size === 0) {
  ...
  if (projectId) {
    ...
    if (dbBest && dbBest.score > result.score) {
      ...
      return {
        ...
        versions_considered: result.versions_considered + 1,
        version_id_source: "meta_json_merged"
      };
    }
  }
...
const PREREQUISITE_GATES = {
  vertical_episode_beats: {
    prerequisite: "episode_grid",
    ci: 80,
    gp: 80,
    composite: 170
  },
  season_script: {
    prerequisite: "vertical_episode_beats",
    ci: 85,
    gp: 85,
    composite: 178
  }
};
async function checkPrerequisiteGate(supabase, jobId, currentDoc, format, job) {
  const NOT_BLOCKED = {
    blocked: false,
    prerequisiteStage: "",
    ci: 0,
...
async function enforcePrereqGateBeforeAdvance(supabase, jobId, targetStage, format, job, stepCount, currentDoc, trigger) {
  ...
  await logStep(supabase, jobId, stepCount + 1, targetStage, "prereq_gate_blocked", `Advance to "${targetStage}" blocked (trigger: ${trigger}): prerequisite "${prereq.prerequisiteStage}" composite ${prereq.composite} < required ${prereq.requiredComposite}. Redirecting to ${prereq.prerequisiteStage}.`, {
    ci: prereq.ci,
    gp: prereq.gp
  }, undefined, {
    prerequisite_stage: prereq.prerequisiteStage,
    composite: prereq.composite,
    required_ci: gate?.ci,
    required_gp: gate?.gp,
    required_composite: prereq.requiredComposite,
    trigger
  });
  await logStep(supabase, jobId, stepCount + 2, prereq.prerequisiteStage, "prereq_block_redirect", `Redirected from "${targetStage}" to prerequisite "${prereq.prerequisiteStage}" due to quality gate (trigger: ${trigger}).`, {}, undefined, {
    from: targetStage,
    to: prereq.prerequisiteStage,
    trigger
  });
...
/** Returns the effective CI+GP gate thresholds for a given doc type and job. */ function resolveDocTypeGates(job, docType) {
  if (DOC_TYPE_GATE_OVERRIDES[docType]) return DOC_TYPE_GATE_OVERRIDES[docType];
  return {
    ci: resolveTargetCI(job),
    gp: 0
 ...
 */ function isExceptionalObjective(_job) {
  return true; // Max Quality — all jobs behave as maximize-quality
}
function extractTargetGP(job) {
  const ct = job?.converge_target_json;
  if (ct !== null && ct !== undefined && typeof ct === "object") {
 ...
 */ async function evaluateCIGate(supabase, jobId, docType, targetCi = GLOBAL_MIN_CI, projectId = null, job) {
  ...
  if (projectId && stageBest?.version_id) {
    try {
      const { data: versionRecord } = await supabase.from("project_document_versions").select("meta_json").eq("id", stageBest.version_id).maybeSingle();
      ...
      if (meta?.canon_drift?.passed === false) {
        ...
        return {
          ...
          gp,
          bestCiSoFar: ci,
          failReason: "canon_drift violations detected"
        };
      }
    } catch (cdErr) {
  ...
  if (override) {
    ...
    const failReason = !ciPass && !gpPass ? `CI ${ci} < ${override.ci} and GP ${gp} < ${override.gp}` : !ciPass ? `CI ${ci} < ${override.ci}` : `GP ${gp} < ${override.gp}`;
    ...
    return {
      ...
      gp,
      bestCiSoFar: ci,
      failReason: pass ? undefined : failReason
    };
  }
  // Default: CI-only gate
  return {
 ...
 */ async function checkEngagementGate(supabase, projectId, docType, versionId) {
  if (!versionId) return true; // no version = skip gate
  const { data: engagementRows } = await supabase.from("scene_engagement_scores").select("total_score, prediction_source").eq("document_version_id", versionId);
  ...
  const avgTotal = Math.round(engagementRows.reduce((s, r)=>s + (r.total_score || 0), 0) / engagementRows.length);
 ...
 */ async function checkMonotonicCIImprovement(supabase, jobId, docType, targetCi = GLOBAL_MIN_CI, projectId = null) {
  // ── IEL: Include both 'review' AND 'rewrite_accepted' steps as CI data points ──
  ...
  const { data: recentReviews } = await supabase.from("auto_run_steps").select("ci, step_index, action").eq("job_id", jobId).eq("document", docType).in("action", CI_SCORED_ACTIONS).not("ci", "is", null).order("step_index", {
    ascending: false
  ...
  if (!recentReviews || recentReviews.length < 2) {
    ...
    return {
      ...
      plateau: false,
      plateauCount: 0,
      bestCi: recentReviews?.[0]?.ci ?? 0,
      currentCi: recentReviews?.[0]?.ci ?? 0
    };
  }
  const currentCi = recentReviews[0].ci;
  const allCIs = recentReviews.map((r)=>r.ci);
...
async function needsFreshReview(supabase, jobId, job, currentDoc) {
  const { data: docRow } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
    ascending: false
  }).limit(1).maybeSingle();
  if (!docRow) return {
    needed: false,
  ...
  const { data: lastScoredStep } = await supabase.from("auto_run_steps").select("output_ref, action").eq("job_id", jobId).eq("document", currentDoc).in("action", [
    "review",
    "rewrite_accepted"
  ]).not("ci", "is", null).order("step_index", {
    ascending: false
  }).limit(1).maybeSingle();
  const lastReviewedVersionId = lastScoredStep?.output_ref?.input_version_id || lastScoredStep?.output_ref?.version_id || lastScoredStep?.output_ref?.output_version_id || null;
  if (activeVersionId !== lastReviewedVersionId) {
    return {
      needed: true,
      activeVersionId,
      activeSource,
      lastReviewedVersionId,
...
function getEffectiveVersionCap(job) {
...
function ielValidateTarget(rawTarget, format) {
...
function getLadderForJob(format) {
...
function canonicalDocType(raw, format) {
...
function nextDoc(current, format) {
  const ladder = getLadderForJob(format);
...
async function isSeedCoreOfficial(supabase, projectId) {
  const { data: docs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", projectId).in("doc_type", SEED_DOC_TYPES);
  ...
  const { data: versions } = await supabase.from("project_document_versions").select("document_id, approval_status").in("document_id", docIds).eq("is_current", true);
  const approvedDocIds = new Set((versions || []).filter((v)=>v.approval_status === "approved").map((v)=>v.document_id));
  const unapproved = [];
...
const INPUT_DOC_TYPES = [
  "idea",
  "concept_brief",
  ...SEED_DOC_TYPES
];
/** Check if plaintext contains stub markers */ function containsStubMarker(text) {
...
async function getDocCharCounts(supabase, projectId, docTypes) {
  const { data: docs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", projectId).in("doc_type", docTypes);
  const docMap = new Map();
  for (const d of docs || []){
    if (!docMap.has(d.doc_type)) docMap.set(d.doc_type, d.id);
  }
  ...
  let versions = [];
  if (docIds.length > 0) {
    const { data: vers } = await supabase.from("project_document_versions").select("document_id, plaintext").in("document_id", docIds).eq("is_current", true);
    versions = vers || [];
  }
  return docTypes.map((dt)=>{
    const docId = docMap.get(dt);
    const ver = docId ? versions.find((v)=>v.document_id === docId) : null;
...
/** Check if project inputs are sufficient to proceed with auto-run */ function checkInputReadiness(counts) {
  const missing = [];
  ...
  const summary = missing.length > 0 ? `INPUT_INCOMPLETE | missing=${missing.join(", ")}` : "inputs_ready";
  return {
...
async function attemptAutoRegenInputs(supabase, supabaseUrl, token, jobId, stepIndex, currentDoc, projectId, insufficients, trigger) {
  try {
    const regenResp = await fetch(`${supabaseUrl}/functions/v1/dev-engine-v2`, {
      ...
      body: JSON.stringify({
        ...
        projectId,
        dryRun: false,
        userId: _requestScopedUserId || undefined
      })
    });
    ...
    await logStep(supabase, jobId, stepIndex, currentDoc, "auto_regen_inputs", ok ? `Auto-regenerated ${regeneratedCount} docs` : `Auto-regeneration attempted but failed (${regenResp.status})`, {}, undefined, {
      trigger,
      insufficients,
      regen_result: regenResult,
      http_status: regenResp.status
    });
    if (!ok) {
      const err = regenResult?.error || `HTTP ${regenResp.status}`;
      console.error("[auto-run] auto-regen failed", {
        jobId,
        trigger,
        err
      });
    ...
    console.log("[auto-run] auto-regen result", {
      jobId,
      trigger,
      regenerated: regeneratedCount,
      skipped: regenResult?.skipped?.length || 0
    });
    return {
      ok: true,
      regenResult
    ...
    await logStep(supabase, jobId, stepIndex, currentDoc, "auto_regen_inputs", `Auto-regeneration threw error: ${err}`, {}, undefined, {
      trigger,
      insufficients,
      error: err
    });
    console.error("[auto-run] auto-regen threw", {
      jobId,
      trigger,
      err
    });
 ...
 */ async function ensureSeedPack(supabase, supabaseUrl, projectId, token, userId) {
  const { data: existingDocs } = await supabase.from("project_documents").select("doc_type").eq("project_id", projectId).in("doc_type", SEED_DOC_TYPES);
  const existingSet = new Set((existingDocs || []).map((d)=>d.doc_type));
  const missing = SEED_DOC_TYPES.filter((dt)=>!existingSet.has(dt));
  if (missing.length === 0) {
    // Verify all have current versions with non-empty plaintext
    const { data: verifiedDocs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", projectId).in("doc_type", SEED_DOC_TYPES);
    const docIds = (verifiedDocs || []).map((d)=>d.id);
    if (docIds.length > 0) {
      const { data: currentVersions } = await supabase.from("project_document_versions").select("document_id, plaintext").in("document_id", docIds).eq("is_current", true);
      const MIN_SEED_CHARS = 20;
      const docsWithContent = new Set((currentVersions || []).filter((v)=>v.plaintext && v.plaintext.trim().length >= MIN_SEED_CHARS).map((v)=>v.document_id));
      // Docs that exist but have no current version at all
      const trulyMissing = SEED_DOC_TYPES.filter((dt)=>{
        const docId = (verifiedDocs || []).find((d)=>d.doc_type === dt)?.id;
        if (!docId) return true;
        // Check if doc has ANY current version (even short)
        const hasCurrentVersion = (currentVersions || []).some((v)=>v.document_id === docId);
        return !hasCurrentVersion;
      });
      // Docs that have a current version but are too short (warning only)
      const shortDocs = SEED_DOC_TYPES.filter((dt)=>!trulyMissing.includes(dt)).filter((dt)=>{
        const docId = (verifiedDocs || []).find((d)=>d.doc_type === dt)?.id;
        return docId && !docsWithContent.has(docId);
      }).map((dt)=>{
        const docId = (verifiedDocs || []).find((d)=>d.doc_type === dt)?.id;
        const ver = (currentVersions || []).find((v)=>v.document_id === docId);
        return {
          doc_type: dt,
          reason: "too_short",
          chars: ver?.plaintext?.trim()?.length || 0
        };
      });
      if (trulyMissing.length > 0) {
        ...
        return {
          ...
          failed: true,
          fail_type: "SEED_PACK_INCOMPLETE",
          error: `Seed docs missing current version: ${trulyMissing.join(", ")}`,
          warnings: shortDocs
        };
      }
      ...
      return {
        ...
        missing: [],
        failed: false,
        warnings: shortDocs.length > 0 ? shortDocs : undefined
      };
    }
  ...
  const { data: project } = await supabase.from("projects").select("title, format, assigned_lane").eq("id", projectId).single();
  let pitch = project?.title || "Untitled project";
  const { data: ideaDocs } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", "idea").order("created_at", {
    ascending: false
  }).limit(1);
  if (ideaDocs?.[0]) {
    const { data: currentVer1 } = await supabase.from("project_document_versions").select("plaintext").eq("document_id", ideaDocs[0].id).eq("is_current", true).eq("approval_status", "approved").limit(1).single();
    const ideaText = currentVer1?.plaintext || "";
  ...
  console.log("[auto-run] calling generate-seed-pack", {
    projectId,
    lane
  });
  ...
  try {
    const { data: foundationDocs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", projectId).in("doc_type", [
      "beat_sheet",
      "treatment",
      "character_bible",
      "story_outline"
    ...
    if (foundationDocs && foundationDocs.length > 0) {
      const ctxDocIds = foundationDocs.map((d)=>d.id);
      const { data: ctxVersions } = await supabase.from("project_document_versions").select("document_id, plaintext").in("document_id", ctxDocIds).eq("is_current", true).eq("approval_status", "approved");
      contextDocuments = foundationDocs.map((d)=>{
        const ver = (ctxVersions || []).find((v)=>v.document_id === d.id);
        return ver ? {
          doc_type: d.doc_type,
  ...
  while(attempt < MAX_SEED_ATTEMPTS){
    ...
    try {
      ...
      console.log("[auto-run] generate-seed-pack http", {
        status: seed_http_status,
        snippet
      });
      if (!seedRes.ok) {
      ...
      if (!seedResult.success) {
        ...
        const truncErr = (seedResult.error || "generate-seed-pack returned success=false").slice(0, 300);
        return {
          ensured: true,
          missing,
          failed: true,
      ...
      const truncErr = (e.message || "Unknown error").slice(0, 300);
  ...
  const { data: postDocs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", projectId).in("doc_type", SEED_DOC_TYPES);
  const postDocIds = (postDocs || []).map((d)=>d.id);
  const { data: postVersions } = postDocIds.length > 0 ? await supabase.from("project_document_versions").select("document_id, plaintext").in("document_id", postDocIds).eq("is_current", true) : {
    data: []
  };
  ...
  const trulyMissingPost = SEED_DOC_TYPES.filter((dt)=>{
    const doc1 = (postDocs || []).find((d)=>d.doc_type === dt);
    if (!doc1) return true;
    const hasCurrentVersion = (postVersions || []).some((v)=>v.document_id === doc1.id);
    return !hasCurrentVersion;
  });
  // Short docs (warning only)
  const shortDocsPost = SEED_DOC_TYPES.filter((dt)=>!trulyMissingPost.includes(dt)).filter((dt)=>{
    const doc1 = (postDocs || []).find((d)=>d.doc_type === dt);
    const ver = (postVersions || []).find((v)=>v.document_id === doc1?.id);
  ...
  if (trulyMissingPost.length > 0) {
    ...
    return {
      ensured: true,
      missing: trulyMissingPost,
      failed: true,
      fail_type: "SEED_PACK_INCOMPLETE",
      error: `Seed pack missing after generation: ${trulyMissingPost.join(", ")}`,
      warnings: shortDocsPost,
      seed_debug: {
        http_status: seed_http_status,
        parsed_success: true,
        attempt,
        post_verify_missing: trulyMissingPost
      }
    };
  }
  ...
  return {
    ...
    missing: [],
    failed: false,
    warnings: shortDocsPost.length > 0 ? shortDocsPost : undefined,
    seed_debug: {
      http_status: seed_http_status,
      parsed_success: true,
      attempt
...
const FEATURE_MIN_WORDS = {
  feature_script: 19800,
  production_draft: 19800
};
function isDownstreamDocSufficient(docType, plaintext, _approvalStatus) {
 ...
 */ async function autoGenerateNEC(supabase, job, jobId, format, gateLabel, resolverHash) {
  const necSlot = await ensureDocSlot(supabase, job.project_id, job.user_id, "nec", {
    source: "generated",
    docRole: "creative_primary"
  });
  const { data: freshCanon } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
  const fc = freshCanon?.canon_json || {};
  const necJson = {
    nec_version: "1.0",
    format_profile: {
  ...
  await createVersion(supabase, {
    ...
    metaJson: {
      nec_json: necJson,
      generated_by: "auto-run-prewrite-setup",
      stale_regen: !!resolverHash
    },
    inputsUsed: {
 ...
 */ async function nextUnsatisfiedStage(supabase, projectId, format, currentStage, targetStage, allowDefaults = false, userId, jobId) {
  const ladder = getLadderForJob(format);
  if (!ladder) return null;
  const currentIdx = ladder.indexOf(currentStage);
  if (currentIdx < 0) {
    // ARCHITECTURE-STRICT: unresolved stage must not silently advance
    console.error(`[auto-run][IEL] nextUnsatisfiedStage UNRESOLVED — currentStage="${currentStage}" not in ${format} ladder=[${ladder.join(",")}]`);
    return null;
  }
  ...
  const { data: allDocs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", projectId);
  ...
  if (allDocIds.length > 0) {
    const { data: vers } = await supabase.from("project_document_versions").select("document_id, plaintext, approval_status, label, status, meta_json").in("document_id", allDocIds).eq("is_current", true);
    currentVersions = vers || [];
  }
  const versionByDocId = new Map();
  for (const v of currentVersions){
    versionByDocId.set(v.document_id, {
      plaintext: v.plaintext,
      approval_status: v.approval_status,
      label: v.label || null,
      status: v.status || null,
      bg_generating: v.meta_json?.bg_generating === true
    });
  }
  // ── REVIEWED-IN-JOB GATE: batch-fetch all reviewed stages for this job in one query ──
  let reviewedSet = new Set();
  if (jobId) {
    const { data: reviewedRows } = await supabase.from("auto_run_steps").select("document").eq("job_id", jobId).eq("action", "review");
    if (reviewedRows) {
      reviewedSet = new Set(reviewedRows.map((r)=>r.document));
    }
  }
      ...
      if (!hasApproved) {
        if (allowDefaults && userId) {
          ...
          for (const docId of docIds){
            const { data: curVer } = await supabase.from("project_document_versions").select("id, meta_json").eq("document_id", docId).eq("is_current", true).limit(1);
            const canonDrift = curVer?.[0]?.meta_json?.canon_drift;
            const hasViolations = canonDrift && canonDrift.passed === false && (canonDrift.violations > 0 || canonDrift.findings && canonDrift.findings.some((f)=>f.severity === "violation"));
...
function ladderIndexOf(d, format) {
  const ladder = getLadderForJob(format);
  return ladder ? ladder.indexOf(d) : -1;
}
function resolveTargetForFormat(targetDoc1, format) {
  if (isOnLadder(targetDoc1, format)) return targetDoc1;
  const ladder = getLadderForJob(format);
  if (!ladder || ladder.length === 0) return targetDoc1; // caller must have validated format already
  const fallback = ladder[ladder.length - 1];
  console.error(`[auto-run][IEL] resolveTargetForFormat FALLBACK — target "${targetDoc1}" not on ${format} ladder, falling back to "${fallback}". ladder=[${ladder.join(",")}]`);
  return fallback;
}
function isStageAtOrBeforeTarget(stage, targetDoc1, format) {
  ...
  const targetIdx = ladderIndexOf(resolveTargetForFormat(targetDoc1, format), format);
  return stageIdx >= 0 && stageIdx <= targetIdx;
}
/** Returns true if nextUnsatisfiedStage returned a background-generation-pending sentinel. */ function isGeneratingSentinel(stage) {
  return typeof stage === "string" && stage.startsWith("__generating__:");
}
/** Extracts the real stage name from a generating sentinel. */ function stageFromSentinel(stage) {
  return stage.replace(/^__generating__:/, "");
}
// ── Mode Config ──
const MODE_CONFIG = {
  fast: {
    max_stage_loops: DEFAULT_MAX_STAGE_LOOPS,
...
const FORMAT_DEFAULTS = {
  "vertical-drama": {
    episode_target_duration_min_seconds: 45,
    episode_target_duration_max_seconds: 90,
    episode_target_duration_seconds: 60,
    season_episode_count: 30
  },
  "limited-series": {
    episode_target_duration_min_seconds: 2700,
    episode_target_duration_max_seconds: 3600,
    episode_target_duration_seconds: 3300,
    season_episode_count: 8
  },
  "tv-series": {
    episode_target_duration_min_seconds: 2400,
    episode_target_duration_max_seconds: 3300,
    episode_target_duration_seconds: 2700,
    season_episode_count: 10
  },
  "anim-series": {
    episode_target_duration_min_seconds: 1200,
    episode_target_duration_max_seconds: 1500,
    episode_target_duration_seconds: 1320,
    season_episode_count: 10
  },
  "documentary-series": {
    episode_target_duration_min_seconds: 2400,
  ...
  "digital-series": {
    episode_target_duration_min_seconds: 480,
    episode_target_duration_max_seconds: 720,
    episode_target_duration_seconds: 600,
    season_episode_count: 10
  },
  "reality": {
    episode_target_duration_min_seconds: 2400,
  ...
  "film": {
    target_runtime_min_low: 85,
    target_runtime_min_high: 110
  },
  "anim-feature": {
    target_runtime_min_low: 80,
    target_runtime_min_high: 100
  },
  "short-film": {
    target_runtime_min_low: 5,
    target_runtime_min_high: 20
  }
};
...
function needsEpisodeQuals(format, _stageIdx) {
  return SERIES_FORMATS.includes(normalizeFormat(format));
}
async function resolveSeriesQualifications(supabase, projectId, format) {
  const fmt = normalizeFormat(format);
  const { data: project } = await supabase.from("projects").select("episode_target_duration_seconds, episode_target_duration_min_seconds, episode_target_duration_max_seconds, season_episode_count, guardrails_config").eq("id", projectId).single();
  if (!project) return {
    episode_target_duration_seconds: null,
    episode_target_duration_min_seconds: null,
    episode_target_duration_max_seconds: null,
  ...
  let canonDurMin = null;
  let canonDurMax = null;
  let canonDurLocked = false;
  let canonDurSource = null;
  try {
    const { data: canonRow } = await supabase.from("project_canon").select("canon_json").eq("project_id", projectId).maybeSingle();
  ...
  if (canonDurMin != null || canonDurMax != null) {
    durMin = canonDurMin;
    durMax = canonDurMax;
    durSource = canonDurSource || "canon";
...
function needsFilmQuals(format, stageIdx) {
  const filmFormats = [
    "film",
    "anim-feature",
    "short-film"
...
async function buildCriteriaSnapshot(supabase, projectId) {
  const { data: p } = await supabase.from("projects").select("format, assigned_lane, budget_range, development_behavior, episode_target_duration_seconds, episode_target_duration_min_seconds, episode_target_duration_max_seconds, season_episode_count, guardrails_config").eq("id", projectId).single();
  if (!p) return {};
...
function estimateDurationSeconds(documentText) {
  ...
  for (const line of lines){
    const t = line.trim();
    if (t.length === 0) {
      total += DURATION_BEAT_SEC;
      inDialogue = false;
      continue;
    }
    if (DURATION_SLUG_RE.test(t)) {
      total += DURATION_SLUGLINE_SEC;
      inDialogue = false;
      continue;
    }
    if (DURATION_PAREN_RE.test(t)) {
      total += DURATION_PAREN_SEC;
      continue;
    }
...
function computeCriteriaHashEdge(criteria) {
  const sorted = Object.keys(criteria).filter((k)=>criteria[k] != null && k !== 'updated_at').sort().map((k)=>`${k}=${JSON.stringify(criteria[k])}`).join('|');
  let hash = 5381;
...
function classifyCriteriaEdge(opts) {
  ...
  if (opts.versionCriteriaHash && opts.currentCriteriaHash && opts.versionCriteriaHash !== opts.currentCriteriaHash) {
    return {
      classification: 'CRITERIA_STALE_PROVENANCE',
      detail: `Criteria hash mismatch: ${opts.versionCriteriaHash} vs ${opts.currentCriteriaHash}`
    };
  }
  ...
  if (!isDurationEligibleDocType(opts.docType, opts.format)) {
    return {
      classification: 'OK',
      detail: `Duration check skipped: docType '${opts.docType ?? 'null'}' is not runtime-bearing (format=${opts.format ?? 'null'})`
    };
  }
  ...
  if (min > 0 || max > 0 && max < Infinity) {
    ...
    if (opts.measuredDurationSeconds < tolMin || opts.measuredDurationSeconds > tolMax) {
      ...
      return {
        ...
        detail: `Duration ${opts.measuredDurationSeconds}s vs target ${min}-${max}s (delta: ${delta > 0 ? '+' : ''}${delta}s)`
      };
    }
  }
  return {
...
async function runPreflight(supabase, projectId, format, currentDoc, allowDefaults = true) {
  const { data: project } = await supabase.from("projects").select("episode_target_duration_seconds, episode_target_duration_min_seconds, episode_target_duration_max_seconds, season_episode_count, assigned_lane, budget_range, guardrails_config").eq("id", projectId).single();
  if (!project) return {
    resolved: {},
    changed: false,
    missing_required: []
  ...
  function resolveValue(field, projectCol) {
  ...
  if (needsEpisodeQuals(format, stageIdx)) {
    ...
    if (!epCount) {
      if (allowDefaults && defaults.season_episode_count) {
        const newGc = updates.guardrails_config || {
          ...gc
        };
        newGc.overrides = newGc.overrides || {};
        newGc.overrides.qualifications = {
          ...newGc.overrides.qualifications || {},
          season_episode_count: defaults.season_episode_count
        };
...
function isQualificationError(msg) {
...
function pickNumberRaw(obj, paths) {
...
function pickNumber(obj, paths, fallback, riskFlags) {
...
function computePromotion(ci, gp, gap, trajectory, doc1, blockersCount, highImpactCount, iterationCount, allowDefaults = false, targetCi = GLOBAL_MIN_CI) {
  const w = WEIGHTS[doc1] || WEIGHTS.concept_brief;
  ...
  let readinessScore = Math.round(ci * w.ci + gp * w.gp + gapScore * w.gap + trajScore * w.traj + hiScore * w.hi - iterPenalty * w.pen);
  ...
  if ((doc1 === "idea" || doc1 === "concept_brief") && highImpactCount > 0) {
    if (allowDefaults && ci >= targetCi) {
      ...
      console.log(`[auto-run][IEL] early_stage_hi_gate_relaxed { doc: "${doc1}", highImpactCount: ${highImpactCount}, ci: ${ci}, allow_defaults: true }`);
...
let _requestScopedUserId = null;
// ── Helper: call another edge function (with retry on qualification errors) ──
async function callEdgeFunction(supabaseUrl, functionName, body, token, forwardUserId) {
  ...
  const enrichedBody = effectiveUserId ? {
    ...body,
  ...
  if (isHtml || !ct.includes("application/json") && raw.length > 0 && !raw.trimStart().startsWith("{")) {
    ...
    const userMsg = resp.status === 502 || resp.status === 503 || resp.status === 504 ? `${functionName} timed out (${resp.status}). Will retry.` : `${functionName} returned non-JSON (${resp.status}).`;
  ...
  try {
    ...
    throw Object.assign(new Error(`${functionName} returned invalid JSON (${resp.status}): ${raw.slice(0, 500)}`), {
      structured: true,
      code: "EDGE_FUNCTION_INVALID_JSON",
      status: resp.status,
      body: raw.slice(0, 1000)
    });
  }
...
async function callEdgeFunctionWithRetry(supabase, supabaseUrl, functionName, body, token, projectId, format, currentDoc, jobId, stepCount, forwardUserId) {
  try {
    ...
    if (preflight.changed) {
      await logStep(supabase, jobId, stepCount, currentDoc, "blockage_resolve", `Resolved missing qualifications: ${Object.keys(preflight.resolved).join(", ")}`);
    }
    // Retry once
    const result = await callEdgeFunction(supabaseUrl, functionName, body, token, forwardUserId);
    return {
...
const STEP_FAILURE_ACTIONS = new Set([
  "ci_gate_blocked",
  "ci_blocker_gate_blocked",
...
const STEP_START_ACTIONS = new Set([
  "start",
  "restart_from_stage",
  "seed_pack_ensured",
  "preflight_resolve",
...
async function logStep(supabase, jobId, stepIndex, document, action, summary, scores = {}, outputText, outputRef) {
  const idx = stepIndex !== null && stepIndex !== undefined ? stepIndex : await nextStepIndex(supabase, jobId);
  ...
  try {
    const { data: jobRow } = await supabase.from("auto_run_jobs").select("project_id").eq("id", jobId).maybeSingle();
    if (jobRow?.project_id) {
      ...
      const eventType = isFailed ? TRANSITION_EVENTS.AUTO_RUN_STEP_FAILED : isStarted ? TRANSITION_EVENTS.AUTO_RUN_STEP_STARTED : TRANSITION_EVENTS.AUTO_RUN_STEP_COMPLETED;
...
async function emitStageTransition(supabase, projectId, jobId, from, to, trigger, extra = {}) {
  try {
    await emitTransition(supabase, {
      projectId,
      ...
      critical: false
    }); // non-critical: stage transition already persisted in job state
  } catch (e) {
...
async function emitScoringTransition(supabase, projectId, jobId, docType, versionId, ci, gp, trigger) {
  try {
    await emitTransition(supabase, {
      projectId,
      eventType: TRANSITION_EVENTS.CI_GP_SCORES_COMPUTED,
 ...
 */ function noteRequiresHuman(note) {
  const sf = note?.suggested_fixes;
  const sfFlag = Array.isArray(sf) && sf.some((x)=>x?.requires_human === true) || sf && typeof sf === "object" && !Array.isArray(sf) && sf.requires_human === true;
  const detailFlag = typeof note?.detail === "string" && /requires_human\s*:\s*true|"requires_human"\s*:\s*true/i.test(note.detail);
 ...
 */ async function buildNoteDirectionsForRewrite(supabase, projectId, docType) {
  try {
    const { data: notes } = await supabase.from("project_notes").select("id, title, summary, detail, suggested_fixes, severity, category").eq("project_id", projectId).eq("doc_type", docType).in("status", [
      "open",
      "in_progress",
      "reopened"
    ]).limit(30);
    if (!notes || notes.length === 0) return [];
    return notes.filter((n)=>!noteRequiresHuman(n)).map((n)=>{
      const fixes = Array.isArray(n.suggested_fixes) ? n.suggested_fixes : [];
      const recommended = fixes.find((f)=>f.recommended) || fixes[0];
      let resolution = "";
      if (recommended) {
        ...
        if (fixes.length > 1) {
          const altTitles = fixes.slice(1, 3).map((f)=>f.title || f.description).filter(Boolean);
...
async function checkActionableNoteExhaustion(supabase, projectId, docType, versionId) {
  try {
    ...
    const { data: notes, error } = await supabase.from("project_notes").select("id, title, suggested_fixes, detail").eq("project_id", projectId).eq("doc_type", docType).in("status", actionableStatuses).limit(50);
    if (error) {
    ...
    try {
      const { data: devNotes } = await supabase.from("development_notes").select("id, severity").eq("project_id", projectId).eq("resolved", false).in("severity", [
        "blocker",
        "high"
      ]).limit(50);
      if (devNotes) {
    ...
    return {
      hasActionable: totalActionable > 0,
 ...
 */ async function autoResolveActionableNotes(supabase, projectId, docType, versionId, jobId, resolverLabel = "auto_run") {
  try {
    const { data: notes, error } = await supabase.from("project_notes").select("id, title, summary, severity, suggested_fixes, detail").eq("project_id", projectId).eq("doc_type", docType).in("status", [
      "open",
      "in_progress",
      "reopened"
    ]).limit(50);
    ...
    const autoNotes = notes.filter((n)=>!noteRequiresHuman(n));
    if (autoNotes.length === 0) return {
      resolved: 0,
      notes: []
    };
    const ids = autoNotes.map((n)=>n.id);
    await supabase.from("project_notes").update({
      status: "resolved",
      updated_at: new Date().toISOString()
    }).in("id", ids);
    // Audit via project_note_events
    await supabase.from("project_note_events").insert(autoNotes.map((n)=>({
        project_id: projectId,
        note_id: n.id,
        event_type: "note_auto_resolved",
        payload: {
          resolved_by: resolverLabel,
          job_id: jobId,
          doc_type: docType,
          version_id: versionId
        },
        created_by: null
      }))).catch((e)=>console.warn(`[auto-run][IEL] note_event_insert_failed: ${e?.message}`));
    ...
    return {
      ...
      notes: autoNotes.map((n)=>({
          id: n.id,
          title: n.title || n.summary?.slice(0, 80) || "untitled"
        }))
    };
  } catch (e) {
 ...
 */ async function evaluateCleanupEligibility(supabase, projectId, docType) {
  const base = {
    eligible: false,
    reason: "",
    noteCount: 0,
  ...
  try {
    const { data: notes, error } = await supabase.from("project_notes").select("id, title, summary, severity, category, suggested_fixes, detail").eq("project_id", projectId).eq("doc_type", docType).in("status", [
      "open",
      "in_progress",
      "reopened"
    ]).limit(50);
    ...
    base.noteIds = notes.map((n)=>n.id);
    base.noteCategories = [
      ...new Set(notes.map((n)=>n.category || "unknown"))
    ];
    if (notes.length === 0) {
    ...
    const unresolvedBlockers = notes.filter((n)=>n.severity === "blocker" && (!Array.isArray(n.suggested_fixes) || n.suggested_fixes.length === 0));
 ...
 */ async function wasCleanupAlreadyAttempted(supabase, jobId, docType) {
  const { data } = await supabase.from("auto_run_steps").select("id").eq("job_id", jobId).eq("document", docType).eq("action", "cleanup_pass_started").limit(1);
 ...
 */ async function runCleanupPass(supabase, supabaseUrl, token, params) {
  ...
  try {
    // Log start
    await logStep(supabase, jobId, stepCount + 1, docType, "cleanup_pass_started", `Cleanup pass started: applying all remaining notes via canonical path. Pre-CI: ${currentBestCi}, Pre-GP: ${currentBestGp}.`, {
      ci: currentBestCi,
      gp: currentBestGp
    }, undefined, {
      target_ci: targetCi,
    ...
    if (!bestVer) {
      ...
      await logStep(supabase, jobId, stepCount + 2, docType, "cleanup_pass_skipped", `Cleanup pass skipped: no eligible version found for ${docType}.`);
      return result;
    }
    ...
    if (noteDirs.length === 0) {
      ...
      await logStep(supabase, jobId, stepCount + 2, docType, "cleanup_pass_skipped", `Cleanup pass skipped: no note directions generated for ${docType}.`);
      return result;
    }
    ...
    if (decisionBundle && decisionBundle.accepted_decisions.length > 0) {
      mergedDirections.push("CRITICAL — MUST FIX (accepted stabilise decisions):", decisionBundle.accepted_decisions_compact_text, "You MUST apply ALL of the above fixes in this rewrite. Do not ignore them.");
    }
    mergedDirections.push(...noteDirs);
    // Resolve behavior
    const { data: proj } = await supabase.from("projects").select("development_behavior, episode_target_duration_seconds, season_episode_count").eq("id", projectId).maybeSingle();
    const behavior = proj?.development_behavior || "market";
    // Fetch protect items from latest analysis
    let protectItems = [];
    try {
      const { data: latestRun } = await supabase.from("development_runs").select("output_json").eq("document_id", bestVer.documentId).eq("run_type", "ANALYZE").order("created_at", {
        ascending: false
      }).limit(1).maybeSingle();
      protectItems = latestRun?.output_json?.protect || latestRun?.output_json?.analysis?.protect || [];
    } catch  {}
    // Rewrite via canonical path
    const { candidateVersionId } = await rewriteWithFallback(supabase, supabaseUrl, token, {
      projectId,
    ...
    if (!candidateVersionId) {
      ...
      await logStep(supabase, jobId, stepCount + 3, docType, "cleanup_pass_rejected", `Cleanup pass rejected: rewrite returned no candidate version.`, {
        ci: currentBestCi,
        gp: currentBestGp
      });
      return result;
    ...
    const reviewResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
      action: "analyze",
      projectId,
      documentId: bestVer.documentId,
    ...
    if (postCi !== null || postGp !== null) {
      await supabase.from("project_document_versions").update({
        meta_json: {
          ci: postCi,
          gp: postGp,
          score_source: "cleanup_pass_review"
        }
    ...
    if (result.rejectionReason) {
      ...
      await logStep(supabase, jobId, stepCount + 4, docType, "cleanup_pass_rejected", `Cleanup pass REJECTED: ${result.rejectionReason}. Pre: CI=${currentBestCi}, GP=${currentBestGp}. Post: CI=${postCi}, GP=${postGp}. Prior version remains authoritative.`, {
        ci: postCi ?? currentBestCi,
        gp: postGp ?? currentBestGp
      }, undefined, {
        pre_ci: currentBestCi,
        pre_gp: currentBestGp,
        post_ci: postCi,
        post_gp: postGp,
    ...
    await logStep(supabase, jobId, stepCount + 4, docType, "cleanup_pass_accepted", `Cleanup pass ACCEPTED: CI ${currentBestCi}→${postCi} (Δ${ciDelta > 0 ? "+" : ""}${ciDelta}), GP ${currentBestGp}→${postGp} (Δ${gpDelta > 0 ? "+" : ""}${gpDelta}). Candidate ${candidateVersionId} is new best.`, {
      ci: postCi,
      gp: postGp
    }, undefined, {
      pre_ci: currentBestCi,
    ...
    await logStep(supabase, jobId, stepCount + 2, docType, "cleanup_pass_error", `Cleanup pass failed with error: ${e?.message}. Falling through to normal escalation.`, {
      ci: currentBestCi,
      gp: currentBestGp
    }, undefined, {
      error: e?.message
 ...
 */ async function tryCleanupBeforeEscalation(supabase, supabaseUrl, token, params) {
  ...
  if (alreadyAttempted) {
    ...
    await logStep(supabase, jobId, null, currentDoc, "cleanup_pass_skipped", `Cleanup pass already attempted for ${currentDoc} in this job. Proceeding to escalation.`);
    return {
      attempted: false,
      accepted: false
    };
  ...
  if (!eligibility.eligible) {
    await logStep(supabase, jobId, null, currentDoc, "cleanup_pass_not_eligible", `Cleanup pass not eligible for ${currentDoc}: ${eligibility.reason}. Proceeding to escalation.`, {
      ci: currentCi
    }, undefined, {
      ...eligibility,
  ...
  const cleanupResult = await runCleanupPass(supabase, supabaseUrl, token, {
    ...
    format,
    stepCount,
    currentBestCi: bestCi,
    currentBestGp: bestGp,
    targetCi
  });
  return {
    attempted: true,
...
async function updateJob(supabase, jobId, fields) {
  ...
  if (fields.current_document) {
    try {
      ...
      const { data: jobRow } = await supabase.from("auto_run_jobs").select("project_id, current_document, last_ci, last_gp, user_id").eq("id", jobId).single();
      ...
      if (jobRow?.project_id && jobRow.current_document !== fields.current_document) {
        await emitStageTransition(supabase, jobRow.project_id, jobId, jobRow.current_document || "unknown", fields.current_document, "updateJob", {
          ci: fields.last_ci ?? jobRow.last_ci,
          gp: fields.last_gp ?? jobRow.last_gp,
          createdBy: jobRow.user_id
        });
      }
    } catch (e) {
...
function buildFormatRulesSeedBlock(format, project) {
...
async function finalizeBest(supabase, jobId, job, explicitCurrentDocId) {
  ...
  if (bestDocId && currentDocId && bestDocId !== currentDocId) {
    console.log("[auto-run] finalizeBest no-op: best_document_id does not match current doc", {
      bestDocId,
      currentDocId,
      explicitCurrentDocId
    });
    return false;
  }
  // Find document for this version + canon_drift check
  const { data: ver } = await supabase.from("project_document_versions").select("document_id, is_current, meta_json").eq("id", bestVersionId).maybeSingle();
  if (!ver) return false;
  ...
  if (docType === "story_outline") {
    ...
    if (chunksCompleted < chunksTotal) {
      ...
      await logStep(supabase, jobId, null, docType, "story_outline_integrity_failed", `Story outline version ${bestVersionId} has incomplete chunks (${chunksCompleted}/${chunksTotal}). Not promoting.`, {}, undefined, {
        version_id: bestVersionId,
  ...
  if (currentDocId && ver.document_id !== currentDocId) {
    console.log("[auto-run] finalizeBest no-op: version document_id mismatch", {
      versionDocId: ver.document_id,
      currentDocId,
      explicitCurrentDocId
    });
    return false;
  ...
  if (error) {
    ...
    await logStep(supabase, jobId, null, job.current_document || "unknown", "finalize_promote_best_failed", `Failed to promote best version ${bestVersionId}: ${error.message}`);
    return false;
  }
  ...
  try {
    const ladder = FORMAT_LADDERS[job.format] || [];
    ...
    if (isOnLadder && job.project_id && job.best_document_id && job.current_document && bestVersionId) {
      ...
      const cascadeFetch = fetch(`${supabaseUrl}/functions/v1/canon-cascade-engine`, {
        ...
        console.warn(`[auto-run][cascade] fire_and_forget_error: ${e?.message}`);
      });
      if (typeof globalThis.EdgeRuntime !== "undefined") {
        globalThis.EdgeRuntime.waitUntil(cascadeFetch);
      }
  ...
  await logStep(supabase, jobId, null, job.current_document || "unknown", "finalize_promote_best", `Job ending — promoted best version ${bestVersionId} (CI=${job.best_ci}, GP=${job.best_gp}, score=${job.best_score})`, {
    ci: job.best_ci,
    gp: job.best_gp
  }, undefined, {
    best_version_id: bestVersionId,
    best_document_id: bestDocId,
    best_ci: job.best_ci,
    best_gp: job.best_gp,
...
async function resolveBestScoredEligibleVersionForDoc(supabase, projectId, docType) {
  const { data: docRow } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", docType).order("created_at", {
    ascending: false
  }).limit(1).maybeSingle();
  if (!docRow?.id) return null;
  const { data: versions, error } = await supabase.from("project_document_versions").select("id, version_number, approval_status, is_current, meta_json").eq("document_id", docRow.id).order("version_number", {
    ascending: false
  });
  if (error || !versions?.length) return null;
  const eligibleScored = (versions || []).map((v)=>{
  ...
  }).filter((v)=>(v.approval_status === "approved" || v.is_current) && v.ci !== null && v.gp !== null);
...
function computePlateauDiagnosisBackend(input) {
  const evidence = [];
  const secondary = [];
  let primary = "unknown";
  let confidence = "low";
  ...
  if (isEarly && ciGap > 15) {
    ...
    primary = "escalation_architecture_weak";
    confidence = "medium";
  ...
  if (hasBP && primary === "seed_limited") {
    secondary.push("blueprint_mismatch");
    evidence.push("Blueprint lineage present but plateau hit");
  }
  if (hasDNA && primary === "seed_limited") {
    secondary.push("dna_mismatch");
    evidence.push("DNA lineage present but plateau hit");
  }
  const seedLimited = primary === "seed_limited" || [
    "weak_hook",
    "weak_conflict_engine"
  ].includes(primary) && rewriteExhausted;
  ...
  if (seedLimited) {
    rec = {
      ...
      recommended_mutations: [
        "Create new pitch idea with stronger hook/conflict",
        hasBP ? "Try different blueprint" : "Consider CI Blueprint",
        hasDNA ? "Review DNA constraints" : "Consider DNA profile"
      ],
    ...
    rec = {
      ...
      short_label: "Continue targeted rewrites",
      rationale: `Small CI gap (${ciGap}). Targeted rewrites may close the gap.`,
      recommended_mutations: [
        "Focus on specific blocker areas"
      ]
    };
  } else {
    rec = {
      ...
      recommended_mutations: [
        "Force-promote best available version to advance the pipeline",
        "Or regenerate DevSeed with stronger structural foundations"
      ]
    };
  }
  return {
 ...
 */ async function persistPlateauDiagnosis(supabase, input) {
  try {
    ...
    if (!totalVersions) {
      const { count } = await supabase.from("project_document_versions").select("id", {
        count: "exact",
        head: true
      }).eq("document_id", (await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", input.currentDoc).limit(1).maybeSingle()).then((r)=>r?.data?.id)).then(undefined, ()=>({
          count: 0
        }));
      totalVersions = count ?? 0;
    }
    // Fetch lineage from project
    const { data: proj } = await supabase.from("projects").select("source_dna_profile_id, source_engine_key, pitch_idea_id").eq("id", job.project_id).maybeSingle().then(undefined, ()=>({
        data: null
      }));
    const row = {
      user_id: job.user_id,
...
async function tryPlateauForcePromote(supabase, params) {
  ...
  if (isExceptionalObjective(job) && (typeof detectedBestCi === "number" ? detectedBestCi : detectedCi) < targetCi) {
    ...
    try {
      const { data: platNotes } = await supabase.from("development_notes").select("note_key, severity").eq("document_id", doc ? doc.id : "unknown").eq("resolved", false);
      var bCount = platNotes ? platNotes.filter(function(n) {
        return n.severity === "blocker";
      }).length : 0;
      var hCount = platNotes ? platNotes.filter(function(n) {
  ...
  try {
    const { data: actionableNotes } = await supabase.from("project_notes").select("id, title, severity").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
      "open",
      "in_progress",
      "reopened"
    ]).in("severity", [
    ...
    if (actionableNotes && actionableNotes.length > 0) {
      await logStep(supabase, jobId, stepCount + 1, currentDoc, "force_promote_deferred_notes", `Force-promote deferred: ${actionableNotes.length} actionable note(s) remain for ${currentDoc}. Applying notes before promoting.`, {
        ci: detectedCi
      }, undefined, {
        plateau_version: plateauVersion,
        note_count: actionableNotes.length,
        note_ids: actionableNotes.map((n)=>n.id).slice(0, 5),
        note_titles: actionableNotes.map((n)=>n.title).slice(0, 5)
      });
  ...
  const effectiveBestCi = Math.max(typeof detectedBestCi === "number" ? detectedBestCi : -Infinity, docBestCi);
  // ── ALL NOTES EXHAUSTED: if no open notes of any severity remain, promote regardless of CI ──
  // If the system has applied every possible note and CI is still below target,
  // the doc is at its ceiling — holding it back helps no one.
  if (effectiveBestCi < GLOBAL_MIN_CI) {
    try {
      const { data: anyOpenNotes } = await supabase.from("project_notes").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
        "open",
        "in_progress",
        "reopened"
      ]).limit(1);
      const allNotesExhausted = !anyOpenNotes || anyOpenNotes.length === 0;
      if (allNotesExhausted) {
        console.log(`[auto-run][IEL] all_notes_exhausted { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${effectiveBestCi}, target: ${targetCi}, action: "bypassing_ci_gate" }`);
        await logStep(supabase, jobId, stepCount + 1, currentDoc, "all_notes_exhausted", `All notes resolved (CI:${effectiveBestCi}, target:${targetCi}). No actionable improvements remain — promoting.`, {
          ci: effectiveBestCi,
          gp: bestForDoc?.gp ?? 0
        }, undefined, {
          action: "bypass_ci_gate",
          all_notes_exhausted: true,
          effective_ci: effectiveBestCi,
          target_ci: targetCi
        });
      // Fall through to promotion logic below
      } else {
        return null; // Notes remain — enforce CI gate
      }
    } catch (_) {
      return null; // Fail closed — enforce CI gate if query fails
    }
  }
  if (!bestForDoc) return null;
  const ciGapFromTarget = targetCi - effectiveBestCi;
  await logStep(supabase, jobId, stepCount + 1, currentDoc, "ci_plateau_auto_promote", `CI plateaued at ${detectedCi} (target: ${targetCi}) but best eligible version is CI:${bestForDoc.ci}, GP:${bestForDoc.gp} (>=${GLOBAL_MIN_CI}). allow_defaults=ON → force-promoting.`, {
    ci: detectedCi,
    gp: bestForDoc.gp
  }, undefined, {
  ...
  if (promoteErr) {
    await logStep(supabase, jobId, stepCount + 2, currentDoc, "ci_plateau_force_promote_failed", `Force-promote failed while setting best version current: ${promoteErr.message}`, {
      ci: bestForDoc.ci,
      gp: bestForDoc.gp
    }, undefined, {
      best_version_id: bestForDoc.versionId,
  ...
  if (nextDoc && isStageAtOrBeforeTarget(nextDoc, job.target_document, format)) {
    await logStep(supabase, jobId, stepCount + 2, currentDoc, "ci_plateau_force_promoted", `Force-promoted ${currentDoc} from best version (CI:${bestForDoc.ci}, GP:${bestForDoc.gp}) after plateau. Skipped hard_gate:blockers (allow_defaults=ON). Advancing to ${nextDoc}.`, {
      ci: bestForDoc.ci,
      gp: bestForDoc.gp
    }, undefined, {
      from: currentDoc,
...
async function routePlateauRecovery(supabase, params) {
  ...
  if (!alreadyCleanedUp) {
    ...
    const cleanup = await tryCleanupBeforeEscalation(supabase, _supabaseUrl, _token, {
      ...
      format,
      stepCount,
      currentCi: effectiveCi,
      currentGp: bestGpForCleanup,
      targetCi,
      escalationSource: `plateau_recovery_${escalationSource}`
    });
    if (cleanup.attempted && cleanup.accepted) {
      await logStep(supabase, jobId, stepCount + 1, currentDoc, "plateau_recovery_cleanup", `PLATEAU_RECOVERY_CLEANUP: cleanup pass accepted, CI improved. Continuing pipeline.`, {
        ci: cleanup.result?.postCi ?? effectiveCi
      }, undefined, {
        plateau_version: plateauVersion,
        source: escalationSource,
        pre_ci: effectiveCi,
        post_ci: cleanup.result?.postCi,
        routing: "cleanup_accepted"
      });
  ...
  if (bestForDoc && promotableCi >= GLOBAL_MIN_CI) {
    ...
    await logStep(supabase, jobId, stepCount + 1, currentDoc, "plateau_recovery_promote", `PLATEAU_RECOVERY_PROMOTED: CI=${promotableCi} >= GLOBAL_MIN_CI (${GLOBAL_MIN_CI}). Force-promoting best version despite target gap of ${ciGap}.`, {
      ci: bestForDoc.ci,
      gp: bestForDoc.gp
    }, undefined, {
      plateau_version: plateauVersion,
    ...
    // Fall through to user pause
      ...
      try {
        await persistVersionScores(supabase, {
          ...
          ci: bestForDoc.ci,
          gp: bestForDoc.gp,
          source: "auto-run-plateau-recovery-promote",
          jobId,
          protectHigher: true,
          docType: currentDoc
        });
      ...
      if (nextDoc && isStageAtOrBeforeTarget(nextDoc, job.target_document, format)) {
        ...
        try {
          ...
          const history = Array.isArray(jobRow?.stage_history) ? jobRow.stage_history : [];
          history.push({
            event: "PLATEAU_RECOVERY_PROMOTED",
            doc_type: currentDoc,
            ci_gap: ciGap,
            best_ci: promotableCi,
            target_ci: targetCi,
            timestamp: new Date().toISOString()
          });
          await supabase.from("auto_run_jobs").update({
        ...
        await updateJob(supabase, jobId, {
          status: "completed",
          stop_reason: "All stages satisfied (plateau recovery promote)"
        });
  ...
  await logStep(supabase, jobId, stepCount + 1, currentDoc, "plateau_recovery_user_required", `PLATEAU_RECOVERY_USER_REQUIRED: CI=${effectiveCi} (best: ${bestCiAvail}) below GLOBAL_MIN_CI (${GLOBAL_MIN_CI}) or no promotable version. Human intervention required.`, {
    ci: effectiveCi
  }, undefined, {
    plateau_version: plateauVersion,
    source: escalationSource,
    target_ci: targetCi,
    best_ci: bestCiAvail,
    routing: "user_required",
    cleanup_attempted: alreadyCleanedUp || !alreadyCleanedUp
  });
  const { data: docForEsc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
    ascending: false
  }).limit(1).maybeSingle();
  ...
  persistPlateauDiagnosis(supabase, {
    ...
    jobId,
    currentDoc,
    bestCi: detectedBestCi,
    finalCi: detectedCi,
    targetCi,
    targetGp: extractTargetGP(job),
    haltReason: "PLATEAU_RECOVERY_EXHAUSTED",
    stepCount,
    stageLoopCount: job.stage_loop_count ?? 0
  }).then(undefined, (e)=>console.error(`[auto-run][DIAG] fire_forget: ${e?.message}`));
  await releaseProcessingLock(supabase, jobId);
  return respondWithJob(supabase, jobId);
}
// ── Helper: get job ──
...
async function acquireProcessingLock(supabase, jobId, userId, isServiceActor = false) {
  ...
  let qA = supabase.from("auto_run_jobs").update({
    is_processing: true,
    processing_started_at: now,
    lock_expires_at: lockExpires,
    last_heartbeat_at: now
  }).eq("id", jobId).eq("status", "running").eq("is_processing", false);
  ...
  let qB = supabase.from("auto_run_jobs").update({
    is_processing: true,
    processing_started_at: now,
    lock_expires_at: lockExpires,
    last_heartbeat_at: now
  }).eq("id", jobId).eq("status", "running").eq("is_processing", true).lt("processing_started_at", staleThreshold);
...
async function releaseProcessingLock(supabase, jobId) {
  await supabase.from("auto_run_jobs").update({
    is_processing: false,
...
async function nextStepIndex(supabase, jobId) {
  ...
  if (error) {
    ...
    const { data: job } = await supabase.from("auto_run_jobs").select("step_count").eq("id", jobId).maybeSingle();
...
function createFallbackDecisions(currentDoc, ci, gp, reason) {
  return [
    {
      id: "force_promote",
      ...
      options: [
        {
          value: "force_promote",
          why: "Skip remaining issues and advance to the next stage"
        },
        {
          value: "retry",
          why: "Run another development cycle at the current stage"
        },
        {
          value: "stop",
          why: "Stop the auto-run and review manually"
        }
      ],
...
function tryAutoAcceptDecisions(decisions, allowDefaults) {
...
async function buildAcceptedDecisionsBundle(supabase, jobId, currentDoc) {
  // Find most recent auto_decided step for this job + document
  const { data: decidedStep } = await supabase.from("auto_run_steps").select("output_ref").eq("job_id", jobId).eq("document", currentDoc).eq("action", "auto_decided").order("step_index", {
    ascending: false
  }).limit(1).maybeSingle();
  if (!decidedStep?.output_ref) return null;
  const selections = decidedStep.output_ref.selections || {};
  const decisionObjects = decidedStep.output_ref.decision_objects || [];
  ...
  for (const key of sortedKeys){
    ...
    const decObj = decisionObjects.find((d)=>d.id === key);
    const chosenOpt = decObj?.options?.find((o)=>o.value === chosenId);
    accepted.push({
      key,
      chosen_id: chosenId,
  ...
  const compactLines = accepted.map((a, i)=>`${i + 1}) [${a.chosen_id}] ${a.question} → FIX: ${a.chosen_text}`);
 ...
 */ function normalizeCanonEntities(raw) {
 ...
 */ function prioritizeCanonEntities(entities) {
  const core = [];
  const secondary = [];
...
async function chunkedRewrite(supabase, supabaseUrl, token, params, jobId, stepCount) {
  ...
  // Step 1: Plan
  const planResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
    action: "rewrite-plan",
    projectId,
    documentId,
    versionId,
  ...
  for(let i = 0; i < totalChunks; i++){
    ...
    const chunkResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
  ...
  const assembleResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
...
async function rewriteWithFallback(supabase, supabaseUrl, token, rewriteBody, jobId, stepCount, format, deliverableType) {
  const projectId = rewriteBody.projectId;
  const sourceVersionId = rewriteBody.versionId;
  try {
    const result = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
      action: "rewrite",
      ...rewriteBody
    ...
    if (projectId) {
      await emitTransition(supabase, {
        projectId,
        eventType: TRANSITION_EVENTS.REWRITE_PASS_EXECUTED,
        docType: deliverableType,
        stage: deliverableType,
        jobId,
        sourceVersionId,
        resultingVersionId: candidateVersionId || undefined,
        trigger: "single_pass",
        sourceOfTruth: "auto-run",
        resultingState: {
          mode: "single_pass",
    ...
    if (e.message && (e.message.includes("needsPipeline") || e.message.includes("too long for single-pass") || e.message.toLowerCase().includes("large-risk doc type"))) {
      ...
      if (projectId) {
        await emitTransition(supabase, {
          ...
          jobId,
          sourceVersionId,
          resultingVersionId: chunkedResult.candidateVersionId || undefined,
          trigger: "chunked_pipeline",
          sourceOfTruth: "auto-run",
          resultingState: {
            mode: "chunked_pipeline",
            candidateVersionId: chunkedResult.candidateVersionId
          }
        });
      }
      return {
    ...
    if (projectId) {
      try {
        await emitTransition(supabase, {
          projectId,
          eventType: TRANSITION_EVENTS.REWRITE_PASS_FAILED,
          docType: deliverableType,
          stage: deliverableType,
          jobId,
          sourceVersionId,
          status: "failed",
          trigger: "rewrite_error",
          sourceOfTruth: "auto-run",
          resultingState: {
...
Deno.serve(async (req)=>{
  ...
  console.log("[auto-run] request_in", {
    method: req.method,
    origin: req.headers.get("origin"),
    hasAuth: !!req.headers.get("authorization")
  });
  console.log("[auto-run] reached_main", {
    method: req.method,
    action
  });
  try {
    if (!FORMAT_LADDERS || typeof FORMAT_LADDERS !== "object" || Object.keys(FORMAT_LADDERS).length === 0) {
      return respond({
        error: "STAGE_LADDERS_LOAD_FAILED"
      }, 500);
    }
    ...
    if (incomingToken === serviceKey) {
      ...
      try {
        ...
        try {
          ...
          if (userErr || !user) return respond({
            error: "Unauthorized"
          }, 401);
          userId = user.id;
        } catch  {
    ...
    console.log("[auto-run] auth", {
      fn: "auto-run",
      isServiceRole: actor === "service_role",
      hasActorUserId: !!userId,
      hasForwardedUserId: !!(body?.userId || body?.user_id),
      action
    });
    // ═══════════════════════════════════════
    ...
    if (action === "debug-completion-gate") {
      ...
      if (!debugProjectId) return respond({
    ...
    if (action === "update-version-cap") {
      ...
      if (!rawCap || isNaN(rawCap)) return respond({
        error: "max_versions_per_doc_per_job must be a number"
      }, 400);
      const clamped = Math.max(MIN_VERSION_CAP, Math.min(MAX_VERSION_CAP, rawCap));
      const { error: updErr } = await supabase.from("auto_run_jobs").update({
        max_versions_per_doc_per_job: clamped
      }).eq("id", jobId).eq("user_id", userId);
      if (updErr) return respond({
        error: updErr.message
      }, 500);
    ...
    if (action === "status") {
      ...
      if (jobId) {
        ...
        const { data: candidates, error: candErr } = await supabase.from("auto_run_jobs").select("*").eq("project_id", projectId).eq("user_id", userId).order("created_at", {
          ascending: false
        }).limit(20);
        statusError = candErr;
      ...
      if (statusError || !job) return respond({
        job: null,
        latest_steps: [],
        next_action_hint: "No job found"
      });
      // Update heartbeat (fire-and-forget, never block status)
      supabase.from("auto_run_jobs").update({
      ...
      if (job.status === "running" && job.is_processing && job.processing_started_at) {
        ...
        if (lockAge > 120_000) {
          console.warn("[auto-run] stuck detection: releasing stale lock", {
            jobId: job.id,
            lockAge
          });
          await supabase.from("auto_run_jobs").update({
      ...
      const { data: steps } = await supabase.from("auto_run_steps").select("*").eq("job_id", job.id).order("step_index", {
        ascending: false
      }).limit(200);
      ...
      let seedPackInfo;
      if (seedProjectId) {
        const { data: seedDocs } = await supabase.from("project_documents").select("doc_type").eq("project_id", seedProjectId).in("doc_type", SEED_DOC_TYPES);
        const seedSet = new Set((seedDocs || []).map((d)=>d.doc_type));
        const seedMissing = SEED_DOC_TYPES.filter((dt)=>!seedSet.has(dt));
        seedPackInfo = {
          present: SEED_DOC_TYPES.length - seedMissing.length,
          total: SEED_DOC_TYPES.length,
          missing: seedMissing
        };
      }
      return respond({
    ...
    if (action === "recover") {
      ...
      if (!job) return respond({
        error: "Job not found"
      }, 404);
      if (!detectFrozen(job)) {
        console.log("[auto-run] recover called but job is not frozen", {
          jobId,
          status: job.status,
    ...
    if (action === "start") {
      ...
      if (!rawFmt) return respond({
        error: "MISSING_FORMAT_FOR_LADDER",
        format: rawFmt,
        original_format: proj?.format
      }, 422);
      const fmt = rawFmt;
      const fmtLadder = getLadderForJob(fmt);
      if (!fmtLadder) return respond({
        error: "MISSING_FORMAT_FOR_LADDER",
        format: fmt,
        original_format: proj?.format
      }, 422);
      ...
      const targetDoc1 = canonicalDocType(rawTarget, fmt);
      // IEL: validate + correct target via unified guard
      const ielResult = ielValidateTarget(targetDoc1, fmt);
      ...
      if (!jobUserId) return respond({
        error: "Cannot determine user_id for job. Provide userId in body."
      }, 400);
      ...
      if (!body.force_new_run) {
        const { data: existingJobs } = await supabase.from("auto_run_jobs").select("id, project_id, status, current_document, step_count, awaiting_approval, stop_reason, pause_reason, converge_target_json, created_at").eq("project_id", projectId).eq("user_id", jobUserId).in("status", [
          "running",
          "paused"
        ]).order("created_at", {
          ascending: false
      ...
      const insertPayload = {
        ...
        converge_target_json: (()=>{
          ...
          if (ct && typeof ct === "object") {
            ...
            const gp = Number(ct.gp);
            if (!isNaN(ci) && ci >= 0 && ci <= 100) return {
              ci,
              gp: !isNaN(gp) ? gp : 95
            };
          }
          return {
            ci: 95,
            gp: 95
          }; // Max Quality default — optimize until progress exhausted
        })(),
        allow_defaults: body.allow_defaults === true,
        follow_latest: body.follow_latest === true ? true : false,
        pipeline_key: fmt,
        max_versions_per_doc_per_job: typeof body.max_versions_per_doc_per_job === "number" ? Math.max(MIN_VERSION_CAP, Math.min(MAX_VERSION_CAP, body.max_versions_per_doc_per_job)) : DEFAULT_MAX_VERSIONS_PER_DOC_PER_JOB
      };
      ...
      if (FILM_FORMATS.has(fmt)) {
        try {
          const { data: projRt } = await supabase.from("projects").select("min_runtime_minutes, min_runtime_hard_floor").eq("id", projectId).single();
      ...
      // ── LADDER DOC-SLOT PREFLIGHT: ensure first N ladder stages have doc slots ──
      {
        ...
        if (ladder && ladder.length > 0) {
          ...
          if (created.length > 0) {
            ...
            await logStep(supabase, job.id, 0, effectiveStartDoc, "doc_slots_ensured", `Created ${created.length} ladder doc slots: ${created.join(", ")}. Already existed: ${existed.length}.`, {}, undefined, {
              created,
              existed,
      ...
      if (preflight.changed) {
        await logStep(supabase, job.id, 0, effectiveStartDoc, "preflight_resolve", `Resolved qualifications: ${Object.keys(preflight.resolved).join(", ")} → ${JSON.stringify(preflight.resolved)}`);
      }
      // ── INPUT READINESS CHECK at start (with auto-regen attempt) ──
      {
        ...
        if (!inputCheck.ready) {
          console.log("[auto-run] INPUT_INCOMPLETE at start — attempting auto-regen", {
            jobId: job.id,
            missing: inputCheck.missing_fields
          });
          const regenAttempt = await attemptAutoRegenInputs(supabase, supabaseUrl, token, job.id, 1, effectiveStartDoc, projectId, inputCheck.missing_fields, "start_gate");
          regenWasOk = regenAttempt.ok;
          if (!regenAttempt.ok) {
            console.warn("[auto-run] start auto-regen did not resolve inputs", {
              jobId: job.id,
              error: regenAttempt.error
            });
          }
          ...
          await logStep(supabase, job.id, 1, effectiveStartDoc, "auto_regen_inputs", inputCheck.ready ? `Post-regen readiness: READY (all inputs satisfied)` : `Post-regen readiness: STILL MISSING ${inputCheck.missing_fields.join(", ")}`, {}, undefined, {
          ...
          if (regenWasOk && inputCheck.ready) {
            console.log("[auto-run] HARD GUARD: regen succeeded + ready — continuing without pause", {
              jobId: job.id
            });
          }
        }
        if (!inputCheck.ready) {
          ...
          console.warn("[auto-run] INPUT_INCOMPLETE at start (after regen attempt)", {
            jobId: job.id,
            missing: inputCheck.missing_fields
          });
          ...
          await logStep(supabase, job.id, 1, effectiveStartDoc, "pause_for_input", `INPUT_INCOMPLETE: ${compactErr}`, {}, undefined, {
            missing_fields: inputCheck.missing_fields,
            regen_was_ok: regenWasOk
          });
          return respond({
            job: {
              ...job,
    ...
    if (action === "update-target") {
      ...
      if (typeof ci !== "number" || typeof gp !== "number" || ci < 0 || ci > 100 || gp < 0 || gp > 100) {
        return respond({
          error: "ci and gp must be numbers 0-100"
        }, 400);
      }
      await updateJob(supabase, jobId, {
        converge_target_json: {
    ...
    if (action === "repair-baseline") {
      ...
      if (!strategy || ![
        ...
        return respond({
          error: "strategy must be 'promote_best_scored' or 'promote_latest'"
        }, 400);
      }
      ...
      if (job.pause_reason !== "BASELINE_MISSING") {
        return respond({
          error: "Job is not paused for BASELINE_MISSING"
        }, 400);
      }
      // Find the document that's missing a baseline
      const currentDoc = job.current_document;
      const { data: doc1 } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", job.project_id).eq("doc_type", currentDoc).limit(1).maybeSingle();
      if (!doc1) return respond({
        error: `No document found for doc_type=${currentDoc}`
      }, 404);
      // Get all versions for this document
      const { data: versions } = await supabase.from("project_document_versions").select("id, version_number, plaintext").eq("document_id", doc1.id).order("version_number", {
        ascending: false
      });
      if (!versions || versions.length === 0) {
        return respond({
          error: "No versions exist for this document — cannot repair"
        }, 400);
      }
      let chosenVersionId;
      if (strategy === "promote_best_scored") {
        ...
        const { data: runs } = await supabase.from("development_runs").select("version_id, output_json").in("version_id", versionIds).eq("run_type", "ANALYZE").order("created_at", {
          ascending: false
        });
        let bestScore = -1;
      ...
      if (promoteErr) {
        return respond({
          error: `Failed to repair baseline: ${promoteErr.message}`
        }, 500);
      }
      await logStep(supabase, jobId, null, currentDoc, "baseline_repaired", `Baseline repaired via ${strategy}: version ${chosenVersionId} set as current.`, {}, undefined, {
        strategy,
        chosenVersionId,
        documentId: doc1.id,
    ...
    if (action === "pause" || action === "stop") {
      ...
      return respond({
        job,
        latest_steps: [],
        next_action_hint: action === "pause" ? "resume" : "none"
      });
    }
    // ═══════════════════════════════════════
    // ACTION: resume
    ...
    if (action === "resume") {
      ...
      const { data: preResumeJob } = await supabase.from("auto_run_jobs").select("current_document, follow_latest, resume_version_id, step_count, max_total_steps, pause_reason").eq("id", jobId).maybeSingle();
    ...
    if (action === "set-resume-source") {
      ...
      if (!documentId || !versionId) return respond({
        error: "documentId and versionId required"
      }, 400);
      const { data: job, error: jobErr } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).eq("user_id", userId).single();
      if (jobErr || !job) return respond({
        error: "Job not found"
      }, 404);
      await updateJob(supabase, jobId, {
        follow_latest: false,
      ...
      await logStep(supabase, jobId, stepCount, job.current_document, "resume_source_set", `Pinned resume source: doc=${documentId} ver=${versionId}`, {}, undefined, {
        documentId,
        versionId,
        follow_latest: false
      });
    ...
    if (action === "approve-decision") {
      ...
      if (!decisionId || !selectedValue) return respond({
        error: "decisionId and selectedValue required"
      }, 400);
      const { data: job, error: jobErr } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).eq("user_id", userId).single();
      if (jobErr || !job) return respond({
        error: "Job not found"
      ...
      const decision = pending.find((d)=>d.id === choiceId || d.id.endsWith(`:${choiceId}`));
      if (!decision) {
        // Decision is stale — return 409 with current job state so UI can self-heal
        console.warn(`[auto-run] Stale decision: ${choiceId} not in pending_decisions [${pending.map((d)=>d.id).join(",")}]`);
        const { data: freshJob } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).maybeSingle();
        const { data: freshSteps } = await supabase.from("auto_run_steps").select("*").eq("job_id", jobId).order("step_index", {
          ascending: false
        }).limit(200);
        return jsonRes({
          code: "STALE_DECISION",
      ...
      if (baseChoiceId === "raise_step_limit_once" && choiceValue === "yes") {
        ...
        await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", `Step limit raised: ${job.max_total_steps} → ${newMax}`, {}, undefined, {
          choiceId,
          choiceValue
        });
        await updateJob(supabase, jobId, {
      ...
      if (baseChoiceId === "raise_step_limit_once" && choiceValue === "no") {
        await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", "User declined step extension — stopping run", {}, undefined, {
          choiceId,
          choiceValue
        });
        await updateJob(supabase, jobId, {
      ...
      if (baseChoiceId === "run_exec_strategy" && choiceValue === "yes") {
        ...
        try {
          const { data: project } = await supabase.from("projects").select("format, development_behavior").eq("id", job.project_id).single();
          const format = (project?.format || "film").toLowerCase().replace(/_/g, "-");
          const behavior = project?.development_behavior || "market";
          const { data: docs } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
            ascending: false
          }).limit(1);
          const doc1 = docs?.[0];
          if (!doc1) throw new Error("No document found for executive strategy");
          const { data: versions } = await supabase.from("project_document_versions").select("id").eq("document_id", doc1.id).order("version_number", {
            ascending: false
          }).limit(1);
          const latestVersion = versions?.[0];
          if (!latestVersion) throw new Error("No version found");
          const stratResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
          ...
          await logStep(supabase, jobId, stepCount, currentDoc, "executive_strategy", strat.summary || `Auto-fixes applied: ${Object.keys(projectUpdates).join(", ") || "none"}`, {}, undefined, {
            strategy: strat,
            updates: projectUpdates
          });
          // If strategy produced blocking decisions, try auto-accept or pause
          const blockingDecisions = mustDecide.filter((d)=>d.impact === "blocking");
          if (blockingDecisions.length > 0) {
            const autoSelections = tryAutoAcceptDecisions(mustDecide, job.allow_defaults !== false);
            if (autoSelections) {
              await logStep(supabase, jobId, stepCount, currentDoc, "auto_decided", `Auto-accepted ${Object.keys(autoSelections).length} decisions (allow_defaults)`, {}, undefined, {
          ...
          await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", `Executive strategy failed: ${stratErr.message}`);
      ...
      if (baseChoiceId === "force_promote" && choiceValue === "yes") {
        ...
        if (next) {
          await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", `Force-promoted: ${currentDoc} → ${next}`, {}, undefined, {
            choiceId,
            choiceValue
          });
          await updateJob(supabase, jobId, {
      ...
      const remainingDecisions = pending.filter((d)=>d.id !== matchedId);
      const hasBlockingRemaining = remainingDecisions.some((d)=>d.impact === "blocking");
      await logStep(supabase, jobId, stepCount, job.current_document, "decision_applied", `${decision.question} → ${choiceValue}`, {}, undefined, {
        decisionId: choiceId,
      ...
      if (hasBlockingRemaining) {
        ...
        if (job.allow_defaults || job.meta_json?.auto_approve_all) {
          const remainingBlocking = remainingDecisions.filter((d)=>d.impact === "blocking");
          for (const rd of remainingBlocking){
            const autoValue = rd.recommended || rd.options?.[0]?.value || "accept";
            await logStep(supabase, jobId, stepCount, job.current_document, "auto_decided_all", `Auto-resolved remaining decision: ${rd.question || rd.id} → ${autoValue}`, {}, undefined, {
              decisionId: rd.id,
              selectedValue: autoValue,
              source: "allow_defaults_remaining"
            });
          }
          await updateJob(supabase, jobId, {
            step_count: stepCount,
        ...
        const nextBlocking = remainingDecisions.find((d)=>d.impact === "blocking");
        await updateJob(supabase, jobId, {
          step_count: stepCount,
      ...
      const ledgerIdsToResolve = [
        matchedId
      ];
      const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      const isUuid = uuidPattern.test(matchedId);
      if (isUuid) {
        const { error: errUuid } = await supabase.from("decision_ledger").update({
          status: "active"
        }).eq("id", matchedId).eq("status", "workflow_pending");
        if (errUuid) {
          console.warn(`[auto-run][IEL] decision_ledger_resolve_failed_id`, JSON.stringify({
            job_id: jobId,
            id: matchedId,
            error: errUuid.message
          }));
        } else {
          console.log(`[auto-run][IEL] decision_ledger_resolved_by_id`, JSON.stringify({
            job_id: jobId,
            id: matchedId
          }));
        }
        ...
        const { error: errKey } = await supabase.from("decision_ledger").update({
          status: "active"
        }).eq("decision_key", matchedId).eq("project_id", job.project_id).eq("status", "workflow_pending");
        if (errKey) {
          console.warn(`[auto-run][IEL] decision_ledger_resolve_failed_key`, JSON.stringify({
            job_id: jobId,
            key: matchedId,
            error: errKey.message
          }));
        } else {
          console.log(`[auto-run][IEL] decision_ledger_resolved_by_key`, JSON.stringify({
            job_id: jobId,
            key: matchedId
          }));
        }
    ...
    if (action === "get-pending-doc") {
      ...
      if (!job.awaiting_approval || !job.pending_doc_id) return respond({
        pending_doc: null,
        job
      }, 200);
      ...
      if (job.pending_version_id) {
        const { data: ver } = await supabase.from("project_document_versions").select("plaintext").eq("id", job.pending_version_id).single();
        docText = ver?.plaintext || "";
      }
      if (!docText && job.pending_doc_id) {
        const { data: docRow } = await supabase.from("project_documents").select("extracted_text, plaintext").eq("id", job.pending_doc_id).single();
    ...
    if (action === "approve-seed-core") {
      ...
      if (!pId) return respond({
        error: "projectId required"
      }, 400);
      // 1. Find all 5 seed docs
      const { data: seedDocs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", pId).in("doc_type", SEED_DOC_TYPES);
      const docMap = new Map();
      ...
      const docIds = Array.from(docMap.values());
      const { data: curVersions } = await supabase.from("project_document_versions").select("id, document_id, approval_status").in("document_id", docIds).eq("is_current", true);
      const versionMap = new Map();
      for (const v of curVersions || []){
        versionMap.set(v.document_id, {
          id: v.id,
          approval_status: v.approval_status
        });
      }
      const missingCurrentVersions = [];
      for (const dt of SEED_DOC_TYPES){
        const docId = docMap.get(dt);
      ...
      if (missingCurrentVersions.length > 0) {
        return respond({
          ...
          stop_reason: "SEED_CORE_MISSING",
          missing_docs: [],
          missing_current_versions: missingCurrentVersions
        });
      }
      ...
      let resumedJob = null;
      if (jobId) {
        const { data: jRow } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).eq("user_id", userId).single();
        if (jRow) {
          const shouldResumeFromSeedGate = jRow.status === "paused" && (jRow.stop_reason === "SEED_CORE_NOT_OFFICIAL" || jRow.approval_type === "seed_core_officialize" || jRow.awaiting_approval === true);
          if (shouldResumeFromSeedGate) {
            ...
            await logStep(supabase, jobId, stepCount, jRow.current_document, "seed_core_approved", `Seed core officialized — ${approvedDocTypes.length} docs approved`, {}, undefined, {
    ...
    if (action === "approve-next") {
      ...
      if (!approvalDecision || ![
        ...
        "revise",
        "stop"
      ].includes(approvalDecision)) {
        return respond({
          error: "decision required: approve | revise | stop"
        }, 400);
      }
      const { data: job, error: jobErr } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).eq("user_id", userId).single();
      if (jobErr || !job) return respond({
      ...
      if (approvalDecision === "stop") {
        ...
        await updateJob(supabase, jobId, {
          step_count: stepCount,
          status: "stopped",
          stop_reason: "User stopped at approval gate",
          awaiting_approval: false,
          approval_type: null,
          approval_payload: null,
          pending_doc_id: null,
      ...
      if (approvalDecision === "revise") {
        ...
        await updateJob(supabase, jobId, {
          ...
          pending_doc_type: null,
          pending_next_doc_type: null,
          stage_loop_count: Math.max(0, job.stage_loop_count - 1)
        });
        return respondWithJob(supabase, jobId, "run-next");
      }
      // approve — advance stage + approve+activate the pending version
      const nextStage = job.pending_next_doc_type;
      ...
      if (approveVersionId) {
        try {
          ...
          const { data: ver } = await supabase.from("project_document_versions").select("id, deliverable_type, label, stage, document_id").eq("id", approveVersionId).single();
          if (ver) {
            const { data: parentDoc } = await supabase.from("project_documents").select("id, doc_type, title, file_name").eq("id", ver.document_id).single();
            const { data: proj } = await supabase.from("projects").select("format").eq("id", job.project_id).single();
            const fmt = (proj?.format || "film").toLowerCase().replace(/_/g, "-");
      ...
      const { data: approveProj } = await supabase.from("projects").select("format").eq("id", job.project_id).single();
      const approveFormat = (approveProj?.format || "film").toLowerCase().replace(/_/g, "-");
      await logStep(supabase, jobId, stepCount, currentDoc, "approval_approved", `User approved ${job.approval_type}: ${currentDoc} → ${nextStage || "continue"}`);
    ...
    if (action === "set-stage") {
      ...
      const { stage } = body;
      if (!stage || !isOnLadder(stage)) return respond({
    ...
    if (action === "force-promote") {
      ...
      const { data: promotingDoc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
        ascending: false
      }).limit(1).maybeSingle();
      let promotionSourceVersionId = null;
      let promotionSourceReason = "no_document";
      if (promotingDoc) {
        const abvr = await resolveActiveVersionForDoc(supabase, job, promotingDoc.id, {
          jobId,
          docType: currentDoc
        });
        if (abvr) {
      ...
      if (!promotionSourceVersionId) {
        ...
        return respond({
          error: `Cannot promote: no eligible approved/current version for ${currentDoc} (${promotionSourceReason}). Approve a version first.`
        }, 400);
      }
      // Persist scores from the promoted version to best_* fields if they're better
      const { data: sourceVer } = await supabase.from("project_document_versions").select("id, meta_json").eq("id", promotionSourceVersionId).single();
      ...
      const stepCount = job.step_count + 1;
      await logStep(supabase, jobId, stepCount, currentDoc, "force_promote", `Force-promoted: ${currentDoc} → ${next} (source_version: ${promotionSourceVersionId}, CI:${srcCI}, GP:${srcGP})`, {}, undefined, {
        promotion_source_version_id: promotionSourceVersionId,
        promotion_source_reason: promotionSourceReason
      });
      // Update job: advance stage AND persist the promoted-from version in best_* and last_analyzed
      const bestUpdateFields = {
        current_document: next,
        stage_loop_count: 0,
        step_count: stepCount,
        stage_exhaustion_remaining: job.stage_exhaustion_default ?? 4,
    ...
    if (action === "apply-rewrite") {
      ...
      let doc1 = null;
      const { data: docs } = await supabase.from("project_documents").select("id, doc_type, plaintext, extracted_text").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
        ascending: false
      }).limit(1);
      doc1 = docs?.[0];
      if (!doc1) {
        ...
        for(let i = ladderIdx - 1; i >= 0; i--){
          const { data: fallbackDocs } = await supabase.from("project_documents").select("id, doc_type, plaintext, extracted_text").eq("project_id", job.project_id).eq("doc_type", jobLadder[i]).order("created_at", {
            ascending: false
          }).limit(1);
          if (fallbackDocs?.[0]) {
            doc1 = fallbackDocs[0];
        ...
        if (!doc1) return respond({
          error: `No document found for stage ${currentDoc} or any prior stage`
        }, 400);
      }
      const { data: versions } = await supabase.from("project_document_versions").select("id, plaintext, version_number").eq("document_id", doc1.id).order("version_number", {
        ascending: false
      }).limit(1);
      const latestVersion = versions?.[0];
      if (!latestVersion) return respond({
        error: "No version found"
      }, 400);
      // Fetch latest notes
      const notesResult = await supabase.from("development_runs").select("output_json").eq("document_id", doc1.id).eq("run_type", "NOTES").order("created_at", {
        ascending: false
      }).limit(1).single();
      ...
      // ── DOC POLICY GUARD (apply-rewrite) — FAIL CLOSED ──
      {
        ...
        try {
          ...
          await logStep(supabase, jobId, stepCount, currentDoc, "doc_type_unregistered", `Doc type "${currentDoc}" is not in the policy registry. Halting.`);
          await updateJob(supabase, jobId, {
            status: "paused",
        ...
        if (applyPolicy.docClass === "AGGREGATE") {
          await logStep(supabase, jobId, stepCount, currentDoc, "aggregate_skip_advance", `AGGREGATE doc "${currentDoc}" is compile-only. Skipping rewrite, advancing to next stage.`);
          ...
          if (nextAfterAgg && isStageAtOrBeforeTarget(nextAfterAgg, job.target_document, format)) {
            ...
            await updateJob(supabase, jobId, {
              status: "completed",
              stop_reason: "All stages satisfied (aggregate skip)"
            });
            return respondWithJob(supabase, jobId);
          }
        }
        if (applyPolicy.docClass === "UNIT" && applyPolicy.requiresEpisodeIndex) {
          ...
          if (typeof epIdx !== "number" || epIdx < 1 || !Number.isInteger(epIdx)) {
            await logStep(supabase, jobId, stepCount, currentDoc, "unit_identity_missing", `UNIT doc "${currentDoc}" requires episode_index but got: ${JSON.stringify(epIdx)}. Halting.`);
            await updateJob(supabase, jobId, {
              status: "paused",
              pause_reason: "UNIT_IDENTITY_MISSING",
      ...
      try {
        const { candidateVersionId: newVersionId_raw } = await rewriteWithFallback(supabase, supabaseUrl, token, {
          projectId: job.project_id,
          documentId: doc1.id,
          versionId: latestVersion.id,
          approvedNotes,
        ...
        await logStep(supabase, jobId, stepCount, currentDoc, "manual_rewrite", `Manual rewrite applied (loop ${newLoopCount}). New version: ${newVersionId}`, {}, undefined, {
          docId: doc1.id,
          newVersionId
        });
        await updateJob(supabase, jobId, {
        ...
        console.log(`[auto-run][IEL] follow_latest_pins_cleared { job_id: "${jobId}", source: "manual_rewrite", doc_id: "${doc1.id}" }`);
        return respondWithJob(supabase, jobId, "run-next");
      } catch (e) {
        await logStep(supabase, jobId, stepCount, currentDoc, "manual_rewrite_failed", `Rewrite failed: ${e.message}`);
        return respond({
          error: `Rewrite failed: ${e.message}`
        }, 500);
      }
    }
    // ═══════════════════════════════════════
    ...
    if (action === "run-strategy") {
      ...
      if (!doc1) {
        ...
        for(let i = ladderIdx - 1; i >= 0; i--){
          const { data: fallbackDocs } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", jobLadder2[i]).order("created_at", {
            ascending: false
          }).limit(1);
          if (fallbackDocs?.[0]) {
            doc1 = fallbackDocs[0];
      ...
      const { data: vers } = await supabase.from("project_document_versions").select("id").eq("document_id", doc1.id).order("version_number", {
        ascending: false
      }).limit(1);
      ...
      try {
        ...
        await logStep(supabase, jobId, stepCount, currentDoc, "manual_strategy", strat.summary || `Executive strategy: auto-fixes=${Object.keys(projectUpdates).join(",") || "none"}, decisions=${mustDecide.length}`, {}, undefined, {
          strategy: strat,
          updates: projectUpdates
        });
        ...
        if (blockingDecisions.length > 0) {
          const autoSelections = tryAutoAcceptDecisions(mustDecide, job.allow_defaults !== false);
          if (autoSelections) {
            await logStep(supabase, jobId, stepCount, currentDoc, "auto_decided", `Auto-accepted ${Object.keys(autoSelections).length} executive strategy decisions`, {}, undefined, {
              selections: autoSelections
            });
        ...
        if (job.allow_defaults || job.meta_json?.auto_approve_all) {
          await logStep(supabase, jobId, stepCount, currentDoc, "decision_mode_skipped", `Executive strategy applied — no blocking decisions, allow_defaults=ON → continuing without pause.`);
          await updateJob(supabase, jobId, {
            step_count: stepCount,
            status: "running",
            stop_reason: null,
        ...
        return respond({
          error: `Strategy failed: ${e.message}`
        }, 500);
      }
    }
    // ═══════════════════════════════════════
    ...
    if (action === "apply-decisions-and-continue") {
      ...
      if (selectedOptions.length === 0) {
        ...
        if (preJob?.allow_defaults) {
          ...
          const normalizeDecisionItem = (d)=>({
              decision_id: d.id || d.decision_key || d.note_id || null,
              options: Array.isArray(d.options) ? d.options : Array.isArray(d.decision_value?.options) ? d.decision_value.options : [],
          ...
          if (decisionItems.length === 0 && preJob.project_id) {
            const { data: ledgerRows } = await supabase.from("decision_ledger").select("id, decision_key, decision_value").eq("project_id", preJob.project_id).eq("status", "workflow_pending").order("created_at", {
              ascending: true
            }).limit(50);
            if (ledgerRows && ledgerRows.length > 0) {
              decisionItems = ledgerRows.map((r)=>normalizeDecisionItem({
                  id: r.id,
          ...
          if (decisionItems.length > 0) {
            ...
            selectedOptions = decisionItems.filter((d)=>d.decision_id) // skip items with no id
            .map((d)=>{
              let optionId;
              if (d.recommended) {
        ...
        if (selectedOptions.length === 0) {
          ...
          if (body.auto_apply === true) {
            console.log(`[auto-run][IEL] apply-decisions: no pending decisions in autonomous mode — falling through to run-next`, {
              jobId
            });
            await updateJob(supabase, jobId, {
              status: "running",
          ...
          return respond({
            error: "selectedOptions array required"
          }, 400);
        }
      }
      const { data: job, error: jobErr } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).eq("user_id", userId).single();
      ...
      const currentDoc = job.current_document;
      const stepCount = job.step_count + 1;
      const CONTROL_NOTE_IDS = new Set([
        "raise_step_limit_once",
        "run_exec_strategy",
        "force_promote",
        "fallback_force_promote"
      ]);
      const selectedMap = new Map(selectedOptions.filter((s)=>s?.note_id && s?.option_id).map((s)=>[
          String(s.note_id),
          String(s.option_id)
        ]));
      ...
      const selectedContentOptions = selectedOptions.filter((s)=>!CONTROL_NOTE_IDS.has(String(s.note_id)));
      const { data: project } = await supabase.from("projects").select("format, development_behavior").eq("id", job.project_id).single();
      const format = (project?.format || "film").toLowerCase().replace(/_/g, "-");
      const behavior = project?.development_behavior || "market";
      ...
      if (selectedContentOptions.length === 0 && (raiseChoice || runExecChoice || forcePromoteChoice)) {
        ...
        if (raiseChoice === "no" || forcePromoteChoice === "stop") {
          await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", forcePromoteChoice === "stop" ? "User chose to stop and review manually" : "User declined step extension — stopping run", {}, undefined, {
            selectedOptions
          });
          await updateJob(supabase, jobId, {
            step_count: stepCount,
        ...
        if (forcePromoteChoice === "retry") {
          await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", "User chose to retry current stage", {}, undefined, {
            selectedOptions
          });
        ...
        await logStep(supabase, jobId, stepCount, currentDoc, "decision_applied", `Applied control decisions: ${controlSummary || "none"}`, {}, undefined, {
          selectedOptions
        });
        await updateJob(supabase, jobId, {
          step_count: stepCount,
      ...
      if (clientSourceVersionId && !docId) {
        const { data: srcVer } = await supabase.from("project_document_versions").select("document_id").eq("id", clientSourceVersionId).single();
        if (srcVer) docId = srcVer.document_id;
      }
      // Fallback: resolve latest document/version for current stage if pending not set
      if (!docId || !versionId) {
        const { data: latestDoc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
          ascending: false
        }).limit(1).single();
        if (latestDoc) {
          docId = latestDoc.id;
          const { data: latestVer } = await supabase.from("project_document_versions").select("id").eq("document_id", latestDoc.id).order("version_number", {
            ascending: false
          }).limit(1).single();
      ...
      if (!docId || !versionId) {
        ...
        if (ladder) {
          ...
          if (curIdx > 0) {
            ...
            const { data: prevDoc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", prevStage).order("created_at", {
              ascending: false
            }).limit(1).single();
            if (prevDoc) {
              docId = prevDoc.id;
              const { data: prevVer } = await supabase.from("project_document_versions").select("id").eq("document_id", prevDoc.id).order("version_number", {
                ascending: false
              }).limit(1).single();
      ...
      if (!docId || !versionId) return respond({
        error: "No document/version found for current stage"
      }, 400);
      // Fetch latest notes for protect items
      const notesResult = await supabase.from("development_runs").select("output_json").eq("document_id", docId).eq("run_type", "NOTES").order("created_at", {
        ascending: false
      }).limit(1).single();
      const notes = notesResult.data?.output_json;
      ...
      // ── DOC POLICY GUARD (apply-decisions-and-continue) — FAIL CLOSED ──
      {
        ...
        if (decPolicy.docClass === "UNIT" && decPolicy.requiresEpisodeIndex) {
          ...
          const { data: unitDocRow } = await supabase.from("project_documents").select("meta_json").eq("id", docId).single();
      ...
      try {
        await logStep(supabase, jobId, stepCount, currentDoc, "apply_decisions", `Applying ${rewriteSelectedOptions.length} decisions with rewrite`, {}, undefined, {
        ...
        const newVersionId = newVersionId_raw || "unknown";
        await logStep(supabase, jobId, stepCount + 1, currentDoc, "decisions_applied_rewrite", `Decisions applied, new version: ${newVersionId}`, {}, undefined, {
          docId,
          newVersionId,
          selectedOptions: rewriteSelectedOptions.length
        });
        ...
        const ledgerIdsToResolve = rewriteSelectedOptions.map((s)=>s?.note_id).filter((id)=>!!id);
        if (ledgerIdsToResolve.length > 0) {
          // Separate UUIDs from string decision_keys
          const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
          const uuidIds = ledgerIdsToResolve.filter((id)=>uuidPattern.test(id));
          const keyIds = ledgerIdsToResolve.filter((id)=>!uuidPattern.test(id));
          let resolvedCount = 0;
          // Resolve by UUID id
          if (uuidIds.length > 0) {
            const { error: err1 } = await supabase.from("decision_ledger").update({
              status: "active"
            }).in("id", uuidIds).eq("status", "workflow_pending");
            if (err1) {
          ...
          if (keyIds.length > 0) {
            const { error: err2 } = await supabase.from("decision_ledger").update({
              status: "active"
            }).in("decision_key", keyIds).eq("project_id", job.project_id).eq("status", "workflow_pending");
            if (err2) {
              console.warn(`[auto-run][IEL] decision_ledger_resolve_failed_key`, JSON.stringify({
                job_id: jobId,
                keys: keyIds,
        ...
        return respond({
          error: `Decisions rewrite failed: ${e.message}`
        }, 500);
      }
    }
    // ═══════════════════════════════════════
    ...
    if (action === "run-next") {
      ...
      console.log("[auto-run] run-next start", {
        jobId
      });
      ...
      let preJob = preJobRaw;
      if (preJobErr || !preJob) return respond({
        error: "Job not found"
      }, 404);
      if (preJob.awaiting_approval) return respond({
        job: preJob,
        latest_steps: [],
        next_action_hint: "awaiting-approval"
      });
      ...
      if (preJob.pending_decisions && preJob.pending_decisions.length > 0) {
        return respond({
          job: preJob,
          latest_steps: [],
          next_action_hint: "approve-decision"
        });
      }
      // Auto-resume stale criteria pause when Auto-Decide is ON
      if (preJob.status !== "running" && preJob.status === "paused" && preJob.pause_reason === "CRITERIA_STALE_PROVENANCE" && preJob.allow_defaults === true) {
      ...
      if (preJob.status !== "running") return respond({
        job: preJob,
        latest_steps: [],
        next_action_hint: getHint(preJob)
      });
      // ── SINGLE-FLIGHT LOCK: acquire processing lock ──
      const job = await acquireProcessingLock(supabase, jobId, userId, actor === "service_role");
      if (!job) {
        console.log("[auto-run] run-next lock not acquired (another invocation processing)", {
          jobId
        });
        return respondWithJob(supabase, jobId, "wait");
      }
      ...
      const rawCurrentDoc = job.current_document;
      const currentDoc = isGeneratingSentinel(rawCurrentDoc) ? stageFromSentinel(rawCurrentDoc) : rawCurrentDoc;
      ...
      {
        const { data: dbDoc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
          ascending: false
        }).limit(1).maybeSingle();
        if (dbDoc) {
          const { data: dbApproved } = await supabase.from("project_document_versions").select("id, meta_json, version_number").eq("document_id", dbDoc.id).eq("is_current", true).eq("approval_status", "approved").maybeSingle();
          if (dbApproved) {
            const dbMeta = dbApproved.meta_json && typeof dbApproved.meta_json === "object" ? dbApproved.meta_json : {};
            const dbCi = typeof dbMeta?.ci === "number" ? dbMeta.ci : null;
            const dbGp = typeof dbMeta?.gp === "number" ? dbMeta.gp : null;
      ...
      // getStageBestFromSteps() is job-scoped; this guard checks DB-persisted approval.
      {
        ...
        const { data: satisfiedDoc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
          ascending: false
        }).limit(1).maybeSingle();
        if (satisfiedDoc) {
          const { data: approvedVers } = await supabase.from("project_document_versions").select("id, meta_json, version_number").eq("document_id", satisfiedDoc.id).eq("approval_status", "approved").order("version_number", {
            ascending: false
          });
          if (approvedVers && approvedVers.length > 0) {
            const best = approvedVers.map((v)=>({
                ...v,
                _ci: parseVersionScores(v.meta_json).ci ?? 0,
                _gp: parseVersionScores(v.meta_json).gp ?? 0
              })).sort((a, b)=>b._ci + b._gp - (a._ci + a._gp))[0];
            if (best._ci >= satisfiedTargetCi) {
              ...
              await logStep(supabase, jobId, stepCount, currentDoc, "stage_already_satisfied", `Stage ${currentDoc} already has approved version CI:${best._ci}, GP:${best._gp} (target: ${satisfiedTargetCi}). Advancing to ${nextStage ?? "complete"}.`, {
                ci: best._ci,
                gp: best._gp
              }, undefined, {
                approved_version_id: best.id,
                target_ci: satisfiedTargetCi,
                next_stage: nextStage
              });
              if (!nextStage) {
                await updateJob(supabase, jobId, {
                  status: "stopped",
                  stop_reason: "All stages complete",
                  last_ci: best._ci,
                  last_gp: best._gp
                });
                await releaseProcessingLock(supabase, jobId);
      ...
      const { data: project } = await supabase.from("projects").select("title, format, development_behavior, episode_target_duration_seconds, season_episode_count, guardrails_config, assigned_lane, budget_range, genres").eq("id", job.project_id).single();
      ...
      // ── IEL: re-validate target_document on every run-next ──
      {
        ...
        if (ielCheck.corrected) {
          ...
          await supabase.from("auto_run_jobs").update({
            target_document: ielCheck.target
          }).eq("id", jobId);
          job.target_document = ielCheck.target;
          await logStep(supabase, jobId, null, currentDoc, "iel_target_corrected", ielCheck.log || `Target corrected to ${ielCheck.target}`);
        }
      }
      ...
      // fresh review guard so we never start an extra ANALYZE call unnecessarily.
      {
        ...
        const { count: reviewCallCount } = await supabase.from("auto_run_steps").select("id", {
          count: "exact",
          head: true
        }).eq("job_id", jobId).eq("document", currentDoc).eq("action", "review");
        const iterCount = reviewCallCount ?? 0;
        if (iterCount >= MAX_STAGE_ITERATIONS) {
          const ciGate1 = await evaluateCIGate(supabase, jobId, currentDoc, targetCiForIterCap, job.project_id, job);
          if (!ciGate1.pass) {
            // Over the iteration cap and still below target — force-promote best or pause
            console.warn(`[auto-run] max_stage_iterations_reached { job_id: "${jobId}", doc_type: "${currentDoc}", iterations: ${iterCount}, best_ci: ${ciGate1.bestCiSoFar}, target: ${targetCiForIterCap} }`);
            // ── EXCEPTIONAL: route through recoverable plateau recovery instead of hard-stopping ──
            if (isExceptionalObjective(job)) {
              console.log(`[auto-run][PLATEAU_RECOVERY] exceptional_iter_cap { job_id: "${jobId}", doc_type: "${currentDoc}", iterations: ${iterCount}, best_ci: ${ciGate1.bestCiSoFar}, target: ${targetCiForIterCap} }`);
              return routePlateauRecovery(supabase, {
                jobId,
                job,
                currentDoc,
                ...
                detectedCi: ciGate1.ci,
                detectedBestCi: ciGate1.bestCiSoFar,
                plateauVersion: "v1",
                escalationSource: "iteration_cap_exceptional"
              });
            } else if ((job.allow_defaults || job.meta_json?.auto_approve_all) && ciGate1.bestCiSoFar >= GLOBAL_MIN_CI) {
              ...
              await logStep(supabase, jobId, stepCount, currentDoc, "stop", `MAX_STAGE_ITERATIONS (${MAX_STAGE_ITERATIONS}) reached for ${currentDoc}. Best CI: ${ciGate1.bestCiSoFar}, target: ${targetCiForIterCap}. Pausing — manual review required.`);
              await updateJob(supabase, jobId, {
                status: "paused",
                stop_reason: "MAX_ITERATIONS_REACHED",
                pause_reason: "MAX_ITERATIONS_REACHED",
                error: `Stage ${currentDoc} reached max ${MAX_STAGE_ITERATIONS} iterations at CI=${ciGate1.bestCiSoFar} (target: ${targetCiForIterCap}).`
              });
              persistPlateauDiagnosis(supabase, {
                job,
                jobId,
                currentDoc,
                bestCi: ciGate1.bestCiSoFar,
                finalCi: ciGate1.ci,
                targetCi: targetCiForIterCap,
                targetGp: extractTargetGP(job),
      ...
      // ── IEL: FRESH REVIEW GUARD — must review latest version before plateau evaluation ──
      {
        ...
        if (freshCheck.needed) {
          ...
          await logStep(supabase, jobId, stepCount, currentDoc, "fresh_review_required", `Skipping CI gate — active version ${freshCheck.activeVersionId} (${freshCheck.activeSource}) not yet reviewed (last reviewed: ${freshCheck.lastReviewedVersionId}). Will review first.`, {}, undefined, {
            active_version_id: freshCheck.activeVersionId,
            active_source: freshCheck.activeSource,
            last_reviewed_version_id: freshCheck.lastReviewedVersionId,
            last_analyzed_version_id: job.last_analyzed_version_id,
            reason: freshCheck.reason
          });
        // Skip CI gate entirely — fall through to analysis block which will review the active version
        } else {
          // ── SIMPLE PROMOTION: if notes exhausted and CI >= GLOBAL_MIN_CI, promote directly ──
          // This is the simple flow Sebastian described: review → no notes → promote.
          // It fires BEFORE the plateau gates so we never get stuck cycling.
          {
            const { data: anyOpenNotes } = await supabase.from("project_notes").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
              "open",
              "in_progress",
            ...
            if (notesExhausted) {
              ...
              if (bestForDoc) {
                if (bestCi >= GLOBAL_MIN_CI) {
                  ...
                  await logStep(supabase, jobId, stepCount + 1, currentDoc, "simple_notes_exhausted_promote", `Notes exhausted (CI:${bestCi} ≥ ${GLOBAL_MIN_CI}). Promoting directly.`, {
                    ci: bestCi,
                    gp: bestForDoc.gp
                  }, undefined, {
                  ...
                  if (!promoteErr) {
                    await persistVersionScores(supabase, {
                      ...
                      ci: bestForDoc.ci,
                      gp: bestForDoc.gp,
                      source: "auto-run-simple-promote",
                      jobId,
                      protectHigher: true,
                      docType: currentDoc
                    }).catch(()=>{});
                    ...
                    const nextDoc = await nextUnsatisfiedStage(supabase, job.project_id, format, currentDoc, job.target_document, job.allow_defaults, job.user_id, jobId);
                    if (nextDoc && isStageAtOrBeforeTarget(nextDoc, job.target_document, format)) {
                      await logStep(supabase, jobId, stepCount + 2, currentDoc, "simple_promoted", `Promoted ${currentDoc} (CI:${bestForDoc.ci}) → ${nextDoc}. No notes remain to apply.`, {
                        ci: bestForDoc.ci,
                        gp: bestForDoc.gp
                      }, undefined, {
                        from: currentDoc,
                  ...
                  await logStep(supabase, jobId, stepCount + 1, currentDoc, "simple_notes_exhausted_below_floor", `Notes exhausted but CI (${bestCi}) < floor (${GLOBAL_MIN_CI}). Promoting best available directly.`, {
                    ci: bestCi,
                    gp: bestForDoc.gp
                  }, undefined, {
                    action: "direct_promote",
                  ...
                  if (!promoteErr) {
                    ...
                    const nextDoc = await nextUnsatisfiedStage(supabase, job.project_id, format, currentDoc, job.target_document, job.allow_defaults, job.user_id, jobId);
                    if (nextDoc && isStageAtOrBeforeTarget(nextDoc, job.target_document, format)) {
                      await logStep(supabase, jobId, stepCount + 2, currentDoc, "simple_below_floor_promoted", `Promoted ${currentDoc} (CI:${bestForDoc.ci}) → ${nextDoc}. CI below floor but no notes remain.`, {
                        ci: bestForDoc.ci,
                        gp: bestForDoc.gp
                      }, undefined, {
                        from: currentDoc,
                        to: nextDoc,
                        best_version_id: bestForDoc.versionId,
                        trigger: "notes_exhausted_below_floor"
                      });
                      await updateJob(supabase, jobId, {
                        current_document: nextDoc,
                        stage_loop_count: 0,
          ...
          // Fail-closes on plateau below GLOBAL_MIN_CI.
          {
            ...
            if (isPlateauV2Enabled()) {
              ...
              console.log(`[auto-run][IEL] plateau_v2_eval ${JSON.stringify({
                job_id: jobId,
                doc_type: currentDoc,
                ...plateauV2
              })}`);
              ...
              if (plateauV2.isCeilingHit) {
                ...
                await logStep(supabase, jobId, stepCount + 1, currentDoc, "ci_ceiling_hit", `CEILING_HIT: ${plateauV2.ceilingDiagnostic} Human review required to promote or abandon.`, {
                  ci: plateauV2.currentCI,
                  estimated_ceiling: plateauV2.estimatedCeiling
                }, undefined, {
                  ceiling_hit: true,
                  ceiling_diagnostic: plateauV2.ceilingDiagnostic,
                  estimated_ceiling: plateauV2.estimatedCeiling,
                  current_ci: plateauV2.currentCI,
                  format
                });
                const { data: docForEsc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                  ascending: false
                ...
                persistPlateauDiagnosis(supabase, {
                  ...
                  jobId,
                  currentDoc,
                  bestCi: plateauV2.currentCI,
                  finalCi: plateauV2.currentCI,
                  targetCi,
                  targetGp: extractTargetGP(job),
                  haltReason: "CEILING_HIT",
                  stepCount,
                  stageLoopCount: job.stage_loop_count ?? 0
                }).then(undefined, (e)=>console.error(`[auto-run][DIAG] fire_forget: ${e?.message}`));
                // ── Create QUALITY_CEILING workflow decision ──
                try {
                  const decisionMode = job.allow_defaults === true || job.meta_json?.auto_decide_all === true || job.full_autopilot === true ? "autonomous" : "strict";
                  await checkQualityCeiling(supabase, job.project_id, jobId, format, currentDoc, plateauV2.currentCI, plateauV2.estimatedCeiling ?? 85, plateauV2.ceilingDiagnostic ?? "Content at structural CI ceiling", decisionMode);
                } catch (e) {
              ...
              if (plateauV2.isPlateaued) {
                ...
                if (job.allow_defaults || job.meta_json?.auto_approve_all) {
                  ...
                  const { data: plateauOpenNotes } = await supabase.from("project_notes").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
                    "open",
                    "in_progress",
                    "reopened"
                  ]).in("severity", [
                  ...
                  if (plateauOpenNotes && plateauOpenNotes.length > 0) {
                    ...
                    await logStep(supabase, jobId, stepCount + 1, currentDoc, "note_driven_rewrite_continue", `CI plateaued (V2) at ${plateauV2.currentCI} but ${plateauOpenNotes.length} blocker/high note(s) remain. Skipping plateau gate → rewrite loop will apply notes.`, {
                      ci: plateauV2.currentCI
                    }, undefined, {
                      plateau_version: "v2",
                      note_count: plateauOpenNotes.length,
                      note_ids: plateauOpenNotes.map((n)=>n.id)
                    });
                  // Fall through to analysis/rewrite block — DO NOT pause
                    ...
                    await logStep(supabase, jobId, stepCount + 1, currentDoc, "notes_unresolvable", `CI plateaued (V2) at ${plateauV2.currentCI}, no blocker/high notes remain, best CI:${bestAvail?.ci ?? "?"} below target ${targetCi}. Cannot improve further.`, {
                      ci: plateauV2.currentCI
                    }, undefined, {
                      plateau_version: "v2",
                      best_ci: bestAvail?.ci,
                      best_gp: bestAvail?.gp,
                      target_ci: targetCi
                    });
                    const { data: docForCap } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                      ascending: false
                    }).limit(1).maybeSingle();
                    ...
                    // ── CLEANUP PASS GATE: try applying remaining polish notes before escalating ──
                    {
                      ...
                      const cleanup = await tryCleanupBeforeEscalation(supabase, _supabaseUrl, _token, {
                        ...
                        format,
                        stepCount,
                        currentCi: plateauV2.currentCI,
                        currentGp: bestAvail?.gp ?? 0,
                        targetCi,
                        escalationSource: "notes_unresolvable_v2"
                      });
                      if (cleanup.attempted && cleanup.accepted) {
                      ...
                      // Fall through to analysis/rewrite block — DO NOT pause
                        ...
                        return routePlateauRecovery(supabase, {
                          ...
                          targetCi,
                          detectedCi: plateauV2.currentCI,
                          detectedBestCi: bestAvail?.ci ?? plateauV2.currentCI,
                          plateauVersion: "v2",
                          escalationSource: "notes_unresolvable_v2"
                        });
                  ...
                  console.error(`[auto-run][IEL] plateau_v2_stop ${JSON.stringify({
                    job_id: jobId,
                    doc_type: currentDoc,
                    ...plateauV2
                  })}`);
                  await logStep(supabase, jobId, stepCount + 1, currentDoc, "ci_plateau_stop", `PLATEAU V2: CI=${plateauV2.currentCI}, blocker_delta=${plateauV2.blockerCountDelta}, hi_delta=${plateauV2.highImpactCountDelta}. Neither CI improving nor notes shrinking. Fail-closed.`, {
                    ci: plateauV2.currentCI
                  }, undefined, {
                    ...plateauV2,
                    global_min_ci: targetCi,
                    plateau_version: "v2"
                  });
                  const { data: docForCap } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                    ascending: false
                  ...
                  persistPlateauDiagnosis(supabase, {
                    ...
                    targetCi,
                    targetGp: extractTargetGP(job),
                    haltReason: "CI_PLATEAU_BELOW_TARGET",
                    stepCount,
                    stageLoopCount: job.stage_loop_count ?? 0
                  }).then(undefined, (e)=>console.error(`[auto-run][DIAG] fire_forget: ${e?.message}`));
                  await releaseProcessingLock(supabase, jobId);
              ...
              if (ciProgress.bestCi >= targetCi) {
                ...
                if (job.allow_defaults || job.meta_json?.auto_approve_all) {
                  ...
                  // ── IEL: tryPlateauForcePromote returned null. Check WHY: blocker/high notes remain or CI too low. ──
                  // Only blocker + high severity notes block promotion. Polish notes do not.
                  const { data: plateauOpenNotesV1 } = await supabase.from("project_notes").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
                    "open",
                    "in_progress",
                    "reopened"
                  ]).in("severity", [
                  ...
                  if (plateauOpenNotesV1 && plateauOpenNotesV1.length > 0) {
                    ...
                    await logStep(supabase, jobId, stepCount + 1, currentDoc, "note_driven_rewrite_continue", `CI plateaued (V1) at ${ciProgress.currentCi} (best: ${ciProgress.bestCi}) but ${plateauOpenNotesV1.length} blocker/high note(s) remain. Skipping plateau gate → rewrite loop will apply notes.`, {
                      ci: ciProgress.currentCi
                    }, undefined, {
                      plateau_version: "v1",
                      note_count: plateauOpenNotesV1.length,
                      note_ids: plateauOpenNotesV1.map((n)=>n.id),
                      plateau_count: ciProgress.plateauCount
                    });
                  // Fall through to analysis/rewrite block — DO NOT pause
                  } else {
                    ...
                    await logStep(supabase, jobId, stepCount + 1, currentDoc, "notes_unresolvable", `CI plateaued (V1) at ${ciProgress.currentCi}, no blocker/high notes remain, best CI:${bestAvailV1?.ci ?? "?"} below target ${targetCi}. Cannot improve further.`, {
                      ci: ciProgress.currentCi
                    }, undefined, {
                      plateau_version: "v1",
                      best_ci: bestAvailV1?.ci,
                      best_gp: bestAvailV1?.gp,
                      target_ci: targetCi,
                      plateau_count: ciProgress.plateauCount
                    });
                    const { data: docForCap } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                    ...
                    // ── CLEANUP PASS GATE: try applying remaining polish notes before escalating ──
                    {
                      ...
                      const cleanupV1 = await tryCleanupBeforeEscalation(supabase, _supabaseUrl, _token, {
                        ...
                        format,
                        stepCount,
                        currentCi: ciProgress.bestCi,
                        currentGp: bestAvailV1?.gp ?? 0,
                        targetCi,
                        escalationSource: "notes_unresolvable_v1"
                      });
                      ...
                      // Fall through to continue pipeline
                        ...
                        // ── Route all plateau cases through recovery router ──
                        return routePlateauRecovery(supabase, {
                          jobId,
                          job,
                          currentDoc,
                  ...
                  await logStep(supabase, jobId, stepCount + 1, currentDoc, "ci_plateau_stop", `CI PLATEAU BELOW ${targetCi}: CI=${ciProgress.currentCi}, best=${ciProgress.bestCi}, ${ciProgress.plateauCount} consecutive non-improving ticks. Fail-closed.`, {
                    ci: ciProgress.currentCi
                  }, undefined, {
                    best_ci: ciProgress.bestCi,
                    plateau_count: ciProgress.plateauCount,
                    global_min_ci: targetCi
                  });
                  const { data: docForCap } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                  ...
                  persistPlateauDiagnosis(supabase, {
                    ...
                    jobId,
                    currentDoc,
                    bestCi: ciProgress.bestCi,
                    finalCi: ciProgress.currentCi,
                    targetCi,
                    targetGp: extractTargetGP(job),
                    haltReason: "CI_PLATEAU_BELOW_TARGET",
                    stepCount,
      ...
      // ── IEL: Version cap guard — SAFETY NET ONLY (monotonic CI loop is primary limiter) ──
      {
        ...
        if (docForCap) {
          ...
          const { count: versionCount } = await supabase.from("project_document_versions").select("id", {
            count: "exact",
            head: true
          }).eq("document_id", docForCap.id).gte("created_at", job.created_at);
          ...
          if (typeof versionCount === "number" && versionCount >= effectiveCap) {
            ...
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "rewrite_cap_reached", `Paused: ${versionCount} job-scoped versions for ${currentDoc} exceed cap of ${effectiveCap} [safety net]`);
            await releaseProcessingLock(supabase, jobId);
            return respondWithJob(supabase, jobId);
          }
        }
      ...
      // Vertical drama ladder enforces minimum composite scores before allowing progression.
      {
        ...
        if (prereqResult.blocked) {
          ...
          await logStep(supabase, jobId, stepCount + 1, currentDoc, "prereq_gate_blocked", `Stage "${currentDoc}" blocked: prerequisite "${prereq.prerequisiteStage}" composite ${prereq.composite} < required ${prereq.requiredComposite}. Redirecting to ${prereq.prerequisiteStage}.`, {
            ci: prereq.ci,
            gp: prereq.gp
          }, undefined, {
            prerequisite_stage: prereq.prerequisiteStage,
            composite: prereq.composite,
            required: prereq.requiredComposite
          });
      ...
      // ── IEL: STAGE IDENTITY GATE — HARD BLOCK malformed documents before review/scoring/convergence ──
      {
        const stageIdDocTypes = new Set([
          "idea",
          "concept_brief"
        ]);
        if (stageIdDocTypes.has(currentDoc)) {
          const { data: sidDoc } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
            ascending: false
          }).limit(1).maybeSingle();
          if (sidDoc) {
            const { data: sidVer } = await supabase.from("project_document_versions").select("id, plaintext, meta_json").eq("document_id", sidDoc.id).eq("is_current", true).eq("approval_status", "approved").maybeSingle();
            if (sidVer?.plaintext) {
              const sidResult = validateStageIdentity(currentDoc, sidVer.plaintext, sidVer.meta_json);
              if (sidResult && !sidResult.pass) {
                const diag = buildDiagnostic(sidResult);
                console.error(`[auto-run][IEL] STAGE_IDENTITY_BLOCKED ${JSON.stringify({
                  job_id: jobId,
                  ...diag
                })}`);
                await logStep(supabase, jobId, stepCount, currentDoc, "stage_identity_blocked", `HARD BLOCK: ${currentDoc} failed stage identity: ${sidResult.violation}. ${sidResult.repair_hint || ""}`, {}, undefined, diag);
                // Persist violation in version meta_json for UI consumption
                const existingMeta = sidVer.meta_json && typeof sidVer.meta_json === "object" ? sidVer.meta_json : {};
                await supabase.from("project_document_versions").update({
                  meta_json: {
                    ...existingMeta,
                    stage_identity: {
                ...
                // HARD GATE: Pause the job. Do NOT allow review, scoring, convergence, or promotion on malformed stage artifacts.
                await updateJob(supabase, jobId, {
                  status: "paused",
                  pause_reason: `STAGE_IDENTITY_BLOCKED: ${sidResult.violation}`,
                  stop_reason: `Stage identity violation: ${sidResult.violation}. ${sidResult.repair_hint || "Regenerate the document with correct stage constraints."}`,
                  last_ui_message: `${currentDoc} document is invalid for its stage: ${sidResult.violation}. ${sidResult.repair_hint || ""}`,
                  pending_decisions: [
                    {
                      id: "stage_identity_repair",
                      question: `The ${currentDoc} document failed stage identity validation: ${sidResult.violation}. How should this be resolved?`,
                      options: [
                        {
                          value: "regenerate",
                          why: `Regenerate as a valid ${currentDoc}-stage artifact with correct constraints`
                        },
                        {
                          value: "compress",
                          why: sidResult.violation === "IDEA_TOO_EXPANDED" ? "Compress the document back to idea-stage density" : `Strip invalid formatting and regenerate as proper ${currentDoc}`
                        }
                      ],
                      recommended: "regenerate",
                      impact: "blocking",
                      source: "stage_identity_gate",
                      violation: sidResult.violation,
                      repair_hint: sidResult.repair_hint
                    }
                  ]
                ...
                await releaseProcessingLock(supabase, jobId);
                return respondWithJob(supabase, jobId, "stage-identity-blocked");
              }
            }
          }
        }
      ...
      {
        const runLadder = getLadderForJob(format) || [];
        const gateResult = await runPendingDecisionGate(supabase, job.project_id, jobId, format, currentDoc, runLadder, job.allow_defaults !== false, job.decision_mode || "strict");
        if (gateResult.shouldPause) {
          await logStep(supabase, jobId, stepCount + 1, currentDoc, "pending_decisions", gateResult.logSummary, {}, undefined, {
            blockingIds: gateResult.blockingIds,
            deferrableIds: gateResult.deferrableIds
          });
          // Fetch full decision details for blocking IDs so UI can render question/options
          let enrichedDecisions = gateResult.blockingIds.map((id)=>({
              id,
              impact: "blocking",
              source: "decision_policy_registry"
            }));
          if (gateResult.blockingIds.length > 0) {
            const { data: fullRows } = await supabase.from("decision_ledger").select("id, decision_key, title, decision_text, decision_value").in("id", gateResult.blockingIds);
            if (fullRows && fullRows.length > 0) {
              enrichedDecisions = fullRows.map((row)=>{
              ...
              console.log(`[auto-run][IEL] pending_decision_emitted`, JSON.stringify({
                ...
                has_options: enrichedDecisions.map((d)=>d.options?.length > 0),
                has_question: enrichedDecisions.map((d)=>!!d.question)
              }));
            }
          }
          ...
          if (isAutonomous && gateResult.blockingIds.length > 0) {
            ...
            for (const d of enrichedDecisions){
              ...
              const recommended = opts.find((o)=>o.is_recommended === true || o.value === d.recommended);
              ...
              await supabase.from("decision_ledger").update({
                ...
                decision_value: {
                  ...
                  resolved_by: "auto_run",
                  resolved_at: new Date().toISOString()
                }
              }).eq("id", d.id).then(()=>{}).then(undefined, (e)=>console.warn(`[auto-run][IEL] decision_auto_apply_failed: ${e?.message}`));
              autoApplied++;
              autoAppliedKeys.push(d.decision_key || d.id);
            }
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "decisions_auto_applied", `Auto-applied ${autoApplied} decision(s) (allow_defaults=ON). Continuing run.`, {}, undefined, {
              auto_applied: autoApplied,
              decision_keys: autoAppliedKeys
            });
      ...
      try {
        ...
        if (seedResult.failed) {
          ...
          await logStep(supabase, jobId, stepCount + 1, currentDoc, "seed_pack_failed", `${stopReason} — cannot proceed. ${compactError.slice(0, 200)}`);
          return respond({
            job: {
              ...job,
              status: "failed",
        ...
        // ── LADDER DOC-SLOT PREFLIGHT on resume ──
        {
          ...
          if (resumeLadder && resumeLadder.length > 0) {
            ...
            if (created.length > 0) {
              ...
              await logStep(supabase, jobId, null, currentDoc, "doc_slots_ensured", `Resume preflight: created ${created.length} ladder doc slots: ${created.join(", ")}`, {}, undefined, {
                created,
                format
              });
            }
        ...
        // ── INPUT READINESS GATE: prevent spinning on empty/stub inputs (with auto-regen) ──
        {
          ...
          let regenWasOk = false;
          if (!inputCheck.ready) {
            console.log("[auto-run] INPUT_INCOMPLETE — attempting auto-regen", {
              jobId,
              missing: inputCheck.missing_fields
            });
            const regenAttempt = await attemptAutoRegenInputs(supabase, supabaseUrl, token, jobId, stepCount + 1, currentDoc, job.project_id, inputCheck.missing_fields, "run_next_gate");
            regenWasOk = regenAttempt.ok;
            if (!regenAttempt.ok) {
              console.warn("[auto-run] run-next auto-regen did not resolve inputs", {
                jobId,
                error: regenAttempt.error
              });
            }
            ...
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "auto_regen_inputs", inputCheck.ready ? `Post-regen readiness: READY` : `Post-regen readiness: STILL MISSING ${inputCheck.missing_fields.join(", ")}`, {}, undefined, {
          ...
          if (!inputCheck.ready) {
            ...
            console.warn("[auto-run] INPUT_INCOMPLETE (after regen attempt)", {
              jobId,
              missing: inputCheck.missing_fields
            });
            const compactErr = inputCheck.summary.slice(0, 500);
            ...
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "pause_for_input", `INPUT_INCOMPLETE: ${compactErr}`, {}, undefined, {
              missing_fields: inputCheck.missing_fields,
              regen_was_ok: regenWasOk
            });
            return respond({
        ...
        {
          const EPISODE_GATED_STAGES = [
            "episode_script",
            "season_master_script"
          ];
          if (EPISODE_GATED_STAGES.includes(currentDoc)) {
            const { data: epProj } = await supabase.from("projects").select("season_episode_count, season_episode_count_locked").eq("id", job.project_id).single();
            ...
            if (typeof epN !== "number" || epN < 1) {
              ...
              await logStep(supabase, jobId, stepCount + 1, currentDoc, "pause_for_input", "Episode count not set — cannot proceed to episode generation");
              return respond({
                job: {
                  ...job,
                  status: "paused",
        ...
        if (stepCount >= job.max_total_steps) {
          ...
          await logStep(supabase, jobId, stepCount + 1, currentDoc, "pause_step_limit", `Auto-paused: step limit reached`, {
            ci: job.last_ci,
            gp: job.last_gp,
            gap: job.last_gap,
            readiness: job.last_readiness
          });
        ...
        if (currentDoc === job.target_document && stageLoopCount > 0) {
          const { data: _tgtDocRow } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", job.target_document).maybeSingle();
          ...
          if (_tgtLen > 0 && _ciMet) {
            await updateJob(supabase, jobId, {
              status: "completed",
              stop_reason: "Reached target document"
            });
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "stop", `Target document reached (CI:${_tgtCi} ≥ ${_tgtMinCi})`);
            return respondWithJob(supabase, jobId);
          }
          if (_tgtLen > 0 && !_ciMet) {
            // Content exists but CI not yet at target — fall through to continue convergence
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "converging", `Target doc '${job.target_document}' exists but CI:${_tgtCi} < target:${_tgtMinCi} — continuing convergence`);
          } else {
            // No content yet — fall through to generate
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "target_doc_not_yet_generated", `Target doc '${job.target_document}' reached but no content yet — generating`);
          }
        }
        ...
        {
          const _fmtCheck = job._cached_format_for_guard; // may be undefined — safe
          const { data: _projFmtRow } = await supabase.from("projects").select("format").eq("id", job.project_id).single();
          const _fmtNorm = (_projFmtRow?.format || "film").toLowerCase().replace(/_/g, "-");
          const EPISODIC_FORMATS = [
            "tv-series",
            "limited-series",
            "vertical-drama",
            "digital-series",
          ...
          if (EPISODIC_FORMATS.includes(_fmtNorm) && currentDoc === "script") {
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "series_writer_handoff", `Episodic format (${_fmtNorm}): "script" stage is owned by Series Writer. AutoRun paused — open Series Writer to generate/revise episodes.`);
            await updateJob(supabase, jobId, {
              step_count: stepCount + 1,
              status: "paused",
        ...
        if (preflight.changed) {
          await logStep(supabase, jobId, stepCount, currentDoc, "preflight_resolve", `Resolved: ${Object.keys(preflight.resolved).join(", ")}`);
        }
        ...
        if (preflight.missing_required.length > 0 && !allowDefaults) {
          ...
          await logStep(supabase, jobId, stepCount + 1, currentDoc, "pause_missing_criteria", `Missing required criteria: ${missingStr}. Please fill in Criteria panel.`);
          await updateJob(supabase, jobId, {
            step_count: stepCount + 1,
            status: "paused",
        ...
        let resolvedQuals = null;
        let resolverHash = null;
        try {
          ...
          if (resolverResp.ok) {
            ...
            const { data: lastStepWithHash } = await supabase.from("auto_run_steps").select("step_resolver_hash").eq("job_id", jobId).not("step_resolver_hash", "is", null).order("step_index", {
              ascending: false
            }).limit(1).single();
            ...
            if (prevStepHash && resolverHash && prevStepHash !== resolverHash) {
              ...
              await logStep(supabase, jobId, stepCount, currentDoc, "qualification_hash_changed", `Resolver hash changed: ${prevStepHash} → ${resolverHash}. Episode count/duration may have changed. Invalidating cached context and re-analyzing with canonical values.`, {
                risk_flags: [
                  "qualification_hash_changed",
                  "episode_count_invalidated"
                ]
              });
        ...
        // ═══════════════════════════════════════
        {
          ...
          if (format === "vertical-drama" && currentDoc === "feature_script") {
            console.warn("[auto-run] VD_GUARD: feature_script illegal for vertical-drama, remapping to season_script", {
              jobId,
              currentDoc,
              format
            });
            await logStep(supabase, jobId, stepCount, currentDoc, "vd_feature_script_remap", `VD guard: remapped feature_script → season_script (feature_script is illegal for vertical-drama)`);
            await updateJob(supabase, jobId, {
              current_document: "season_script"
            });
            return respondWithJob(supabase, jobId, "run-next");
          }
          if (needsSetup) {
            console.log(`[auto-run] ${gateLabel} gate entered`, {
              jobId,
              currentDoc,
              format,
              isWritingStage,
            ...
            if (nextRetryAt) {
              ...
              if (now < retryTime) {
                ...
                console.log(`[auto-run] ${gateLabel}: inline backoff wait (${waitSec}s)`, {
                  jobId,
                  nextRetryAt
                });
                await updateJob(supabase, jobId, {
                ...
                await new Promise((res)=>setTimeout(res, waitMs + 200));
              }
            ...
            const { data: canonRow } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
            const cj = canonRow?.canon_json || {};
                ...
                try {
                  ...
                  if (extractResp.ok) {
                    ...
                    if (extractResult.updated || extractResult.reason === "already_populated") {
                      ...
                      // ── FIX: sync canon fields to guardrails_config.story_setup ──
                      // canon_os_extract_from_seed_docs writes to canon_json but episode_grid
                      // reads from guardrails_config.overrides.story_setup — sync them now.
                      try {
                        const { data: canonRow } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
                        const cj = canonRow?.canon_json || {};
                        if (cj && typeof cj === "object") {
                          ...
                          if (Object.keys(storySetup).length > 0) {
                            const { data: proj } = await supabase.from("projects").select("guardrails_config").eq("id", job.project_id).maybeSingle();
                            ...
                            console.log(`[auto-run] ${gateLabel}: story_setup synced to guardrails_config`, {
                              fields: Object.keys(storySetup)
                            });
                          }
                        }
                      } catch (syncErr) {
                        console.warn(`[auto-run] ${gateLabel}: story_setup sync failed (non-fatal):`, syncErr.message);
                      }
                      break;
                    }
                  }
                } catch (e) {
                  console.warn(`[auto-run] ${gateLabel}: canon extract attempt ${attempt + 1} failed`, {
                    error: e.message
                  });
                }
                // Re-check DB in case another process already populated it
                const { data: recheck } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
                const rcj = recheck?.canon_json || {};
                const recheckOk = Array.isArray(rcj.world_rules) && rcj.world_rules.length > 0 && typeof rcj.logline === "string" && rcj.logline.trim().length > 0;
                if (recheckOk) {
                  extractOk = true;
                  setupResolved.push("canon_os_found_on_recheck");
                  // ── FIX: sync canon fields to guardrails_config.story_setup (recheck path) ──
                  try {
                    const { data: canonRow } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
                    const cj = canonRow?.canon_json || {};
                    if (cj && typeof cj === "object") {
                      ...
                      if (Object.keys(storySetup).length > 0) {
                        ...
                        console.log(`[auto-run] ${gateLabel}: story_setup synced to guardrails_config (recheck)`, {
                          fields: Object.keys(storySetup)
                        });
                      }
                    }
                  } catch (syncErr) {
                    console.warn(`[auto-run] ${gateLabel}: story_setup sync failed (recheck, non-fatal):`, syncErr.message);
                  }
                  break;
                }
                if (attempt + 1 < MAX_SETUP_ATTEMPTS) {
                  const retryDelay = BACKOFF_DELAYS[attempt] || 30;
                  console.log(`[auto-run] ${gateLabel}: canon extract failed, inline wait ${retryDelay}s before retry`, {
                    attempt: attempt + 1
                  });
                  await logStep(supabase, jobId, stepCount, currentDoc, `${gateLabel.toLowerCase()}_retry`, `Canon OS extract failed (attempt ${attempt + 1}/${MAX_SETUP_ATTEMPTS}). Retrying in ${retryDelay}s...`, {}, undefined, {
                    attempt: attempt + 1
                  });
            ...
            const lane = project?.assigned_lane || "independent-film";
            const { data: prefsRow } = await supabase.from("project_lane_prefs").select("prefs").eq("project_id", job.project_id).eq("lane", lane).maybeSingle();
            const lanePrefs = prefsRow?.prefs || {};
            const hasVoice = !!lanePrefs.writing_voice?.id || !!lanePrefs.team_voice?.id;
            if (!hasVoice) {
              if (allowDefaults) {
            ...
            if (!hasCanonComps) {
              const { count } = await supabase.from("comparable_candidates").select("id", {
                count: "exact",
                head: true
            ...
            if (!hasCanonComps && !hasTableComps) {
              ...
              if (Array.isArray(packComps) && packComps.length > 0) {
                ...
                const { data: currentCanon } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
                const updatedCanon = {
                  ...currentCanon?.canon_json || {},
                  comparables: packComps.slice(0, 12)
                };
                await supabase.from("project_canon").update({
                  canon_json: updatedCanon,
                  updated_by: job.user_id
                }).eq("project_id", job.project_id);
                setupResolved.push(`comparables_from_pack=${packComps.length}`);
                console.log(`[auto-run] ${gateLabel}: init comparables from seed_intel_pack`, {
                  count: packComps.length
                });
                  ...
                  try {
                    ...
                    if (compResp.ok) {
                      ...
                      if (compResult.generated) {
                        ...
                        console.log(`[auto-run] ${gateLabel}: auto-generated comparables via LLM`, {
                      ...
                      console.warn(`[auto-run] ${gateLabel}: comparables generation HTTP error`, {
                        status: compResp.status
                      });
                    }
                  } catch (compErr) {
                    console.warn(`[auto-run] ${gateLabel}: comparables auto-generate attempt ${attempt + 1} failed`, {
            ...
            if (isWritingStage) {
              const { data: necDoc } = await supabase.from("project_documents").select("id, latest_version_id").eq("project_id", job.project_id).eq("doc_type", "nec").limit(1).maybeSingle();
              ...
              if (!hasNEC) {
                if (allowDefaults) {
                  ...
                  try {
                    ...
                    console.warn(`[auto-run] ${gateLabel}: NEC generation failed`, {
                      error: necErr.message
                    });
                ...
                const { data: necVer } = await supabase.from("project_document_versions").select("id, depends_on_resolver_hash").eq("document_id", necDoc.id).eq("is_current", true).eq("approval_status", "approved").limit(1).maybeSingle();
                ...
                if (necIsStale) {
                  ...
                  try {
                    ...
                    console.warn(`[auto-run] ${gateLabel}: NEC stale regen failed (non-fatal)`, {
                      error: necErr.message
                    });
            ...
            if (setupResolved.length > 0 || setupMissing.length > 0) {
              await logStep(supabase, jobId, stepCount, currentDoc, gateLabel.toLowerCase(), setupMissing.length > 0 ? `${gateLabel} incomplete: missing=[${setupMissing.join(",")}] resolved=[${setupResolved.join(",")}]` : `${gateLabel} complete: resolved=[${setupResolved.join(",")}]`, {}, undefined, {
                setupMissing,
                setupResolved
              });
            }
            ...
            if (setupMissing.length > 0 && allowDefaults) {
              ...
              await logStep(supabase, jobId, stepCount, currentDoc, `${gateLabel.toLowerCase()}_setup_skipped`, `SETUP_SKIPPED (allow_defaults=true): [${failedItems}] could not be auto-populated after ${MAX_SETUP_ATTEMPTS} attempts. Proceeding without — generation will use available context only.`);
              console.warn(`[auto-run] setup skipped in autonomous mode`, {
                jobId,
                currentDoc,
                failedItems
              });
        ...
        if (currentDoc === "idea") {
          const { data: ideaDocs } = await supabase.from("project_documents").select("id, plaintext, extracted_text").eq("project_id", job.project_id).eq("doc_type", "idea").order("created_at", {
            ascending: false
          }).limit(1);
          ...
          if (ideaText.length < 400 || wordCount < 80) {
            ...
            if (ideaDoc) {
              const { data: ideaVersions } = await supabase.from("project_document_versions").select("id").eq("document_id", ideaDoc.id).order("version_number", {
                ascending: false
              }).limit(1);
              const ideaVersion = ideaVersions?.[0];
              if (ideaVersion) {
                try {
                  await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
                    action: "convert",
                    projectId: job.project_id,
                  ...
                  console.error(`[auto-run] thin-idea upsift conversion failed:`, {
                    error: e.message,
                    project_id: job.project_id,
                    document_id: ideaDoc.id,
                    version_id: ideaVersion.id
                  });
                  await logStep(supabase, jobId, stepCount, currentDoc, "conversion-failed", `Thin idea → concept brief conversion failed: ${e.message}. Continuing at concept_brief stage.`, {}, undefined, {
                    error: e.message
                  });
                }
              }
            ...
            await logStep(supabase, jobId, upshiftStep, "idea", "auto_skip_thin_idea", `Idea too thin (${wordCount} words, ${ideaText.length} chars); converting to concept brief`);
            await updateJob(supabase, jobId, {
        ...
        const resumeRiskFlags = [];
        if (!job.follow_latest && job.resume_document_id && job.resume_version_id) {
          // Pinned source — validate it
          const { data: pinnedDoc } = await supabase.from("project_documents").select("id, doc_type, plaintext, extracted_text").eq("id", job.resume_document_id).eq("project_id", job.project_id).single();
          const { data: pinnedVer } = await supabase.from("project_document_versions").select("id, plaintext, version_number").eq("id", job.resume_version_id).eq("document_id", job.resume_document_id).single();
        ...
        if (!doc1) {
          ...
          if (!seedCheck.official) {
            if ((job.allow_defaults || job.meta_json?.auto_approve_all) && seedCheck.missing.length === 0) {
              ...
              await logStep(supabase, jobId, gateStep, currentDoc, "seed_core_auto_approved", `Seed core auto-approved (allow_defaults) — ${seedCheck.unapproved.join(", ")}`);
              await updateJob(supabase, jobId, {
                step_count: gateStep
              });
            // Fall through to continue generation
              ...
              const gateStep = stepCount + 1;
              await logStep(supabase, jobId, gateStep, currentDoc, "seed_core_block", `Seed core not official — missing: [${seedCheck.missing.join(",")}], unapproved: [${seedCheck.unapproved.join(",")}]`);
              await updateJob(supabase, jobId, {
                status: "paused",
                ...
                error: JSON.stringify({
                  missing_seed_docs: seedCheck.missing,
                  unapproved_seed_docs: seedCheck.unapproved
                })
              });
          ...
          if (ladderIdx <= 0) {
            await updateJob(supabase, jobId, {
              status: "failed",
              error: "No source document found for initial stage"
            });
            return respondWithJob(supabase, jobId);
          }
          const prevStage = runNextLadder[ladderIdx - 1];
          const { data: prevDocs } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", prevStage).order("created_at", {
            ascending: false
          }).limit(1);
          ...
          if (!prevDoc) {
            await updateJob(supabase, jobId, {
              status: "failed",
              error: `No document for ${prevStage} to convert from`
            });
            return respondWithJob(supabase, jobId);
          }
          // Prefer approved+current (authoritative) version; fallback to latest by version_number
          const { data: prevVersions } = await supabase.from("project_document_versions").select("id, approval_status, is_current, version_number").eq("document_id", prevDoc.id).order("version_number", {
            ascending: false
          });
          const prevVersionCandidates = prevVersions || [];
          const prevVersion = prevVersionCandidates.find((v)=>v.approval_status === "approved" && v.is_current === true) || prevVersionCandidates.find((v)=>v.approval_status === "approved") || prevVersionCandidates[0] || null;
          if (!prevVersion) {
            await updateJob(supabase, jobId, {
              status: "failed",
              error: `No version for ${prevStage} document`
            });
            return respondWithJob(supabase, jobId);
          }
          ...
          const EPISODE_DOC_TYPES_SET = new Set([
            "episode_grid",
            "vertical_episode_beats",
            "episode_beats",
            "season_script"
          ...
          try {
            ...
            if (useChunkedGenerator) {
              ...
              console.log("[auto-run] Using generate-document chunked pipeline for", currentDoc, {
                projectId: job.project_id
              });
              ...
              if (genResult?.generating === true) {
                ...
                await logStep(supabase, jobId, newStep, currentDoc, "generate", `Background generation started for ${currentDoc} (${genResult?.episode_count ?? "?"} episodes) — waiting for completion`, {}, bgDocId ? `Doc ${bgDocId}` : undefined, bgDocId ? {
                  docId: bgDocId,
                  versionId: bgVersionId,
                  generating: true
                } : undefined);
                await updateJob(supabase, jobId, {
                  step_count: newStep,
                  stage_loop_count: job.stage_loop_count || 0
                });
              ...
              if (!convertedDocId) {
                const { data: newDocs } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                  ascending: false
                }).limit(1);
                convertedDocId = newDocs?.[0]?.id || null;
              }
              if (convertedDocId && !convertedVersionId) {
                const { data: newVers } = await supabase.from("project_document_versions").select("id").eq("document_id", convertedDocId).order("version_number", {
                  ascending: false
                }).limit(1);
              ...
              const ASSIMILATION_TARGETS = new Set([
                "feature_script",
                "production_draft",
                "season_script",
                "episode_script"
              ]);
              let assimilationContext;
              if (ASSIMILATION_TARGETS.has(currentDoc)) {
                assimilationContext = {};
                for (const stage of ASSIMILATION_STAGES){
                  try {
                    const { data: stageDocs } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", stage).order("created_at", {
                      ascending: false
                    }).limit(1);
                    if (stageDocs?.[0]) {
                      const { data: stageVers } = await supabase.from("project_document_versions").select("plaintext, approval_status, is_current").eq("document_id", stageDocs[0].id).order("version_number", {
                        ascending: false
                      });
                      const approved = (stageVers || []).find((v)=>v.approval_status === "approved" && v.is_current) || (stageVers || []).find((v)=>v.approval_status === "approved") || (stageVers || [])[0];
              ...
              const { result: convertResult } = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
                action: "convert",
                projectId: job.project_id,
              ...
              if (convertedDocId) {
                const { data: cvs } = await supabase.from("project_document_versions").select("id").eq("document_id", convertedDocId).order("version_number", {
                  ascending: false
                }).limit(1);
              ...
              if (!convertedDocId) {
                ...
                if (newDocRow) {
                  const { data: newVers } = await supabase.from("project_document_versions").select("id").eq("document_id", newDocRow.id).order("version_number", {
                    ascending: false
                  }).limit(1);
                  convertedVersionId = newVers?.[0]?.id || null;
                }
            ...
            const newStep = stepCount + 1;
            await logStep(supabase, jobId, newStep, currentDoc, "generate", `Generated ${currentDoc} from ${prevStage}${useChunkedGenerator ? ' (chunked pipeline)' : ''}`, {}, convertedDocId ? `Created doc ${convertedDocId}` : undefined, convertedDocId ? {
            ...
            if (job.allow_defaults || job.meta_json?.auto_approve_all) {
              ...
              if (convertedVersionId) {
                ...
                let autoRunCCEMeta = {};
                try {
                  const { data: cvVerRow } = await supabase.from("project_document_versions").select("plaintext, meta_json").eq("id", convertedVersionId).maybeSingle();
                ...
                const { data: existingVer } = await supabase.from("project_document_versions").select("meta_json").eq("id", convertedVersionId).maybeSingle();
                const mergedMeta = {
                  ...existingVer?.meta_json || {},
                  source_version_id: prevVersion.id,
                  ...autoRunCCEMeta
                };
                await supabase.from("project_document_versions").update({
                  approval_status: "approved",
              ...
              await logStep(supabase, jobId, null, currentDoc, "auto_approved_convert", `Auto-approved generated ${currentDoc} (allow_defaults)`, {}, undefined, {
                docId: convertedDocId,
                versionId: convertedVersionId,
                doc_type: currentDoc,
                from_stage: prevStage
              });
              // Re-fetch doc and latestVersion so downstream code has valid references
              if (convertedDocId) {
                const { data: freshDoc } = await supabase.from("project_documents").select("id, doc_type, plaintext, extracted_text").eq("id", convertedDocId).single();
                if (freshDoc) doc1 = freshDoc;
              }
              if (convertedVersionId) {
                const { data: freshVer } = await supabase.from("project_document_versions").select("id, plaintext, version_number").eq("id", convertedVersionId).single();
                if (freshVer) latestVersion = freshVer;
              }
            // Continue — don't pause
            } else {
              await logStep(supabase, jobId, null, currentDoc, "approval_required", `Review generated ${currentDoc} before continuing`, {}, undefined, {
                docId: convertedDocId,
                versionId: convertedVersionId,
                doc_type: currentDoc,
                from_stage: prevStage
        ...
        if (!latestVersion) {
          const abvr = await resolveActiveVersionForDoc(supabase, job, doc1.id);
          if (abvr) {
            const { data: resolvedVer } = await supabase.from("project_document_versions").select("id, plaintext, version_number").eq("id", abvr.versionId).single();
            if (resolvedVer) {
        ...
        if (!latestVersion) {
          ...
          console.warn(`[auto-run] Doc slot ${doc1.id} (${currentDoc}) has no versions — will generate`);
          doc1 = null;
          ...
          if (!seedCheck2.official) {
            if ((job.allow_defaults || job.meta_json?.auto_approve_all) && seedCheck2.missing.length === 0) {
              ...
              await updateJob(supabase, jobId, {
                ...
                approval_type: "seed_core_officialize",
                error: JSON.stringify({
                  missing_seed_docs: seedCheck2.missing,
                  unapproved_seed_docs: seedCheck2.unapproved
                })
              });
              return respondWithJob(supabase, jobId, "seed-core-not-official");
            }
          ...
          if (!prevStage2) {
            await updateJob(supabase, jobId, {
              status: "failed",
              error: `No previous stage to generate ${currentDoc} from (empty doc slot)`
            });
            return respondWithJob(supabase, jobId);
          }
          const { data: prevDocs2 } = await supabase.from("project_documents").select("id, doc_type, plaintext, extracted_text").eq("project_id", job.project_id).eq("doc_type", prevStage2).order("created_at", {
            ascending: false
          }).limit(1);
          ...
          if (!prevDoc2) {
            await updateJob(supabase, jobId, {
              status: "failed",
              error: `No ${prevStage2} document for empty-slot recovery`
            });
            return respondWithJob(supabase, jobId);
          }
          const { data: prevVersions2 } = await supabase.from("project_document_versions").select("id, approval_status, is_current, version_number").eq("document_id", prevDoc2.id).order("version_number", {
            ascending: false
          });
          const prevVersion2Candidates = prevVersions2 || [];
          const prevVersion2 = prevVersion2Candidates.find((v)=>v.approval_status === "approved" && v.is_current === true) || prevVersion2Candidates.find((v)=>v.approval_status === "approved") || prevVersion2Candidates[0] || null;
          if (!prevVersion2) {
            await updateJob(supabase, jobId, {
              status: "failed",
              error: `No version for ${prevStage2} in empty-slot recovery`
            });
            return respondWithJob(supabase, jobId);
          }
          const useChunked2 = new Set([
            "episode_grid",
            "vertical_episode_beats",
            "episode_beats"
          ]).has(currentDoc) || isLargeRiskDocType(currentDoc);
          try {
            let genDocId2 = null;
            let genVerId2 = null;
            if (useChunked2) {
              ...
              const { result: convertResult2 } = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
                action: "convert",
                projectId: job.project_id,
            ...
            if (!genDocId2) {
              const { data: newDocs2 } = await supabase.from("project_documents").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
                ascending: false
              }).limit(1);
              genDocId2 = newDocs2?.[0]?.id || null;
            }
            if (genDocId2 && !genVerId2) {
              const { data: newVers2 } = await supabase.from("project_document_versions").select("id").eq("document_id", genDocId2).order("version_number", {
                ascending: false
              }).limit(1);
            ...
            if (!genDocId2 || !genVerId2) {
              ...
              await logStep(supabase, jobId, stepCount + 1, currentDoc, "empty_slot_recovery_failed", reason, {}, undefined, {
                currentDoc,
                targetDoc: currentDoc,
                reason,
                required_inputs_missing: [
                  !genDocId2 ? "generated_document" : null,
                  !genVerId2 ? "generated_version" : null
                ].filter(Boolean),
                job_id: jobId,
            ...
            await logStep(supabase, jobId, ns2, currentDoc, "generate", `Generated ${currentDoc} from ${prevStage2} (empty-slot recovery)`, {}, genDocId2 ? `Created doc ${genDocId2}` : undefined, genDocId2 ? {
              docId: genDocId2
            } : undefined);
            await updateJob(supabase, jobId, {
              step_count: ns2,
            ...
            if (job.allow_defaults || job.meta_json?.auto_approve_all) {
              if (genVerId2) {
                ...
                let arCCEMeta2 = {};
                try {
                  const { data: cvr2 } = await supabase.from("project_document_versions").select("plaintext").eq("id", genVerId2).maybeSingle();
                  if (cvr2?.plaintext && cvr2.plaintext.length > 100) {
                    const { data: cr2 } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
                    ...
                    if (dr2.constraintsUsed) {
                      arCCEMeta2 = {
                        canon_drift: {
                          passed: dr2.passed,
                          violations: dr2.findings.filter((f)=>f.severity === "violation").length,
                          warnings: dr2.findings.filter((f)=>f.severity === "warning").length,
                          domains_checked: dr2.domains_checked,
                          checked_at: dr2.checkedAt
                        }
                      };
                    }
                  }
                } catch (_e) {}
                const { data: existingVer2 } = await supabase.from("project_document_versions").select("meta_json").eq("id", genVerId2).maybeSingle();
                const mergedMeta2 = {
                  ...existingVer2?.meta_json || {},
                  source_version_id: prevVersion2.id,
                  ...arCCEMeta2
                };
                await supabase.from("project_document_versions").update({
                  approval_status: "approved",
              ...
              await logStep(supabase, jobId, null, currentDoc, "auto_approved_convert", `Auto-approved generated ${currentDoc} (allow_defaults, empty-slot)`, {}, undefined, {
                docId: genDocId2,
                versionId: genVerId2,
                doc_type: currentDoc,
                from_stage: prevStage2
              });
            // Continue — don't pause
            } else {
              await logStep(supabase, jobId, null, currentDoc, "approval_required", `Review generated ${currentDoc} before continuing`, {}, undefined, {
              ...
              await updateJob(supabase, jobId, {
                ...
                awaiting_approval: true,
                approval_type: "convert",
                pending_doc_id: genDocId2 || prevDoc2.id,
                pending_version_id: genVerId2,
                pending_doc_type: currentDoc,
                pending_next_doc_type: currentDoc
              });
              return respondWithJob(supabase, jobId, "awaiting-approval");
            ...
            if (e2.message?.includes("CANON_MISMATCH")) {
              ...
              if (retryCount <= MAX_CANON_LOCK_RETRIES) {
                ...
                await logStep(supabase, jobId, stepCount + 1, currentDoc, "canon_lock_retry", `CANON_MISMATCH retry ${retryCount}/${MAX_CANON_LOCK_RETRIES} (empty-slot recovery, core=${coreEntities.length}, secondary=${secondaryEntities.length}, attempt_id=${attemptId}): ${e2.message.slice(0, 200)}`, {}, undefined, {
                  retry_count: retryCount,
                  max_retries: MAX_CANON_LOCK_RETRIES,
                  entity_count: canonEntityPack.length,
                  core_count: coreEntities.length,
                  secondary_count: secondaryEntities.length,
                  attempt_id: attemptId,
                  error_excerpt: e2.message.slice(0, 300),
                  doc_type: currentDoc,
                  path: "empty_slot_recovery"
                });
                const updatedRetries = {
                  ...canonRetries,
                  [currentDoc]: retryCount
                };
                await updateJob(supabase, jobId, {
                  step_count: stepCount + 1,
                  meta_json: {
              ...
              await logStep(supabase, jobId, stepCount + 1, currentDoc, "canon_mismatch_stuck", `CANON_MISMATCH stuck after ${retryCount - 1} retries (empty-slot recovery): ${e2.message.slice(0, 200)}`, {}, undefined, {
        ...
        if (!doc1) {
          const { data: recoveredDocs } = await supabase.from("project_documents").select("id, doc_type, plaintext, extracted_text").eq("project_id", job.project_id).eq("doc_type", currentDoc).order("created_at", {
            ascending: false
          }).limit(1);
          doc1 = recoveredDocs?.[0] || null;
          if (doc1 && !latestVersion) {
            const { data: recoveredVers } = await supabase.from("project_document_versions").select("id, plaintext, version_number, criteria_hash").eq("document_id", doc1.id).order("version_number", {
              ascending: false
            }).limit(1);
          ...
          if (!doc1) {
            ...
            await updateJob(supabase, jobId, {
              status: "paused",
              pause_reason: "EMPTY_SLOT_RECOVERY_FAILED",
              error: `Document ${currentDoc} still missing after empty-slot recovery`
            });
            return respondWithJob(supabase, jobId);
          }
          console.log(`[auto-run][IEL] doc_refetched_after_recovery { docId: "${doc1.id}", currentDoc: "${currentDoc}" }`);
        }
        // Log resume source usage
        if (resumeSourceUsed) {
          await logStep(supabase, jobId, stepCount, currentDoc, "resume_source_used", `Using pinned source: doc=${doc1.id} ver=${latestVersion?.id}`, {}, undefined, {
            documentId: doc1.id,
            versionId: latestVersion?.id,
            follow_latest: false
          });
        ...
        const reviewTextForDuration = latestVersion?.plaintext || doc1?.extracted_text || doc1?.plaintext || "";
        ...
        let versionCriteriaHash = latestVersion?.criteria_hash || null;
        if (!versionCriteriaHash && latestVersion?.id) {
          const { data: verRow } = await supabase.from("project_document_versions").select("criteria_hash").eq("id", latestVersion.id).maybeSingle();
        ...
        if (criteriaResult.classification === 'CRITERIA_STALE_PROVENANCE') {
          ...
          if (job.allow_defaults || job.meta_json?.auto_approve_all) {
            ...
            try {
              ...
              const regeneratedCount = Array.isArray(regenResult?.regenerated) ? regenResult.regenerated.length : 0;
              await logStep(supabase, jobId, stepCount + 1, currentDoc, "criteria_stale_auto_regen", `Auto-regen triggered for stale criteria provenance (allow_defaults=true). Regenerated ${regeneratedCount} docs. ${criteriaResult.detail}`, {
                risk_flags: [
                  "criteria_stale_provenance_auto_resolved"
                ]
              });
            ...
            if (latestVersion?.id && currentCriteriaHash) {
              const { error: rebaseErr } = await supabase.from("project_document_versions").update({
            ...
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "criteria_stale_provenance", `Criteria provenance mismatch: ${criteriaResult.detail}`, {
              risk_flags: [
        ...
        if (criteriaResult.classification === 'CRITERIA_FAIL_DURATION') {
          ...
          if (!isDurationEligibleDocType(currentDoc, format)) {
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "duration_scope_skipped", `Skipped duration criteria/repair for non-runtime doc type: ${currentDoc} (format=${format})`, {}, undefined, {
              currentDoc,
              format,
              measuredDurationSeconds: measuredDuration,
              targetMin: latestCriteriaSnapshot.episode_target_duration_min_seconds,
              targetMax: latestCriteriaSnapshot.episode_target_duration_max_seconds,
              reason: "NON_DURATION_DOC_TYPE"
            });
            await updateJob(supabase, jobId, {
              step_count: stepCount + 1
            });
          // Continue to normal analysis flow — no duration repair
            ...
            const durationRepairAttempts = job.duration_repair_attempts || 0;
            if (durationRepairAttempts >= 2) {
              // Already tried 2 repairs — pause with clear explanation
              await logStep(supabase, jobId, stepCount + 1, currentDoc, "criteria_fail_duration_exhausted", `Duration repair exhausted after ${durationRepairAttempts} attempts. ${criteriaResult.detail}`, {
                risk_flags: [
            ...
            const durSourceLabel = (()=>{
              try {
                const snap = latestCriteriaSnapshot;
                return snap._duration_source ? `source=${snap._duration_source}${snap._duration_locked ? ', locked' : ''}` : '';
              } catch  {
                return '';
              }
            })();
            await logStep(supabase, jobId, stepCount + 1, currentDoc, "duration_repair_attempt", `Duration repair #${durationRepairAttempts + 1}: measured=${measuredDuration}s target=${targetMin}-${targetMax}s delta=${measuredDuration - targetMid}s${durSourceLabel ? ` (${durSourceLabel})` : ''}`, {
              risk_flags: [
                "criteria_fail_duration",
                "duration_repair"
              ]
            ...
            await supabase.from("auto_run_jobs").update({
              approval_payload: {
                ...job.approval_payload || {},
                duration_repair_attempts: durationRepairAttempts + 1,
                duration_target: {
                  min: targetMin,
                  max: targetMax,
        ...
        let reviewText = latestVersion?.plaintext || doc1?.extracted_text || doc1?.plaintext || "";
        ...
        if (reviewCharCount === 0 && latestVersion?.id) {
          const { data: versionStatusRow } = await supabase.from("project_document_versions").select("meta_json, created_at").eq("id", latestVersion.id).maybeSingle();
          if (versionStatusRow?.meta_json?.bg_generating === true) {
            const bgStarted = versionStatusRow?.meta_json?.bg_started_at || versionStatusRow?.created_at;
            ...
            if (ageMs < BG_STALE_THRESHOLD_MS) {
              ...
              const newStep = stepCount + 1;
              await logStep(supabase, jobId, newStep, currentDoc, "generate", `Background generation in progress for ${currentDoc} (age ${Math.round(ageMs / 1000)}s) — yielding until complete`, {}, undefined, {
                versionId: latestVersion.id,
                status: "generating",
            ...
            const existingMeta = versionStatusRow?.meta_json || {};
            const mergedMeta = {
              ...existingMeta,
              bg_generating: false,
              bg_stale: true,
            ...
            const { data: stuckChunks } = await supabase.from("project_document_chunks").select("id, meta_json").eq("version_id", latestVersion.id).eq("status", "running");
            const stuckIds = (stuckChunks || []).map((c)=>c.id);
            if (stuckIds.length > 0) {
              console.warn(`[auto-run][IEL] stale_chunks_converted`, JSON.stringify({
                job_id: jobId,
                version_id: latestVersion.id,
                chunk_count: stuckIds.length,
                from_status: "running",
                to_status: "failed"
              }));
              for (const sc of stuckChunks || []){
                const chunkMeta = {
                  ...sc.meta_json || {},
                  stale_reason: "bg_generating_timeout",
        ...
        if (docIsStub) {
          ...
          const regenAttempt = await attemptAutoRegenInputs(supabase, supabaseUrl, token, jobId, stepCount + 1, currentDoc, job.project_id, stageInsufficients, "stub_at_current_stage");
          if (!regenAttempt.ok) {
            console.warn("[auto-run] auto-regen for current stub doc failed", {
              jobId,
              currentDoc,
              error: regenAttempt.error
            });
          ...
          const { data: regenVers } = await supabase.from("project_document_versions").select("id, plaintext, version_number").eq("document_id", doc1.id).eq("is_current", true).limit(1);
          if (regenVers?.[0]) {
        ...
        if (reviewCharCount === 0) {
          ...
          await logStep(supabase, jobId, stepCount + 1, currentDoc, "stop", `Input text empty for ${currentDoc} (docId=${doc1.id} verId=${latestVersion.id}). Cannot proceed.`);
          return respondWithJob(supabase, jobId);
        }
        // ── review_input visibility step ──
        const simpleHash = reviewCharCount > 0 ? reviewText.slice(0, 64).replace(/\s+/g, " ").trim() : "(empty)";
        await logStep(supabase, jobId, stepCount, currentDoc, "review_input", `Reviewing ${currentDoc} docId=${doc1.id} versionId=${latestVersion.id} chars=${reviewCharCount}`, {}, reviewText.slice(0, 500), {
          docId: doc1.id,
          versionId: latestVersion.id,
          doc_type: currentDoc,
          char_count: reviewCharCount,
          preview_hash: simpleHash
        });
        ...
        const bgTask = (async ()=>{
          try {
            console.log("[auto-run] dev-engine analyze (bg) START", {
              jobId,
              currentDoc,
              docId: doc1.id
            });
            const { result: rawAnalyzeResult } = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
              action: "analyze",
              projectId: job.project_id,
              documentId: doc1.id,
              versionId: latestVersion.id,
            ...
            console.log("[auto-run] dev-engine analyze (bg) DONE", {
              jobId
            });
            ...
            if (!rawAnalyzeResult || rawAnalyzeResult.success === false) {
              ...
              console.error("[auto-run] dev-engine analyze returned failure", {
                jobId,
                errMsg,
                where
              });
              ...
              await logStep(supabase, jobId, stepCount + 1, currentDoc, "stop", `DEV_ENGINE_FAILED: ${fullErr}`.slice(0, 500));
            ...
            const scoreRiskFlags = [
              ...resumeRiskFlags
            ];
            const ciRaw = pickNumberRaw(analyzeResult, [
              "ci_score",
              "scores.ci_score",
              "scores.ci",
              "ci"
            ]);
            const gpRaw = pickNumberRaw(analyzeResult, [
              "gp_score",
              "scores.gp_score",
              "scores.gp",
              "gp"
            ...
            const gap = pickNumber(analyzeResult, [
              "gap",
              "scores.gap"
            ], Math.abs(ci - gp));
            const trajectory = analyzeResult?.trajectory ?? analyzeResult?.convergence?.trajectory ?? null;
            const blockers = pickArray(analyzeResult, [
              "blocking_issues",
              "blockers",
              "scores.blocking_issues"
            ]);
            const highImpact = pickArray(analyzeResult, [
              "high_impact_notes",
              "high_impact"
            ]);
            ...
            await logStep(supabase, jobId, null, currentDoc, "promotion_check", `${promo.recommendation} (readiness: ${promo.readiness_score}, flags: ${promo.risk_flags.join(",") || "none"})`, {
              ci,
              gp,
              gap,
              readiness: promo.readiness_score,
              confidence: promo.confidence,
              risk_flags: promo.risk_flags
            });
            ...
            await updateJob(supabase, jobId, {
              ...
              last_gp: gp,
              last_gap: gap,
              last_blocker_count: blockersCount,
              last_readiness: promo.readiness_score,
              last_confidence: promo.confidence,
            ...
            let remediationReason = undefined;
            // ── HARD STOPS ──
            if (promo.risk_flags.includes("hard_gate:thrash")) {
              await updateJob(supabase, jobId, {
                status: "stopped",
                stop_reason: "Thrash detected — run Executive Strategy Loop"
              });
            ...
            if (promo.risk_flags.includes("hard_gate:eroding_trajectory") || promo.recommendation === "escalate") {
              ...
              const escalateReason = promo.risk_flags.includes("hard_gate:eroding_trajectory") ? "Trajectory eroding" : `Escalation: readiness ${promo.readiness_score}/100`;
              try {
                // Call dev-engine-v2 "options" to generate decision options for escalation
                const optionsResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
                  action: "options",
                  projectId: job.project_id,
                  documentId: doc1.id,
                  versionId: latestVersion.id,
                ...
                await logStep(supabase, jobId, null, currentDoc, "escalate_options_generated", `${escalateReason}. Generated ${normalizedDecisions.length} decision sets.`, {
                  ci,
                  gp,
                  gap,
                  readiness: promo.readiness_score,
                ...
                if (autoSelections) {
                  await logStep(supabase, jobId, null, currentDoc, "auto_decided", `Auto-accepted ${Object.keys(autoSelections).length} escalation decisions`, {
                    ci,
                    gp,
                    gap
                  }, undefined, {
                ...
                const escalateDecisions = [
                  {
                    ...
                    options: [
                      {
                        value: "yes",
                        why: "Skip remaining loops and advance to the next document stage"
                      },
                      {
                        value: "no",
                        why: "Stay at current stage"
                      }
                    ],
                  ...
                  },
                  {
                    ...
                    options: [
                      {
                        value: "yes",
                        why: "Continue the current development cycle with more steps"
                      },
                      {
                        value: "no",
                        why: "Stop the run"
                ...
                if (autoSelections) {
                  await logStep(supabase, jobId, null, currentDoc, "auto_decided", `Auto-accepted escalation decisions (allow_defaults)`, {
                    ci,
                    gp,
                    gap,
                    readiness: promo.readiness_score,
                ...
                // Fall through — don't pause
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "pause_for_approval", `${escalateReason} — options generation failed, awaiting user decision`, {
                    ci,
                    gp,
                    gap,
                    readiness: promo.readiness_score,
            ...
            if (promo.recommendation === "stabilise") {
              ...
              if (blockersCount > 0 || newLoopCount <= 1 && highImpactCount > 0 || remediationReason === "ci_gate_blocked") {
                ...
                if (remediationReason === "ci_gate_blocked") {
                  await logStep(supabase, jobId, null, currentDoc, "stabilise_entered", `Entering stabilise for ${currentDoc} (reason: ${remediationReason})`, {
                    ci: ciGate.ci,
                    gp
                  }, undefined, {
                    remediationReason
                ...
                try {
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "options_generated", `Generated ${stabiliseDecisions.length} decision sets for ${blockersCount} blockers + ${highImpactCount} high-impact notes`, {
                    ci,
                    gp,
                    gap,
                    readiness: promo.readiness_score
                    ...
                    optionsRunId,
                    decisions: stabiliseDecisions.length,
                    global_directions: optionsData.global_directions?.length || 0,
                    decision_objects: stabiliseDecisions
                  });
                  // ── ALL NOTES EXHAUSTED: 0 decisions + 0 blockers + 0 high-impact → promote ──
                  if (stabiliseDecisions.length === 0 && blockersCount === 0 && highImpactCount === 0) {
                    const { data: remainingNotes } = await supabase.from("project_notes").select("id").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
                      "open",
                      "in_progress",
                      "reopened"
                    ]).limit(1).maybeSingle();
                    if (!remainingNotes) {
                      const exhaustTargetCi = resolveTargetCI(job);
                      console.log(`[auto-run][IEL] stabilise_all_notes_exhausted { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ci}, gp: ${gp} }`);
                      await logStep(supabase, jobId, null, currentDoc, "all_notes_exhausted", `All notes resolved (CI:${ci}, target:${exhaustTargetCi}). No actionable improvements remain.`, {
                        ci,
                        gp
                      }, undefined, {
                        blockers: blockersCount,
                        high_impact: highImpactCount,
                        action: "promote_due_to_note_exhaustion"
                      });
                      // Skip fallback decisions — promote immediately
                      const promoteNext = await nextUnsatisfiedStage(supabase, job.project_id, format, currentDoc, job.target_document, job.allow_defaults, job.user_id, jobId);
                      if (promoteNext && isStageAtOrBeforeTarget(promoteNext, job.target_document, format)) {
                        try {
                          await supabase.from("project_document_versions").update({
                            approval_status: "approved",
                            approved_at: new Date().toISOString(),
                          ...
                          console.warn("[auto-run] non-fatal note-exhaustion approve failed:", e?.message || e);
                        }
                        await logStep(supabase, jobId, null, currentDoc, "auto_approved_promote", `Note exhaustion: CI=${ci} vs target ${exhaustTargetCi}, 0 blockers, 0 high-impact, 0 open notes. Promoting ${currentDoc} → ${promoteNext}.`, {
                          ci,
                          gp
                        }, undefined, {
                          docId: doc1.id,
                          versionId: latestVersion.id,
                          trigger: "note_exhaustion_promote"
                        });
                        await updateJob(supabase, jobId, {
                          stage_loop_count: 0,
                          current_document: promoteNext,
                          stage_exhaustion_remaining: job.stage_exhaustion_default ?? 4,
                          status: "running",
                          stop_reason: null,
                          awaiting_approval: false,
                        ...
                        console.log(`[auto-run][IEL] stage_transition { job_id: "${jobId}", from: "${currentDoc}", to: "${promoteNext}", trigger: "note_exhaustion_promote" }`);
                        return respondWithJob(supabase, jobId, "run-next");
                      }
                    }
                  }
                  ...
                  if (autoSelections) {
                    await logStep(supabase, jobId, null, currentDoc, "auto_decided", `Auto-accepted ${Object.keys(autoSelections).length} stabilise decisions`, {
                      ci,
                      gp,
                      gap
                    }, undefined, {
                      selections: autoSelections,
                      decision_objects: finalDecisions
                    });
                    ...
                    if (earlyBestCi >= earlyTargetCi && blockersCount === 0 && (job.allow_defaults !== false || job.meta_json?.auto_approve_all)) {
                      ...
                      if (earlyNext && isStageAtOrBeforeTarget(earlyNext, job.target_document, format)) {
                        ...
                        await logStep(supabase, jobId, null, currentDoc, "auto_approved_promote", `Early convergence after auto-decided stabilise: best_CI=${earlyBestCi}≥${earlyTargetCi}, blockers=0. Auto-promoting ${currentDoc} → ${earlyNext} (allow_defaults).`, {
                          ci,
                          gp,
                          gap
                        }, undefined, {
                          ...
                          versionId: latestVersion.id,
                          doc_type: currentDoc,
                          next_doc_type: earlyNext,
                          trigger: "early_convergence_after_auto_decided"
                        });
                        await updateJob(supabase, jobId, {
                          stage_loop_count: 0,
                          current_document: earlyNext,
                          stage_exhaustion_remaining: job.stage_exhaustion_default ?? 4,
                          status: "running",
                          stop_reason: null,
                          awaiting_approval: false,
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "options_failed", `Options generation failed: ${optErr.message}. Falling back to rewrite.`);
                }
              }
              ...
              const hasActiveDecisions = Array.isArray(jobAfterOptions?.pending_decisions) && jobAfterOptions.pending_decisions.length > 0;
              ...
              if (!optionsGeneratedThisStep && !hasActiveDecisions && newLoopCount >= job.max_stage_loops) {
                ...
                const convergeTarget = typeof job.converge_target_json === 'object' && job.converge_target_json ? job.converge_target_json : {
                  ci: GLOBAL_MIN_CI,
                  gp: 100
                };
                ...
                if (!convergedEnough) {
                  ...
                  if (job.step_count < job.max_total_steps) {
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "soft_max_loops_continue", `Soft limit exceeded; continuing until CI>=${convergeTarget.ci} and GP>=${convergeTarget.gp} or step budget exhausted (CI=${ci}, GP=${gp})`);
                ...
                // If step budget exhausted, the step-limit guard at the top of run-next will catch it
                  ...
                  const ciGate1 = await evaluateCIGate(supabase, jobId, currentDoc, promoteTargetCi, job.project_id, job);
                  const docOverride = DOC_TYPE_GATE_OVERRIDES[currentDoc];
                  console.log(`[auto-run][IEL] ci_gate_eval { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ciGate1.ci}, gp: ${ciGate1.gp}, min_ci: ${docOverride ? docOverride.ci : promoteTargetCi}, min_gp: ${docOverride ? docOverride.gp : 0}, override: ${!!docOverride}, rule: "converge_promote_gate" }`);
                  if (!ciGate1.pass) {
                    // Gate failed — CI below target (or GP below target for overridden doc types)
                    const blockReason = ciGate1.failReason || `CI=${ciGate1.ci} < ${promoteTargetCi}`;
                    console.warn(`[auto-run][IEL] ci_gate_blocked_promote { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ciGate1.ci}, gp: ${ciGate1.gp}, reason: "${blockReason}", trigger: "converge_promote" }`);
                    // ── TRANSITION LEDGER: promotion_gate_evaluated (blocked) ──
                    await emitTransition(supabase, {
                      projectId: job.project_id,
                      eventType: TRANSITION_EVENTS.PROMOTION_GATE_EVALUATED,
                      docType: currentDoc,
                      stage: currentDoc,
                      jobId,
                      resultingVersionId: latestVersion?.id,
                      status: "failed",
                      trigger: "ci_hard_gate",
                      sourceOfTruth: "auto-run",
                      ci: ciGate1.ci,
                      previousState: {
                        min_ci: docOverride ? docOverride.ci : promoteTargetCi,
                        min_gp: docOverride?.gp ?? 0
                      },
                      resultingState: {
                        pass: false,
                        reason: docOverride ? "doc_type_gate_failed" : "ci_below_threshold",
                        detail: blockReason
                      }
                    });
                    await logStep(supabase, jobId, null, currentDoc, "ci_gate_blocked", `Promotion blocked: ${blockReason}. Continuing stabilise.`, {
                      ci: ciGate1.ci,
                      gp: ciGate1.gp
                    }, undefined, {
                      min_ci: docOverride ? docOverride.ci : promoteTargetCi,
                      min_gp: docOverride?.gp ?? 0,
                      trigger: "converge_promote",
                      doc_type_override: !!docOverride
                    });
                    await updateJob(supabase, jobId, {
                      stage_loop_count: newLoopCount
                    });
                  // Fall through to rewrite below
                    ...
                    await emitTransition(supabase, {
                      ...
                      trigger: "ci_blocker_gate",
                      sourceOfTruth: "auto-run",
                      ci: blockerGate.ci,
                      gp: blockerGate.gp,
                      previousState: {
                        blockerCount: blockerGate.blockerCount,
                        highImpactCount: blockerGate.highImpactCount
                      },
                      resultingState: {
                        pass: blockerGate.pass,
                        blockReasons: blockerGate.blockReasons
                      }
                    });
                    if (!blockerGate.pass) {
                      ...
                      await logStep(supabase, jobId, null, currentDoc, "ci_blocker_gate_blocked", `Promotion blocked by CI Blocker Gate: ${blockerGate.blockReasons.join(", ")}. Continuing stabilise.`, {
                        ci: blockerGate.ci,
                        gp: blockerGate.gp
                      }, undefined, {
                        blockerCount: blockerGate.blockerCount,
                    ...
                    // Fall through to rewrite below
                      ...
                      if (!engagementGatePass) {
                        ...
                        await logStep(supabase, jobId, null, currentDoc, "engagement_gate_blocked", `Promotion blocked: engagement score below threshold for ${currentDoc}. Continuing stabilise.`, {
                          ci: blockerGate.ci,
                          gp: blockerGate.gp
                        }, undefined, {
                          version_id: latestVersion?.id,
                          trigger: "blocker_gate_promote"
                        });
                        await updateJob(supabase, jobId, {
                          stage_loop_count: newLoopCount
                        });
                      // Fall through to rewrite below
                        ...
                        if (noteExhaust.hasActionable) {
                          ...
                          await logStep(supabase, jobId, null, currentDoc, "note_exhaustion_blocked", `Promotion blocked: ${noteExhaust.count} actionable note(s) remain for ${currentDoc}. Continuing stabilise to apply/resolve notes.`, {
                            ci: blockerGate.ci,
                            gp: blockerGate.gp
                          }, undefined, {
                            actionable_count: noteExhaust.count,
                            trigger: "blocker_gate_promote"
                          });
                          await updateJob(supabase, jobId, {
                            stage_loop_count: newLoopCount
                        ...
                        // Fall through to rewrite below
                          ...
                          if (next && isStageAtOrBeforeTarget(next, job.target_document, format)) {
                            if (job.allow_defaults || job.meta_json?.auto_approve_all) {
                              ...
                              await logStep(supabase, jobId, null, currentDoc, "auto_approved_promote", `Converged (CI=${ci}, GP=${gp}). Auto-promoting ${currentDoc} → ${next} (allow_defaults, blocker_gate_passed)`, {}, undefined, {
                                docId: doc1.id,
                                versionId: latestVersion.id,
                                doc_type: currentDoc,
                                next_doc_type: next
                            ...
                            await logStep(supabase, jobId, null, currentDoc, "approval_required", `Converged (CI=${ci}, GP=${gp}). Review ${currentDoc} before promoting to ${next}`, {}, undefined, {
                              docId: doc1.id,
                              versionId: latestVersion.id,
                              doc_type: currentDoc,
                              next_doc_type: next
                            ...
                            await updateJob(supabase, jobId, {
                              stage_loop_count: newLoopCount,
                              status: "paused",
                              stop_reason: `Converged — review ${currentDoc} before promoting to ${next}`,
                              awaiting_approval: true,
                              approval_type: "promote",
                              pending_doc_id: doc1.id,
                              pending_version_id: latestVersion.id,
                              pending_doc_type: currentDoc,
                              pending_next_doc_type: next
                            });
                            return respondWithJob(supabase, jobId, "awaiting-approval");
                          }
                    ...
                    console.log(`[auto-run][IEL] ci_gate_passed { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ciGate1.ci} }`);
                    ...
                    if (noteExhaustOrig.hasActionable) {
                      ...
                      await logStep(supabase, jobId, null, currentDoc, "note_exhaustion_blocked", `Promotion blocked: ${noteExhaustOrig.count} actionable note(s) remain for ${currentDoc}. Continuing stabilise.`, {
                        ci: ciGate1.ci
                      }, undefined, {
                        actionable_count: noteExhaustOrig.count,
                        trigger: "converge_promote_flag_off"
                      });
                      await updateJob(supabase, jobId, {
                        stage_loop_count: newLoopCount
                      });
                    // Fall through to rewrite below
                      ...
                      if (next && isStageAtOrBeforeTarget(next, job.target_document, format)) {
                        if (job.allow_defaults || job.meta_json?.auto_approve_all) {
                          ...
                          await logStep(supabase, jobId, null, currentDoc, "auto_approved_promote", `Converged (CI=${ci}, GP=${gp}). Auto-promoting ${currentDoc} → ${next} (allow_defaults)`, {}, undefined, {
                            docId: doc1.id,
                            versionId: latestVersion.id,
                            doc_type: currentDoc,
                            next_doc_type: next
              ...
              try {
                ...
                await logStep(supabase, jobId, null, currentDoc, "doc_type_unregistered", `Doc type "${currentDoc}" is not in the policy registry. Halting.`, {
                  ci,
                  gp,
                  gap
                });
              ...
              if (isAggregate(currentDoc)) {
                ...
                const { data: unitDocs } = await supabase.from("project_documents").select("id, doc_type").eq("project_id", job.project_id).in("doc_type", [
                  "episode_outline",
                  "episode_script"
                ]);
                const unitDocIds = (unitDocs || []).map((d)=>d.id);
                let sourceVersionKey = "[]";
                if (unitDocIds.length > 0) {
                  const { data: unitVersions } = await supabase.from("project_document_versions").select("id, document_id").in("document_id", unitDocIds).eq("is_current", true).order("document_id");
                  const sortedIds = (unitVersions || []).map((v)=>v.id).sort();
                  sourceVersionKey = JSON.stringify(sortedIds);
                }
                // Check if current aggregate version was compiled with this same source key
                const aggCurrentVer = await getCurrentVersionForDoc(supabase, doc1.id);
                const aggMeta = aggCurrentVer ? aggCurrentVer.meta_json : null;
                const cachedKey = aggMeta?.source_version_key;
                if (cachedKey === sourceVersionKey && aggCurrentVer) {
                  await logStep(supabase, jobId, null, currentDoc, "aggregate_compile_skipped", `Aggregate "${currentDoc}" already compiled with current source versions. Skipping.`, {
                    ci,
                    gp,
                    gap
                  }, undefined, {
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "aggregate_skip_advance", `Doc type "${currentDoc}" is AGGREGATE (compile-only). Skipping LLM rewrite, advancing to next stage.`, {
                    ci,
                    gp,
                    gap
                  });
                ...
                if (nextAfterAggregate && isStageAtOrBeforeTarget(nextAfterAggregate, job.target_document, format)) {
                  ...
                  await updateJob(supabase, jobId, {
                    status: "completed",
                    stop_reason: "All stages satisfied up to target (aggregate skip)"
                  });
              ...
              // ── IEL: UPSTREAM NOTE DEBT GATE (UNIFIED — reads all 3 note systems) ──
              {
                ...
                if (upstreamBlockers.length > 0) {
                  const sourceDocs = [
                    ...new Set(upstreamBlockers.map((n)=>n.source_doc_type).filter(Boolean))
                  ];
                  const byTable = {
                    deferred: upstreamBlockers.filter((b)=>b.source_table === "project_deferred_notes").length,
                    notes: upstreamBlockers.filter((b)=>b.source_table === "project_notes").length,
                    dev_state: upstreamBlockers.filter((b)=>b.source_table === "project_dev_note_state").length
                  };
                  ...
                  if (job.allow_defaults || job.meta_json?.auto_approve_all) {
                    ...
                    const noteBlockerIds = upstreamBlockers.filter((b)=>b.source_table === "project_notes").map((b)=>b.id);
                    if (noteBlockerIds.length > 0) {
                      await supabase.from("project_notes").update({
                        status: "resolved",
                        updated_by: "auto_run_upstream_debt",
                        updated_at: new Date().toISOString()
                      }).in("id", noteBlockerIds);
                    }
                    // Auto-resolve deferred note blockers
                    const deferredBlockerIds = upstreamBlockers.filter((b)=>b.source_table === "project_deferred_notes").map((b)=>b.id);
                    ...
                    const devNoteBlockerIds = upstreamBlockers.filter((b)=>b.source_table === "project_dev_note_state").map((b)=>b.id);
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "note_auto_resolved", `Auto-resolved ${upstreamBlockers.length} upstream note debt blocker(s) from [${sourceDocs.join(", ")}] (allow_defaults=true). Continuing rewrite. (notes=${noteBlockerIds.length}, deferred=${deferredBlockerIds.length}, dev_state=${devNoteBlockerIds.length})`, {
                      ci,
                      gp,
                      gap
                    }, undefined, {
                      ...
                      sourceDocs,
                      byTable,
                      resolver: "auto_run_upstream_debt",
                      auto_resolved: true
                    });
                  // Fall through to rewrite — do NOT pause
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "upstream_note_debt_paused", `${upstreamBlockers.length} unresolved upstream note(s) from [${sourceDocs.join(", ")}] target ${currentDoc}. Repair upstream docs first. (unified: deferred=${byTable.deferred}, notes=${byTable.notes}, dev_state=${byTable.dev_state})`, {
                      ci,
                      gp,
                      gap
                    }, undefined, {
                      ...
                      sourceDocs,
                      byTable,
                      noteKeys: upstreamBlockers.map((n)=>n.note_key_or_fingerprint)
                    });
                    await updateJob(supabase, jobId, {
                      stage_loop_count: newLoopCount,
                      status: "paused",
              ...
              let injectedProjectNotes = [];
              try {
                const { data: pnotes } = await supabase.from("project_notes").select("id, title, summary, detail, severity, suggested_fixes, category").eq("project_id", job.project_id).eq("doc_type", currentDoc).in("status", [
                  "open",
                  "in_progress",
                  "reopened"
                ]).limit(30);
                if (pnotes && pnotes.length > 0) {
                  injectedProjectNotes = pnotes.map((n)=>{
                    const fixes = Array.isArray(n.suggested_fixes) ? n.suggested_fixes : [];
                    const recommended = fixes.find((f)=>f.recommended) || fixes[0];
                    const resolutionDirective = recommended ? `Apply: "${recommended.title || recommended.description}".${recommended.what_changes ? ` Changes: ${Array.isArray(recommended.what_changes) ? recommended.what_changes.join("; ") : recommended.what_changes}` : ""}` : undefined;
                    return {
                      id: n.id,
                  ...
                  console.log(`[auto-run][IEL] project_notes_injected_into_rewrite { doc_type: "${currentDoc}", count: ${injectedProjectNotes.length}, severities: ${JSON.stringify(pnotes.map((n)=>n.severity))} }`);
                }
              } catch (e) {
              ...
              const allNotesForStrategy = {
                blocking_issues: [
                  ...
                  ...injectedProjectNotes.filter((n)=>n.severity === "blocker")
                ],
                high_impact_notes: [
                  ...notes?.high_impact_notes || analyzeResult?.high_impact_notes || [],
                  ...injectedProjectNotes.filter((n)=>n.severity === "high")
                ],
                polish_notes: [
                  ...notes?.polish_notes || analyzeResult?.polish_notes || [],
                  ...injectedProjectNotes.filter((n)=>n.severity !== "blocker" && n.severity !== "high")
                ]
              };
              ...
              if (attemptNumber > MAX_TOTAL_ATTEMPTS_PER_TARGET) {
                await logStep(supabase, jobId, null, currentDoc, "max_target_attempts_reached", `Exceeded max target attempts (${MAX_TOTAL_ATTEMPTS_PER_TARGET}). Halting.`, {
                  ci,
                  gp,
                  gap
                }, undefined, {
              ...
              const strategyNotes = strategyNotesRaw.map((n)=>{
                ...
                const fixes = Array.isArray(n.suggested_fixes) ? n.suggested_fixes : [];
                const recommendedFix = fixes.find((f)=>f.recommended) || fixes[0];
                if (recommendedFix) {
                  ...
                  return {
                    ...n,
                    resolution_directive: directive
                  };
                }
                ...
                if (decisions.length > 0) {
                  ...
                  const recommended = decisions.find((d)=>d.recommended) || decisions[0];
              ...
              const enrichedCount = strategyNotes.filter((n)=>n.resolution_directive).length;
              const noteCategories = [
                ...new Set(strategyNotes.map((n)=>n.category || "unknown"))
              ];
              console.log(`[auto-run][IEL] AUTORUN_NOTES_ENRICHED ${JSON.stringify({
                project_id: job.project_id,
                document_id: doc1.id,
                attempt: attemptNumber,
                note_count: strategyNotes.length,
              ...
              if (autoSelectedOptions.length > 0) {
                console.log(`[auto-run][IEL] AUTORUN_DECISIONS_AUTOSELECTED ${JSON.stringify({
                  ...
                  selections: autoSelectedOptions.map((s)=>({
                      note_id: s.note_id,
              ...
              console.log(`[auto-run][IEL] AUTORUN_NOTE_PAYLOAD_STRATEGY ${JSON.stringify({
                ...
                enriched_count: enrichedCount,
                categories: noteCategories,
                selected_option_count: autoSelectedOptions.length,
                reviewer_global_directions_attached: reviewerGlobalDirections.length > 0,
                version_id: job.frontier_version_id || null
              })}`);
              await logStep(supabase, jobId, null, currentDoc, "convergence_strategy_selected", `Attempt ${attemptNumber}: strategy=${strategy}, notes=${strategyNotes.length}, directions=${strategyDirections.length}, enriched=${enrichedCount}, reviewerGD=${reviewerGlobalDirections.length}, autoDecisions=${autoSelectedOptions.length}`, {
                ci,
                gp,
                gap
              }, undefined, {
                attemptNumber,
                strategy,
                noteCount: strategyNotes.length,
                enrichedCount,
                reviewerGDCount: reviewerGlobalDirections.length,
                autoSelectedCount: autoSelectedOptions.length,
                noteCategories
              });
              // ── 1) BASELINE PINNING: ensure current accepted baseline exists (auto-repair/seed once) ──
              let currentAccepted = await getCurrentVersionForDoc(supabase, doc1.id);
              if (!currentAccepted) {
                const { data: latestAnyVersion } = await supabase.from("project_document_versions").select("id, version_number").eq("document_id", doc1.id).order("version_number", {
                  ascending: false
                }).limit(1).maybeSingle();
                if (latestAnyVersion?.id) {
                  ...
                  if (!repairErr) {
                    await logStep(supabase, jobId, null, currentDoc, "baseline_repaired", `Repaired missing baseline by promoting latest version ${latestAnyVersion.id} as current.`, {
                      ci,
                      gp,
                      gap
                    }, undefined, {
                      documentId: doc1.id,
                      docType: currentDoc,
                      chosenVersionId: latestAnyVersion.id
                    });
                    currentAccepted = await getCurrentVersionForDoc(supabase, doc1.id);
                  }
                }
              }
              if (!currentAccepted) {
                const seedText = (doc1?.plaintext || doc1?.extracted_text || "").trim();
                if (seedText.length > 0) {
                  const { data: maxRow } = await supabase.from("project_document_versions").select("version_number").eq("document_id", doc1.id).order("version_number", {
                    ascending: false
                  }).limit(1).maybeSingle();
                  ...
                  const seededVersion = await createVersion(supabase, {
                    ...
                    deliverableType: currentDoc,
                    metaJson: {
                      seed_source: "auto_run_baseline_seed",
                      seeded_at: new Date().toISOString()
                    },
                    generatorId: "auto-run-seed",
                    inputsUsed: {
                      generator_id: "auto-run-seed",
                      document_id: doc1.id,
                      doc_type: currentDoc,
                      project_id: job.project_id,
                      job_id: jobId
                  ...
                  if (seededVersion?.id) {
                    await supabase.from("project_documents").update({
                      latest_version_id: seededVersion.id
                    }).eq("id", doc1.id);
                    await logStep(supabase, jobId, null, currentDoc, "baseline_seeded", `Seeded baseline from document plaintext and set version ${seededVersion.id} as current.`, {
                      ci,
                      gp,
                      gap
                    }, undefined, {
                      documentId: doc1.id,
                      docType: currentDoc,
                      seededVersionId: seededVersion.id
                    });
                    currentAccepted = await getCurrentVersionForDoc(supabase, doc1.id);
                  }
                }
              ...
              if (!currentAccepted) {
                ...
                const hasSeedText = (doc1?.plaintext || doc1?.extracted_text || "").trim().length > 0;
                if (!hasSeedText && (!versionCount || versionCount === 0)) {
                  await logStep(supabase, jobId, null, currentDoc, "baseline_missing_no_text", `No baseline exists and no plaintext source is available for ${currentDoc}.`, {
                    ci,
                    gp,
                    gap
                  }, undefined, {
                ...
                await logStep(supabase, jobId, null, currentDoc, "baseline_missing", `No current accepted version found for document ${doc1.id} (${versionCount ?? 0} versions exist).`, {
                  ci,
                  gp,
                  gap
                }, undefined, {
              ...
              const abvrResolved = await resolveActiveVersionForDoc(supabase, job, doc1.id, {
                jobId,
                docType: currentDoc
              });
              if (!abvrResolved?.versionId) {
                await logStep(supabase, jobId, null, currentDoc, "baseline_resolution_failed", `ABVR could not resolve an active version for ${currentDoc}. Failing closed.`, {}, undefined, {
                  documentId: doc1.id,
                  docType: currentDoc,
                  reason: "abvr_no_version"
                });
                await updateJob(supabase, jobId, {
                  stage_loop_count: newLoopCount,
                  status: "paused",
              ...
              if (job.follow_latest && currentAccepted?.id && baselineVersionId !== currentAccepted.id) {
                console.log(`[auto-run][IEL] abvr_baseline_divergence { job_id: "${jobId}", doc_type: "${currentDoc}", document_id: "${doc1.id}", abvr_version_id: "${baselineVersionId}", abvr_reason: "${abvrResolved.reason}", current_version_id: "${currentAccepted.id}", note: "ABVR and is_current differ while follow_latest=true" }`);
              }
              ...
              let baselineGP;
              {
                const jobLastAnalyzed2 = job.last_analyzed_version_id;
                const jobLastCI2 = job.last_ci;
                const jobLastGP2 = job.last_gp;
                const canReuseFromJob = jobLastAnalyzed2 === baselineVersionId && typeof jobLastCI2 === "number" && typeof jobLastGP2 === "number";
                ...
                if (persisted.ci !== null && persisted.gp !== null) {
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "baseline_score_reused", `Reused DB-persisted scores for baseline ${baselineVersionId}: CI=${baselineCI} GP=${baselineGP}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    source: "version_meta_json",
                    score_source: persisted.scoreSource,
                    baselineVersionId,
                    last_analyzed_version_id: jobLastAnalyzed2
                  });
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "baseline_score_reused", `Reused job cache and backfilled version scores for baseline ${baselineVersionId}: CI=${baselineCI} GP=${baselineGP}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    source: "job_cache_backfill",
                    baselineVersionId,
                    last_analyzed_version_id: jobLastAnalyzed2
                  });
                } else {
                  await logStep(supabase, jobId, null, currentDoc, "baseline_score_rescored", `Re-scoring baseline ${baselineVersionId} (last_analyzed=${jobLastAnalyzed2 || 'null'}, mismatch=${jobLastAnalyzed2 !== baselineVersionId})`, {}, undefined, {
                    baselineVersionId,
                    last_analyzed_version_id: jobLastAnalyzed2,
                    reason: jobLastAnalyzed2 !== baselineVersionId ? "version_mismatch" : "scores_missing"
                  });
                  ...
                  try {
                    const baselineScoreResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
                      action: "analyze",
                      projectId: job.project_id,
                      documentId: doc1.id,
                      versionId: baselineVersionId,
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "baseline_score_failed", `Baseline scoring failed: ${bsErr.message}. Halting.`, {}, undefined, {
                      baselineVersionId,
                      error: bsErr.message
                    });
                    await updateJob(supabase, jobId, {
              ...
              async function scoreCandidate(candVersionId, label) {
                ...
                const doScoreCall = async ()=>{
                  const postScoreResult = await callEdgeFunctionWithRetry(supabase, supabaseUrl, "dev-engine-v2", {
                    action: "analyze",
                    projectId: job.project_id,
                    documentId: doc1.id,
                    versionId: candVersionId,
                  ...
                  const candBlockers = pickArray(analysisObj, [
                    "blocking_issues",
                    "blockers",
                    "scores.blocking_issues"
                  ]);
                  return {
                    ci: scores.ci,
                    gp: scores.gp,
                    blockerCount: candBlockers.length
                  };
                };
                try {
                  ...
                  const errBody = (e.body ?? e.message ?? "").slice(0, 300);
                  ...
                  if (is5xx) {
                    ...
                    try {
                      ...
                      const retryBody = (retryErr.body ?? retryErr.message ?? "").slice(0, 300);
                      console.error(`[auto-run] scoreCandidate(${label}) retry also failed: status=${retryStatus} body=${retryBody}`);
                      await logStep(supabase, jobId, null, currentDoc, "post_score_failed_detail", `Scoring failed after retry: status=${retryStatus}, deliverableType=${currentDoc}, versionId=${candVersionId}`, {
                        ci: baselineCI,
                        gp: baselineGP
                      }, undefined, {
                        ...scorePayloadMeta,
                        error_status: retryStatus,
                        response_excerpt: retryBody,
                        retried: true
                      });
                      return null;
                    }
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "post_score_failed_detail", `Scoring failed (${errStatus}): deliverableType=${currentDoc}, versionId=${candVersionId}. ${errBody.slice(0, 120)}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    ...scorePayloadMeta,
                    error_status: errStatus,
                    response_excerpt: errBody,
                    retried: false
                  });
                  return null;
                }
              }
              function threeWayGate(candCI, candGP) {
                const ciDrop = baselineCI - candCI;
                const gpDrop = baselineGP - candGP;
                const worstDrop = Math.max(ciDrop, gpDrop);
                if (worstDrop <= REGRESSION_THRESHOLD) return {
                  decision: "PROMOTE",
                  ciDrop,
                  gpDrop,
                  worstDrop
                };
                if (worstDrop <= EXPLORE_THRESHOLD) return {
                  decision: "EXPLORE",
                  ciDrop,
                  gpDrop,
                  worstDrop
              ...
              async function promoteCandidate(candVersionId, candCI, candGP, meta, candBlockerCount) {
                const { error: promoteErr } = await supabase.rpc("set_current_version", {
                  p_document_id: doc1.id,
                  p_new_version_id: candVersionId
                });
                if (promoteErr) {
                  await logStep(supabase, jobId, null, currentDoc, "promote_failed", `set_current_version failed: ${promoteErr.message}. Failing closed.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    ...meta,
                ...
                try {
                  ...
                  const latestMeta = latestScoreSnapshot.meta && typeof latestScoreSnapshot.meta === "object" && !Array.isArray(latestScoreSnapshot.meta) ? latestScoreSnapshot.meta : {};
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "candidate_approval_stamp_failed", `Approval/score persistence failed for accepted candidate ${candVersionId}. Failing closed.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    job_id: jobId,
                    document_id: doc1.id,
                    version_id: candVersionId,
                    error: approvalStampErr?.message || "unknown",
                    rls_hint: "verify UPDATE policy/service role write path"
                  });
                  await updateJob(supabase, jobId, {
                    stage_loop_count: newLoopCount,
                    status: "paused",
                ...
                console.log(`[auto-run][IEL] candidate_accepted_persisted { document_id: "${doc1.id}", accepted_version_id: "${candVersionId}", approval_status_set: "approved", job_id: "${jobId}", step_index: ${newStep}, ci: ${acceptedCI}, gp: ${acceptedGP} }`);
                ...
                const globalBestCI = job.best_ci ?? null;
                const globalBestGP = job.best_gp ?? null;
                ...
                // ── STAGNATION TRACKING ──
                const lastBlockerCount = job.last_blocker_count ?? null;
                const prevStagnation = job.stagnation_no_blocker_count ?? 0;
                ...
                await logStep(supabase, jobId, null, currentDoc, "rewrite_accepted", `Candidate accepted (attempt ${attemptNumber}, ${strategy}). CI: ${baselineCI}→${acceptedCI}, GP: ${baselineGP}→${acceptedGP}. Blockers: ${blockersCount}→${cbc}${blockersImproved ? ' ✓ improved' : ''}${isStageBest ? ' [STAGE BEST]' : ''}${isGlobalBest ? ' [GLOBAL BEST]' : ''}`, {
                  ci: acceptedCI,
                  gp: acceptedGP
                }, undefined, {
                  ...meta,
                  ...
                  isStageBest,
                  isGlobalBest,
                  blocker_count_before: blockersCount,
                  blocker_count_after: cbc,
                  blockers_improved: blockersImproved,
                  stagnation_count: stagnationCount,
                  accepted_version_id: candVersionId,
                  score_source: "auto-run-fork"
                });
                const jobUpdate = {
                  stage_loop_count: newLoopCount,
                  follow_latest: true,
                  resume_document_id: doc1.id,
                ...
                if (isGlobalBest) {
                  ...
                  jobUpdate.best_document_id = doc1.id;
                ...
                if (noteResolution.resolved > 0) {
                  await logStep(supabase, jobId, null, currentDoc, "note_auto_resolved", `Resolved ${noteResolution.resolved} note(s) on ${currentDoc} after rewrite: [${noteResolution.notes.map((n)=>n.title).join("; ")}]. All notes exhausted — ready to promote.`, {
                    ci: acceptedCI,
                    gp: acceptedGP
                  }, undefined, {
                    resolved_count: noteResolution.resolved,
                    resolved_notes: noteResolution.notes,
                    resolver: "auto_run_rewrite",
                    version_id: candVersionId
                  });
                }
                console.log(`[auto-run][IEL] acceptance_anchor_set { job_id: "${jobId}", doc_type: "${currentDoc}", document_id: "${doc1.id}", accepted_version_id: "${candVersionId}", last_analyzed_version_id: "${candVersionId}", last_ci: ${acceptedCI}, last_gp: ${acceptedGP}, score_source: "auto-run-fork", eligibility_reason: "approved_after_accept" }`);
                return true;
              }
              ...
              async function setFrontier(candVersionId, candCI, candGP, meta) {
                // Read latest persisted frontier state to avoid stale in-memory data
                const { data: freshJob } = await supabase.from("auto_run_jobs").select("frontier_version_id, frontier_attempts").eq("id", jobId).maybeSingle();
                ...
                await logStep(supabase, jobId, null, currentDoc, "frontier_set", `Frontier set (attempt ${attemptNumber}, ${strategy}): CI=${candCI}, GP=${candGP}. Baseline preserved (CI=${baselineCI}, GP=${baselineGP}). is_current unchanged. frontier_attempts=${newAttempts}`, {
                  ci: candCI,
                  gp: candGP
                }, undefined, {
                  ...meta,
              ...
              const prevStagnationCount = job.stagnation_no_blocker_count ?? 0;
              const STAGNATION_LIMIT = 4;
              if (hasBlockers && prevStagnationCount >= STAGNATION_LIMIT) {
                await logStep(supabase, jobId, null, currentDoc, "stagnation_no_blocker_progress", `Blocker count has not decreased in ${prevStagnationCount} consecutive attempts (blockers=${blockersCount}). Pausing for review.`, {
                  ci: baselineCI,
                  gp: baselineGP
                }, undefined, {
                  blockersCount,
                  stagnation_count: prevStagnationCount,
                  STAGNATION_LIMIT
                });
                await updateJob(supabase, jobId, {
                  stage_loop_count: newLoopCount,
              ...
              const EARLY_STAGE_DOCS = new Set([
                "idea",
                "concept_brief"
              ]);
              if (EARLY_STAGE_DOCS.has(currentDoc) && hasBlockers && newLoopCount >= 3) {
                ...
                const { data: recentDecidedSteps } = await supabase.from("auto_run_steps").select("output_ref").eq("job_id", jobId).eq("document", currentDoc).eq("action", "auto_decided").order("step_index", {
                  ascending: false
                }).limit(3);
                if (recentDecidedSteps && recentDecidedSteps.length >= 3) {
                  const hashes = recentDecidedSteps.map((s)=>{
                    const sel = s.output_ref?.selections || {};
                    const keys = Object.keys(sel).sort();
                    return keys.map((k)=>`${k}=${sel[k]}`).join("|");
                  });
                  const allSame = hashes.every((h)=>h === hashes[0]) && hashes[0].length > 0;
                  if (allSame) {
                    const blockerKeys = Object.keys(recentDecidedSteps[0]?.output_ref?.selections || {}).slice(0, 10);
                    await logStep(supabase, jobId, null, currentDoc, "stuck_blocker_loop", `STUCK_BLOCKER_LOOP: Same decisions repeated 3+ times for ${currentDoc}. Blocker keys: ${blockerKeys.join(", ")}`, {
                      ci: baselineCI,
                      gp: baselineGP
                    }, undefined, {
                      blockersCount,
                      repeating_keys: blockerKeys,
              ...
              if (decisionBundle && decisionBundle.accepted_decisions.length > 0) {
                ...
                console.log("[auto-run][TEL] OPTION_SELECTED count=" + decisionBundle.accepted_decisions.length + " doc=" + currentDoc + " note: Observing decisions.");
                await logStep(supabase, jobId, null, currentDoc, "decisions_injected", `Injected ${decisionBundle.accepted_decisions.length} accepted decisions into rewrite (hash=${decisionBundle.accepted_decisions_hash})`, {
                  ci: baselineCI,
                  gp: baselineGP
                }, undefined, {
              ...
              if (currentDoc === "vertical_episode_beats" || currentDoc === "episode_beats") {
                try {
                  const { data: ebProj } = await supabase.from("projects").select("meta_json").eq("id", job.project_id).maybeSingle();
...
- If a continuity issue exists between specific episodes, RESOLVE it explicitly in the beats — do not ignore it.\n`);
              ...
              if ([
                ...
                try {
                  ...
                  const populatedCount = Object.values(existing).filter((v)=>typeof v === "string" && v.trim().length > 0).length;
                  if (populatedCount < 3) {
                    const { data: canonRow } = await supabase.from("project_canon").select("canon_json").eq("project_id", job.project_id).maybeSingle();
                    const cj = canonRow?.canon_json || {};
                    if (cj && typeof cj === "object" && Object.keys(cj).length > 0) {
                      ...
                      if (Object.keys(storySetup).length > 0) {
                        ...
                        console.log(`[auto-run][story-setup-sync] synced before ${currentDoc}`, {
                          fields: Object.keys(storySetup)
                        });
                      }
                    }
              ...
              if (currentDoc === "episode_grid") {
                try {
                  const { data: egProj } = await supabase.from("projects").select("title, meta_json").eq("id", job.project_id).maybeSingle();
...
- Dialogue or scripted content (→ belongs in Season Script)\n`);
              ...
              if (currentDoc === "format_rules") {
                try {
                  const { data: fmtProj } = await supabase.from("projects").select("title, format, meta_json, episode_count").eq("id", job.project_id).maybeSingle();
              ...
              if (SPINE_ELIGIBLE_STAGES.includes(currentDoc)) {
                try {
                  const { data: spineProj } = await supabase.from("projects").select("narrative_spine_json").eq("id", job.project_id).maybeSingle();
              ...
              if (isCPMEnabled() && currentDoc === "episode_grid") {
                const cpRepair = buildCPRepairDirections(allNotesForStrategy.blocking_issues || [], allNotesForStrategy.high_impact_notes || []);
                if (cpRepair.failClosed) {
                  logCPM("cpm_v1_repair_blocked_missing_blockers", {
                    job_id: jobId,
                    doc_type: currentDoc
                  });
                  await logStep(supabase, jobId, null, currentDoc, "cpm_repair_blocked", `CPM repair blocked: ${cpRepair.reason}. No freeform rewrite for CP fields.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    reason: cpRepair.reason
                  });
                } else {
                  mergedDirections.push(...cpRepair.directions);
                  logCPM("cpm_v1_applied", {
                    job_id: jobId,
                    doc_type: "episode_grid",
                    repair_directions: cpRepair.directions.length
                  });
                }
              ...
              if (isRewriteTargetingEnabled()) {
                const compiled = compileRewriteDirectives(allNotesForStrategy.blocking_issues || [], allNotesForStrategy.high_impact_notes || [], currentDoc);
                if (compiled.failClosed) {
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "rewrite_targeting_fail_closed", `Rewrite targeting fail-closed: ${compiled.reason}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  });
                  ...
                  console.log(`[dev-engine-v2][IEL] rewrite_targeting_compiled { job_id: "${jobId}", doc_type: "${currentDoc}", directive_count: ${compiled.directives.length}, categories: ${JSON.stringify([
                    ...new Set(compiled.directives.map((d)=>d.category))
                  ])} }`);
                  await logStep(supabase, jobId, null, currentDoc, "rewrite_targeting_compiled", `Compiled ${compiled.directives.length} targeted rewrite directives`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    directive_count: compiled.directives.length,
                    categories: [
                      ...new Set(compiled.directives.map((d)=>d.category))
                    ]
                  });
              ...
              const jobMeta = job.meta_json || {};
              let canonLockWasApplied = false;
              let canonLockAttemptId = null;
              if (jobMeta.canon_lock_mode && (Array.isArray(jobMeta.canon_lock_core_entities) || Array.isArray(jobMeta.canon_lock_entities))) {
                const coreEntities = jobMeta.canon_lock_core_entities || [];
                const secondaryEntities = jobMeta.canon_lock_secondary_entities || [];
                ...
                if (hasStructured) {
                  ...
                  if (coreList) {
                    mergedDirections.push(`CANON-LOCK: You MUST preserve all CORE canonical entities exactly as named and not contradict them: ${coreList}`, `Do NOT rename, omit, or mutate any CORE entity.`);
                  }
                  if (secondaryList) {
                    mergedDirections.push(`Increase canon coverage by weaving in these SECONDARY entities naturally where appropriate (do not force): ${secondaryList}`);
                  }
                  mergedDirections.push(`You may introduce minor new entities only if narratively required — do not invent major new named characters, locations, or concepts not in the canon.`);
                } else {
                  // Legacy flat list fallback
                  const entityList = (jobMeta.canon_lock_entities || []).slice(0, 80).join(", ");
                  mergedDirections.push(`CANON-LOCK: Preserve these canonical entities and increase coverage naturally: ${entityList}`, `Do NOT rename or omit canonical entities. Do not invent major new named entities.`);
                }
              ...
              // ── PHASE 2C: REWRITE ELIGIBILITY GATE (delta-based churn prevention) ──
              {
                ...
                const eligInput = await buildEligibilityInput(supabase, job.project_id, currentDoc, baselineVersionId, {
                  ...
                  strategy,
                  frontierVersionId: job.frontier_version_id || null
                });
                ...
                console.log(`[auto-run][IEL] rewrite_eligibility_eval ${JSON.stringify({
                  job_id: jobId,
                  doc_type: currentDoc,
                  scope_key: eligScopeKey,
                  eligible: eligResult.eligible,
                  fingerprint: eligResult.fingerprint,
                  prev_fingerprint: eligResult.previousFingerprint,
                  material_changes: eligResult.materialChanges,
                  reason: eligResult.reason
                })}`);
                if (!eligResult.eligible) {
                  await logStep(supabase, jobId, null, currentDoc, "rewrite_eligibility_denied", `Rewrite denied: ${eligResult.reason}. No material delta since last attempt. [scope=${eligScopeKey}]`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    fingerprint: eligResult.fingerprint,
                    previousFingerprint: eligResult.previousFingerprint,
                    unchangedInputs: eligResult.unchangedInputs,
                    blockingFactors: eligResult.blockingFactors,
                    scopeKey: eligScopeKey
                  });
                  await updateJob(supabase, jobId, {
                    stage_loop_count: newLoopCount,
                    status: "paused",
                ...
                await logStep(supabase, jobId, null, currentDoc, "rewrite_eligibility_approved", `Rewrite eligible: ${eligResult.materialChanges.join(", ")}`, {
                  ci: baselineCI,
                  gp: baselineGP
                }, undefined, {
                  fingerprint: eligResult.fingerprint,
                  materialChanges: eligResult.materialChanges
                });
              }
              try {
                ...
                if (strategy === "FORK_CONSERVATIVE_AGGRESSIVE") {
                  ...
                  const forkInputVersionId = job.frontier_version_id ?? baselineVersionId;
                  const forkSelectedOptions = autoSelectedOptions.length > 0 ? autoSelectedOptions.map((s)=>({
                      note_id: s.note_id,
                      option_id: s.option_id
                    })) : undefined;
                  const rewriteBase = {
                  ...
                  for (const forkCandId of [
                    ...
                    try {
                      ...
                      await supabase.from("project_document_versions").update({
                        ...
                        criteria_json: latestCriteriaSnapshot,
                        measured_metrics_json: {
                          measured_duration_seconds: candMeasured,
                          estimated_at: new Date().toISOString(),
                          estimator: 'edge_deterministic'
                        }
                      }).eq("id", forkCandId);
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "fork_candidates_created", `Fork: conservative=${candA || 'FAILED'}, aggressive=${candB || 'FAILED'}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    baselineVersionId,
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "fork_candidates_scored", `Conservative: CI=${scoreA?.ci ?? 'N/A'} GP=${scoreA?.gp ?? 'N/A'}, Aggressive: CI=${scoreB?.ci ?? 'N/A'} GP=${scoreB?.gp ?? 'N/A'}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    scoreA,
                  ...
                  const allCandidates = [];
                  if (candA && scoreA && gateA) allCandidates.push({
                    versionId: candA,
                    ci: scoreA.ci,
                    gp: scoreA.gp,
                    blockerCount: scoreA.blockerCount,
                    label: "conservative",
                    decision: gateA.decision
                  });
                  if (candB && scoreB && gateB) allCandidates.push({
                    versionId: candB,
                    ci: scoreB.ci,
                    gp: scoreB.gp,
                    blockerCount: scoreB.blockerCount,
                    label: "aggressive",
                  ...
                  function chooseForkWinner(candidates) {
                  ...
                  if (promotable.length > 0) {
                    ...
                    const promoted = await promoteCandidate(winner.versionId, winner.ci, winner.gp, {
                      baselineVersionId,
                      forkInputVersionId,
                      candA,
                      candB,
                      forkWinner: winner.label,
                      forkLoser: loser?.label,
                      winner_version_id: winner.versionId,
                      loser_version_id: loser?.versionId,
                      winner_scores: {
                        ci: winner.ci,
                        gp: winner.gp,
                        blockers: winner.blockerCount
                      },
                      loser_scores: loser ? {
                        ci: loser.ci,
                        gp: loser.gp,
                        blockers: loser.blockerCount
                      } : null,
                      selection_reason: reason,
                      policy: FORK_SELECTION_POLICY,
                      scoreA,
                      scoreB,
                      gateA,
                      gateB
                  ...
                  if (explorable.length > 0) {
                    const { data: freshJobFork } = await supabase.from("auto_run_jobs").select("frontier_attempts").eq("id", jobId).maybeSingle();
                    ...
                    if (frontierAttempts < MAX_FRONTIER_ATTEMPTS) {
                      ...
                      await setFrontier(best.versionId, best.ci, best.gp, {
                        ...
                        candA,
                        candB,
                        forkWinner: best.label,
                        winner_version_id: best.versionId,
                        loser_version_id: explorLoser?.versionId,
                        selection_reason: explorReason,
                        policy: FORK_SELECTION_POLICY,
                        scoreA,
                        scoreB,
                        gateA,
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "frontier_cleared", `Frontier exhausted after ${frontierAttempts} attempts. Clearing frontier, returning to baseline.`, {
                      ci: baselineCI,
                      gp: baselineGP
                    }, undefined, {
                      frontierAttempts,
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "fork_both_rejected", `Both fork candidates rejected/frontier exhausted. Baseline preserved.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    gateA,
                ...
                let episodicRepairMeta = null;
                let episodicTargetEpisode = null;
                let originalFullContent = null;
                {
                  try {
                    ...
                    if (isEpisodicBlockRepairSupported(currentDoc)) {
                      const { data: verContent } = await supabase.from("project_document_versions").select("plaintext").eq("id", baselineVersionId).maybeSingle();
                      ...
                      if (docContent.length > 100) {
                        ...
                        const primaryIssue = blockers && blockers.length > 0 ? {
                          episodeIndex: blockers[0]?.episodeIndex,
                          category: blockers[0]?.category,
                          title: blockers[0]?.title || blockers[0]?.objective,
                          summary: blockers[0]?.summary || blockers[0]?.detail,
                          anchor: blockers[0]?.anchor,
                          constraint_key: blockers[0]?.constraint_key
                        } : highImpact && highImpact.length > 0 ? {
                          episodeIndex: highImpact[0]?.episodeIndex,
                          category: highImpact[0]?.category,
                          title: highImpact[0]?.title || highImpact[0]?.objective,
                          summary: highImpact[0]?.summary || highImpact[0]?.detail,
                          anchor: highImpact[0]?.anchor,
                          constraint_key: highImpact[0]?.constraint_key
                        } : null;
                        if (primaryIssue) {
                          ...
                          if (target.repair_target_type === "episode_block" && target.episode_number != null) {
                            ...
                            mergedDirections.push(`EPISODE-TARGETED REPAIR: Focus ALL changes on Episode ${target.episode_number}. Preserve ALL other episodes EXACTLY as they are — do not modify, reorder, rename, or paraphrase any content outside Episode ${target.episode_number}.`);
                ...
                let sceneRepairMeta = null;
                let sceneTargetNumber = null;
                  ...
                  try {
                    ...
                    if (isSceneRepairSupported(currentDoc)) {
                      ...
                      if (originalFullContent && originalFullContent.length > 100) {
                        const primaryIssue = blockers && blockers.length > 0 ? {
                          scene_number: blockers[0]?.scene_number,
                          scene_numbers: blockers[0]?.scene_numbers || blockers[0]?.target?.scene_numbers,
                          category: blockers[0]?.category,
                          title: blockers[0]?.title || blockers[0]?.objective,
                          summary: blockers[0]?.summary || blockers[0]?.detail,
                          anchor: blockers[0]?.anchor,
                          ...
                          scene_number: highImpact[0]?.scene_number,
                          scene_numbers: highImpact[0]?.scene_numbers || highImpact[0]?.target?.scene_numbers,
                          category: highImpact[0]?.category,
                          title: highImpact[0]?.title || highImpact[0]?.objective,
                          summary: highImpact[0]?.summary || highImpact[0]?.detail,
                          anchor: highImpact[0]?.anchor,
                        ...
                        if (primaryIssue) {
                          ...
                          if (target.repair_target_type === "scene" && target.scene_number != null) {
                            ...
                            mergedDirections.push(`SCENE-TARGETED REPAIR: Focus ALL changes on Scene ${target.scene_number} (${target.scene_heading || ""}). Preserve ALL other scenes EXACTLY as they are — do not modify, reorder, rename, or paraphrase any content outside Scene ${target.scene_number}.`);
                ...
                let sectionRepairMeta = null;
                let sectionTargetKey = null;
                      ...
                      if (originalFullContent && originalFullContent.length > 100) {
                        ...
                        if (primaryIssue) {
                          ...
                          if (target.repair_target_type === "section" && target.section_key) {
                            ...
                            mergedDirections.push(`SECTION-TARGETED REPAIR: Focus ALL changes on the "${target.section_label || target.section_key}" section. Preserve ALL other sections EXACTLY as they are — do not modify, reorder, rename, or paraphrase any content outside this section.`);
                ...
                const selectedOptions = autoSelectedOptions.length > 0 ? autoSelectedOptions.map((s)=>({
                    note_id: s.note_id,
                    option_id: s.option_id
                  })) : undefined;
                // ── SINGLE CANDIDATE PATH (all other strategies) ──
                // Use frontier as input if available; compare against BASELINE
                const singleInputVersionId = job.frontier_version_id ?? baselineVersionId;
                const { candidateVersionId } = await rewriteWithFallback(supabase, supabaseUrl, token, {
                  projectId: job.project_id,
                  documentId: doc1.id,
                ...
                const rewriteOutcome = !newVersionId ? "rewrite_failed" : newVersionId === sourceVersionId ? "rewrite_noop_same_content" : "rewrite_created_new_version";
                await logStep(supabase, jobId, null, currentDoc, rewriteOutcome, rewriteOutcome === "rewrite_created_new_version" ? `New version created: ${newVersionId}` : rewriteOutcome === "rewrite_noop_same_content" ? `Rewrite produced identical content — no new version created` : `Rewrite failed: ${newVersionId === null ? "rewrite returned null" : "unknown error"}`, {
                  ci,
                  gp,
                  gap
                }, undefined, {
                  newVersionId: newVersionId || null,
                  sourceVersionId,
                ...
                if (rewriteOutcome === "rewrite_failed") {
                  await updateJob(supabase, jobId, {
                    status: "failed",
                    error: `Rewrite failed: candidateVersionId was null`
                  });
                  return respondWithJob(supabase, jobId);
                }
                ...
                if (episodicTargetEpisode != null && originalFullContent && candidateVersionId) {
                  try {
                    ...
                    const { data: candidateVer } = await supabase.from("project_document_versions").select("plaintext").eq("id", candidateVersionId).maybeSingle();
                ...
                if (episodicRepairMeta) {
                  await logStep(supabase, jobId, null, currentDoc, "episode_block_repair_execution", `Episode block repair: ${episodicRepairMeta.repair_target_type} [ep=${episodicRepairMeta.episode_number || "none"}] exec=${episodicRepairMeta.block_execution_mode} preserved=${episodicRepairMeta.episodes_preserved || 0}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, episodicRepairMeta);
                }
                // Log scene repair metadata for provenance
                if (sceneRepairMeta) {
                  await logStep(supabase, jobId, null, currentDoc, "scene_repair_execution", `Scene repair: ${sceneRepairMeta.repair_target_type} [scene=${sceneRepairMeta.scene_number || "none"}] exec=${sceneRepairMeta.scene_execution_mode} preserved=${sceneRepairMeta.scenes_preserved || 0} corrected=${sceneRepairMeta.scenes_corrected || 0}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, sceneRepairMeta);
                }
                if (sectionRepairMeta) {
                  await logStep(supabase, jobId, null, currentDoc, "section_repair_execution", `Section repair: ${sectionRepairMeta.repair_target_type} [${sectionRepairMeta.section_key || "full_doc"}] exec=${sectionRepairMeta.section_execution_mode} corrected=${sectionRepairMeta.sections_corrected || 0}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, sectionRepairMeta);
                }
                // ── Clear canon_lock_mode AFTER successful rewrite ──
                if (canonLockWasApplied) {
                  const postRewriteMeta = {
                    ...job.meta_json || {},
                    canon_lock_mode: false,
                    canon_lock_applied_at_step: newStep + 2
                  };
                  await updateJob(supabase, jobId, {
                    meta_json: postRewriteMeta
                  });
                ...
                if (!candidateVersionId || candidateVersionId === baselineVersionId) {
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "rewrite_no_candidate", `Rewrite did not produce a new version id (attempt ${attemptNumber}, ${strategy}). Baseline preserved. Halting.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    baselineVersionId,
                    singleInputVersionId,
                  ...
                  const { data: casRow } = await supabase.from("auto_run_jobs").update({
                    stage_loop_count: newLoopCount,
                    follow_latest: false,
                    resume_document_id: doc1.id,
                    resume_version_id: baselineVersionId,
                  ...
                  }).eq("id", jobId).eq("status", "running").eq("step_count", casStepCount).select("id").maybeSingle();
                  if (!casRow) {
                ...
                await logStep(supabase, jobId, null, currentDoc, "rewrite_candidate_created", `Candidate ${candidateVersionId} created (attempt ${attemptNumber}, ${strategy}). Input=${singleInputVersionId}. Scoring before acceptance.`, {
                  ci: baselineCI,
                  gp: baselineGP
                }, undefined, {
                  baselineVersionId,
                ...
                if (!candScore) {
                  await logStep(supabase, jobId, null, currentDoc, "post_score_failed", `Post-rewrite scoring failed (attempt ${attemptNumber}, ${strategy}). Candidate rejected. Baseline preserved.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    baselineVersionId,
                ...
                await logStep(supabase, jobId, null, currentDoc, "rewrite_candidate_scored", `Candidate scored: CI=${candidateCI}, GP=${candidateGP}, blockers=${candidateBlockerCount} (baseline CI=${baselineCI}, GP=${baselineGP}, blockers=${blockersCount}). Blockers ${candidateBlockerCount < blockersCount ? 'improved ✓' : candidateBlockerCount === blockersCount ? 'unchanged' : 'worsened ✗'}`, {
                  ci: candidateCI,
                  gp: candidateGP
                }, undefined, {
                  baselineVersionId,
                  ...
                  candidateGP,
                  blocker_count_before: blockersCount,
                  blocker_count_after: candidateBlockerCount,
                  blockers_improved: candidateBlockerCount < blockersCount,
                  attemptNumber,
                  strategy
                });
                // ── THREE-WAY ACCEPTANCE GATE ──
                const { decision: gateDecision, ciDrop, gpDrop, worstDrop } = threeWayGate(candidateCI, candidateGP);
                await logStep(supabase, jobId, null, currentDoc, "gate_decision", `Gate: ${gateDecision} | CI ${baselineCI}→${candidateCI} (drop ${ciDrop}), GP ${baselineGP}→${candidateGP} (drop ${gpDrop}) | blockers ${blockersCount}→${candidateBlockerCount} | PROMOTE_DELTA=${REGRESSION_THRESHOLD}, EXPLORE_DELTA=${EXPLORE_THRESHOLD}`, {
                  ci: candidateCI,
                  gp: candidateGP
                }, undefined, {
                ...
                if (gateDecision === "PROMOTE") {
                  ...
                  const promoted = await promoteCandidate(candidateVersionId, candidateCI, candidateGP, {
                    baselineVersionId,
                    singleInputVersionId,
                    candidateVersionId,
                    baselineCI,
                ...
                if (gateDecision === "EXPLORE") {
                  ...
                  // Read persisted frontier state for deterministic attempt counting
                  const { data: freshJobSingle } = await supabase.from("auto_run_jobs").select("frontier_attempts").eq("id", jobId).maybeSingle();
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "frontier_cleared", `Frontier exhausted after ${frontierAttempts} attempts (max ${MAX_FRONTIER_ATTEMPTS}). Clearing frontier, returning to baseline.`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    frontierAttempts,
                ...
                await logStep(supabase, jobId, null, currentDoc, "rewrite_rejected_regression", `Candidate rejected (attempt ${attemptNumber}, ${strategy}): CI ${baselineCI}→${candidateCI} (drop ${ciDrop}), GP ${baselineGP}→${candidateGP} (drop ${gpDrop}). worstDrop=${worstDrop}, PROMOTE_DELTA=${REGRESSION_THRESHOLD}, EXPLORE_DELTA=${EXPLORE_THRESHOLD}. Baseline preserved.`, {
                  ci: baselineCI,
                  gp: baselineGP
                }, undefined, {
                  baselineVersionId,
                ...
                if (e.message?.includes("CANON_MISMATCH")) {
                  if (job.allow_defaults === true || job.meta_json?.auto_approve_all) {
                    await logStep(supabase, jobId, null, currentDoc, "canon_mismatch_advisory", `CANON_MISMATCH suppressed in autonomous mode (allow_defaults=true) — treating as advisory, not blocking`, {
                      ci: baselineCI,
                      gp: baselineGP
                    });
                    return respondWithJob(supabase, jobId, "run-next");
                  ...
                  if (retryCount <= MAX_CANON_LOCK_RETRIES) {
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "canon_lock_retry", `CANON_MISMATCH retry ${retryCount}/${MAX_CANON_LOCK_RETRIES} (core=${coreEntities.length}, secondary=${secondaryEntities.length}, attempt_id=${attemptId}): ${e.message.slice(0, 200)}`, {
                      ci: baselineCI,
                      gp: baselineGP
                    }, undefined, {
                      retry_count: retryCount,
                      ...
                      secondary_count: secondaryEntities.length,
                      attempt_id: attemptId,
                      error_excerpt: e.message.slice(0, 300),
                      doc_type: currentDoc
                    });
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "canon_mismatch_stuck", `CANON_MISMATCH stuck after ${retryCount - 1} retries: ${e.message.slice(0, 200)}`, {
                    ci: baselineCI,
                    gp: baselineGP
                  }, undefined, {
                    retry_count: retryCount - 1,
            ...
            if (promo.recommendation === "promote") {
              ...
              const ciGate1 = await evaluateCIGate(supabase, jobId, currentDoc, writeTargetCi, job.project_id, job);
              console.log(`[auto-run][IEL] ci_gate_eval { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ciGate1.ci}, min_ci: ${writeTargetCi}, rule: "promote_gate_writing" }`);
              if (!ciGate1.pass) {
                remediationReason = "ci_gate_blocked";
                finalRecommendation = "stabilise";
                console.warn(`[auto-run][IEL] ci_gate_blocked_promote { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ciGate1.ci}, min_ci: ${writeTargetCi}, trigger: "promote_writing" }`);
                await logStep(supabase, jobId, null, currentDoc, "ci_gate_blocked", `Promotion blocked: CI=${ciGate1.ci} < ${writeTargetCi}. Converting to stabilise.`, {
                  ci: ciGate1.ci,
                  gp
                }, undefined, {
                  min_ci: writeTargetCi,
                  readiness: promo.readiness_score,
                  trigger: "promote_writing"
                });
                await updateJob(supabase, jobId, {
                  stage_loop_count: stageLoopCount + 1
                });
              } else {
                console.log(`[auto-run][IEL] ci_gate_passed { job_id: "${jobId}", doc_type: "${currentDoc}", ci: ${ciGate1.ci} }`);
              }
              ...
              if (finalRecommendation === "stabilise") {
                ...
                if (writeNoteExhaust.hasActionable) {
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "note_exhaustion_blocked", `Promotion blocked: ${writeNoteExhaust.count} actionable note(s) remain for ${currentDoc}. Continuing stabilise.`, {
                    ci: ciGate1.ci,
                    gp
                  }, undefined, {
                    actionable_count: writeNoteExhaust.count,
                    trigger: "writing_promote"
                  });
                  await updateJob(supabase, jobId, {
                    stage_loop_count: stageLoopCount + 1
                  });
                ...
                if (ciGate1.pass !== true || remediationReason === "ci_gate_blocked") {
                  console.error(`[auto-run][PATCH4] BLOCKED: auto_approved_promote called with ciGate.pass=${ciGate1.pass}, remediationReason=${remediationReason}. Forcing stabilise.`);
                  await updateJob(supabase, jobId, {
                    stage_loop_count: stageLoopCount + 1
                  });
                  return respondWithJob(supabase, jobId, "run-next");
                ...
                if (next && isStageAtOrBeforeTarget(next, job.target_document, format)) {
                  if (job.allow_defaults || job.meta_json?.auto_approve_all) {
                    ...
                    await logStep(supabase, jobId, null, currentDoc, "auto_approved_promote", `Promote recommended: ${currentDoc} → ${next}. Auto-promoting (allow_defaults). CI=${ciGate1.ci}≥${writeTargetCi}✓`, {
                      ci,
                      gp,
                      gap,
                      readiness: promo.readiness_score,
                  ...
                  await logStep(supabase, jobId, null, currentDoc, "approval_required", `Promote recommended: ${currentDoc} → ${next}. Review before advancing.`, {
                    ci,
                    gp,
                    gap,
                    readiness: promo.readiness_score,
            ...
            if (isUpstreamOutage(e)) {
              ...
              if (!retried) {
                ...
                await logStep(supabase, jobId, errStep, currentDoc, "dev_engine_unavailable", compactErr.slice(0, 500));
              }
            } else {
              const errIdx = await nextStepIndex(supabase, jobId);
              await updateJob(supabase, jobId, {
                status: "failed",
                error: `Step failed: ${(e.message || '').slice(0, 500)}`
              });
              ...
              if (postJob && postJob.status === "running" && !postJob.awaiting_approval && !postJob.is_processing) {
                ...
                if (postJob.step_count < postJob.max_total_steps) {
                  ...
                  if (postJob.step_count >= 3) {
                    try {
                      const { data: recentSteps } = await supabase.from("auto_run_steps").select("step_number, output_json").eq("job_id", jobId).not("output_json", "is", null).order("step_number", {
                        ascending: false
                      }).limit(3);
        ...
        if (scheduled) {
          console.log("[auto-run] run-next returning early (bg scheduled via waitUntil)", {
            jobId
          });
        } else {
          ...
          bgTask.catch((e)=>console.error("[auto-run] bgTask unhandled", e?.message || e));
          console.log("[auto-run] run-next returning early (fire-and-forget, no waitUntil)", {
            jobId
          });
        }
...
async function respondWithJob(supabase, jobId, hint) {
  const { data: job } = await supabase.from("auto_run_jobs").select("*").eq("id", jobId).maybeSingle();
  const { data: steps } = await supabase.from("auto_run_steps").select("*").eq("job_id", jobId).order("step_index", {
    ascending: false
  }).limit(200);
  ...
  if (job && job.pause_reason === "pending_decisions" && Array.isArray(job.pending_decisions) && job.pending_decisions.length > 0) {
    const needsEnrichment = job.pending_decisions.some((d)=>!d.question && !d.options);
    if (needsEnrichment) {
      const legacyCount = job.pending_decisions.filter((d)=>!d.question && !d.options).length;
      console.log(`[auto-run][IEL] pending_decisions_legacy_detected`, JSON.stringify({
        job_id: jobId,
        doc_type: job.current_document,
        count: job.pending_decisions.length,
        legacy_count: legacyCount
      }));
      try {
        const idsToFetch = job.pending_decisions.filter((d)=>!d.question && !d.options && d.id).map((d)=>d.id);
        if (idsToFetch.length > 0) {
          const { data: fullRows, error: fetchErr } = await supabase.from("decision_ledger").select("id, decision_key, title, decision_text, decision_value").in("id", idsToFetch);
          if (fetchErr) {
            console.error(`[auto-run][IEL] pending_decisions_enrich_failed`, JSON.stringify({
              job_id: jobId,
              error: fetchErr.message
            }));
          } else {
            const rowMap = new Map((fullRows || []).map((r)=>[
                r.id,
                r
              ]));
            const enriched = job.pending_decisions.map((d)=>{
            ...
            await supabase.from("auto_run_jobs").update({
              pending_decisions: enriched
            }).eq("id", jobId);
            job.pending_decisions = enriched;
            console.log(`[auto-run][IEL] pending_decisions_enriched`, JSON.stringify({
              job_id: jobId,
              enriched_count: idsToFetch.length,
              missing_rows: idsToFetch.filter((id)=>!rowMap.has(id)),
              option_counts: enriched.map((d)=>(d.options || []).length)
            }));
          }
        }
      } catch (e) {
  ...
  if (job && Array.isArray(job.pending_decisions)) {
    job.pending_decisions = job.pending_decisions.map((d)=>{
      if (d.recommended && typeof d.recommended === "object" && d.recommended.value) {
        return {
          ...d,
          recommended: String(d.recommended.value)
        };
      }
      return d;
    });
