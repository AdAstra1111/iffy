...
async function handleExtract(projectId: string) {
  ...
  const staleRecovery = await recoverStaleRunning(admin, projectId, "genre").catch(() => ({ recovered: 0 }));
  if (staleRecovery.recovered > 0) {
    console.log("[StaleRecovery] Recovered " + staleRecovery.recovered + " stale genre atoms on status check");
  }
