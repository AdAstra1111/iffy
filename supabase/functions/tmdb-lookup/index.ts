...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (mode === 'search') {
      const results = searchData.results.slice(0, 8).map((p)=>({
          tmdb_id: p.id,
          name: p.name,
    ...
    const detailUrl = (path)=>`https://api.themoviedb.org/3${path}${isReadAccessToken ? '' : `?api_key=${apiKey}&`}${isReadAccessToken ? '?language=en-US' : 'language=en-US'}`;
    const fetchOpts = {
      headers: authHeaders
    };
    ...
    const profileUrl = detail.profile_path ? `https://image.tmdb.org/t/p/w500${detail.profile_path}` : null;
    const result = {
      found: true,
      tmdb_id: tmdbId,
