...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Seed-pack docs are auto-generated by the development pipeline — skip them
    const SKIP_TYPES = ['nec', 'canon'];
    const filteredDocTypes = docTypes.filter((dt) => {
      if (SKIP_TYPES.includes(dt)) {
        console.warn(`[regenerate-stale-docs] Skipping seed-pack docType "${dt}" — auto-generated by pipeline`);
        return false;
      }
      return true;
    });
    ...
    for (const docType of filteredDocTypes) {
      try {
