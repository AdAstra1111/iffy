...
export interface ApplyGraphMutationsOutput {
  ok: boolean;
  applied: number;
  entities_created: ApplyResult[];
  errors?: string[];
}

export async function applyGraphMutationsHandler(
  supabase: SupabaseClient,
  serviceSupabase: SupabaseClient,
  input: ApplyGraphMutationsInput
): Promise<ApplyGraphMutationsOutput> {
  const { projectId, proposalIds, approved, reviewComment } = input;
  const entitiesCreated: ApplyResult[] = [];
  const errors: string[] = [];
