...
export async function classifyMutationsHandler(supabase, serviceSupabase, input) {
  ...
  for (const note of approvedNotes || []){
    ...
    try {
      ...
      const { text: extractionText } = await composeSystem(ENTITY_EXTRACTION_SYSTEM, `Extract character entity from this development note:\n\n${noteText}\n\nReturn JSON: { "name": string, "role": string, "description": string, "confidence": number }`, {
      ...
      if (existingEntity) {
        ...
        const { data: duplicateProposal } = await serviceSupabase.from('graph_mutation_proposals').insert({
        ...
        if (duplicateProposal) {
          proposals.push({
            ...duplicateProposal,
            _duplicate_warning: true
          });
        }
        continue;
      }
      // 6. Insert into graph_mutation_proposals
      const { data: newProposal, error: insertError } = await serviceSupabase.from('graph_mutation_proposals').insert({
        project_id: projectId,
        source_note_id: noteId,
        mutation_type: 'add_entity',
        entity_type: 'character',
