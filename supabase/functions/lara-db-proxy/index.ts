  ...
  try {
    ...
    switch (op) {
      ...
      case "list_projects": {
        ...
        const { data, error } = await supabase
          .from("projects")
          .select("id, title, format, created_at, updated_at, user_id")
      ...
      // ── Get owner user_id for a project ──
      ...
      // ── Insert a project row (transfer use) ──
      case "insert_project": {
        const { project_data } = params;
        if (!project_data || !project_data.user_id) throw new Error("insert_project requires project_data with user_id");
        const { data, error } = await supabase
          .from("projects")
          .insert(project_data)
          .select("id, title, user_id")
      ...
      // ── Insert a project document row ──
      case "insert_document": {
        const { doc_data } = params;
        if (!doc_data) throw new Error("insert_document requires doc_data");
        const { data, error } = await supabase
          .from("project_documents")
          .insert(doc_data)
      ...
      // ── Insert a document version row ──
      case "insert_document_version": {
        const { version_data } = params;
        if (!version_data) throw new Error("insert_document_version requires version_data");
        const { data, error } = await supabase
          .from("project_document_versions")
          .insert(version_data)
        ...
        const APPROVED_MIGRATIONS: Record<string, string> = {
          ...
          "insert_yeti_script_doc": `
            ...
            SELECT
              ...
              'yeti_script.txt',
              '/uploads/yeti_script.txt'
            ON CONFLICT (project_id, doc_type) DO UPDATE SET plaintext = EXCLUDED.plaintext;
          `,
          ...
          "rename_proj_docs_extracted_text": `
            ALTER TABLE public.project_documents
              ADD COLUMN IF NOT EXISTS plaintext TEXT,
              ADD COLUMN IF NOT EXISTS latest_version_id UUID;
            UPDATE public.project_documents SET plaintext = extracted_text WHERE plaintext IS NULL AND extracted_text IS NOT NULL;
          `,

          "check_proj_docs_cols": `
            SELECT column_name FROM information_schema.columns WHERE table_name = 'project_documents' AND table_schema = 'public';
          `,

          "check_yeti_proj_docs": `
            SELECT id, LEFT(plaintext, 50) as p_text, LEFT(extracted_text, 50) as e_text
          ...
          "test_yeti_insert": `
            INSERT INTO public.project_documents
              (id, project_id, doc_type, doc_role, title, plaintext, user_id, file_name, file_path)
            VALUES (
              '6b145f62-aaaa-bbbb-cccc-00000000000b'::uuid,
              '6b145f62-d482-4cd4-99cc-0ea57079d98a'::uuid,
              'source_script',
              'source_script',
              'Yeti Test',
              'TEST - if you see this the insert works',
              '00000000-0000-0000-0000-000000000001'::uuid,
              'test.txt',
              '/uploads/test.txt'
            )
          ...
          "insert_yeti_test_v2": `
            INSERT INTO public.project_documents
              (id, project_id, doc_type, doc_role, title, plaintext, user_id, file_name, file_path)
            SELECT
              '6b145f62-aaaa-bbbb-cccc-00000000000c'::uuid,
              '6b145f62-d482-4cd4-99cc-0ea57079d98a'::uuid,
              'source_script',
              'source_script',
              'Yeti Test V2',
