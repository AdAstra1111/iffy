...
function resolveStylePolicy(format, genres) {
  const f = format.toLowerCase();
  const gs = genres.map((g)=>g.toLowerCase());
  if (ANIMATION_FORMATS.some((af)=>f.includes(af))) {
    return {
      mode: "stylised_animation",
      styleDirectives: "Stylised animated visual language. Professional animation studio quality.",
  ...
  if (gs.some((g)=>GRAPHIC_GENRES.some((gg)=>g.includes(gg)))) {
    return {
      ...
      styleDirectives: "Graphic novel / comic book visual style. Bold ink work.",
      negativeStyleConstraints: "photorealistic, live-action, stock photo"
    };
  }
  return {
...
/** Shot-type to narrative selection strategy */ const SHOT_NARRATIVE_STRATEGY = {
  wide: {
    prefer: 'establishing'
  },
  atmospheric: {
...
/** Scene-role to shot-type affinity scores (additive to strategy score) */ const SCENE_ROLE_SHOT_PREFERENCE = {
  setup: {
    wide: 5,
    medium: 3,
    close_up: 0,
...
function selectNarrativeMoment(moments, shotType, variantIndex, shotPlanContext) {
  ...
  if (shotPlanContext?.narrativeTarget) {
    ...
    const matching = moments.filter((m)=>m.purpose && m.purpose.toLowerCase().includes(target) || m.slugline && m.slugline.toLowerCase().includes(target) || m.summary && m.summary.toLowerCase().includes(target));
...
function buildNarrativeMomentBlock(moment) {
  const lines = [
    '[NARRATIVE MOMENT — FROM THIS PROJECT\'S SCRIPT]',
    ''
  ];
...
async function loadNarrativeMoments(sb, projectId) {
  // Load active scene IDs
  const { data: scenes } = await sb.from('scene_graph_scenes').select('id').eq('project_id', projectId).is('deprecated_at', null).limit(50);
  if (!scenes?.length) return [];
  const sceneIds = scenes.map((s)=>s.id);
  // Load latest version per scene (highest version_number)
  const { data: versions } = await sb.from('scene_graph_versions').select('scene_id, slugline, summary, content, characters_present, location, time_of_day, purpose, tension_delta, scene_roles, canon_location_id, version_number').eq('project_id', projectId).in('scene_id', sceneIds).order('version_number', {
    ascending: false
  });
  ...
  for (const v of versions){
    ...
    const chars = Array.isArray(v.characters_present) ? v.characters_present.filter((c)=>typeof c === 'string') : [];
    moments.push({
      slugline: v.slugline || '',
      summary: v.summary || '',
...
const SECTION_BINDING_RELEVANCE = {
  character: {
    characters: true,
    locations: false,
    world: true
...
async function resolveCharacterBindings(sb, projectId, sectionKey, explicitCharacterName, explicitCharacterNames) {
...
async function resolveLocationBindings(sb, projectId, sectionKey, explicitLocationId, explicitLocationName, explicitLocationIds) {
  ...
  if (explicitLocationIds?.length) {
    const { data: exactRows } = await sb.from("canon_locations").select("id, canonical_name, description, location_type, geography, interior_or_exterior, era_relevance, story_importance").eq("project_id", projectId).eq("active", true).in("id", explicitLocationIds);
    if (exactRows?.length) {
      return exactRows.map((loc)=>({
          location_id: loc.id,
          canonical_name: loc.canonical_name,
          description: loc.description,
          location_type: loc.location_type || 'unspecified',
  ...
  let q = sb.from("canon_locations").select("id, canonical_name, description, location_type, geography, interior_or_exterior, era_relevance, story_importance").eq("project_id", projectId).eq("active", true);
  if (explicitLocationId) q = q.eq("id", explicitLocationId);
  q = q.order("story_importance", {
    ascending: true
  }).limit(6);
  ...
  if (explicitLocationName && !explicitLocationId) {
    ...
    const match = locRows.filter((l)=>l.canonical_name?.toLowerCase().includes(norm));
...
function resolveWorldBinding(canonJson) {
  if (!canonJson) return {
    era: '',
    geography: '',
    architecture: '',
  ...
  const cj = canonJson;
  const worldRules = [];
  if (Array.isArray(cj.world_rules)) worldRules.push(...cj.world_rules.filter((r)=>typeof r === 'string'));
  ...
  return {
    ...
    geography: cj.geography || '',
    architecture: cj.architecture || '',
...
function buildCharacterBindingBlock(chars, shotType) {
...
function buildLocationBindingBlock(locs) {
  if (!locs.length) return '';
  const lines = [
    '[CANONICAL LOCATION BINDING — ARCHITECTURAL CONTINUITY REQUIRED]',
    ''
  ];
  for (const l of locs){
...
function buildWorldBindingBlock(world) {
  ...
  if (world.world_rules.length > 0) {
    lines.push(`WORLD RULES:`);
    for (const r of world.world_rules.slice(0, 5))lines.push(`  - ${r}`);
  }
  lines.push('', `ENFORCE: All imagery must belong to THIS project's world. No generic fantasy substitution.`);
  return lines.join('\n');
}
async function resolveCanonicalBindings(sb, projectId, sectionKey, canonJson, explicitCharacterName, explicitLocationId, explicitLocationName, explicitCharacterNames, explicitLocationIds) {
  ...
  const missing = [];
  const rel = SECTION_BINDING_RELEVANCE[sectionKey] || {
    characters: false,
    locations: false,
    world: true
  };
  ...
  const targeting_mode = hasExactCharTarget || hasExactLocTarget ? 'exact' : characters.length > 0 || locations.length > 0 ? 'derived' : 'heuristic';
...
const SHOT_TYPE_IDENTITY_CONSTRAINTS = {
 ...
 */ function buildIdentityLockMandate(characterName, identitySignatureBlock) {
  const lines = [
...
async function enrichWithCharacterAtoms(sb, projectId, characterNames) {
  ...
  const { data: atoms, error } = await sb.from("atoms").select("canonical_name, attributes").eq("project_id", projectId).eq("atom_type", "character").eq("generation_status", "complete").eq("readiness_state", "generated");
...
async function enrichWithLocationAtoms(sb, projectId, locationNames) {
  ...
  const { data: atoms, error } = await sb.from("atoms").select("canonical_name, attributes").eq("project_id", projectId).eq("atom_type", "location").eq("generation_status", "complete").eq("readiness_state", "generated");
  if (error || !atoms?.length) {
...
async function enrichWithCostumeAtoms(sb, projectId, characterNames) {
  if (!characterNames?.length) return {
    atomBlock: '',
    bound: 0,
    total: 0,
  ...
  const lowerNames = characterNames.map((n)=>n.toLowerCase());
  const { data: atoms, error } = await sb.from("atoms").select("canonical_name, attributes").eq("project_id", projectId).eq("atom_type", "costume").eq("generation_status", "complete").eq("readiness_state", "generated");
  if (error || !atoms?.length) {
...
function buildIdentityPrompt(characterName, shotType, ctx) {
 ...
 */ function filterWorldNarrativeMoment(moment) {
...
function buildPackPrompt(assetGroup, shotType, ctx, variantIndex = 0) {
  const framing = SHOT_FRAMING[shotType];
  // ── Narrative moment selection ──
  let narrativeMoment = ctx.narrativeMoments?.length ? selectNarrativeMoment(ctx.narrativeMoments, shotType, variantIndex, ctx.shotPlanContext) : null;
  ...
  switch(assetGroup){
    case "character":
      subjectDescription = ctx.characterName ? `Character: ${ctx.characterName}. ${ctx.characters || "A compelling screen presence with emotional depth."}` : ctx.characters || "The protagonist — a compelling screen presence with emotional depth.";
    ...
    case "key_moment":
      {
        ...
        if (narrativeMoment?.summary) {
          ...
          const castLine = sceneChars.length ? `Characters who MUST appear: ${sceneChars.join(', ')}. ` : ctx.boundCharacterNames?.length ? `Characters who MUST appear: ${ctx.boundCharacterNames.join(', ')}. ` : '';
          ...
          const castLine = ctx.boundCharacterNames?.length ? `Characters who MUST appear: ${ctx.boundCharacterNames.join(', ')}. ` : '';
          subjectDescription = [
            castLine,
  ...
  if (narrativeMoment && assetGroup !== 'visual_language') {
    if (isWorldSlot) {
      ...
      const worldNarrativeLines = [
        '[LOCATION CONTEXT — FROM SCRIPT]',
        ''
      ];
  ...
  promptParts.push(``, `PHOTOREALISM MANDATE: ${ctx.stylePolicy.styleDirectives}`, ``, `ABSOLUTE PROHIBITIONS:`, `- ${ctx.stylePolicy.negativeStyleConstraints}`, `- No text, titles, watermarks, or typography of any kind`, `- No illustrated or painted look`, `- ${driftExclusions}`, `- Must look indistinguishable from a still frame from a theatrically released film`);
  // ── Environment-specific negative prompt additions ──
  if (isEnvironmentSlot) {
    promptParts.push(`- No pottery wheels, ceramics, or craft processes`, `- No artisan workshop activity`, `- No protagonist performing their trade or occupation`, `- No character-centered composition — the PLACE is the subject`);
  }
  promptParts.push(``, `TECHNICAL: Premium cinematic quality. Anamorphic lens characteristics.`);
  return promptParts.join("\n");
}
function buildSectionPrompt(section, ctx, variantIndex) {
  ...
  return buildPackPrompt(section, shotType, ctx, variantIndex);
}
  ...
  if (choices?.length) {
    ...
    if (Array.isArray(content)) {
      for (const part of content){
        if (part?.type === "image_url" && part?.image_url?.url) return parseDataUrl(part.image_url.url);
...
async function generateImage(apiKey, prompt, model, gatewayUrl, referenceImageUrls, requestedWidth, requestedHeight) {
  const content = [];
 ...
 */ function measureImageDimensions(bytes) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { project_id, section, count = 4, entity_id, character_name, character_names: requestedCharacterNames, asset_group: requestedAssetGroup, pack_mode = false, base_look_mode = false, location_name, location_description, location_ref_mode = false, location_id = null, location_ids: requestedLocationIds, state_key = null, state_label = null, state_prompt_modifier = null, identity_mode = false, identity_anchor_paths = null, identity_notes = null, identity_canon_facts = null, identity_traits_block = null, identity_signature_block = null, forced_shot_type = null, custom_prompt: clientCustomPrompt = null, generation_purpose: clientGenerationPurpose = null, subject: clientSubject = null, // ── AUTO-COMPLETE CONTEXT: requirement-origin metadata from pipeline ──
    auto_complete_context: autoCompleteContext = null, // ── VERTICAL COMPLIANCE: explicit aspect ratio from client ──
    width: requestedWidth = null, height: requestedHeight = null, aspect_ratio: requestedAspectRatio = null, dataset_provenance: clientDatasetProvenance = null, shot_plan_context: shotPlanContextRaw = null } = body;
    ...
    if (!project_id || !section) {
      return new Response(JSON.stringify({
        error: "project_id and section required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    let cinematicStyleLock = null;
    let cinematicStylePromptBlock = '';
    let cinematicStyleHash = '';
    {
      const { data: vsRow } = await supabase.from("project_visual_style").select("style_lock_json").eq("project_id", project_id).maybeSingle();
      if (vsRow?.style_lock_json && typeof vsRow.style_lock_json === 'object') {
        cinematicStyleLock = vsRow.style_lock_json;
    ...
    const SHOT_INTENT_MAP = {
      characters: {
        framing: 'Close-up',
        subject: 'Character dominant',
        angle: 'Eye-level',
        dof: 'Shallow',
        motion: 'Static'
      },
      ...
      visual_language: {
        framing: 'Close-up',
        subject: 'Environment/texture dominant',
        angle: 'Eye-level',
        dof: 'Shallow',
        motion: 'Static'
      },
      themes: {
        framing: 'Wide',
        subject: 'Environment/atmosphere dominant',
        angle: 'Eye-level',
        dof: 'Deep',
        motion: 'Static'
      },
    ...
    const COMPOSITION_RULE_MAP = {
      characters: {
        balance: 'Rule-of-thirds — subject at power points',
        subject_scale: 'Dominant — fills 40-70% of frame',
        density: 'Balanced',
        horizon: 'Mid',
        headroom: 'Tight — minimal space above subject',
        negative_space: 'Low'
      },
      world: {
        balance: 'Environment-weighted — space dominates',
        subject_scale: 'Small in frame — under 20%',
        density: 'Balanced',
        horizon: 'Low — sky/ceiling dominant',
        headroom: 'Airy — generous overhead',
        negative_space: 'High — significant empty space'
      },
      key_moments: {
        balance: 'Rule-of-thirds — dynamic staging',
        subject_scale: 'Balanced — 20-40% of frame',
        density: 'Balanced',
        horizon: 'Mid',
        headroom: 'Balanced',
      ...
      story_engine: {
        balance: 'Rule-of-thirds — relational tension',
      ...
      visual_language: {
        balance: 'Centered — formal, anchored',
        subject_scale: 'Dominant — fills frame',
        density: 'Dense — rich detail, layered textures',
        horizon: 'Mid',
        headroom: 'Tight',
        negative_space: 'Low'
      },
      themes: {
        balance: 'Environment-weighted — atmosphere dominates',
        subject_scale: 'Small in frame',
        density: 'Minimal — clean negative space',
        horizon: 'Low',
        headroom: 'Airy',
        negative_space: 'High'
      },
      cover: {
        balance: 'Centered — iconic, intentional',
      ...
      closing: {
        balance: 'Symmetrical — formal bookend',
        subject_scale: 'Small in frame',
        density: 'Minimal',
        horizon: 'Low',
        headroom: 'Airy',
      ...
      poster_directions: {
        balance: 'Centered — key art iconic',
        subject_scale: 'Dominant',
        density: 'Balanced',
        horizon: 'Mid',
        headroom: 'Tight',
    ...
    if (resolvedCompositionRule) {
      ...
      compositionRuleHash = [
        resolvedCompositionRule.balance,
        resolvedCompositionRule.subject_scale,
        resolvedCompositionRule.density
      ].join('|');
    }
    // ── CAMERA LANGUAGE: Resolve for current section/slide ──
    const CAMERA_LANGUAGE_MAP = {
      characters: {
        style: 'Observational — documentary intimacy',
        movement: 'Static — locked-off, deliberate',
        lens: 'Compressed — telephoto, subject isolation'
      },
      world: {
        style: 'Observational — present but unobtrusive',
        movement: 'Slow push — gradual reveal',
        lens: 'Stable — natural perspective'
      },
      key_moments: {
        style: 'Dynamic — kinetic energy, urgency',
        movement: 'Tracking — following action',
        lens: 'Stable — undistorted'
      },
      story_engine: {
        style: 'Observational — discovering the scene',
        movement: 'Slow push — building intimacy',
        lens: 'Stable — clean perspective'
      },
      visual_language: {
        style: 'Formal — deliberate, composed',
        movement: 'Static — photograph quality',
        lens: 'Stable — clean rendering'
      },
      themes: {
        style: 'Observational — atmosphere discovery',
        movement: 'Slow push — building mood',
        lens: 'Stable — natural'
      },
      cover: {
        style: 'Formal — every element placed with intention',
        movement: 'Static — composed stillness',
        lens: 'Compressed — flattened depth, isolation'
      },
      closing: {
        style: 'Observational — reflective distance',
        movement: 'Slow push — gentle retreat',
        lens: 'Stable — natural perspective'
      },
      poster_directions: {
        style: 'Formal — iconic, deliberate',
        movement: 'Static — composed',
        lens: 'Compressed — heroic isolation'
      }
    };
    ...
    if (resolvedCameraLanguage) {
      ...
      cameraLanguageHash = [
        resolvedCameraLanguage.style.split(' ')[0],
        resolvedCameraLanguage.movement.split(' ')[0],
        resolvedCameraLanguage.lens.split(' ')[0]
      ].join('|');
    }
    // ── Load canon early (needed by production design + world context) ──
    const { data: canon } = await supabase.from("project_canon").select("canon_json").eq("project_id", project_id).maybeSingle();
    // ── PRODUCTION DESIGN: Resolve from canon ──
    const canonData = canon?.canon_json;
    const productionMaterials = [];
    let productionArchitecture = 'naturalistic';
    const productionEnvRules = [];
    if (canonData) {
      ...
      for (const [kws, style] of ARCH_KW){
        if (kws.some((k)=>wdLower.includes(k))) {
          productionArchitecture = style;
          break;
        }
      }
      if (canonData.world_rules && Array.isArray(canonData.world_rules)) {
        productionEnvRules.push(...canonData.world_rules.filter((r)=>typeof r === 'string').slice(0, 4));
      }
    ...
    if (productionMaterials.length > 0 || productionEnvRules.length > 0) {
      ...
      if (productionEnvRules.length > 0) {
        lines.push('ENVIRONMENT RULES:');
        for (const r of productionEnvRules)lines.push(`  - ${r}`);
      }
    ...
    const { data: project } = await supabase.from("projects").select("title, format, genres, tone, default_prestige_style").eq("id", project_id).single();
    ...
    const prestigeComposite = assemblePrestigePrompt(resolvedLane, resolvedStyleKey, styleSource);
    ...
    if (canon?.canon_json) {
      ...
      if (cj.characters && Array.isArray(cj.characters)) {
        characters = cj.characters.map((c)=>typeof c === "string" ? c : `${c.name || "Unknown"} (${c.role || ""})`).slice(0, 4).join(", ");
      }
    }
    ...
    let shotListContextItems = [];
    let shotListId = null;
    {
      const { data: lists } = await supabase.from("shot_lists").select("id").eq("project_id", project_id).order("created_at", {
        ascending: false
      }).limit(1);
      if (lists?.length) {
        shotListId = lists[0].id;
        const { data: items } = await supabase.from("shot_list_items").select("id, order_index, scene_number, scene_heading, shot_type, framing, camera_movement, action, characters_present, location, time_of_day").eq("shot_list_id", shotListId).order("order_index", {
          ascending: true
        }).limit(500);
        if (items?.length) {
          shotListContextItems = items.map((r)=>({
              id: r.id,
              ...
              characters_present: Array.isArray(r.characters_present) ? r.characters_present.filter((c)=>typeof c === 'string') : [],
    ...
    if (shotListContextItems.length > 0) {
      ...
      function scoreShotForSlideServer(item, st, total) {
        ...
        switch(st){
          ...
          case 'poster_directions':
            ...
            if (hasCh) sc += 5;
            if (posRatio < 0.4) sc += 4;
            break;
          case 'world':
            if ([
              'WS',
              'AERIAL'
            ].includes(fr) || sht === 'AERIAL') sc += 10;
            if (item.location) sc += 6;
            if (!hasCh) sc += 4;
            break;
          case 'characters':
            if ([
            ...
            if (item.characters_present.length === 1) sc += 6;
            break;
          case 'key_moments':
          case 'key_moment':
            if (hasCh) sc += 4;
            if (act.includes('fight') || act.includes('confront') || act.includes('reveal')) sc += 8;
            if (mv !== 'STATIC') sc += 3;
            if (posRatio > 0.3 && posRatio < 0.8) sc += 4;
            break;
          case 'story_engine':
            if (item.characters_present.length >= 2) sc += 8;
            if ([
              'OTS',
              '2SHOT'
            ].includes(sht)) sc += 6;
          ...
          case 'visual_language':
            ...
            if (!hasCh) sc += 4;
            if (item.time_of_day) sc += 3;
            break;
          case 'closing':
            if (posRatio > 0.7) sc += 8;
            if (act.includes('end') || act.includes('depart')) sc += 6;
            break;
        }
        return sc;
      }
      const total = shotListContextItems.length;
      const scored = shotListContextItems.map((it)=>({
          it,
          sc: scoreShotForSlideServer(it, slideType, total)
        }));
      ...
      if (top.length > 0) {
        const primary = top[0];
        const lines = [
          '[SHOT LIST CONTEXT — CANONICAL CINEMATIC SOURCE]',
          ''
        ];
    ...
    if (identity_anchor_paths && (identity_anchor_paths.headshot || identity_anchor_paths.fullBody)) {
      ...
      const resolveAnchor = async (path)=>{
    ...
    if (SECTION_BINDING_RELEVANCE[section]?.locations && location_name) {
      const locResult = await enrichWithLocationAtoms(supabase, project_id, [
        location_name
      ]);
    ...
    for(let i = 0; i < genCount; i++){
      ...
      if (clientCustomPrompt) {
        ...
        if (isPDCustomPrompt) {
          console.log(`[PD-route-audit] family=${clientSubject || 'unknown'} slot=${shotType || 'auto'} section=${section} asset_group=${assetGroup} dataset_mode=${body?.dataset_provenance?.dataset_resolution_mode || 'none'} cinematicStyleSkipped=true locationBound=${!!body?.location_id}`);
        }
      } else if (isIdentityGeneration && isIdentityShot && character_name) {
        prompt = buildIdentityPrompt(character_name, shotType, ctx);
        ...
        prompt = shotType ? buildPackPrompt(assetGroup, shotType, ctx, i) : buildSectionPrompt(section, ctx, i);
...
\n${prompt}`;
      ...
      if (shotPlanContextRaw && !isIdentityGeneration && !promptOverrideUsed) {
        const spc = shotPlanContextRaw;
        const lines = [
          '[PIPELINE CONTEXT — ORCHESTRATION METADATA]',
          ''
        ];
      ...
      if (!isIdentityGeneration) {
        ...
        if (canonicalBindings.characters.length > 0) {
          const shotSpecificCharBlock = buildCharacterBindingBlock(canonicalBindings.characters, shotType);
      ...
      try {
        ...
        const { error: uploadErr } = await supabase.storage.from("project-posters").upload(storagePath, imageResult.rawBytes, {
        ...
        const resolvedSubjectType = assetGroup === "character" ? "character" : assetGroup === "world" && location_name ? "location" : assetGroup === "world" ? "world" : assetGroup === "key_moment" ? "moment" : assetGroup === "visual_language" ? "production_design" : null;
        ...
        const { data: imgRecord, error: insertErr } = await supabase.from("project_images").insert({
          ...
          generation_config: {
            ...
            requested_location_ids: requestedLocationIds || (location_id ? [
              location_id
            ] : []),
            ...
            shot_list_framing: shotListContextItems.length > 0 && shotListMappedItemIds.length > 0 ? shotListContextItems.find((i)=>i.id === shotListMappedItemIds[0])?.framing || null : null,
            shot_list_camera_movement: shotListContextItems.length > 0 && shotListMappedItemIds.length > 0 ? shotListContextItems.find((i)=>i.id === shotListMappedItemIds[0])?.camera_movement || null : null,
            shot_list_location: shotListContextItems.length > 0 && shotListMappedItemIds.length > 0 ? shotListContextItems.find((i)=>i.id === shotListMappedItemIds[0])?.location || null : null,
            shot_list_time_of_day: shotListContextItems.length > 0 && shotListMappedItemIds.length > 0 ? shotListContextItems.find((i)=>i.id === shotListMappedItemIds[0])?.time_of_day || null : null,
            ...
            ...clientDatasetProvenance ? {
              dataset_provenance: clientDatasetProvenance
            } : {},
          ...
          subject_type: assetGroup === "character" ? "character" : assetGroup === "world" && location_name ? "location" : assetGroup === "world" ? "world" : assetGroup === "key_moment" ? "moment" : assetGroup === "visual_language" ? "production_design" : null,
          subject_ref: character_name || location_name || null,
          location_ref: location_name || null,
          canon_location_id: location_id || null,
          generation_purpose: clientGenerationPurpose ? clientGenerationPurpose : isIdentityGeneration ? "character_identity" : state_key ? `state_variant_${state_key}` : base_look_mode ? "character_reference" : location_ref_mode ? "location_reference" : `lookbook_${section}`,
          state_key: state_key || null,
