...
async function resolveVisualStyleProfile(sb, projectId) {
  ...
  try {
    ...
    if (!data?.style_profile_json) return {
      promptBlock: null
    };
    const profile = data.style_profile_json;
    const parts = [];
    for (const [k, v] of Object.entries(profile)){
      if (v && typeof v === "string") parts.push(`${k}: ${v}`);
    }
    return {
      promptBlock: parts.length ? parts.join("\n") : null
    };
  } catch  {
    return {
      promptBlock: null
...
async function resolveCharacterTruth(sb, projectId) {
  ...
  for (const b of castBindings || []){
    if (b.ai_actor_version_id) {
      bindMap.set(b.character_key, {
        actorId: b.ai_actor_id,
        versionId: b.ai_actor_version_id
      });
    }
  }
  const actorIds = [
  ...
  let anchorCountMap = new Map();
  let refUrlMap = new Map();
  if (actorIds.length > 0) {
    const { data: actors } = await sb.from("ai_actors").select("id, name").in("id", actorIds);
    for (const a of actors || [])actorNameMap.set(a.id, a.name);
  }
  if (versionIds.length > 0) {
    const { data: assets } = await sb.from("ai_actor_assets").select("actor_version_id, asset_type, public_url").in("actor_version_id", versionIds).in("asset_type", [
      "reference_image",
      "reference_headshot",
      "reference_full_body"
    ]);
  ...
  const unboundNames = characterNames.filter((n)=>!bindMap.has(n));
  const identityAnchorMap = new Map();
  if (unboundNames.length > 0) {
    ...
    const originalNames = unboundNames.map((n)=>nameToOriginal.get(n) || n);
    const { data: identityImages } = await sb.from("project_images").select("subject, shot_type, storage_path, storage_bucket").eq("project_id", projectId).eq("generation_purpose", "character_identity").eq("is_primary", true).eq("curation_state", "active").in("shot_type", [
      "identity_headshot",
      "identity_full_body"
    ...
    if (identityImages?.length) {
      for (const img of identityImages){
        ...
        const { data: signedData } = await sb.storage.from(bucket).createSignedUrl(img.storage_path, 3600);
...
async function resolveLocationDataset(sb, projectId, locationKey) {
  // Normalize key for matching
  const normKey = locationKey.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '');
  // Try canon_locations first
  const { data: canonLoc } = await sb.from("canon_locations").select("id, canonical_name, description, geography, interior_or_exterior, location_type").eq("project_id", projectId).eq("normalized_name", normKey).eq("active", true).maybeSingle();
  if (!canonLoc) {
    // Fuzzy match: try partial name match
    const { data: fuzzyLoc } = await sb.from("canon_locations").select("id, canonical_name").eq("project_id", projectId).eq("active", true).ilike("normalized_name", `%${normKey}%`).limit(1).maybeSingle();
...
async function resolveLocationDatasetById(sb, projectId, canonLocationId, locationName, canonLoc) {
  const { data: dataset } = await sb.from("location_visual_datasets").select("*").eq("project_id", projectId).eq("canon_location_id", canonLocationId).eq("is_current", true).maybeSingle();
  const extractJsonLabel = (obj)=>{
    if (!obj || typeof obj !== 'object') return '';
    const parts = [];
    for (const [k, v] of Object.entries(obj)){
      if (typeof v === 'string' && v.trim()) parts.push(`${k}: ${v}`);
      else if (Array.isArray(v)) parts.push(`${k}: ${v.filter((s)=>typeof s === 'string').join(', ')}`);
    }
    return parts.join('. ');
  };
  ...
  const promptParts = [
    `[LOCATION — PRODUCTION DESIGN TRUTH]`,
...
function resolveWardrobePromptBlocks(wardrobeStateMap, canonJson) {
  ...
  const temporalTruth = resolveTemporalTruthFromCanon(canonJson);
  const results = [];
  for (const [charKey, stateKey] of Object.entries(wardrobeStateMap)){
    ...
    if (profile) {
      ...
      const stateMatch = charStates.find((s)=>s.state_key === stateKey);
      const sceneExplicitGarments = stateMatch?.explicit_or_inferred === 'explicit' ? stateMatch.garment_adjustments || [] : [];
      ...
      if (stateMatch) {
        const filteredGarmentAdj = (stateMatch.garment_adjustments || []).filter((g)=>{
          const lower = g.toLowerCase().replace(/[·.\-–—()\[\]]/g, '').replace(/scene$/, '').trim();
          return !temporalTruth || !temporalTruth.forbidden_garment_families.some((f)=>f.toLowerCase() === lower);
        });
...
async function loadSceneIndex(sb, projectId) {
  const { data, error } = await sb.from("scene_index").select("scene_number, title, location_key, character_keys, wardrobe_state_map").eq("project_id", projectId).order("scene_number");
  if (error) {
  ...
  return (data || []).map((row)=>({
      ...
      wardrobe_state_map: row.wardrobe_state_map && typeof row.wardrobe_state_map === 'object' ? row.wardrobe_state_map : null
    }));
}
// ── Scene-Level Narrative Classification ─────────────────────────────────────
function classifyNarrativeFunction(summary, characters, index, total) {
...
function scoreDramaticIntensity(summary, characters, content) {
  let score = 20;
...
async function loadSceneBoundMoments(sb, projectId, canonJson, targetCount = 13) {
  ...
  const { data: scenes } = await sb.from("scene_graph_scenes").select("id, scene_key").eq("project_id", projectId).is("deprecated_at", null).limit(200);
  const sceneContentMap = new Map();
  if (scenes?.length) {
    const { data: versions } = await sb.from("scene_graph_versions").select("scene_id, slugline, summary, content, time_of_day, characters_present, version_number").eq("project_id", projectId).in("scene_id", scenes.map((s)=>s.id)).order("version_number", {
      ascending: false
    });
    if (versions?.length) {
      const seen = new Set();
      for (const v of versions){
        ...
        seen.add(v.scene_id);
        const sceneRow = scenes.find((s)=>s.id === v.scene_id);
        if (sceneRow) {
  ...
  const uniqueLocationKeys = [
    ...new Set(sceneEntries.map((e)=>e.location_key).filter(Boolean))
  ];
  const locationDatasetMap = new Map();
...
function assessHeroWorthiness(moment, characters, canonJson) {
  const reasons = [];
  let score = 0;
...
function buildHeroFramePrompt(projectTitle, projectLogline, canonJson, characters, worldBlock, styleBlock, moment, cpieCanonBlock) {
  ...
  const momentCharNorm = moment.characterKeys.map((c)=>c.toLowerCase().trim().replace(/\s+/g, " "));
  const relevantCharacters = momentCharNorm.length > 0 ? characters.filter((c)=>momentCharNorm.includes(c.name.toLowerCase().trim().replace(/\s+/g, " "))) : [];
  ...
  try {
    ...
    for(let i = 0; i < effectiveCount; i++){
      ...
      if (target_narrative_function) {
        moment.narrativeFunction = target_narrative_function;
      }
      ...
      try {
        ...
        const { error: uploadErr } = await supabase.storage.from("project-posters").upload(storagePath, new Blob([
          rawBytes
        ], {
          type: `image/${format}`
        }), {
        ...
        const gateResult = computeEdgeQualityGate({
          ...
          generation_config: {
            ...
            identity_source: allRefUrls.length > 0 ? anchorInjectionLog.some((a)=>a.source === 'actor_assets') ? 'actor_assets' : 'identity_anchors' : 'none',
