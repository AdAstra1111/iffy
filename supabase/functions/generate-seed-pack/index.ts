...
import { upsertDoc, SEED_CORE_TYPES, createVersion, ensureDocSlot } from "../_shared/doc-os.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (commitOnly) {
      ...
      try {
        const newVer = await createVersion(adminClient, {
          documentId,
          docType: necCfg.doc_type,
