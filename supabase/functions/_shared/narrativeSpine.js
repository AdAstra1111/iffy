...
export function isCompleteSpine(s) {
...
export async function getSpineState(supabase, projectId) {
    ...
    if (!spine)
        return { state: 'none', spine: null, entryId: null };
    ...
    if (pendingEntry && !activeEntry)
        return { state: 'confirmed', spine, entryId: pendingEntry.id };
    if (activeEntry && supersededEntries.length > 0)
        return { state: 'locked_amended', spine, entryId: activeEntry.id };
    if (activeEntry)
        return { state: 'locked', spine, entryId: activeEntry.id };
    return { state: 'provisional', spine, entryId: null };
}
// ── Amendment Helpers ──
/** Returns constitutional severity for a given axis */
export function getAxisSeverity(axis) {
...
export function getRevalidationFloorIndex(axis, ladder) {
    ...
    if (floor === 'next_unapproved')
        return -1; // caller resolves at amendment time
...
export function spineToPromptBlock(spine, state = 'locked') {
    ...
    if (spine.story_engine)
        lines.push(`- Story Engine: ${spine.story_engine}`);
    if (spine.pressure_system)
        lines.push(`- Pressure System: ${spine.pressure_system}`);
    if (spine.central_conflict)
        lines.push(`- Central Conflict: ${spine.central_conflict}`);
    if (spine.inciting_incident)
        lines.push(`- Inciting Incident: ${spine.inciting_incident}`);
    if (spine.resolution_type)
        lines.push(`- Resolution Type: ${spine.resolution_type}`);
    if (spine.stakes_class)
        lines.push(`- Stakes: ${spine.stakes_class}`);
    if (spine.protagonist_arc)
        lines.push(`- Protagonist Arc: ${spine.protagonist_arc}`);
    if (spine.midpoint_reversal)
        lines.push(`- Midpoint Reversal: ${spine.midpoint_reversal}`);
    if (spine.tonal_gravity)
        lines.push(`- Tonal Gravity: ${spine.tonal_gravity}`);
    if (lines.length === 0)
        return '';
...
export function parseClassASpineCheckOutput(parsed) {
    ...
    for (const c of parsed.checks) {
        if (!validAxes.has(c.axis) || !validStatuses.has(c.status))
            continue;
...
export function buildClassBSpineCheckUserPrompt(spine, docType, documentText, projectTitle, lane) {
    const spineLines = [];
    if (spine.pressure_system)
        spineLines.push(`- Pressure System: "${spine.pressure_system}"`);
    if (spine.central_conflict)
        spineLines.push(`- Central Conflict: "${spine.central_conflict}"`);
    if (spine.resolution_type)
        spineLines.push(`- Resolution Type: "${spine.resolution_type}"`);
    if (spine.stakes_class)
        spineLines.push(`- Stakes Class: "${spine.stakes_class}"`);
...
export function parseClassBSpineCheckOutput(parsed) {
    if (!parsed || !Array.isArray(parsed.checks))
        return null;
    const validAxes = new Set(['pressure_system', 'central_conflict', 'resolution_type', 'stakes_class']);
    const validStatuses = new Set(['aligned', 'contradicted', 'unclear']);
    const checks = [];
    for (const c of parsed.checks) {
        if (!validAxes.has(c.axis) || !validStatuses.has(c.status))
