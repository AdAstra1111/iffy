...
export function resolveGateway() {
    ...
    if (openrouterKey)
        return { url: GATEWAY_URL_OPENROUTER, apiKey: openrouterKey };
    const openaiKey = Deno.env.get("OPENAI_API_KEY");
    if (openaiKey)
        return { url: GATEWAY_URL_OPENAI, apiKey: openaiKey };
 ...
 */
export async function parseJsonSafe(raw, apiKey) {
    try {
        return JSON.parse(extractJSON(raw));
    }
    catch {
        if (!apiKey)
            throw new Error("Unparseable JSON and no apiKey for repair");
...
function logParseTelemetry(raw, handler, model, phase, repairAttempted, retryTriggered) {
...
function makeParseError(raw, handler, model, phase, detail) {
    const e = new Error(`AI_JSON_PARSE_ERROR [${handler}]: ${detail}`);
 ...
 */
export function parseAiJson(raw, opts) {
 ...
 */
export async function callLLMWithJsonRetry(llmOpts, parseOpts) {
    const { handler, validate } = parseOpts;
    const model = llmOpts.model;
    // Attempt 1
    const r1 = await callLLM(llmOpts);
    try {
        return parseAiJson(r1.content, { handler, model, validate });
    }
    catch {
    ...
    return parseAiJson(r2.content, { handler, model, validate });
}
/**
 ...
 */
export async function callLLMChunked(opts) {
    ...
    const allItems = [];
    const seenKeys = dedupeMode ? new Map() : null;
    for (let i = 0; i < batches.length; i++) {
        ...
        const result = await callLLMWithJsonRetry({ ...llmOpts, user: userPrompt }, { handler: `${handler}_batch_${i + 1}`, validate });
        const extracted = extractItems(result);
        if (onBatchResult)
            onBatchResult(i, extracted);
        if (getKey && seenKeys) {
