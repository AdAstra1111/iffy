...
function resolveGuardrailsConfig(project) {
    if (!project)
        return null;
...
// ─── Resolve engine mode ───
function resolveEngineMode(pt, engineName, explicitMode, config) {
    ...
    if (config?.overrides?.engineMode)
        return config.overrides.engineMode;
 ...
 */
export function buildGuardrailBlock(input) {
 ...
 */
export function validateOutput(text, policy) {
    const violations = [];
 ...
 */
export function buildRegenerationPrompt(violations) {
 ...
 */
export async function getCorpusCalibration(db, productionType, genre) {
    try {
        ...
        if (goldData?.length) {
            const match = goldData.find((d) => {
                const cpt = (d.production_type || "").toLowerCase();
                return cpt === pt || pt.includes(cpt) || cpt.includes(pt);
            }) || goldData.find((d) => d.production_type === "all");
            if (match?.pattern)
                return match.pattern;
        }
    }
