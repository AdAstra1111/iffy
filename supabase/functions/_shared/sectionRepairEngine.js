 ...
 */
import { getSectionConfig, findSectionDef, } from "./deliverableSectionRegistry.ts";
// ── Section Parsing ──
/**
 ...
 */
export function parseSections(content, docType) {
 ...
 */
export function resolveIssueToSectionKey(issue, docType, content) {
    const config = getSectionConfig(docType);
 ...
 */
export function extractSection(content, docType, sectionKey) {
    const parsed = parseSections(content, docType);
 ...
 */
export function replaceSection(content, docType, sectionKey, newSectionContent) {
    const parsed = parseSections(content, docType);
    const config = getSectionConfig(docType);
    if (!config) {
        return {
 ...
 */
export function getRepairTarget(issue, docType, content) {
    const config = getSectionConfig(docType);
 ...
 */
function buildCategoryMap(docType) {
    switch (docType) {
 ...
 */
export function findVerbatimInSections(sections, verbatimQuote) {
    const NONE = { verified: false, section_key: null, line_start: null, line_end: null, match_method: 'none' };
    if (!verbatimQuote || verbatimQuote.trim().length < 5)
        return NONE;
    ...
    for (const boundary of sections) {
        ...
        if (!normSection.includes(normQuote))
            continue;
        ...
        for (let i = 0; i < sectionLines.length; i++) {
            if (!normalise(sectionLines[i]).includes(quotePrefix.slice(0, 10)))
                continue;
 ...
 */
function buildSectionKeywords(sec) {
    const keywords = [];
    // From section_key
