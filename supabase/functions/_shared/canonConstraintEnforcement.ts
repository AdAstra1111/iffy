...
/** Profession/role extraction patterns for generated text analysis */ const PROFESSION_PATTERNS = [
...
/** Relationship type keywords for semantic comparison */ const RELATIONSHIP_TYPES = {
  parent: [
    "father",
    "mother",
    "dad",
...
/** Tone register keywords */ const TONE_REGISTERS = {
  dark: [
    "dark",
    "bleak",
    "grim",
...
function extractSentencesAround(text, keyword, windowChars = 400) {
...
/** Extract what role/profession the text asserts for a named character */ function extractAssertedProfessions(text, charName) {
...
/** Classify the relationship type from free text */ function classifyRelationshipType(relText) {
...
/** Check if two relationship type sets are contradictory */ function areRelationshipsContradictory(canonTypes, observedTypes) {
  if (canonTypes.length === 0 || observedTypes.length === 0) return {
    contradicts: false,
    reason: ""
  };
...
/** Extract the dominant supernatural classification from generated text */ function classifyGeneratedSupernatural(text) {
  const lower = text.toLowerCase();
  ...
  if (confirmations.length > 0 && !hasAmbiguity) return {
    level: "present",
    keywords: found,
    confirmations
  };
  if (found.length >= 3 && !hasAmbiguity) return {
    level: "present",
    keywords: found,
    confirmations
  };
  if (found.length >= 1 || confirmations.length > 0) return {
    level: "ambiguous",
    keywords: found,
    confirmations
  };
...
/** Classify the dominant tone register of a text */ function classifyToneRegister(text) {
  const lower = text.toLowerCase();
  const scores = [];
...
/** Check if observed tone contradicts canonical tone */ function isToneContradiction(canonTone, observedRegisters) {
  ...
  for (const [canonGroup, conflictGroup] of toneContradictions){
    ...
    if (canonMatch && observedMatch) {
      return {
        contradicts: true,
        reason: `Canon tone "${canonTone}" conflicts with observed registers: ${observedRegisters.join(", ")}`
      };
    }
  }
  return {
...
/** Check if a canonical fact is contradicted in generated text */ function checkFactContradiction(generatedText, canonFact) {
...
/** Check if a specific fact from forbidden_changes is violated */ function checkForbiddenChangeViolation(text, forbiddenFact) {
 ...
 */ export function extractCanonConstraints(canonJson) {
  ...
  for (const char of characters){
    ...
    if (role && charName !== protagonistName) {
      ...
      const relIndicators = [
        {
          pattern: /love interest/i,
          relation: `love interest / romantic partner of ${protagonistName}`
        },
        {
          pattern: /handler/i,
          relation: `handler / supervisor of ${protagonistName}`
        },
        {
          pattern: /mentor/i,
          relation: `mentor to ${protagonistName}`
        },
        {
          pattern: /(?:mother|father|parent|dad|mom)\b/i,
          relation: `parent of ${protagonistName}`
        },
        {
          pattern: /(?:brother|sister|sibling)\b/i,
          relation: `sibling of ${protagonistName}`
        },
        {
          pattern: /(?:husband|wife|spouse)\b/i,
          relation: `spouse of ${protagonistName}`
        },
        {
          pattern: /(?:friend|companion|ally)\b/i,
          relation: `friend / ally of ${protagonistName}`
        },
        {
          pattern: /(?:rival|enemy|nemesis|antagonist|adversary)\b/i,
          relation: `antagonist / rival of ${protagonistName}`
        }
      ];
  ...
  if (Array.isArray(canonJson.world_rules)) {
    worldRules.push(...canonJson.world_rules.filter((r)=>typeof r === "string"));
  } else if (typeof canonJson.world_rules === "string" && canonJson.world_rules.trim()) {
    worldRules.push(canonJson.world_rules);
  }
  const supernaturalMode = classifySupernatural(worldRules, canonJson);
  // Tone
  const toneClass = canonJson.tone || canonJson.tone_style || null;
  // Core incident facts from premise/logline
  const coreIncidentFacts = [];
  if (canonJson.premise && typeof canonJson.premise === "string") {
    coreIncidentFacts.push(canonJson.premise);
  }
  if (canonJson.logline && typeof canonJson.logline === "string") {
    coreIncidentFacts.push(canonJson.logline);
  }
  // Forbidden changes
  const forbiddenChanges = [];
  if (Array.isArray(canonJson.forbidden_changes)) {
    forbiddenChanges.push(...canonJson.forbidden_changes.filter((f)=>typeof f === "string"));
  } else if (typeof canonJson.forbidden_changes === "string" && canonJson.forbidden_changes.trim()) {
    forbiddenChanges.push(canonJson.forbidden_changes);
  }
  const canonicalCharacterNames = characters.map((c)=>c.name).filter((n)=>typeof n === "string" && n.trim());
  const hasContent = !!(protagonist.name || relationships.length > 0 || worldRules.length > 0 || toneClass || coreIncidentFacts.length > 0 || canonicalCharacterNames.length > 0);
  return {
    protagonist,
    relationships,
    worldRuleMode: {
      supernatural: supernaturalMode,
      rules: worldRules
    },
    toneClass,
    coreIncidentFacts,
    ...
    logline: canonJson.logline || null,
    premise: canonJson.premise || null,
...
/** Extract profession from role field like "Lead Protagonist, Courtesan & Spy" */ function extractProfessionFromRole(role) {
  if (!role) return null;
...
/** Extract profession from description like "A young courtesan trained..." */ function extractProfessionFromDescription(desc) {
  if (!desc) return null;
...
function classifySupernatural(worldRules, canonJson) {
  const allText = [
    ...worldRules,
    canonJson.premise || "",
    canonJson.logline || "",
 ...
 */ export function buildCanonConstraintBlock(constraints) {
  ...
  if (constraints.protagonist.name) {
    const parts = [
      `Protagonist name: ${constraints.protagonist.name}`
    ];
 ...
 */ export function detectCanonDrift(generatedText, constraints) {
  if (constraints.extractedFrom === "empty") {
    return {
      passed: true,
      findings: [],
      ...
      if (constraints.protagonist.profession) {
        ...
        if (assertedProfs.length > 0) {
          ...
          const matchesCanon = assertedProfs.some((p)=>{
            ...
            return p.includes(canonProf) || canonProf.includes(p) || canonProf.split(/\s+/).some((w)=>w.length > 3 && p.includes(w));
          });
          if (!matchesCanon) {
            findings.push({
              domain: "identity",
 ...
 */ export function logDriftResult(tag, projectId, docType, result) {
  const logEntry = {
