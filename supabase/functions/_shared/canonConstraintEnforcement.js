...
function extractAssertedProfessions(text, charName) {
...
function classifyRelationshipType(relText) {
...
function areRelationshipsContradictory(canonTypes, observedTypes) {
    ...
    for (const ct of canonTypes) {
        for (const ot of observedTypes) {
            if (ct === ot)
                continue; // Same type, no contradiction
...
function classifyGeneratedSupernatural(text) {
    const lower = text.toLowerCase();
    ...
    for (const pat of confirmPatterns) {
        ...
        if (m)
            confirmations.push(m[0]);
    }
    const hasAmbiguity = ambiguousPatterns.some(p => p.test(text));
    if (confirmations.length > 0 && !hasAmbiguity)
        return { level: "present", keywords: found, confirmations };
    if (found.length >= 3 && !hasAmbiguity)
        return { level: "present", keywords: found, confirmations };
    if (found.length >= 1 || confirmations.length > 0)
        return { level: "ambiguous", keywords: found, confirmations };
    return { level: "none", keywords: found, confirmations };
}
/** Classify the dominant tone register of a text */
function classifyToneRegister(text) {
    const lower = text.toLowerCase();
    const scores = [];
...
function checkFactContradiction(generatedText, canonFact) {
...
function checkForbiddenChangeViolation(text, forbiddenFact) {
...
export function extractCanonConstraints(canonJson) {
    ...
    const hasContent = !!(protagonist.name || relationships.length > 0 || worldRules.length > 0 ||
...
function findProtagonist(characters) {
    const result = { name: null, role: null, profession: null, background: null };
    for (const char of characters) {
        const c = char;
...
function extractProfessionFromRole(role) {
    if (!role)
        return null;
...
function extractProfessionFromDescription(desc) {
    if (!desc)
        return null;
...
function classifySupernatural(worldRules, canonJson) {
    ...
    if (!allText.trim())
        return "unknown";
    if (allText.includes("no supernatural") || allText.includes("grounded realism") || allText.includes("purely realistic"))
        return "none";
    if (allText.includes("ambiguous supernatural") || allText.includes("magical realism") || allText.includes("may or may not be real"))
        return "ambiguous";
...
export function buildCanonConstraintBlock(constraints) {
    ...
    if (constraints.protagonist.name) {
        ...
        if (constraints.protagonist.role)
            parts.push(`Role: ${constraints.protagonist.role}`);
        if (constraints.protagonist.profession)
            parts.push(`Profession: ${constraints.protagonist.profession}`);
        if (constraints.protagonist.background)
            parts.push(`Background: ${constraints.protagonist.background}`);
