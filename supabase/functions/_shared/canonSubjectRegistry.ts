 ...
 */ export function getSubjectClassesForSourceDoc(docType) {
  return Object.values(SUBJECT_CLASS_CONFIGS).filter((c)=>c.active_in_initial_rollout && c.source_doc_types.includes(docType)).map((c)=>c.subject_class);
}
// ── Subject Extraction ──
/**
    ...
    if (Array.isArray(canonJson.characters)) {
      for (const char of canonJson.characters){
        ...
        for (const f of [
          ...
          if (char[f]) valParts.push(`${f}:${String(char[f]).trim()}`);
        }
        subjects.push({
 ...
 */ export function diffCanonicalSubjects(previous, current) {
  const deltas = [];
  const prevMap = new Map(previous.map((s)=>[
      s.subject_id,
      s
    ]));
  const currMap = new Map(current.map((s)=>[
      s.subject_id,
      s
    ]));
  // Added or changed
 ...
 */ export function getSubjectProjectionTargets(deltas, lane, sourceDocType) {
  ...
  const affected = [
    ...reachableDocs
  ].filter((dt)=>projectionSet.has(dt));
  const unaffected = [
    ...reachableDocs
  ].filter((dt)=>!projectionSet.has(dt));
  return {
 ...
 */ export function buildSubjectPropagationPlan(previousCanon, currentCanon, sourceDocType, lane) {
  ...
  const narrowingRatio = totalReachable > 0 ? 1 - Object.keys(affectedProjections).length / totalReachable : 0;
  return {
