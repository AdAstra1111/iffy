 ...
 */
export async function extractEntityMentionsForVersion(supabase, projectId, documentId, versionId, docType, 
...
plaintextOverride) {
    ...
    for (const section of sections) {
        ...
        if (secStart > secEnd || secStart < 0 || secStart >= lines.length)
            continue;
...
export async function extractEntityMentionsForProject(supabase, projectId) {
    ...
    const docTypeMap = new Map(docs.map(d => [d.id, d.doc_type]));
    ...
    for (const v of versions) {
        ...
        if (!effectiveDocType || !isSectionRepairSupported(effectiveDocType))
            continue;
...
export async function syncSceneEntityLinksForProject(supabase, projectId) {
    ...
    const sceneKeyMap = new Map(scenes.map((s) => [s.id, s.scene_key]));
...
export function extractDialogueHeadings(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (SLUGLINE_RE.test(line) || TRANSITION_RE.test(line))
            continue;
...
export async function syncDialogueCharactersForProject(supabase, projectId) {
    ...
    if (!linkFetchErr && allLinks) {
        ...
        for (const row of allLinks) {
            ...
            if (!name)
                continue;
            if (!linksByScene.has(row.scene_id))
                linksByScene.set(row.scene_id, []);
