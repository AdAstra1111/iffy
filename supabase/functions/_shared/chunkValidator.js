...
export function hasScreenplayFormat(content, docType) {
    if (!PROSE_ONLY_DOC_TYPES.has(docType))
        return false;
...
function extractEpisodeNumbers(text) {
    const patterns = [
...
function detectCollapsedRanges(text) {
    const hits = [];
...
function findSectionHeadings(content) {
    const found = new Set();
 ...
 */
export function validateEpisodicContent(content, expectedCount, docType = "episode_grid") {
    const failures = [];
    const bannedHits = [];
 ...
 */
export function validateEpisodicChunk(chunkContent, expectedEpisodes, docType = "episode_grid") {
    const failures = [];
    const bannedHits = [];
 ...
 */
export function validateSectionedContent(content, expectedSections, docType) {
    const failures = [];
    const bannedHits = [];
    ...
    for (const expectedSection of expectedSections) {
        ...
        const found = [...foundHeadings].some(h => h.includes(normalized) || normalized.includes(h) || h === normalized);
 ...
 */
export function hasBannedSummarizationLanguage(content) {
 ...
 */
export function validateEpisodicProgress(content, expectedCount, docType = "episode_grid", options = {}) {
    const blockers = [];
    const warnings = [];
    // 1. Extract episode numbers present
    const foundEpisodes = extractEpisodeNumbers(content);
