 ...
 */
export async function resolveStructuralLineage(supabase, projectId) {
    const empty = {
            ...
            if (dnaId) {
                ...
                try {
                    const { data: dna } = await supabase
                        ...
                        .eq("id", dnaId)
                        .maybeSingle();
                    if (dna?.thematic_spine)
                        result.dna_thematic_spine = dna.thematic_spine;
                }
    ...
    if (proj.source_dna_profile_id) {
        try {
            ...
            if (dna) {
                ...
                if (dna.primary_engine_key)
                    result.engine_key = dna.primary_engine_key;
...
function buildBlueprintBlock(l) {
    const ep = l.execution_pattern || {};
    const lines = [
        `\n=== STRUCTURAL EXECUTION PATTERN (AUTHORITATIVE — from Blueprint Family: ${l.blueprint_family_key}) ===`,
    ];
    if (l.structural_summary)
        lines.push(`STRUCTURAL SUMMARY: ${l.structural_summary}`);
    if (l.engine_key)
        lines.push(`Narrative Engine: ${l.engine_key}`);
    if (l.dna_thematic_spine)
        lines.push(`Thematic Spine: ${l.dna_thematic_spine}`);
    ...
    for (const key of patternKeys) {
        if (ep[key])
            lines.push(`${key}: ${typeof ep[key] === "string" ? ep[key] : JSON.stringify(ep[key])}`);
    }
...
function buildEngineBlock(l) {
    const lines = [
        `\n=== STRUCTURAL GUIDANCE (from Narrative Engine: ${l.engine_key}) ===`,
    ];
    if (l.engine_label)
        lines.push(`Engine: ${l.engine_label}`);
    if (l.engine_structural_pattern)
        lines.push(`Structural Pattern: ${l.engine_structural_pattern}`);
    if (l.dna_thematic_spine)
        lines.push(`Thematic Spine: ${l.dna_thematic_spine}`);
...
function buildDnaBlock(l) {
    ...
    if (l.dna_thematic_spine)
        lines.push(`Thematic Spine: ${l.dna_thematic_spine}`);
    if (l.engine_key)
        lines.push(`Derived Engine: ${l.engine_key}`);
