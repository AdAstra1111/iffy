...
export const CP_FIELD_DESCRIPTIONS = {
 ...
 */ export function buildCPRepairDirections(blockers, highImpactNotes) {
  ...
  if (epMentions.size > 0) {
    directions.push(`Episodes requiring attention: ${[
      ...epMentions
    ].join(", ")}`);
  }
  // Generic CP strengthening if no specific field mentions found
  if (directions.length <= 5) {
    directions.push("Ensure ALL episodes have complete CP fields: pressure_source, internal_dilemma, relationship_shift, micro_transformation, cliffhanger_cause.", "Ensure every episode has an EPISODE FUNCTION statement.", "Make cliffhanger_cause derive from a CHARACTER CHOICE, not coincidence.");
  }
  return {
    directions,
    failClosed: false
 ...
 */ export function logCPM(tag, data) {
