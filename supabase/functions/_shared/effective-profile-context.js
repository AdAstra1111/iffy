 ...
 */
export function buildEffectiveProfileContextBlock(input) {
    ...
    // Top 6 comparables
    const topComps = sorted.filter((c) => c.title).slice(0, 6);
    if (topComps.length > 0) {
        const lines = topComps.map((c, i) => {
    ...
    if (toneBand)
        voiceParts.push(`tone=${toneBand}`);
    if (pacing)
        voiceParts.push(`pacing=${pacing}`);
    if (packTone.dialogue_density)
        voiceParts.push(`dialogue_density=${packTone.dialogue_density}`);
    if (packTone.humor_darkness)
        voiceParts.push(`humor=${packTone.humor_darkness}`);
    if (voiceParts.length > 0)
        parts.push(`Voice Profile: ${voiceParts.join(", ")}`);
    // Constraints
    const pc = pack?.constraints_suggestions || {};
    const cParts = [];
    if (project?.budget_range || pc.budget_band)
        cParts.push(`budget=${project?.budget_range || pc.budget_band}`);
    if (pc.runtime_band)
        cParts.push(`runtime=${pc.runtime_band}`);
    if (pc.rating)
        cParts.push(`rating=${pc.rating}`);
    if (cParts.length > 0)
    ...
    if (parts.length <= 1)
        return ""; // Only market profile — not worth injecting
