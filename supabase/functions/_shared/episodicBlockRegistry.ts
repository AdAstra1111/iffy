 ...
 */ export function resolveIssueToEpisodeNumber(issue, docType, content) {
  ...
  const matches = [
    ...searchText.matchAll(epPattern)
  ];
  const extractedNums = [
    ...new Set(matches.map((m)=>parseInt(m[1], 10)))
  ].filter((n)=>!isNaN(n) && episodeSet.has(n));
 ...
 */ export function replaceEpisodeBlock(content, episodeNumber, newBlockContent) {
  ...
  if (!blocks.find((b)=>b.episodeNumber === episodeNumber)) {
    return {
      success: false,
      new_content: content,
      reason: `episode_${episodeNumber}_not_found`
    };
  }
  const result = mergeEpisodeBlocks(content, {
    [episodeNumber]: newBlockContent
  });
  return {
    success: true,
 ...
 */ export function enforceEpisodeBlockIntegrity(originalContent, rewrittenContent, targetEpisodeNumber) {
  ...
  // Validate the merge
  const validation = validateEpisodeMerge(originalContent, mergeResult.mergedText, [
    targetEpisodeNumber
  ]);
  return {
    ok: validation.ok,
    ...
    reason: validation.ok ? `integrity_ok:replaced_ep_${targetEpisodeNumber}:preserved=${mergeResult.preservedEpisodes.length}` : `integrity_issues:${validation.errors.join("; ")}`
  };
}
