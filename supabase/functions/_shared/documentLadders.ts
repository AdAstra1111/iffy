...
export const BASE_DOC_TYPES = {
  idea: {
    label: 'Idea / Logline',
    description: 'Initial concept or logline'
  },
  concept_brief: {
    label: 'Concept Brief',
    description: 'One-pager expanding the idea'
  },
  market_sheet: {
    label: 'Market Sheet',
  ...
  vertical_market_sheet: {
    label: 'Vertical Market Sheet',
    description: 'Market sheet for vertical formats'
  },
  treatment: {
    label: 'Treatment',
    description: 'Treatment / series bible / structural overview'
  },
  story_outline: {
    label: 'Story Outline',
    description: 'Plot outline / structural architecture'
  },
  character_bible: {
    label: 'Character Bible',
    description: 'Character profiles and arcs'
  },
  beat_sheet: {
    label: 'Beat Sheet',
    description: 'Scene-by-scene beat structure'
  },
  episode_beats: {
    label: 'Episode Beats',
    description: 'Beat sheets for series episodes'
  },
  feature_script: {
    label: 'Feature Script',
    description: 'Full screenplay for film / feature'
  },
  episode_script: {
    label: 'Episode Script',
    description: 'Script for a single episode'
  },
  season_script: {
    label: 'Season Script',
    description: 'Full-season continuous script (vertical drama / feature)'
  },
  season_master_script: {
    label: 'Season Master Script',
    description: 'Compiled season scripts'
  },
  production_draft: {
    label: 'Production Draft',
    description: 'Final production-ready draft'
  },
  deck: {
    label: 'Deck',
    description: 'Pitch deck / lookbook'
  },
  documentary_outline: {
    label: 'Documentary Outline',
    description: 'Story structure for documentary'
  },
  format_rules: {
    label: 'Format Rules',
    description: 'Vertical drama format constraints'
  },
  season_arc: {
    label: 'Season Arc',
    description: 'Season-level arc and episode progression'
  },
  episode_grid: {
    label: 'Episode Grid',
    description: 'Grid of all episodes with hooks/turns'
  },
  vertical_episode_beats: {
    label: 'Vertical Episode Beats',
    description: 'Beat sheets for vertical drama episodes'
  },
  topline_narrative: {
    label: 'Topline Narrative',
    description: 'Synopsis + logline + story pillars'
  },
  // Seed pack doc types
  project_overview: {
    label: 'Project Overview',
    description: 'High-level project summary for packaging'
  },
  creative_brief: {
    label: 'Creative Brief',
    description: 'Creative vision and artistic direction'
  },
  market_positioning: {
    label: 'Market Positioning',
    description: 'Market positioning and sales strategy'
  },
  canon: {
    label: 'Canon Snapshot',
    description: 'Canonical world/character state snapshot'
  },
  nec: {
    label: 'NEC',
    description: 'Narrative Evaluation Card — quality assessment'
  },
  complete_season_script: {
    label: 'Complete Season Script',
    description: 'Full compiled season script for vertical drama'
  },
  // Visual output documents
  visual_project_bible: {
    label: 'Visual Project Bible',
    description: 'Assembled visual development document from canonical truth'
  },
  // Derived (non-ladder) doc types
  scene_graph: {
    label: 'Scene Index',
    description: 'Deterministic scene segmentation index'
  },
  change_report: {
    label: 'Change Report',
    description: 'Deterministic diff and impact analysis'
  }
};
export const LANE_DOC_LADDERS = {
  feature_film: [
    'idea',
    'concept_brief',
    'character_bible',
...
export const DOC_LABEL_ALIASES = {
  blueprint: 'treatment',
...
export const DOC_LABEL_ALIASES_BY_LANE = {
  vertical_drama: {
...
export function normalizeDocType(input, lane, format) {
  if (!input) return input;
  const key = input.trim().toLowerCase().replace(/[\s-]+/g, '_');
  const effectiveLane = lane ?? (format ? formatToLane(format) : 'unspecified');
  const laneAliases = DOC_LABEL_ALIASES_BY_LANE[effectiveLane];
...
const OUTPUT_DOC_TYPE_SET = new Set([
  'market_sheet',
  'vertical_market_sheet',
  'deck',
  'trailer_script',
 ...
 */ export function isOutputDocType(docType) {
  return OUTPUT_DOC_TYPE_SET.has(docType);
}
export function formatToLane(format) {
...
export function buildLadderPromptBlock(lane) {
  const ladder = getLaneLadder(lane);
