...
export function requireDocPolicy(docType) {
    const p = REGISTRY[docType];
    if (!p)
        throw new Error(`DOC_TYPE_UNREGISTERED:${docType}`);
    return p;
}
/**
...
export function validateCanonAlignment(generatedText, canonEntities, opts = {}) {
    ...
    for (const name of foundNames) {
        ...
        if (canonLower.has(lower))
            continue;
        if (GENERIC_PHRASES.has(lower))
            continue;
        // Skip phrases starting with common articles/prepositions that indicate structural text
        if (/^(The|A|An|In|On|At|By|For|With|From|But|And|Or|As|To)\s/i.test(name) && name.split(/\s+/).length <= 3)
            continue;
...
const STRUCTURAL_TERMS = new Set([
    ...
    "FATAL FLAW", "SEASON ARC", "SEASON PROMISE", "SEASON ENGINE",
    "SEASON ARC SNAPSHOT", "SERIES PREMISE", "ARC STRUCTURE",
    "KEY EPISODE ANCHORS", "THEMATIC ARC", "TONE MAP",
    "DRAMATIC SPINE", "ESTABLISHMENT", "ESCALATION",
    "CHARACTER ARCS", "RELATIONSHIP ARC", "STORY PILLARS",
]);
...
export async function buildCanonEntitiesFromDB(supabase, projectId) {
    ...
    if (entities.length === 0 && sources.length === 0)
        return null;
    // Deduplicate
...
export async function runCanonAlignmentGate(supabase, projectId, generatedText) {
    ...
    if (!canon)
        return null; // No canon sources — caller decides policy
