 ...
 */
export async function emitTransition(supabase, payload, options) {
    ...
    try {
        ...
        if (err?.message?.startsWith("TRANSITION_PERSIST_FAILED:"))
            throw err;
 ...
 */
export async function emitTransitions(supabase, payloads) {
 ...
 */
export async function queryTransitions(supabase, filters) {
    let query = supabase
        .from("pipeline_transitions")
        .select("*")
...
function inferDomain(eventType) {
    if (eventType.startsWith("version_") || eventType.startsWith("authoritative_"))
        return EVENT_DOMAINS.VERSION;
    if (eventType.startsWith("stage_"))
        return EVENT_DOMAINS.STAGE;
    if (eventType.startsWith("auto_run_"))
        return EVENT_DOMAINS.AUTO_RUN;
    if (eventType.startsWith("ci_gp_") || eventType.startsWith("scoring_"))
        return EVENT_DOMAINS.SCORING;
    if (eventType.startsWith("rewrite_"))
        return EVENT_DOMAINS.REWRITE;
    if (eventType.startsWith("analysis_"))
        return EVENT_DOMAINS.ANALYSIS;
    if (eventType.startsWith("canon_"))
        return EVENT_DOMAINS.CANON;
    if (eventType.startsWith("decision_"))
        return EVENT_DOMAINS.DECISION;
    if (eventType.startsWith("promotion_") || eventType.startsWith("stale_"))
        return EVENT_DOMAINS.GATE;
    if (eventType.startsWith("impact_") || eventType.startsWith("affected_") || eventType.startsWith("bounded_"))
        return "impact";
    if (eventType.startsWith("projection_"))
        return "projection";
    if (eventType.startsWith("narrative_"))
        return "validation";
