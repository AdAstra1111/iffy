...
export const DOC_TYPE_REGISTRY = {
    ...
    visual_canon_brief: { title: "Visual Canon Brief", file_name: "visual_canon_brief.md", is_seed_core: false, is_ladder: false, doc_category: "canon" },
    // Assembled visual output — downstream read-only assembly of canonical visual truth
    visual_project_bible: { title: "Visual Project Bible", file_name: "visual_project_bible.md", is_seed_core: false, is_ladder: false, doc_category: "output" },
...
export async function ensureDocSlot(supabase, projectId, userId, docType, opts) {
    ...
    const meta = { ...(opts?.metaJson || {}) };
    if (epIdx != null)
        meta.episode_index = epIdx;
    if (Object.keys(meta).length > 0)
        insertPayload.meta_json = meta;
...
export function shouldRunCanonAlignment(format, docType, generatorId) {
    ...
    if ((docType === "production_draft" || docType === "season_script" || docType === "feature_script") &&
        (generatorId === "dev-engine-v2-rewrite-chunked" || generatorId === "dev-engine-v2-rewrite-episodic" || generatorId === "dev-engine-v2-scene-rewrite")) {
...
export async function createVersion(supabase, opts) {
    ...
    const runAlignment = (() => {
        if (!isSystemGenerated || !opts.plaintext)
            return false;
        if (opts.format) {
    ...
    if (opts.changeSummary)
        insertPayload.change_summary = opts.changeSummary;
    if (opts.inheritedCore)
        insertPayload.inherited_core = opts.inheritedCore;
    if (opts.sourceDocumentIds)
        insertPayload.source_document_ids = opts.sourceDocumentIds;
    if (opts.dependsOn)
        insertPayload.depends_on = opts.dependsOn;
    if (opts.generatorId)
        insertPayload.generator_id = opts.generatorId;
    // Convergence fields
    if (opts.isStale !== undefined)
        insertPayload.is_stale = opts.isStale;
    if (opts.staleReason !== undefined)
        insertPayload.stale_reason = opts.staleReason;
    if (opts.generatorRunId)
        insertPayload.generator_run_id = opts.generatorRunId;
    if (opts.styleTemplateVersionId)
        insertPayload.style_template_version_id = opts.styleTemplateVersionId;
    ...
    const { data: newVersion, error } = await supabase
        ...
        .single();
    if (error)
        throw new Error(`createVersion(${key} v${nextVersion}): ${error.message}`);
    ...
    if (transitionProjectId && newVersion?.id && opts.plaintext) {
        try {
            ...
            const mentionResult = await extractEntityMentionsForVersion(supabase, transitionProjectId, opts.documentId, newVersion.id, key, // resolved canonical doc type
            opts.plaintext);
