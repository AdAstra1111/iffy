 ...
 */
export async function validateEpisodeCount(supabaseClient, projectId) {
    ...
    const { data: proj } = await supabaseClient
        ...
        .eq("id", projectId)
        .single();
    const N = (typeof proj?.season_episode_count === "number" && proj.season_episode_count > 0)
    ...
    for (const d of (epDocs || [])) {
        const idx = d.meta_json?.episode_index;
