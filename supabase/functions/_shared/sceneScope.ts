 ...
 */ /** A parsed scene segment with heading and body text. */ /** Scene heading pattern: INT., EXT., I/E., INT/EXT, or numbered scene headings */ const SCENE_HEADING_RE = /^(?:\d+[\.\)]\s*)?(?:INT\.|EXT\.|I\/E\.|INT\/EXT\.?)\s*.+$/im;
/**
 ...
 */ export function parseScenes(scriptText) {
 ...
 */ export function detectOutOfScopeChanges(originalScenes, updatedScenes, allowedSceneNumbers) {
  ...
  if (sorted.length === 0) {
    return {
      ok: true,
      outOfScopeScenes: [],
      message: 'All changes within scope'
    };
  }
  return {
    ok: false,
    outOfScopeScenes: sorted,
    message: `Out-of-scope changes detected in scene(s): ${sorted.join(', ')}. Only scenes ${[
      ...allowedSet
    ].sort((a, b)=>a - b).join(', ')} were allowed.`
  };
}
/**
 ...
 */ export function resolveApplyScope(changePlan, clientApplyScope) {
 ...
 */ export function computeScopedShrink(originalText, rewrittenText, allowedScenes) {
  const isSelective = allowedScenes.length > 0;
  if (!isSelective) {
    // Full rewrite: global comparison
    const shrinkFraction = originalText.length > 0 ? (originalText.length - rewrittenText.length) / originalText.length : 0;
    return {
      shrinkFraction,
  ...
  const originalTargeted = originalScenes.filter((s)=>allowedSet.has(s.sceneNumber)).map((s)=>s.heading + '\n' + s.body).join('\n');
  const rewrittenTargeted = rewrittenScenes.filter((s)=>allowedSet.has(s.sceneNumber)).map((s)=>s.heading + '\n' + s.body).join('\n');
  const origLen = originalTargeted.length;
  const rewriteLen = rewrittenTargeted.length;
  const shrinkFraction = origLen > 0 ? (origLen - rewriteLen) / origLen : 0;
  return {
    shrinkFraction,
    shrinkPct: Math.round(shrinkFraction * 100),
    isSelective: true,
...
/* ── Internal helpers ── */ function normalizeHeading(heading) {
