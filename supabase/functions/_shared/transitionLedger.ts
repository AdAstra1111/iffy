...
export const TRANSITION_EVENTS = {
  ...
  // Patch Planning Events (PPE Phase 0A)
  PATCH_PLANNED: "patch_planned",
  PATCH_EXECUTING: "patch_executing",
  PATCH_FAILED: "patch_failed",
  PATCH_REVERTED: "patch_reverted",
  // Extraction Events (PPE Phase 0A)
 ...
 */ export async function emitTransition(supabase, payload, options) {
  ...
  try {
    const { data, error } = await supabase.from("pipeline_transitions").insert(row).select("id").single();
    if (error) {
 ...
 */ export async function emitTransitions(supabase, payloads) {
  for (const p of payloads){
    await emitTransition(supabase, p, {
      critical: false
    });
  }
}
 ...
 */ export async function queryTransitions(supabase, filters) {
  let query = supabase.from("pipeline_transitions").select("*").eq("project_id", filters.projectId).order("created_at", {
    ascending: true
  });
...
function inferDomain(eventType) {
  ...
  if (eventType.startsWith("patch_") || eventType.startsWith("extraction_")) return "patching";
  return "pipeline";
}
