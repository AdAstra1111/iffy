...
export const SECTION_DEPENDENCY_ORDER = CONCEPT_BRIEF_SECTIONS.map(s => s.key);
/**
 ...
 */
export function getSectionForNoteCategory(category) {
 ...
 */
export function validateSectionDependencies(sectionKey, completedSections) {
    const section = CONCEPT_BRIEF_SECTIONS.find(s => s.key === sectionKey);
    if (!section)
        return `Unknown section key: ${sectionKey}`;
    for (const dep of section.dependencies) {
        if (!completedSections.has(dep)) {
