 ...
 */
import { AXIS_METADATA, SPINE_AXES } from "./narrativeSpine.ts";
// ── NDG v2: Risk Scoring ──────────────────────────────────────────────────
/**
...
export const NARRATIVE_DEPENDENCY_EDGES = [
 ...
 */
export function getDirectDownstream(axis) {
 ...
 */
export function getDirectUpstream(axis) {
    return NARRATIVE_DEPENDENCY_EDGES
        .filter(e => e.to === axis)
        .map(e => e.from);
}
 ...
 */
export function getDownstreamAxes(axis) {
 ...
 */
export function getUpstreamAxes(axis) {
    const visited = new Set();
    const queue = [axis];
    const result = [];
    while (queue.length > 0) {
 ...
 */
export function getDependencyChain(from, to) {
    if (from === to)
        return [from];
 ...
 */
export function getImpactedAxes(changedAxes) {
 ...
 */
export function computePropagatedRisk(changedAxes) {
    const result = [];
    for (const axis of changedAxes) {
        const downstream = getDownstreamAxes(axis);
        if (downstream.length === 0)
            continue;
        const chains = downstream
 ...
 */
export function getDependencyPosition(axis, changedAxes) {
 ...
 */
export function getConfidenceFactor(unitMeta) {
    if (!unitMeta)
        return 0.50;
    ...
    if (vqv === true && sk && sk !== "__preamble")
        return 1.00; // canonical passage
    if (vqv === true)
        return 0.90; // verified, any section
    if (vq)
        return 0.75; // quote present, unverified
 ...
 */
export function computeDependencyRisk(sourceAxis, targetAxis, unitMeta) {
    const chain = getDependencyChain(sourceAxis, targetAxis);
    if (!chain || chain.length < 2)
        return null; // no path or trivial
 ...
 */
export function computeDownstreamRiskScores(sourceAxis, downstreamUnitMeta) {
 ...
 */
export function computeRewritePriorityScore(axis) {
 ...
 */
export const SEQUENCE_BUCKET_RANK = {
    root_fix: 1,
    upstream_fix: 2,
 ...
 */
export function getSequenceBucket(dependencyPosition) {
 ...
 */
export function getSequenceReason(axis, bucket, reason) {
 ...
 */
export function sequenceRewriteTargets(targets) {
    if (targets.length === 0)
        return [];
    const REASON_RANK = { contradicted: 1, stale: 2 };
    const SPINE_ORDER = new Map(SPINE_AXES.map((ax, i) => [ax, i]));
    const sorted = [...targets].sort((a, b) => {
        ...
        // 1. Bucket rank
        if (aBucketRank !== bBucketRank)
            return aBucketRank - bBucketRank;
 ...
 */
export function getRecommendedRepairOrder(sourceAxis) {
    ...
    return downstream
        ...
        sequence_bucket: "upstream_fix", // all downstream of an amendment are upstream-fix relative to terminals
 ...
 */
export function getAffectedScenesForAxes(affectedAxes, sceneIndex) {
    if (!affectedAxes.length || sceneIndex.size === 0)
        return [];
    const results = [];
    const seen = new Set(); // dedupe by scene_id
 ...
 */
export function buildSceneImpactIndex(spineLinkRows, sceneKeyMap) {
    const index = new Map();
    for (const row of spineLinkRows) {
        ...
        if (!index.has(row.axis_key))
            index.set(row.axis_key, []);
