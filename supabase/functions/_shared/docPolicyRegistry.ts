...
const REGISTRY = {
  ...
  episode_outline: {
    docClass: "UNIT",
    requiresEpisodeIndex: true,
    acceptanceProfile: "UNIT_PROFILE_DEFAULT"
  },
  episode_script: {
    docClass: "UNIT",
    requiresEpisodeIndex: true,
  ...
  concept_brief: {
    docClass: "SINGLE",
    acceptanceProfile: "SINGLE_PROFILE_DEFAULT"
  },
  logline: {
    docClass: "SINGLE",
    acceptanceProfile: "SINGLE_PROFILE_DEFAULT"
  ...
  episode_grid: {
    docClass: "AGGREGATE",
    acceptanceProfile: "AGG_PROFILE_DEFAULT"
  },
 ...
 */ export function getDocPolicy(docType) {
 ...
 */ export function requireDocPolicy(docType) {
  const p = REGISTRY[docType];
 ...
 */ export function isAggregate(docType) {
  return requireDocPolicy(docType).docClass === "AGGREGATE";
}
export function getRegressionThreshold(docType) {
...
export function getExploreThreshold(docType) {
  const policy = requireDocPolicy(docType);
  const profile = ACCEPTANCE_PROFILES[policy.acceptanceProfile];
  return profile?.exploreThreshold ?? 15;
}
export function getMaxFrontierAttempts(docType) {
  const policy = requireDocPolicy(docType);
  const profile = ACCEPTANCE_PROFILES[policy.acceptanceProfile];
 ...
 */ export function validateLadderIntegrity(ladder) {
  const missing = [];
 ...
 */ export function validateCanonAlignment(generatedText, canonEntities, opts = {}) {
  ...
  for (const name of foundNames){
    ...
    for (const ce of canonLower){
      if (lower.includes(ce) || ce.includes(lower)) {
        partialMatch = true;
        break;
      }
    }
  ...
  return {
    ...
    reason: !pass ? coverage < minCoverage ? `entity_coverage_${Math.round(coverage * 100)}%_below_${Math.round(minCoverage * 100)}%` : `foreign_entity_ratio_${Math.round(foreignRatio * 100)}%_above_${Math.round(maxForeignRatio * 100)}%` : undefined
  };
}
...
const STRUCTURAL_TERMS = new Set([
  ...
  "RESOLUTION",
  "DENOUEMENT",
  // Season arc / vertical drama structural headers — NOT narrative entities
  "FATAL FLAW",
  "SEASON ARC",
  "SEASON PROMISE",
  "SEASON ENGINE",
...
function isStructuralTerm(name) {
 ...
 */ function stripWorldCharactersSection(text) {
 ...
 */ export async function buildCanonEntitiesFromDB(supabase, projectId) {
  const CANON_SOURCE_TYPES = [
    "canon",
    "character_bible",
    "vertical_episode_beats",
    "season_arc"
  ...
  for (const docType of CANON_SOURCE_TYPES){
    const { data: doc } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", docType).order("created_at", {
      ascending: false
    }).limit(1).maybeSingle();
    if (!doc) continue;
    const { data: ver } = await supabase.from("project_document_versions").select("plaintext").eq("document_id", doc.id).eq("is_current", true).maybeSingle();
    if (ver?.plaintext) {
      // Strip world characters section from character_bible to prevent
      // non-canonical entities entering the canon entity set
      const textForExtraction = docType === "character_bible" ? stripWorldCharactersSection(ver.plaintext) : ver.plaintext;
 ...
 */ export async function runCanonAlignmentGate(supabase, projectId, generatedText) {
