 ...
 */ // ─── Types ───
// ─── Constants ───
/** Episode doc types that use episode-scoped processing */ export const EPISODE_DOC_TYPES = new Set([
  'episode_grid',
  'episode_beats',
 ...
 */ export function parseEpisodeBlocks(text) {
 ...
 */ export function mergeEpisodeBlocks(originalText, replacements) {
  const originalBlocks = parseEpisodeBlocks(originalText);
  const replacedEpisodes = [];
  const preservedEpisodes = [];
 ...
 */ export function validateEpisodeMerge(originalText, mergedText, targetedEpisodes) {
  const originalBlocks = parseEpisodeBlocks(originalText);
  const mergedBlocks = parseEpisodeBlocks(mergedText);
  const targetedSet = new Set(targetedEpisodes);
  const errors = [];
  const missingEpisodes = [];
  const mutatedUntouchedEpisodes = [];
  ...
  for (const ob of originalBlocks){
    ...
    if (!targetedSet.has(ob.episodeNumber)) {
      if (ob.rawBlock !== mb.rawBlock) {
        ...
        errors.push(`Episode ${ob.episodeNumber} was NOT targeted but was modified (original: ${ob.rawBlock.length} chars, merged: ${mb.rawBlock.length} chars)`);
      }
    }
  }
  return {
 ...
 */ export function extractEpisodeNumbersFromOutput(text) {
  const headerPattern = /^(?:#{1,3}\s+|\*{2})?(?:EPISODE\s+|EP\.?\s*)(\d+)\b/gim;
  const matches = [
    ...text.matchAll(headerPattern)
  ];
  const nums = [
 ...
 */ export function detectCollapsedRangeSummaries(text) {
 ...
 */ export function findEpisodesWithCollapse(text) {
 ...
 */ export function buildEpisodeScaffold(episodeCount) {
  const parts = [];
