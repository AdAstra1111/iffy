...
import { getDocPurposeClass } from "./docPurposeRegistry.ts";
// ── Constants ──
...
const NIE_ELIGIBLE_PURPOSES = new Set([
    "DEVELOPMENT_ARCHITECTURE",
    "SCRIPT_EXECUTION",
]);
// ── Ladder Utilities ──
function resolveLane(assignedLane) {
    const laneMap = {
...
function getAdjacentDocTypes(docType, lane) {
...
export async function loadAdjacentDocPack(supabaseClient, projectId, docType, assignedLane) {
    const lane = resolveLane(assignedLane);
    const { upstream, downstream } = getAdjacentDocTypes(docType, lane);
    const loadDoc = async (targetDocType, direction) => {
        try {
            ...
            const { data: ver } = await supabaseClient
                ...
                .eq("is_current", true)
                .maybeSingle();
            if (!ver?.plaintext || ver.plaintext.trim().length < 50)
                return null;
            return {
    ...
    try {
        ...
        if (canonRow?.canon_json) {
            ...
            if (Array.isArray(cj.characters)) {
                parts.push(`Characters: ${cj.characters.map((c) => c.name || c).join(", ")}`);
            }
            if (Array.isArray(cj.world_rules)) {
                parts.push(`World Rules:\n- ${cj.world_rules.join("\n- ")}`);
            }
            if (cj.themes)
                parts.push(`Themes: ${Array.isArray(cj.themes) ? cj.themes.join(", ") : cj.themes}`);
 ...
 */
export function shouldRunNIE(docType) {
    if (!NIE_PHASE1_DOC_TYPES.has(docType))
        return false;
 ...
 */
function buildNIEPrompt(docType, docText, pack) {
    const contextParts = [];
    if (pack.canon_text) {
 ...
 */
export async function evaluateNarrativeIntegrity(callAIFn, parseAIJsonFn, apiKey, model, docType, docText, pack) {
    const { system, user } = buildNIEPrompt(docType, docText, pack);
    const comparedDocs = [];
    if (pack.upstream)
        comparedDocs.push({ doc_type: pack.upstream.doc_type, doc_id: pack.upstream.doc_id, direction: "upstream" });
    if (pack.downstream)
        comparedDocs.push({ doc_type: pack.downstream.doc_type, doc_id: pack.downstream.doc_id, direction: "downstream" });
    ...
    try {
        ...
        const state = parsed.integrity_state || (score >= 90 ? "clear" : score >= 60 ? "warning" : "blocked");
        // Map domains
        const domains = {};
        const rawDomains = parsed.domains || {};
        for (const key of ["character", "plot_payoff", "canon", "ladder_readiness"]) {
            const d = rawDomains[key];
            if (d) {
                domains[key] = {
                    status: (["clear", "warning", "blocked"].includes(d.status) ? d.status : "clear"),
