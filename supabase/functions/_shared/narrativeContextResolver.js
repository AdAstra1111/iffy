...
import { extractCanonConstraints, buildCanonConstraintBlock, } from "./canonConstraintEnforcement.ts";
import { resolveStructuralLineage } from "./structuralLineageResolver.ts";
// ── Caps ──
const SIGNALS_CAP = 6;
...
function parseTier(match, fallback) {
    if (!match)
        return fallback;
...
function parseTensionSources(text) {
    const tsmMatch = text.match(/tension(?:\s+source(?:\s+matrix)?)[;:?\s]+(.+?)(?=\n\n|\n[A-Z]|$)/is);
    if (!tsmMatch)
        return [];
    ...
    for (const entry of rawEntries) {
        ...
        if (seen.has(core.toLowerCase()))
            continue;
        if (/^(the|and|or|for|with|from|by|to|of|on|in|at|is|are|tier|tiers?)$/i.test(core))
            continue;
        if (core.length < 3)
            continue;
 ...
 */
export async function resolveNarrativeContext(supabase, projectId, opts = {}) {
    const provenance = {};
    const counts = {};
    ...
    try {
        ...
        const parts = [];
        if (cj.title) {
            canon.title = cj.title;
            parts.push(`Title: ${cj.title}`);
        }
        if (cj.logline && typeof cj.logline === "string" && cj.logline.trim()) {
            canon.logline = cj.logline;
            parts.push(`Logline: ${cj.logline}`);
        }
        if (cj.premise && typeof cj.premise === "string" && cj.premise.trim()) {
            canon.premise = cj.premise;
            parts.push(`Premise: ${cj.premise}`);
        }
        if (cj.format)
            parts.push(`Format: ${cj.format}`);
        if (cj.genre)
            parts.push(`Genre: ${cj.genre}`);
        if (cj.tone)
            parts.push(`Tone: ${cj.tone}`);
        if (cj.tone_style && typeof cj.tone_style === "string" && cj.tone_style.trim())
            parts.push(`Tone & Style: ${cj.tone_style}`);
        // Episode meta
        ...
        if (epCount)
            parts.push(`Episode count: ${epCount}`);
        if (epMin != null && epMax != null)
            parts.push(`Episode duration range: ${epMin}–${epMax}s`);
        ...
        if (Array.isArray(cj.characters) && cj.characters.length > 0) {
            ...
            canon.characters = chars.map((c) => ({
                name: c.name,
                detail: [c.role, c.goals, c.traits].filter(Boolean).join("; "),
            }));
            canon.entityAnchors = chars.map((c) => c.name);
                    ...
                    if (cbVer?.plaintext && cbVer.plaintext.length > 50) {
                        ...
                        const addName = (raw) => {
                            ...
                            if (name.length <= 1 || name.length > 30)
                                return;
                            if (STRUCTURAL_TERMS.has(nameUpper) || STRUCTURAL_TERMS.has(nameNoThe))
                                return;
                            // Skip names that look like section descriptors rather than characters
                            if (/^(DARK[- ]STREAM|VISUAL|SETTING|LOCATION|THEME|FORMAT|SERIES)/i.test(name))
                                return;
                            nameSet.add(name);
                        };
        ...
        if (cj.timeline && typeof cj.timeline === "string" && cj.timeline.trim())
            parts.push(`Timeline: ${cj.timeline}`);
        if (cj.locations && typeof cj.locations === "string" && cj.locations.trim())
            parts.push(`Locations: ${cj.locations}`);
        if (cj.ongoing_threads && typeof cj.ongoing_threads === "string" && cj.ongoing_threads.trim())
            parts.push(`Ongoing threads: ${cj.ongoing_threads}`);
        // World rules
        ...
        if (Array.isArray(cj.forbidden_changes) && cj.forbidden_changes.length > 0)
            parts.push(`Forbidden changes: ${cj.forbidden_changes.join("; ")}`);
        else if (typeof cj.forbidden_changes === "string" && cj.forbidden_changes.trim())
            parts.push(`Forbidden changes: ${cj.forbidden_changes}`);
        if (cj.format_constraints && typeof cj.format_constraints === "string" && cj.format_constraints.trim())
            parts.push(`Format constraints: ${cj.format_constraints}`);
        if (parts.length > 0) {
        ...
        const canonConstraints = extractCanonConstraints(cj);
        ...
        counts.worldRules = canon.worldRules.length;
        // ── 3. Signals ──
        let signals = { topSignals: [], blockText: "" };
        if (opts.includeSignals !== false) {
            try {
                ...
                const influence = projSettings?.signals_influence ?? 0.5;
                const applyConfig = projSettings?.signals_apply ?? { pitch: true, dev: true, grid: true, doc: true };
        ...
        // ── 4. Locked Decisions ──
        let lockedDecisions = { items: [], blockText: "" };
        try {
            ...
            if (decisions && decisions.length > 0) {
                ...
                const bullets = decisions.map((d) => `- [${d.decision_key}] ${d.decision_text}`).join("\n");
        ...
        // ── 5. Voice ──
        let voice = { voiceId: null, blockText: "" };
        ...
        try {
            ...
            if (!voice.voiceId)
                provenance.voice = provenance.voice || "none";
 ...
 */
export function buildNarrativeContextBlock(ctx) {
    return [
        ctx.nec.blockText,
        ctx.canon.blockText,
...
=== END WORLD POPULATION LAYER ===`;
function buildWorldPopulationBlock(density) {
    if (density === "rich")
        return WORLD_POP_RICH;
    return WORLD_POP_MODERATE; // default for anything non-minimal
}
// ── Internal NEC block builders ──
function buildNECBlock(text, prefTier, maxTier, docId) {
