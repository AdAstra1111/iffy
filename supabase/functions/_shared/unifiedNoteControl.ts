 ...
 */ import { getInvalidationPlan } from "./deliverableDependencyRegistry.ts";
const DEFAULT_BLOCKING_SEVERITIES = [
  "blocker",
  "high"
];
const UNRESOLVED_DEFERRED_STATUSES = [
  "open",
  "pinned"
];
const UNRESOLVED_NOTE_STATUSES = [
  "open",
  "in_progress",
  "reopened"
];
const UNRESOLVED_DEV_NOTE_STATUSES = [
  "open"
];
/**
 * Get all unresolved upstream note blockers targeting a specific doc type,
 * normalized across all three note systems.
 */ export async function getUnifiedUpstreamNoteBlockers(supabase, projectId, targetDocType, options) {
  ...
  try {
    const { data: deferred } = await supabase.from("project_deferred_notes").select("id, note_key, source_doc_type, target_deliverable_type, severity, category, note_json, pinned").eq("project_id", projectId).eq("target_deliverable_type", targetDocType).in("status", UNRESOLVED_DEFERRED_STATUSES).in("severity", blockingSeverities).limit(limitPerTable);
  ...
  if (includeProjectNotes) {
    try {
      const { data: notes } = await supabase.from("project_notes").select("id, doc_type, destination_doc_type, status, severity, category, timing, title, summary, legacy_key").eq("project_id", projectId).eq("destination_doc_type", targetDocType).in("status", UNRESOLVED_NOTE_STATUSES).in("severity", blockingSeverities).limit(limitPerTable);
  ...
  if (includeDevNoteState) {
    try {
      const { data: devNotes } = await supabase.from("project_dev_note_state").select("id, doc_type, defer_to_doc_type, status, severity, tier, note_fingerprint, objective").eq("project_id", projectId).eq("defer_to_doc_type", targetDocType).in("status", UNRESOLVED_DEV_NOTE_STATUSES).limit(limitPerTable);
  ...
  if (staleDevNoteBlockers.length > 0) {
    const fps = staleDevNoteBlockers.map((b)=>b.note_key_or_fingerprint).filter(Boolean);
    if (fps.length > 0) {
      try {
        const { data: resolvedNotes } = await supabase.from("development_notes").select("note_key").eq("resolved", true).in("note_key", fps);
        if (resolvedNotes && resolvedNotes.length > 0) {
          const resolvedFps = new Set(resolvedNotes.map((r)=>r.note_key));
          const staleCount = blockers.filter((b)=>b.source_table === "project_dev_note_state" && resolvedFps.has(b.note_key_or_fingerprint)).length;
          if (staleCount > 0) {
 ...
 * the given doc type.
 */ export function getDownstreamDocTypes(repairedDocType, ladder) {
 ...
 */ export async function invalidateDescendants(supabase, projectId, repairedDocType, ladder, newVersionId, lane) {
  ...
  let subjectPropagation;
  let subjectNarrowedDocTypes = null;
  try {
    const { isSubjectSourceDocType, buildSubjectPropagationPlan, extractCanonicalSubjects } = await import("./canonSubjectRegistry.ts");
    if (isSubjectSourceDocType(repairedDocType)) {
      ...
      if (canonRow?.canon_json) {
        ...
        const { data: prevVersion } = await supabase.from("project_canon_versions").select("canon_json").eq("project_id", projectId).order("created_at", {
          ascending: false
        }).limit(2);
        // Use the second-most-recent version as "previous" (first is the current update)
        const prevCanon = prevVersion && prevVersion.length >= 2 ? prevVersion[1].canon_json || {} : {};
        const currentCanon = canonRow.canon_json;
        const propagationPlan = buildSubjectPropagationPlan(prevCanon, currentCanon, repairedDocType, effectiveLane);
        if (propagationPlan && propagationPlan.deltas.length > 0) {
          ...
          subjectPropagation = {
            ...
            subject_classes: [
              ...new Set(propagationPlan.deltas.map((d)=>d.subject_class))
            ],
  ...
  for (const entry of plan.entries){
    ...
    try {
      const { data: docs } = await supabase.from("project_documents").select("id").eq("project_id", projectId).eq("doc_type", entry.doc_type);
      if (!docs || docs.length === 0) continue;
      for (const doc of docs){
        if (effectivePolicy === "stale") {
          // Hard invalidation — mark stale
          const { data: updated, error } = await supabase.from("project_document_versions").update({
          ...
          }).eq("document_id", doc.id).eq("is_current", true).select("id");
  ...
  const reanalyzeDocs = plan.entries.filter((e)=>e.revalidation_policy === "must_reanalyze").map((e)=>e.doc_type);
  if (reanalyzeDocs.length > 0) {
    try {
      const { data: activeJobs } = await supabase.from("auto_run_jobs").select("id, current_document").eq("project_id", projectId).in("status", [
        "running",
        "paused"
