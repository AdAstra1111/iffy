 ...
 */ import { parseScenes, detectOutOfScopeChanges } from "./sceneScope.ts";
// ── Registry ──
const SCENE_REPAIR_REGISTRY = {
  feature_script: {
 ...
 */ export function isSceneRepairSupported(docType) {
  return SCENE_REPAIR_REGISTRY[docType]?.scene_repair_supported === true;
}
/**
 * Get scene repair config. Returns null if unsupported.
 */ export function getSceneRepairConfig(docType) {
  return SCENE_REPAIR_REGISTRY[docType] || null;
}
/**
 * List doc types supporting scene-level repair.
 */ export function listSceneRepairDocTypes() {
  return Object.keys(SCENE_REPAIR_REGISTRY).filter((k)=>SCENE_REPAIR_REGISTRY[k].scene_repair_supported);
}
 ...
 */ export function resolveIssueToSceneTarget(issue, docType, content) {
  ...
  const matches = [
    ...searchText.matchAll(scenePattern)
  ];
  const extractedNums = [
    ...new Set(matches.map((m)=>parseInt(m[1], 10)))
  ].filter((n)=>!isNaN(n) && sceneNumberSet.has(n));
  ...
  const slugMatches = [
    ...searchText.matchAll(sluglinePattern)
  ];
  if (slugMatches.length === 1) {
    const slugFragment = slugMatches[0][0].trim().toUpperCase();
    const matchedScene = scenes.find((s)=>s.heading.toUpperCase().includes(slugFragment) || normalizeHeading(s.heading).includes(normalizeHeading(slugFragment)));
    if (matchedScene) {
      return {
 ...
 */ export function extractScene(content, sceneNumber) {
  const scenes = parseScenes(content);
 ...
 */ export function replaceScene(content, sceneNumber, newSceneContent) {
  const scenes = parseScenes(content);
  ...
  if (targetIdx === -1) {
    return {
      success: false,
      new_content: content,
      reason: `scene_${sceneNumber}_not_found`
    };
  }
  // Extract preamble (content before the first scene)
  const preamble = scenes.length > 0 && scenes[0].startOffset > 0 ? content.slice(0, scenes[0].startOffset) : "";
 ...
 */ export function getSceneRepairTarget(issue, docType, content) {
  const config = SCENE_REPAIR_REGISTRY[docType];
 ...
 */ export function enforceSceneIntegrity(originalContent, rewrittenContent, targetSceneNumber) {
  ...
  let effectiveTarget = targetInRewritten || null;
  if (!effectiveTarget) {
    ...
    if (originalTarget) {
      ...
      effectiveTarget = rewrittenScenes.find((s)=>normalizeHeading(s.heading) === normalizedOrigHeading) || null;
    }
  }
  if (!effectiveTarget) {
    return {
  ...
  const preamble = originalScenes.length > 0 && originalScenes[0].startOffset > 0 ? originalContent.slice(0, originalScenes[0].startOffset) : "";
  const parts = [];
  if (preamble.trim()) {
    parts.push(preamble.trimEnd());
  }
  ...
  const scopeCheck = detectOutOfScopeChanges(originalScenes, finalScenes, [
    targetSceneNumber
  ]);
  return {
    ok: scopeCheck.ok,
    ...
    reason: scopeCheck.ok ? `integrity_ok:replaced_scene_${targetSceneNumber}:preserved=${scenesPreserved}:corrected=${scenesCorrected}` : `integrity_issues:${scopeCheck.message}`
  };
}
// ── Internal Helpers ──
