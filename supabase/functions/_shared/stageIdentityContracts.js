...
function hasScreenplayFormatting(text) {
    ...
    for (const line of lines) {
        ...
        if (SCENE_HEADING_RE.test(trimmed))
            scene_heading_count++;
        if (DIALOGUE_CUE_RE.test(trimmed) && trimmed.length < 40 && trimmed === trimmed.toUpperCase())
            dialogue_cue_count++;
        if (PARENTHETICAL_RE.test(trimmed) && trimmed.startsWith("("))
            parenthetical_count++;
    }
    return {
 ...
 */
export function validateIdeaStageIdentity(plaintext, metaJson) {
    ...
    if (screenplay.scene_headings || screenplay.dialogue_cues || screenplay.vo_os) {
        if (screenplay.scene_headings)
            violations.push(`Found ${screenplay.scene_heading_count} scene heading(s) (INT./EXT.)`);
        if (screenplay.dialogue_cues)
            violations.push("Contains screenplay dialogue cues");
        if (screenplay.vo_os)
            violations.push("Contains V.O./O.S. annotations");
        if (screenplay.parentheticals)
            violations.push("Contains parenthetical directions");
        return {
            pass: false,
 ...
 */
export function validateConceptBriefStageIdentity(plaintext) {
    const text = (plaintext || "").trim();
    const charCount = text.length;
    const wordCount = countWords(text);
    const sectionCount = countSections(text);
    ...
    if (screenplay.scene_heading_count >= 3 || screenplay.dialogue_cues || screenplay.vo_os) {
        if (screenplay.scene_headings)
            violations.push(`Found ${screenplay.scene_heading_count} scene heading(s)`);
        if (screenplay.dialogue_cues)
            violations.push("Contains screenplay dialogue cues");
        if (screenplay.vo_os)
            violations.push("Contains V.O./O.S. annotations");
 ...
 */
export function validateStageIdentity(docType, plaintext, metaJson) {
    switch (docType) {
 ...
 */
export function getStageIdentityPromptBlock(docType) {
    switch (docType) {
        case "idea":
            return `
═══════════════════════════════════════
...
export function buildDiagnostic(result, repairInvoked = false) {
    return {
