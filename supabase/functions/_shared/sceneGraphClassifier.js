 ...
 */
export async function classifySceneGraphState(supabase, projectId) {
    ...
    if (sceneErr)
        throw new Error(`[sceneGraphClassifier] scenes query failed: ${sceneErr.message}`);
    ...
    if (verErr)
        throw new Error(`[sceneGraphClassifier] versions query failed: ${verErr.message}`);
    const versionedSceneIds = new Set((versionedRows ?? []).map((r) => r.scene_id));
    ...
    const { data: orderedRows, error: ordErr } = await supabase
        ...
        .in('scene_id', [...allSceneIds]);
    if (ordErr)
        throw new Error(`[sceneGraphClassifier] order query failed: ${ordErr.message}`);
    const orderedSceneIds = new Set((orderedRows ?? []).map((r) => r.scene_id));
    // ── Compute signals ───────────────────────────────────────────────────────
    // Orphan count: scenes with no version
    let orphanCount = 0;
    for (const id of allSceneIds) {
        if (!versionedSceneIds.has(id))
            orphanCount++;
    }
    // Missing order count: scenes with no active order entry
    let missingOrderCount = 0;
    for (const id of allSceneIds) {
        if (!orderedSceneIds.has(id))
            missingOrderCount++;
    }
    ...
    const signals = [];
    if (orphanCount > 0)
        signals.push(`orphan_scenes=${orphanCount}`);
    if (missingOrderCount > 0)
        signals.push(`missing_order=${missingOrderCount}`);
    if (keyGapCount > 0)
        signals.push(`key_gaps=${keyGapCount}`);
    const state = signals.length > 0 ? 'PARTIAL_GRAPH' : 'POPULATED_GRAPH';
    return {
        state,
