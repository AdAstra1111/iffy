 ...
 */
export function containsFailedPlaceholders(text) {
    return FAILED_CHUNK_PLACEHOLDER_RE.test(text);
}
// ── Token budgets per strategy/docType ──
function maxTokensForChunk(strategy, docType) {
    if (strategy === "episodic_indexed")
        return 16000;
    if (docType.includes("script") || docType === "screenplay_draft" || docType === "production_draft")
        return 32000;
    if (docType.includes("treatment"))
        return 24000;
    return 16000;
}
// ── LLM Gateway ──
async function callChunkLLM(apiKey, gatewayUrl, system, user, model = "google/gemini-2.5-flash", maxTokens = 16000) {
...
async function initializeChunks(supabase, documentId, versionId, plan) {
...
async function generateSingleChunk(opts, chunk, previousChunkEnding) {
...
`;
            ...
            const PER_SECTION_TARGETS = isLong ? {
...
function chunksNeedingGeneration(plan, existingMap) {
    return plan.chunks.filter(c => {
        ...
        if (["pending", "failed", "failed_validation", "needs_regen"].includes(existing.status))
            return true;
...
// ── Main Runner ──
export async function runChunkedGeneration(opts) {
    const { supabase, documentId, versionId, plan, docType, maxChunkRepairs = 2, episodeCount, requestId, } = opts;
    ...
    const chunkMap = new Map((existingChunks || []).map((c) => [c.chunk_index, c]));
    // Pre-fill content array from existing done chunks
    const chunkContents = new Array(plan.totalChunks).fill("");
        ...
        for (let attempt = 0; attempt <= maxChunkRepairs; attempt++) {
            try {
                ...
                if (plan.strategy === "episodic_indexed" && chunk.episodeStart && chunk.episodeEnd) {
                    const expectedEps = Array.from({ length: chunk.episodeEnd - chunk.episodeStart + 1 }, (_, i) => chunk.episodeStart + i);
    ...
    for (let repairPass = 0; repairPass <= MAX_ASSEMBLY_REPAIR_PASSES; repairPass++) {
        ...
        if (plan.strategy === "episodic_indexed" && episodeCount) {
            ...
            validationResult = validateSectionedContent(assembledContent, plan.chunks.map(c => c.chunkKey), docType);
        }
        if (validationResult.pass || repairPass >= MAX_ASSEMBLY_REPAIR_PASSES)
            break;
        ...
        if (validationResult.missingIndices?.length > 0 && plan.strategy === "episodic_indexed") {
            ...
            for (const missingEp of validationResult.missingIndices) {
                const owningChunk = plan.chunks.find(c => c.episodeStart != null && c.episodeEnd != null &&
        ...
        if (chunksToRegen.length === 0)
            break; // No actionable repair
    ...
    if (SCREENPLAY_CLASS_DOCS.has(docType) && assembledWordCount < SCREENPLAY_WORD_FLOOR) {
        belowFloor = true;
        console.error(`[chunkRunner][IEL] SCREENPLAY FLOOR VIOLATION: ${docType} assembled with ${assembledWordCount} words (floor=${SCREENPLAY_WORD_FLOOR}). ` +
 ...
 */
export async function resumeChunkedGeneration(opts) {
