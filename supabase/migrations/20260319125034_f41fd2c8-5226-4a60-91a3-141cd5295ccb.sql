...
-- ═══════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.canon_locations (
...
CREATE INDEX IF NOT EXISTS idx_canon_locations_project ON public.canon_locations(project_id);

-- ═══════════════════════════════════════════════════════════════
-- 2. Entity Visual States table (story-aware state variants)
-- ═══════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.entity_visual_states (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id uuid NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_entity_visual_states_project ON public.entity_visual_states(project_id, entity_type, entity_name);
