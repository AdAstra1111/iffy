...
-- Fix lock_visual_set: skip evaluation requirement for non-character domains
-- Environment/atmosphere/texture/motif sets don't have character DNA evaluations
...
BEGIN
  ...
  -- Determine if this domain requires image evaluations
  -- Character identity and costume domains require evaluations (identity gate)
  -- Production design domains (location, atmosphere, texture, motif) do NOT
  ...
  LOOP
    IF v_slot.is_required THEN
      ...
      -- 5. Evaluation check — only for domains that require it
...
-- Fix resolve_visual_set_readiness: skip evaluation requirement for non-character domains
  ...
  LOOP
    IF v_slot.is_required THEN
      ...
      IF v_slot.state = 'approved' OR v_slot.state = 'locked' THEN
        IF v_requires_evaluation THEN
          -- Verify latest evaluation (character domains only)
          SELECT * INTO v_eval FROM public.image_evaluations
            WHERE project_id = v_set.project_id
              AND image_id = v_slot.selected_image_id
          ...
          END IF;
        ELSE
          -- Non-character domains: approved slot is sufficient for lock readiness
          v_required_approved := v_required_approved + 1;
        END IF;
      ELSE
