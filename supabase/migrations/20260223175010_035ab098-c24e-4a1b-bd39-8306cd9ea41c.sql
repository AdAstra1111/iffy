...
CREATE TABLE IF NOT EXISTS public.storyboard_runs (
...
CREATE INDEX IF NOT EXISTS idx_storyboard_runs_project ON public.storyboard_runs (project_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_storyboard_panels_project_run ON public.storyboard_panels (project_id, run_id);
CREATE INDEX IF NOT EXISTS idx_storyboard_panels_project_unit ON public.storyboard_panels (project_id, unit_key);
...
CREATE TABLE IF NOT EXISTS public.storyboard_pipeline_frames (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id uuid NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_sb_pipeline_frames_panel ON public.storyboard_pipeline_frames (project_id, panel_id, created_at DESC);
