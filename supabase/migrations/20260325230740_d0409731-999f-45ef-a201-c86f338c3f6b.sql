CREATE OR REPLACE FUNCTION public.rebind_project_ai_cast(p_project_id uuid, p_character_key text, p_next_actor_id uuid, p_reason text, p_changed_by uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 2. Lock + fetch current binding (case-insensitive match against stored keys)
  SELECT id, ai_actor_id, ai_actor_version_id, character_key
  INTO v_current
  FROM public.project_ai_cast
  WHERE project_id = p_project_id AND lower(character_key) = v_norm_key
  FOR UPDATE;
  ...
  IF p_next_actor_id IS NULL THEN
    ...
    -- Delete ALL matching rows defensively (handles duplicates)
    DELETE FROM public.project_ai_cast
    WHERE project_id = p_project_id AND lower(character_key) = v_norm_key;
    GET DIAGNOSTICS v_deleted_count = ROW_COUNT;

    IF v_deleted_count = 0 THEN
      RAISE EXCEPTION 'Unbind failed: no rows deleted for character_key %', v_norm_key;
    END IF;
  ...
  -- 6. Upsert binding (delete old if case-mismatched, then insert/update)
