...
CREATE TABLE IF NOT EXISTS public.project_coverage_subjects (
...
CREATE INDEX IF NOT EXISTS idx_proj_cov_subjects_project ON public.project_coverage_subjects(project_id);
CREATE INDEX IF NOT EXISTS idx_proj_cov_subjects_type ON public.project_coverage_subjects(subject_type);
CREATE UNIQUE INDEX IF NOT EXISTS idx_proj_cov_subjects_doc_version ON public.project_coverage_subjects(document_version_id) WHERE document_version_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_proj_cov_subjects_bundle ON public.project_coverage_subjects(project_id, bundle_key);
...
CREATE TABLE IF NOT EXISTS public.project_coverage_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id uuid NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_proj_cov_runs_project ON public.project_coverage_runs(project_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_proj_cov_runs_subject ON public.project_coverage_runs(subject_id, created_at DESC);
